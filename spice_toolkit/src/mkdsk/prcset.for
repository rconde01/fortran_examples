...
C        GETP05  {Get plate type 5 parameters}
C
...
C$ Version
C
C-    MKDSK Version 4.0.0, 01-DEC-2020 (NJB)
C
C        Added check for missing angular or distance units in the
C        INPUT_DATA_UNITS setup file assignment. Previously only
C        the presence of two assignments was verified, so either a
C        duplicated angular unit assignment or distance unit
C        assignment would pass the test.
C
...
C
     ...
     .            //            'specification in setup file: #'    )
            ...
            IF ( AUNITS .EQ. ' ' ) THEN
               CALL SETMSG ( 'Angular units were not set by the '
     .         //            'INPUT_DATA_UNITS setup file variable.' )
               CALL SIGERR ( 'SPICE(SYNTAXERROR)'                    )
               CALL CHKOUT ( 'GETGEN'                                )
               RETURN
            END IF
            ...
            IF ( DUNITS .EQ. ' ' ) THEN
               CALL SETMSG ( 'Distance units were not set by the '
     .         //            'INPUT_DATA_UNITS setup file variable.' )
               CALL SIGERR ( 'SPICE(SYNTAXERROR)'                    )
               CALL CHKOUT ( 'GETGEN'                                )
               RETURN
