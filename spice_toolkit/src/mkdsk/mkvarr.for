...
C                fill in each column of the output grid in "top-down"
C                order.
C
...
C                file. In other words, the data from the input file
C                fill in each row of the output grid in "left-right"
C                order.
C
...
C                input file. When the input file is row-major and
C                LEFTRT is .TRUE., the rows themselves are in
C                left-right order: the first element of each sequence
C                of NCOLS tokens belongs to the left column of the
C                output grid.
C
...
C
C                Units are given by the DISTANCE setting of the
C                setup file's INPUT_DATA_UNITS value.
C
...
C$ Version
C
C-    MKDSK Version 1.0.1, 27-DEC-2021 (NJB)
C
C        Corrected description of units of the height reference value.
C        Updated descriptions of TOPDWN and LEFTRT inputs. Corrected 
C        typo in header comments.
C
