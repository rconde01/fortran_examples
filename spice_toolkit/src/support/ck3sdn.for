...
C
C
C-    Beta Version 1.3.0, 29-AUG-2019 (BVS)
C
C        BUG FIX: changed to declare fit to be OK for cases when KEEPF
C        and KEEPL points are next to each other but do not pass the
C        too close to 180 degrees apart check. In such cases the
C        algorithm went into indefinite loop.
C
...
C
C           If KEEPF and KEEPL points are next to each other, we will
C           declare the fit to be OK even if it does not pass the ``too
C           close to 180 degrees apart'' test and set LEFT equal to
C           RIGHT to move KEEPF forward. If we don't do this, the
C           algorithm goes into indefinite loop.
C
            IF ( KEEPF + 1 .EQ. KEEPL .AND. .NOT. FITOK ) THEN
               FITOK = .TRUE.
               LEFT  = RIGHT
            END IF
