...
C        error NSPIO(BOGUSENTRY) is signaled.
C
C     2) See entry points for exceptions specific to them.
C
C$ Files
C
...
C$ Version
C
C-    NSPIO Version 2.0.1, 06-DEC-2021 (BVS)
C
C        Cleaned up non-printing characters.
C
...
C        file, this routine simply leaves the port un-open and
C        returns.
C
...
C$ Restrictions
C
C     1) NAME should point to a non-existent file that can be opened
C        for write access.
C
