...
C$ Version
C
C-     META/2 Configured Version 2.0.1, 06-DEC-2021 (BVS)
C
C         Filled in $Index_Entries, cleaning up non-printing
C         characters.
C
C         Re-ordered header sections.
C
C-     META/2 Configured Version 2.0.0, 09-MAY-1994 (WLT)
C
C         This is the configured version of the Command Loop
C         software as of May 9, 1994
C
C
C-     META/2 Configured Version 1.0.0, 09-MAY-1994 (WLT)
C
...
C$ Index_Entries
C
C     pretty print syntax definition
C
C-&
...
C        BEGIN  we have not begun processing a switch
...
C
      ...
      CRLAST = .FALSE.
C
C     Process the string a word at a time  until we've seen it all.
C
      DO WHILE (E .NE. 0 )
...
C
            ...
            ELSE
C
C              This word is to be treated as an ordinary word.
C
               WORD   = STRING(B:E)
               CRLAST = .FALSE.
