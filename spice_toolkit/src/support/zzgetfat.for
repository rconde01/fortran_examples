...
C                 when opened.  An integer, returned to the calling
C                 routine.
C
C$ Parameters
C
...
C          some reason, the error SPICE(INQUIREERROR) will be signaled.
C
C      2)  If the file specified by FILE is already open, the error
C          SPICE(FILECURRENTLYOPEN) will be signaled.
C
...
C          fail, the error SPICE(FILEREADFAILED) will be signaled.
C
C$ Files
C
...
C$ Version
C
C-    Beta Version 1.26.0, 28-NOV-2021 (BVS)
C
