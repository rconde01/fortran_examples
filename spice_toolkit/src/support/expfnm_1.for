...
C        output string, the error SPICE(STRINGTOOSMALL) is signaled.
C
...
C
C     The operating system does filename expansion itself, so this
...
C$ Version
C
C-    Beta Version 3.26.0, 28-NOV-2021 (BVS)
C
...
C        an environment variable directory specification plus a
...
C
C     Because the operating system takes care of any necessary filename
C     translations, just return the input filename.
C
      OUTFIL = INFIL
      CALL CHKOUT ( 'EXPFNM_1' )
      RETURN
