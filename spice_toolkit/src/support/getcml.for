...
C
C     Call the FORTRAN library function NARGS to determine how many
C     words are on the command line. This number includes the command
C     itself; we won't include that in the output string. Get the
C     words one at a time and construct the output string and we're
C     done.
C
      HOWMNY = NARGS() - 1
      I      = 1
