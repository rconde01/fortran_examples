...
C         SPICE(SYSTEMCALLFAILED) is signaled.
C
C$ Files
C
C     None.
...
C     operating system's command interpreter. This routine encapsulates
...
C$ Version
C
C-    Beta Version 2.27.0, 28-NOV-2021 (BVS)
C
...
C
      ...
      INTEGER             GETLASTERRORQQ
      EXTERNAL            GETLASTERRORQQ
...
C
      SUCCES  =  SYSTEMQQ (   CMD( :RTRIM(CMD)  )   )
 
      IF ( .NOT. SUCCES ) THEN
C
C        Uh, we've got a problem.
C
         STATUS  =  GETLASTERRORQQ()
 
         CALL SETMSG ( 'The "SYSTEMQQ" call '     //
     .                 'returned code # in response to command #.'  )
         CALL ERRINT ( '#',  STATUS                                 )
         CALL ERRCH  ( '#',  CMD                                    )
