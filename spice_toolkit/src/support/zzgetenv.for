...
C     ENVLEN     P   Maximum length of an environment variable or symbol
...
C$ Particulars
C
C     Call the function 'GETENVQQ( ENVVAR, VALUE )', provided to obtain
C     environment variable values. Given the name of an environment
C     variable, and it returns the length of the value, or zero if an
C     error occurs, storing in VALUE the value of the specified
...
C$ Version
C
C-    SPICELIB Version 2.26.0, 28-NOV-2021 (BVS)
C
...
C
      ...
      ELSE
C
C        Attempt to get the value for the environment variable. A
C        LENGTH of zero (0) is returned if the environment variable
C        is not defined, otherwise LENGTH is the length of the value.
C
         LENGTH = GETENVQQ ( ENVVAR, MYVALU )
 
         IF ( LENGTH .GT. 0 ) THEN
 
            IF ( LENGTH .GT. LEN (VALUE) ) THEN
               MYVALU = ' '
            END IF
