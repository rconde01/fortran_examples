...
C$ Abstract
C
C     CLCOMM provides command line functionality to the COMMNT
C     program.
C
C$ Disclaimer
C
...
C$ Detailed_Output
C
C     Performance of the action defined by the command line options.
C
C$ Parameters
C
C     None.
...
C$ Version
C
C-    COMMNT Version 1.2.0, 14-OCT-2021 (BVS)
C
C        Bug fix: fixed routine name in CHKIN/CHKOUT calls
C        (CLCOMMNT -> CLCOMM).
C
C        Bug fix: corrected usage strings (clcommnt -> commnt).
C
C-    SPICELIB Version 1.1.0, 04-AUG-2006 (EDW)
C
...
C        Edited header.
C
C-    SPICELIB Version 1.0.1, 23-JAN-1997 (WLT)
C
...
C
      ...
      PARAMETER           ( NUMOPT = 5 )
C
C     Number of different option names.
C
      INTEGER               OPTLEN
      PARAMETER           ( OPTLEN = 16 )
C
...
C
         IF ( NWORDS .NE. 2 ) THEN
            CALL SETMSG ( 'Usage is: commnt -a <kernel file>'
     .      //            ' <comment file>'                      )
            CALL ERRCH  ( '#', USAGE                             )
...
C
         IF ( NWORDS .NE. 1 ) THEN
            CALL SETMSG ( 'Usage is: commnt -d <kernel file>' )
            CALL ERRCH  ( '#', USAGE                            )
            CALL SIGERR ( 'SPICE(USAGEERROR)'                   )
         END IF
...
C
         IF ( NWORDS .NE. 2 ) THEN
            CALL SETMSG ( 'Usage is: commnt -e <kernel file>'
     .      //            ' <comment file>'                      )
            CALL ERRCH  ( '#', USAGE                             )
            CALL SIGERR ( 'SPICE(USAGEERROR)'                    )
         END IF
...
C
         IF ( NWORDS .NE. 1 ) THEN
            CALL SETMSG ( 'Usage is: commnt -r <kernel file>' )
            CALL ERRCH  ( '#', USAGE                            )
            CALL SIGERR ( 'SPICE(USAGEERROR)'                   )
         END IF
