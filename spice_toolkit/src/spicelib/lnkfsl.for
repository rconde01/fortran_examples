...
C         error SPICE(INVALIDNODE) is signaled. POOL will not be
C         modified.
C
C     2)  If either of HEAD or TAIL are valid node numbers but are not
...
C         containing HEAD, the error SPICE(INVALIDSUBLIST) is signaled.
