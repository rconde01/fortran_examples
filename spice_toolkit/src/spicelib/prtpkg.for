...
C$ Exceptions
C
C     1)  If PRTPKG is called directly, the error SPICE(BOGUSENTRY) is
C         signaled.
C
C$ Files
C
...
C        MSGSEL
C
C     There is no reason to call this subroutine. The purpose of this
C     subroutine is to make the declarations required by the various
C     entry points. This routine has no run-time function.
C
C$ Examples
C
...
C
C-    SPICELIB Version 3.26.0, 17-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header of the PRTPKG umbrella routine and all its
...
C
C-    SPICELIB Version 2.0.0, 09-NOV-1993 (HAN)
C
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 17-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
...
C
C     The function returns the value .TRUE. if the message type
C     indicated by TYPE has been selected for output to the error output
C     device.
C
C$ Detailed_Input
C
C     TYPE     is the type of error message.
C
C              Possible values of TYPE are 'SHORT', 'EXPLAIN', 'LONG',
C              'DEFAULT', and 'TRACEBACK'.
C
C$ Detailed_Output
C
C     The function returns the value .TRUE. if the message type
C     indicated by TYPE has been selected for output to the error
C     output device.
C
C$ Parameters
C
...
C$ Exceptions
C
C     1)  If an invalid value of TYPE is detected, the error
C         SPICE(INVALIDMSGTYPE) is signaled. The handling of this error
C         is a special case; to avoid recursion problems, SIGERR is not
C         called when the error is detected. Instead, the short and long
C         error messages are output directly.
C
C$ Files
C
C     None.
...
C        type declaration in $Declarations section.
C
C-    SPICELIB Version 3.0.5, 17-JUL-2002 (BVS)
C
C        Added MAC-OSX environments.
