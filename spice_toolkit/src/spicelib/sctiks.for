...
C$ Exceptions
C
C     1)  If the spacecraft clock type is not supported, the
...
C     clock type is supported by SPICE, then the SPICELIB routine TIKSnn
C     is called to handle the actual conversion from clock format to
C     number of ticks. The nn in TIKSnn refers to the spacecraft clock
C     type code. Different spacecraft have distinct clock formats but
C     can still be of the same clock type.
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 01-NOV-2021 (JDR)
C
