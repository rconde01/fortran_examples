...
C              coordinate axis numbers; each pair ANGLEx and AXISx
C              specifies a coordinate transformation consisting of a
C              rotation by ANGLEx radians about the coordinate axis
C              indexed by AXISx. These coordinate transformations are
C              typically symbolized by
C
C                 [ ANGLEx ]     .
C                           AXISx
C
C              See the $Particulars section below for details concerning
C              this notation.
C
C              Note that these coordinate transformations rotate vectors
C              by
C
C                 -ANGLEx
C
...
C
C              The values of AXISx may be 1, 2, or 3, indicating the X,
C              Y, and Z axes respectively.
C
C$ Detailed_Output
C
C     R        is a rotation matrix representing the composition of the
C              rotations defined by the input angle-axis pairs.
C              Together, the three pairs specify a composite
C              transformation that is the result of performing the
C              rotations about the axes indexed by AXIS1, AXIS2, and
C              AXIS3, in that order. So,
C
...
C              this notation.
C
C              The resulting matrix R may be thought of as a coordinate
C              transformation; applying it to a vector yields the
C              vector's coordinates in the rotated system.
C
C              Viewing R as a coordinate transformation matrix, the
C              basis that R transforms vectors to is created by rotating
C              the original coordinate axes first by ANGLE1 radians
C              about the coordinate axis indexed by AXIS1, next by
C              ANGLE2 radians about the coordinate axis indexed by
C              AXIS2, and finally by ANGLE3 radians about coordinate
C              axis indexed by AXIS3. At the second and third steps of
C              this process, the coordinate axes about which rotations
C              are performed belong to the bases resulting from the
C              previous rotations.
C
C$ Parameters
C
C     None.
...
C$ Restrictions
C
C     1)  Beware: more than one definition of "RA, DEC and twist"
C         exists.
C
C$ Literature_References
C
C     [1]  W. Owen, "Galileo Attitude and Camera Models," JPL
C          Interoffice Memorandum 314-323, Nov. 11, 1983. NAIF document
C          number 204.0.
C
...
C        The current names reflect the order of operator
C        application when the Euler angle rotations are applied to
C        a vector: the rightmost matrix
C
C           [ ANGLE1 ]
C                     AXIS1
C
...
C                     AXIS3
C
C        Previously, the names reflected the order in which the
C        Euler angle matrices appear on the page, from left to
C        right. This naming convention was found to be a bit
C        obtuse by a various users.
C
...
C
C        Also, the header was upgraded to describe the notation in
C        more detail. The symbol
C
C           [ x ]
C                i
C
C        is explained at mind-numbing length. An example was
C        added that shows a specific set of inputs and the
C        resulting output matrix.
C
C-&
