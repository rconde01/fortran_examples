...
C         output argument SCLKDP will not be modified. This routine
C         assumes that that an SCLK kernel appropriate to the spacecraft
...
C         SCLKDP will not be modified. When using SCLK kernels that map
C         SCLK to a time system other than ET (also called barycentric
C         dynamical time---`TDB'), it is necessary to have a leapseconds
C         kernel loaded at the time this routine is called.
C
...
C         SC, an error is signaled by a routine in the call tree of this
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 13-APR-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Moved the
C        required readings present in $Literature_References section to
C        $Required_Reading.
C
