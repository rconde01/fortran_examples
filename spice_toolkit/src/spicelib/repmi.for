...
C$ Parameters
C
C     MAXLI    is the maximum expected length of the text representation
C              of an integer. 11 characters are sufficient to hold any
C              integer whose absolute value is less than 10 billion.
C
C              This routine assumes that the input integer is such that
C              its string representation contains no more than MAXLI
C              characters.
C
C$ Exceptions
C
...
C
C     1) The following example illustrate the use of REPMI to
C        replace a marker within a string with the text representation
C        of an integer value.
C
C
...
C
C              CALL REPMI ( INSTR, MARKER, 75, OUTSTR )
C
...
C
C              CALL REPMI ( INSTR, MARKER, 2035, OUTSTR )
C
...
C            Output: Invalid value. The value was:  75
C
C         Case 2: Multiple markers.
C            Input : Left > Right endpoint. Left: XX; Right: XX
C            Output: Left > Right endpoint. Left: 2035; Right: XX
C
C
C$ Restrictions
C
...
C        $Particulars and extended MAXLI parameter description in
C        $Parameters.
C
