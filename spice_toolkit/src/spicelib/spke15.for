...
C$ Abstract
C
C     Evaluate a single SPK data record from a segment of type 15
...
C         has zero length, the error SPICE(BADVECTOR) is signaled.
C
C     4)  If the trajectory pole vector and the periapsis vector are not
C         orthogonal, the error SPICE(BADINITSTATE) is signaled. The
C         test for orthogonality is very crude. The routine simply
C         checks that the absolute value of the dot product of the unit
C         vectors parallel to the trajectory pole and periapse vectors
C         is less than 0.00001. This check is intended to catch
C         blunders, not to enforce orthogonality to double precision
C         tolerance.
C
...
C          pp.345-347, Willman-Bell, 1989.
C
C     [2]  R. H. Battin, "Astronautical Guidance," pp.199, McGraw-Hill
C          Book Company, 1964.
C
...
C$ Version
C
C-    SPICELIB Version 1.3.0, 14-APR-2021 (JDR)
C
