...
C$ Exceptions
C
C     1)  If an error occurs while converting the input ASCII text DAF
C         file to binary format, the error is signaled by a routine in
C         the call tree of this routine.
C
C     2)  If an error occurs while writing data to the output binary DAF
...
C$ Restrictions
C
C     1)  DAFA2B cannot be executed while any other DAF is open
C         for writing.
C
...
C$ Index_Entries
C
C     DEPRECATED ascii DAF to binary
C
C-&
