C$Procedure SPKACS ( SPK, aberration corrected state )
...
C
C
C              PROGRAM SPKACS_EX1
C              IMPLICIT NONE
C        C
C        C     Local constants
C        C
C        C     The meta-kernel name shown here refers to a file whose
C        C     contents are those shown above. This file and the
C        C     kernels it references must exist in your current working
C        C     directory.
C        C
C        C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'spkacs_ex1.tm' )
C        C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 23-AUG-2021 (JDR) (NJB)
C
