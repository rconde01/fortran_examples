...
C$ Abstract
C
C     Locate the point on the surface of an ellipsoid that is nearest
C     to a specified position. Also return the altitude of the position
C     above the ellipsoid.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     POSITN   is a 3-vector giving the position of a point with respect
...
C              parallel to the x-axis of the body-fixed reference frame.
C
C     B        is the length of the semi-axis of the ellipsoid that is
C              parallel to the y-axis of the body-fixed reference frame.
C
C     C        is the length of the semi-axis of the ellipsoid that is
C              parallel to the z-axis of the body-fixed reference frame.
C
C$ Detailed_Output
C
...
C         error SPICE(BADAXISLENGTH) is signaled.
C
...
C
C     This routine solves for the location, N, on the surface of an
C     ellipsoid nearest to an arbitrary location, P, relative to that
C     ellipsoid.
C
C$ Examples
C
...
C$ Version
C
C-    SPICELIB Version 2.0.0, 26-OCT-2021 (NJB) (JDR) (EDW)
C
C        Edit to logic to reduce unneeded operations when
C        error or projection vectors equal zero. Addition
C        of details concerning the "ellipsoid near point"
...
C$ Revisions
C
C-    SPICELIB Version 1.1.0, 27-NOV-1990 (NJB)
C
C        Bug fix: added initialization of variable SNGLPT to support
C                  operation under the Macintosh Intel Fortran
C                  compiler. Note that this bug did not affect
...
C     ----------------------------------------------------------------
C
C     You can describe this problem in two ways, either using a
C     geometric description of the required conditions for the
C     solution, or a functional description based on minimization.
C
...
C
C     Moreover, if POSITN = (Px,Py,Pz) then lambda must satisfy
C     the equation:
C
C              2                 2                2
C           Px                 Py               Pz
...
C
C     The problem also defines a minimization condition solvable with
C     the technique of Lagrange Multipliers. In this case,
C     minimize the distance between the ellipsoid surface (set) and the
C     location P subject to the ellipsoid constraint, the solution,
C     NPOINT, an element of the ellipsoid set.
C
C     Define the Lagrange expression, L, as
C
C        L = f - lambda * g                                         [8]
C
C     with f the function to minimize, and g the constraint on f.
C
C     Solve
C
C        D*L = 0                                                    [9]
...
C        g = 0                                                     [10]
C
C     which gives us the expressions
C
C        D*f = lambda * D*g                                        [11]
C        g = 0
C
C     where D = (Dx, Dy, Dz).
C                                     2
C      Choose f(x,y,z) as ||P-NPOINT|| rather than ||P-NPOINT||
C      to simplify the resulting derivatives. Both functions have the
C      same minimum.
C
C        f(x,y,z) = (Px - X)**2 + (Py-Y)**2 + (Pz-Z)**2            [12]
C
C       The constrain equation
C
C                    X**2       Y**2       Z**2
C        g(x,y,z) = ------  +  ------  +  ------  - 1              [13]
C                    A**2       B**2       C**2
C
C      The resulting equations from [11]
C
C         (Px-X) = lambda *   X                                    [14]
C                            ---
C                            A**2
...
C          A**2       B**2       C**2
C
C     Solve [14], [15], [16] for X, Y, Z, then substitute into [5]
C
C              2                 2                2
C           Px                 Py               Pz
C      -------------   +  -------------  +  ------------  - 1 = 0  [18]
...
C              A                 B                 C
C
C     We see expression [18] identical to expression [6].
C
...
C     ----------------------------------------------------------------
C
C     The solution as implemented uses the geometric description
C     of the problem.
C
...
C     whether we should continue solving for NEARPT. The logic relies
C     on the initial value of SOLVNG as TRUE, maintaining that
C     value unless explicitly changed.
C
      SNGLPT =  4
      SOLUTN =  1
      SOLVNG = .TRUE.
...
C
C           Check for a zero projection. If so, set iteration flag to
C           prevent an iteration pass after the current finishes.
C
            IF ( VZERO(ERRP) ) THEN
               SOLVNG = .FALSE.
            END IF
...
C
     ...
     .                          'POSITN = ( #, #, # )'        )
            ...
            IF (  (SPOINT(1) .EQ. POINT(1)) .AND.
     .            (SPOINT(2) .EQ. POINT(2)) .AND.
     .            (SPOINT(3) .EQ. POINT(3))       ) THEN
               SOLVNG = .FALSE.
            END IF
...
C
C           Check for a zero difference. If so, set iteration flag to
C           prevent an iteration pass after the current finishes.
C
            IF ( VZERO(ERR) ) THEN
               SOLVNG = .FALSE.
...
C              Determine the magnitude of the error due to our
C              sharpened estimate, if error non zero.
C
               NEWERR = VNORM ( ERR )
            END IF
...
C
C                 Check for a zero projection. If so, set iteration
C                 flag to prevent an iteration pass after the current
C                 finishes.
C
                  IF ( VZERO(ERRP) ) THEN
                     SOLVNG = .FALSE.
