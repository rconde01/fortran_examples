...
C         is signaled.
C
C     3)  If OBSRVR and TARGET map to the same NAIF integer ID codes,
...
C         coordinates have not been loaded prior to calling SUBSOL, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     5)  If the specified aberration correction is not recognized, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     6)  If insufficient ephemeris data have been loaded prior to
C         calling SUBSOL, an error is signaled by a
C         routine in the call tree of this routine.
C
C     7)  If the triaxial radii of the target body have not been loaded
C         into the kernel pool prior to calling SUBSOL, an error is
C         signaled by a routine in the call tree of this routine.
C
C     8)  If the size of the TARGET body radii kernel variable is not
C         three, an error is signaled by a routine in the call tree of
C         this routine.
C
C     9)  If any of the three TARGET body radii is less-than or equal to
C         zero, an error is signaled by a routine in the call tree of
C         this routine.
C
C     10) If PCK data supplying a rotation model for the target body
C         have not been loaded prior to calling SUBSOL, an error is
C         signaled by a routine in the call tree of this routine.
C
C$ Files
C
...
C$ Restrictions
C
C     1)  The appropriate kernel data must have been loaded before this
...
C
C-    SPICELIB Version 1.0.1, 01-NOV-1999 (WLT)
C
