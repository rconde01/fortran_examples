...
C         contains no such data, the error SPICE(NOAVDATA) is signaled.
C
C$ Files
C
...
C
C     The following code fragment searches backwards through a file
C     (represented by HANDLE) for all segments applicable to the
C     Voyager 2 wide angle camera, for a particular spacecraft clock
C     time, which have data type 1. It then evaluates the pointing for
C     that epoch and prints the result.
C
...
C$ Version
C
C-    SPICELIB Version 1.2.2, 12-AUG-2021 (NJB) (JDR)
C
...
C           is signaled, the FOUND flag will definitely be false.
C
