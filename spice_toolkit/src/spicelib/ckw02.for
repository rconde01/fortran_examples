...
C         SPICE(INVALIDSCLKTIME) is signaled.
C
C     5)  If the second or any subsequent START times are negative, the
C         error SPICE(TIMESOUTOFORDER) is signaled.
C
C     6)  If any of the STOP times are negative, the error
C         SPICE(DEGENERATEINTERVAL) is signaled.
C
C     7)  If the STOP time of any of the intervals is less than or equal
C         to the START time, the error SPICE(DEGENERATEINTERVAL) is
C         signaled.
C
C     8)  If the START times are not strictly increasing, the
C         error SPICE(TIMESOUTOFORDER) is signaled.
C
C     9)  If the STOP time of one interval is greater than the START
C         time of the next interval, the error SPICE(BADSTOPTIME)
C         is signaled.
C
C     10) If BEGTIM is greater than START(1) or ENDTIM is less than
C         STOP(NREC), the error SPICE(INVALIDDESCRTIME) is
C         signaled.
C
C     11) If the name of the reference frame is not one of those
C         supported by the routine NAMFRM, the error
C         SPICE(INVALIDREFFRAME) is signaled.
C
C     12) If NREC, the number of pointing records, is less than or
C         equal to 0, the error SPICE(INVALIDNUMRECS) is signaled.
C
...
C
C     1) The following example creates a CK file with a type-2 segment,
C        with data for a simple time dependent rotation and angular
C        velocity.
C
...
C        C
C              CHARACTER*(*)         CK2
C              PARAMETER           ( CK2 = 'ckw02_ex1.bc' )
C
C              DOUBLE PRECISION      SPTICK
C              PARAMETER           ( SPTICK = 0.001D0 )
C
C              INTEGER               INST
C              PARAMETER           ( INST = -77702 )
C
C              INTEGER               MAXREC
C              PARAMETER           ( MAXREC = 21 )
C
C              INTEGER               NAMLEN
...
C              DOUBLE PRECISION      RATES  (     MAXREC )
C              DOUBLE PRECISION      RWMAT  ( 3, 3 )
C              DOUBLE PRECISION      SPACES
C              DOUBLE PRECISION      STARTS (     MAXREC )
C              DOUBLE PRECISION      STOPS  (     MAXREC )
...
C        C
C              SEGID  = 'Test type 2 CK segment'
C              IFNAME = 'Test CK type 2 segment created by CKW02'
C
C
C        C
...
C
C        C
C        C     Set the initial value of the encoded ticks. The interval
C        C     associated with each quaternion will start at the epoch
C        C     of the quaternion and will extend 0.8 * STICKS forward in
C        C     time, leaving small gaps between the intervals.
C        C
C        C     The clock rates array will have a constant SPTICK value.
C        C
C              STARTS(1) = 1000.D0
C              STOPS(1)  = STARTS(1) + ( 0.8D0 * STICKS )
C              RATES(1)  = SPTICK
C
C        C
...
C        C        1000.0 ticks.
C        C
C                 STARTS(I) = 1000.D0 + (I-1) * STICKS
C                 STOPS(I)  = STARTS(I) + ( 0.8D0 * STICKS )
C                 RATES(I)  = SPTICK
C
C        C
...
C        C
C              BEGTIM = STARTS(1)
C              ENDTIM = STOPS(MAXREC)
C
C        C
C        C     All information ready to write. Write to a CK type 2
C        C     segment to the file indicated by HANDLE.
C        C
C              CALL CKW02 ( HANDLE, BEGTIM, ENDTIM, INST,  REF,
C             .             SEGID,  MAXREC, STARTS, STOPS, QUATS,
C             .             AVVS,   RATES                       )
C
C        C
...
C$ Version
C
C-    SPICELIB Version 3.0.1, 26-MAY-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Created
C        complete code example from existing fragment.
C
C        Updated Exception #12 to describe the actual check and error
C        produced by this routine.
C
