...
C
C     The function returns .TRUE. if ITEM is in the kernel pool, .FALSE.
C     otherwise.
C
C$ Detailed_Input
C
...
C              requested.
C
C     ITEM     is the item to be returned. Together, the body and item
C              name combine to form a variable name, e.g.,
C
C                    'BODY599_RADII'
C                    'BODY4_POLE_RA'
C
C$ Detailed_Output
C
C     The function returns .TRUE. if ITEM is in the kernel pool, and
...
C     error to be signaled whenever the specified item is not found.
...
C$ Version
C
C-    SPICELIB Version 1.3.0, 12-AUG-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Updated
C        input argument BODY detailed description. Added SPK and PCK to
C        the list of required readings.
C
