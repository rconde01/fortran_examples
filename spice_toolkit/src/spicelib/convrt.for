...
C              specified by IN.
C
C     IN       is the identifier of the units associated with the
C              measurement X. Acceptable units are:
C
C                 Angles:                 'RADIANS'
C                                         'DEGREES'
C                                         'ARCMINUTES'
...
C
C     OUT      is the identifier of the units desired for the
C              measurement X. See the description of IN.
C
C$ Detailed_Output
C
...
C$ Restrictions
C
C     1)  This routine does not do any checking for overflow. The caller
C         is required to make sure that the units used for the
C         measurement are such that no floating point overflow will
C         occur when the conversion is performed.
C
C     2)  Some of the units are not "defined" quantities. In such a case
C         a best estimate is provided as of the date of the current
C         version of this routine. Those estimated quantities are:
C
C            AU               The astronomical unit. The value was taken
C                             from the JPL ephemeris DE125. This value
C                             is an approximation and should not be used
C                             for high-accuracy work. It agrees with the
C                             value used in the JPL planetary ephemeris
C                             DE430 (149597870.700 km) at the 100m
C                             level.
C
C            TROPICAL_YEARS   The tropical year is the time from equinox
C                             to equinox. This varies slightly with
C                             time.
C
C            PARSECS          The parsec is the distance to an object
C                             whose parallax angle is one arcsecond. Its
C                             value is dependent upon the value of the
C                             astronomical unit.
C
...
C
C        Corrected a typo in $Restrictions section.
C
