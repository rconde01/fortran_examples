...
C     data that are missing or invalid are treated in the usual way.
...
C     and MSG string.
C
C     In the case of any SPICELIB error occurring, ERROR is initialized
...
C     performed by data type specific routines. The following exceptions
C     may occur:
C
C     7)  The input spacecraft clock string is blank.
C
C     8)  The remainder clock string cannot be parsed by the SCLK data
C         type specific routine. The precise issue detected while
C         parsing the clock string is provided in the MSG string.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 1.3.0, 22-NOV-2021 (JDR) (NJB)
C
...
C
C        Added entries #7 and #8 to $Exceptions section.
C
...
C        Missing partition data should be caught by the earlier SCTYPE
C        call, which accesses the type 1 SCLK database.
C
         CALL CHKOUT ( 'SCPARS' )
         RETURN
...
C
     ...
     .         'string #.'
      ...
      IF ( POS .EQ. LEN(SCLKCH) ) THEN
         ...
         MSG = 'SCLK string ends with slash.'
...
C
C     Parse the portion of the clock string following the slash,
C     if any, or the whole string if the slash is absent.
C
...
C
C        This code should be unreachable but is provided for safety.
C        Missing SCLK data should be caught by the earlier SCTYPE call,
C        which accesses the type 1 SCLK database.
C
         CALL CHKOUT ( 'SCPARS' )
         RETURN
      END IF
...
C
C     Check if the SCPSxx routine encountered a problem.
C
      IF ( PSERR ) THEN
