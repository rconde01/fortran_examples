...
C              with ID code set found for the file PCKFNM.
C
...
C              of its contents upon input with the set of reference
C              frame class ID codes of each frame for which data are
C              present in the indicated PCK file. The elements of
C              SPICE sets are unique; hence each ID code in IDS
C              appears only once, even if the PCK file contains multiple
C              segments for that ID code.
C
...
C         contain the actual number of ID codes of frames covered by the
C         indicated PCK file, an error is signaled by a routine in the
