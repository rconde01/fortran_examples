...
C              integer within the bounds of representable integers
C              as defined by the SPICELIB routines INTMAX and INTMIN.
C              Otherwise, FOUND is .FALSE.
C
C$ Parameters
C
C     MAXL     is the maximum allowable length of a body name. The
C              current value of this parameter is 36. See the SPICELIB
C              include file zzbodtrn.inc for details.
C
C$ Exceptions
C
C     1)  If there is any problem with the body name-ID mapping kernel
C         variables present in the kernel pool, an error is signaled by
C         a routine in the call tree of this routine.
C
C     2)  Body name strings are upper-cased, their leading and trailing
C         blanks removed, and embedded blanks are compressed out, after
C         which they get truncated to the maximum body name length MAXL.
C         Therefore, two body names that differ only after that maximum
C         length are considered equal.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 07-AUG-2020 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Updated output argument FOUND description in $Detailed_Output.
C
C        Edited the header to comply with NAIF standard. Fixed minor
C        typos in header. Added description of MAXL parameter. Added
