...
C
C     7)  If FIRST is greater than LAST, the error
C         SPICE(BADDESCRTIMES) is signaled.
C
C     8)  If STEP is non-positive, the error SPICE(INVALIDSTEPSIZE) is
C         signaled.
C
C     9)  If the first epoch BEGTIM is greater than FIRST, the error
C         SPICE(BADDESCRTIMES) is signaled.
C
...
C         is signaled. See the $Parameters section for a description of
C         the tolerance.
C
C$ Files
C
...
C
C     1) This example demonstrates how to create an SPK type 12 kernel
C        containing only one segment, given an evenly-spaced
C        time-ordered set of discrete states.
C
C
C        Example code begins here.
C
C
C              PROGRAM SPKW12_EX1
C              IMPLICIT NONE
C
C        C
...
C        C
C              CHARACTER*(*)         SPK12
C              PARAMETER           ( SPK12 = 'spkw12_ex1.bsp' )
C
C              CHARACTER*(*)         REF
...
C        C
C              IFNAME = 'Type 12 SPK internal file name.'
C              SEGID  = 'SPK type 12 test segment'
C
C        C
C        C     Open a new SPK file.
C        C
C              CALL SPKOPN( SPK12, IFNAME, NCOMCH, HANDLE )
C
C        C
C        C     Write the segment.
C        C
C              CALL SPKW12 (  HANDLE, BODY,   CENTER, REF,
C             .               FIRST,  LAST,   SEGID,  DEGREE,
C             .               NSTATS, STATES, FIRST,  STEP   )
C
C        C
...
C        screen. After run completion, a new SPK type 12 exists in
