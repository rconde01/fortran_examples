...
C
C        New checks on item sizes have been added: sizes are now
C        compared against upper bounds as well as lower bounds.
C        Previously only lower bounds were used.
C
C        Bug fix: corrected index error in SCLD01 modulus range check.
C        Corrected comments about count checks in SCLI01. Made
C        cosmetic changes to code and comments in both SCLD01 and
C        SCLI01, and in this routine. Deleted unused parameter MXNCFF.
C
C        Edited the umbrella routine and all its entry points headers
...
C
C     MXCOFA is the maximum size of the coefficient array.
C
      INTEGER               MXCOFA
      PARAMETER           ( MXCOFA = 3 * MXCOEF )
...
C
C     COFIDX is the index of the coefficient name in NAMLST.
C
      INTEGER               COFIDX
      PARAMETER           ( COFIDX =  1 )
...
C
C     OFFIDX is the index of the SCLK offsets in NAMLST.
C
      INTEGER               OFFIDX
      PARAMETER           ( OFFIDX =  5 )
...
C     associated values.
C
      DATA (   NAMLST(I),            LB(I),  UB(I),   I = 1, NKITEM ) /
     .
     .      'SCLK01_COEFFICIENTS',      3,   MXCOFA,
     .      'SCLK_PARTITION_START',     1,   MXPART,
     .      'SCLK_PARTITION_END',       1,   MXPART,
     .      'SCLK01_N_FIELDS',          1,   1,
     .      'SCLK01_OFFSETS',           1,   MXNFLD,
     .      'SCLK01_MODULI',            1,   MXNFLD,
     .      'SCLK01_OUTPUT_DELIM',      1,   1,
     .      'SCLK01_KERNEL_ID',         1,   1,
     .      'SCLK01_TIME_SYSTEM',       0,   1                        /


      DATA NFDMSG / '# not found. Did you load the SCLK kernel?'      /

      DATA NUMMSG / '# values found for #: valid range is #:#.'       /
...
C         modified.
C
C     2)  If the item specified by NAME and SC is found but does not
C         have numeric type, the error SPICE(BADKERNELVARTYPE) is
C         signaled.
C
C     3)  This routine can check certain data for validity. If any of
...
C
C     5)  If the dimension of the requested item is outside of the
C         limits for that item, the error SPICE(INVALIDSIZE) is
C         signaled.
C
C$ Files
C
...
C        Previously only lower bounds were used.
C
C        Corrected comments about count checks. Made cosmetic changes
C        to code and comments.
C
...
C
      ...
      IF ( N .GT. MAXNV ) THEN
         ...
         CALL SETMSG ( 'Item # for SCLK # has size # but '
     .   //            'output array has size #.'         )
         CALL ERRCH  ( '#',  TMPNAM                       )
         CALL ERRINT ( '#',  SC                           )
         CALL ERRINT ( '#',  N                            )
         CALL ERRINT ( '#',  MAXNV                        )
...
C
C     Check the data type of the variable.
C
      IF (  FOUND  .AND.  ( TYPE .NE. 'N' )  ) THEN

         CALL SETMSG ( 'Kernel variable # for spacecraft '
     .   //            'clock # does not have numeric type.' )
         CALL ERRCH  ( '#', TMPNAM                           )
         CALL ERRINT ( '#', SC                               )
         CALL SIGERR ( 'SPICE(BADKERNELVARTYPE)'             )
         CALL CHKOUT ( 'SCLI01'                              )
         RETURN
...
C
C        - The SCLK field count kernel variable is a scalar.
C
C        - The delimiter code kernel variable is a scalar.
C
C        - The time system code kernel variable, if present, is a
C          scalar. At this point in the code, the variable is known
C          to be present.
C
C     Note that the kernel pool doesn't allow a kernel variable to
C     not have associated values. Checks against the lower bound 1 are
C     done just to simplify the code.
C
...
C
      ...
      IF ( I .NE. 0 ) THEN
         ...
         IF (  (  N .LT. LB(I) ) .OR. ( N .GT. UB(I) )  ) THEN

            CALL REPMI  (  NUMMSG, '#', N,      ERRMSG )
            CALL REPMC  (  ERRMSG, '#', TMPNAM, ERRMSG )
            CALL REPMI  (  ERRMSG, '#', LB(I),  ERRMSG )
            CALL REPMI  (  ERRMSG, '#', UB(I),  ERRMSG )
            CALL SETMSG (  ERRMSG                      )
            CALL SIGERR ( 'SPICE(SIZEOUTOFRANGE)'      )
            CALL CHKOUT ( 'SCLI01'                     )
            RETURN
...
C
C     5)  If the dimension of the requested item exceeds MAXNV, the
C         error SPICE(ARRAYTOOSMALL) is signaled.
C
C     6)  If the dimension of the requested item is outside of the
C         limits for that item, the error SPICE(INVALIDSIZE) is
C         signaled.
C
C     7)  If the dimension of the coefficient kernel variable is
C         not a multiple of 3, the error SPICE(INVALIDSIZE) is
C         signaled.
C
C$ Files
C
...
C        New checks on item sizes have been added: sizes are now
C        compared against upper bounds as well as lower bounds.
C        Previously only lower bounds were used. A check has been
C        added to verify that the coefficient count is a multiple
C        of 3.
C
C        Bug fix: corrected index error in modulus range check. Made
C        cosmetic changes to code and comments.
C
C        Edited the header to comply with NAIF standard.
C
C-    SPICELIB Version 2.2.0, 20-NOV-2006 (NJB) (EDW)
...
C     appropriate range. We test for the following conditions:
C
C        - The number of coefficients is at least 3 and no more than
C          3 * MXCOEF
C
C        - The number of partition start values is at least 1 and no
C          more than MXPART
C
C        - The number of partition end values is at least 1 and no more
C          than MXPART
C
C        - The number of moduli is at least 1, no more than MXNFLD,
C          and matches the field count.
C
C        - The number of offsets is at least 1, no more than MXNFLD,
C          and matches the field count.
C
C     Note that the kernel pool doesn't allow a kernel variable to
C     not have associated values. Checks against the lower bound 1 are
...
C
      ...
      IF ( NAME .EQ. NAMLST(COFIDX) ) THEN

         IF (  ( (N/3)*3 )  .NE.  N  ) THEN

            CALL SETMSG ( 'Coefficient count for # must '
     .      //            'be multiple of 3 but was #.'  )
            CALL ERRCH  ( '#', TMPNAM                    )
            CALL ERRINT ( '#', N                         )
...
C
C     Check the values of the moduli themselves. Also check the
C     moduli count against the field count.
C
      IF ( NAME .EQ. NAMLST(MODIDX) ) THEN

         DO I = 1, N

            IF ( DVAL(I) .LT. 1.D0 ) THEN
               CALL REPMC  (  BVLMSG, '#', TMPNAM,      ERRMSG )
               CALL REPMD  (  ERRMSG, '#', DVAL(I), 14, ERRMSG )
               CALL SETMSG (  ERRMSG                           )
               CALL SIGERR ( 'SPICE(VALUEOUTOFRANGE)'          )
               CALL CHKOUT ( 'SCLD01'                          )
               RETURN
      ...
      IF (       ( NAME .EQ. NAMLST(MODIDX) )
     .     .OR.  ( NAME .EQ. NAMLST(OFFIDX) )  ) THEN
C
C        Get the field count for this clock.
C
         KVNAME = NAMLST( NFLIDX )(:KVNMLN)

         CALL SUFFIX ( '_#',    0,        KVNAME )
         CALL REPMI  ( KVNAME, '#', -SC,  KVNAME )

         CALL GIPOOL ( KVNAME, 1, 1, NF, NFIELD, FOUND )
         ...
         IF ( .NOT. FOUND ) THEN
            ...
            CALL SETMSG ( 'Field count was not found for SCLK #.' )
            CALL ERRINT ( '#', SC                    )
            CALL SIGERR ( 'SPICE(KERNELVARNOTFOUND)' )
            CALL CHKOUT ( 'SCLD01'                   )
            RETURN
         ...
         IF ( N .NE. NFIELD ) THEN
            ...
            IF ( NAME .EQ. NAMLST(MODIDX) ) THEN
               ...
               CALL SETMSG ( 'Modulus count # does not match '
     .         //            'field count # for SCLK #.'      )

            ELSE

               CALL SETMSG ( 'Offset count # does not match '
     .         //            'field count # for SCLK #.'      )
