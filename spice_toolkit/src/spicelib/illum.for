...
C         target, and Sun at the time specified by ET, an error is
C         signaled by a routine in the call tree of this routine. If
C         light time corrections are used, SPK data for the target body
C         must be available at the time ET - LT, where LT is the one-way
...
C         body are unavailable, an error is signaled by a routine in the
...
C
C     6)  If radii for TARGET are not found in the kernel pool, an error
C         is signaled by a routine in the call tree of this routine.
C
C     7)  If the size of the TARGET body radii kernel variable is not
C         three, an error is signaled by a routine in the call tree of
C         this routine.
C
C     8)  If any of the three TARGET body radii is less-than or equal to
...
C        C     from Mars Global Surveyor at time ET.
C        C
...
C              WRITE (*,*) 'The emission and phase angles should '//
C             .            'should be equal.'
C
C              WRITE (*,*) ' '
...
C              WRITE (*,*) 'The solar incidence and phase angles '//
C             .            'should be equal.'
C
C              END
C
...
C
C
C         UTC epoch is 2003 OCT 13 06:00:00
C
C         Illumination angles at the sub-solar point:
C
C         Phase angle           (deg.):    138.36942994721122
C         Solar incidence angle (deg.):    7.1119364739076138E-015
C         Emission angle        (deg.):    138.36942994721122
C
C         The solar incidence angle should be 0.
C         The emission and phase angles should should be equal.
C
C         Illumination angles at the sub-s/c point:
C
C         Phase angle           (deg.):    101.43985783237235
C         Solar incidence angle (deg.):    101.43985783237240
C         Emission angle        (deg.):    5.0087425211137720E-014
C
...
C        arguments TARGET and OBSRVR: calls to BODN2C were replaced by
C        calls to BODS2C.
C
