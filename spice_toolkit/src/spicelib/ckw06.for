...
C     AVFLAG     I   Flag indicating if the segment will contain angular
C                    velocity.
...
C
C
C     1)  If FIRST is greater than LAST, the error
...
C
C     12) If the elements of the array SCLKDP corresponding to a given
...
C
C     15) If an error occurs while writing the output segment, the error
...
C         -Q(I+1).
C
C         If a pair of quaternions violating this condition is found in
C         the input array PACKTS, the error SPICE(BADQUATSIGN) is
C         signaled.
C
C     18) If any element of the input RATES array is non-positive, the
C         error SPICE(INVALIDSCLKRATE) is signaled.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 02-JUN-2021 (JDR)
C
