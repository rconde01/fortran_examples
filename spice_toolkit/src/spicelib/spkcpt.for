...
C
C     2)  If the reference frame OUTREF is unrecognized, an error
C         is signaled by a routine in the call tree of this
C         routine.
C
C     3)  If the reference frame TRGREF is unrecognized, an error is
...
C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'spkcpt_ex1.tm' )
C
C              CHARACTER*(*)         TIMFMT
C              PARAMETER           ( TIMFMT =
...
C
C        Modified code example output format for the solution to fit
