...
C$ Detailed_Input
C
C     STATUS   is a flag that provides the new status. When .TRUE., it
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 26-OCT-2021 (JDR) (NJB)
C
...
C$ Abstract
C
C     Return .TRUE. if an error condition has been signaled via SIGERR.
...
C     has yet signaled an error via SIGERR, or the status
...
C$ Restrictions
C
C     1)  This routine automatically detects errors occurring in
