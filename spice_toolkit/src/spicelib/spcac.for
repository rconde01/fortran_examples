...
C
C              Use the SPICELIB routine DAFOPW to open the file for
C              write access and get HANDLE. Upon exit, this binary file
C              will contain the specified text from the comment file in
C              its comment area, appended to whatever text may already
C              have been stored there. SPCAC will include an extra blank
C              line between the original text and the appended text.
C
...
C$ Exceptions
C
C     1)  If the specified DAF file is not open for write access, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     2)  If there is a problem reading from the comment area of the
C         binary file, the error SPICE(FILEREADFAILED) is signaled.
C
C     3)  If there is a problem writing to the comment area of the
C         binary file, the error SPICE(FILEWRITEFAILED) is signaled.
C
C     4)  If there is a problem reading from the text file, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     5)  If a non-printing ASCII character is encountered in the
C         comments, an error is signaled by a routine in the call tree
C         of this routine.
C
C$ Files
C
C     See arguments HANDLE and UNIT.
C
C$ Particulars
C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 13-AUG-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard.
C
C        Moved the contents of the $Files section to the description of
C        HANDLE and UNIT in $Detailed_Input section, and referred to
C        them from $Files. Removed unnecessary entries from $Revisions
C        section.
C
...
C        longer signaled, since if no EOT is found in the reserved
...
C     and EMARK at the beginning.  Once we have located the markers,
