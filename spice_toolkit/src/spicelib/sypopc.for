...
C     FOUND      O   .TRUE. if the symbol exists, .FALSE. otherwise.
C
C$ Detailed_Input
C
...
C     TABVAL   are the components of a character symbol table.
C
C              On output, the value is removed from the symbol table,
C              and the remaining values associated with the symbol are
C              moved forward in the value table. If no other values are
...
C              symbol NAME.
C
C     FOUND    is .TRUE. if NAME is in the symbol table, otherwise it is
C              .FALSE.
C
C$ Parameters
C
...
C     The call,
C
C        CALL SYPOPC ( 'EINSTEIN', TABSYM, TABPTR, TABVAL,
C       .                                  VALUE,  FOUND  )
C
...
C                        BROWNIAN MOTION
C
C      FOUND is .TRUE., and VALUE is 'NUCLEAR FISSION'. Note that
C      because "FERMI" had only one value associated with it, it was
C      removed from the symbol table.
C
C$ Restrictions
C
C     None.
