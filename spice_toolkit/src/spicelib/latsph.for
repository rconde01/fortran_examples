...
C     SLON       O   Angle of the point from the XZ plane (radians).
C
C$ Detailed_Input
C
...
C$ Detailed_Output
C
C     RHO      is the distance of the point from the origin.
C
C     COLAT    is the angle between the vector from the origin to the
C              point and the positive Z axis in radians. COLAT is
C              computed as PI/2 - LAT.
C
C     SLON     is the angle of the point from the XZ plane (radians).
C              SLON is set equal to LON.
C
C$ Parameters
C
C     None.
...
C
C     1) Co-latitude is obtained by subtracting latitude from HALFPI()
...
C
C
C     2) Compute the latitudinal coordinates of the position of the Moon
C        as seen from the Earth, and convert them to spherical and
...
C
C        C
C        C     Convert the latitudinal coordinates to spherical.
C        C
C              CALL LATSPH ( RADIUS, LON, LAT, R, COLAT, SLON )
C
C        C
...
C
C
C     3) Create a table showing a variety of latitudinal coordinates
C        and the corresponding spherical coordinates.
C
C        Corresponding latitudinal and spherical coordinates are
...
C                 RLAT = LAT(I) * RPD()
C
C                 CALL LATSPH( RADIUS(I), RLON, RLAT, R, COLAT, SLON )
C
C                 WRITE (*,'(6F9.3)') RADIUS(I), LON(I), LAT(I),
C             .                       R, COLAT * DPR(), SLON * DPR()
C
C              END DO
C
C              END
...
C
C
C           RADIUS    LON      LAT        R     COLAT     SLON
C          -------  -------  -------  -------  -------  -------
C            0.000    0.000   90.000    0.000    0.000    0.000
C            1.000    0.000    0.000    1.000   90.000    0.000
C            1.000   90.000    0.000    1.000   90.000   90.000
C            1.000    0.000   90.000    1.000    0.000    0.000
C            1.414  180.000   45.000    1.414   45.000  180.000
C            1.000  -90.000    0.000    1.000   90.000  -90.000
C            1.000    0.000  -90.000    1.000  180.000    0.000
C            1.000   45.000    0.000    1.000   90.000   45.000
C            1.414  180.000  -45.000    1.414  135.000  180.000
C            1.000  180.000   90.000    1.000    0.000  180.000
C            0.000   33.000    0.000    0.000   90.000   33.000
C
C
C$ Restrictions
C
...
C
C        Changed the argument names LONG and LONGS to LON and SLON for
