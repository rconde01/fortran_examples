...
C     time scales are TAI, GPS, TT, TDT, TDB, ET, JED, JDTDB, JDTDT.
C
C$ Disclaimer
C
...
C                 'TT'      Terrestrial Time, identical to TDT.
...
C                 'GPS'     Global Positioning System Time.
C
C              The routine is not sensitive to the case of the
C              characters in INSYS; 'tai' 'Tai' and 'TAI' are all
C              equivalent from the point of view of this routine.
C
...
C            'DELTET/M'
C
C         If these are not present, the error SPICE(MISSINGTIMEINFO) is
C         signaled. (These variables are typically inserted into the
C         kernel pool by loading a leapseconds kernel with the SPICE
C         routine FURNSH.)
C
C     2)  If the names of either the input or output time types are
C         unrecognized, the error SPICE(BADTIMETYPE) is signaled.
C
C$ Files
C
C     None.
...
C     represented by a number) and additive. A numeric time system is
C     additive if given the representations, E1 and E2, of any pair of
C     successive epochs, the time elapsed between the epochs is given by
C     E2 - E1.
C
C     Given an epoch in one of the uniform time scales specified by
C     INSYS, the function returns the equivalent representation in the
C     scale specified by OUTSYS. A list of the recognized uniform time
C     scales is given in the detailed input for INSYS.
C
C$ Examples
C
...
C$ Restrictions
C
C     1)  The appropriate variable must be loaded into the SPICE kernel
C         pool (normally by loading a leapseconds kernel with FURNSH)
...
C$ Version
C
C-    SPICELIB Version 1.5.0, 05-SEP-2020 (EDW) (JDR)
C
C        Added time system name 'TT' (Terrestrial Time) as alternate
C        assignment of 'TDT' (Terrestrial Dynamical Time).
C
C        Included GPS time system mapping.
C
C        Edited the header to comply with NAIF standard.
C
C        Removed references to FURNSH, CLPOOL, KCLEAR, UNLOAD, and
C        Required Reading documents and tutorials from the "variables
...
C
C     Constant shift between GPS time system and TAI time system.
C
      DOUBLE PRECISION      GPSTAI
      PARAMETER           ( GPSTAI = 19.D0 )
...
C
            ...
            CALL SETMSG ( 'The following variables, needed to convert'//
     ...
     .                    'to routines that clear the kernel pool. '  )
...
C
     ...
     .             'TAI, GPS, TT, TDT, JDTDT, TDB, ET, JED, JDTDB. ' //
     .             'At least one of the inputs (#, #) was not in '   //
...
C
C        TAI, GPS, and JDTDT will be converted to TDT
C        JED and JDTDB will be converted to TDB.
C        ET means TDB; TT means TDT.
C
C
      IF      ( MYIN .EQ. 'TAI'   ) THEN
      ...
      ELSE IF ( MYIN .EQ. 'GPS' ) THEN

         MYTIME = MYTIME + (DTA + GPSTAI)
