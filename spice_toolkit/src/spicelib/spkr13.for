...
C         is signaled.
C
C     3)  If any other error occurs while looking up SPK data, the error
...
C$ Restrictions
C
C     1)  This subroutine should not be called directly by a casual
C         user. It is intended for use by the subroutine SPKPVN, and
C         certain tests for error conditions are not performed here, as
C         SPKPVN will have already performed them.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 14-APR-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Updated entry
C        #3 in $Exceptions section. Moved SPK required reading from
