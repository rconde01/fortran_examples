...
C$ Abstract
C 
C     Compute the state of a massless body at time t_0 + DT by applying 
C     the two-body force model to a given central mass and a given body 
C     state at time t_0.
C
C$ Disclaimer
C
...
C$ Exceptions
C
C     1)  If GM is not positive, the error SPICE(NONPOSITIVEMASS) is
C         signaled.
C
C     2)  If the position of the initial state is the zero vector, the
C         error SPICE(ZEROPOSITION) is signaled.
C
C     3)  If the velocity of the initial state is the zero vector, the
C         error SPICE(ZEROVELOCITY) is signaled.
C
...
C         is signaled.
C
C     5)  If DT is so large that there is a danger of floating point
C         overflow during computation, the error SPICE(DTOUTOFRANGE) is
C         signaled and a message is generated describing the problem.
C         The value of DT must be "reasonable". In other words, DT
C         should be less than 10**20 seconds for realistic solar system
C         orbits specified in the MKS system. (The actual bounds on DT
C         are much greater but require substantial computation.) The
C         "reasonableness" of DT is checked at run-time.
C
C$ Files
C
C     None.
...
C
C     1) When the eccentricity of an orbit is near 1, and the epoch
...
C
C        By using this routine, you need not go from a state to
C        elements and back to a state. Instead, you can get the state
C        from an initial state.
C
C        If PVINIT is your initial state and you want the state 3600
...
C           DT = 3600.0D0
C
C           CALL PROP2B ( GM, PVINIT, DT, PVPROP )
C
C        After the call, PVPROP will contain the state of the
C        object 3600 seconds after the time it had state PVINIT.
C
C     2) Use the two-body force model to propagate the state of a
C        massless body orbiting the Earth at 100,000,000 km after half
C        a period.
C
C        In circular two-body motion, the orbital speed is
C
C           s     = sqrt(mu/r)
C
C        where mu is the central mass. After tau/2 = pi*r/s seconds
C        (half period), the state should equal the negative of the
C        original state.
C
C        Example code begins here.
C
C
C              PROGRAM PROP2B_EX2
C              IMPLICIT NONE
C
C        C
...
C              DOUBLE PRECISION      PVINIT ( 6 )
C              DOUBLE PRECISION      R
C              DOUBLE PRECISION      SPEED
...
C        C     Initial values.
C        C
C              MU    =  3.9860043543609598D+05
C              R     =  1.0D+08
C              SPEED =  SQRT( MU / R )
C              T     =  PI( )*R/SPEED
C
C              PVINIT(1) =  0.0D0
C              PVINIT(2) =  R/SQRT(2.0D0)
C              PVINIT(3) =  R/SQRT(2.0D0)
C              PVINIT(4) =  0.0D0
C              PVINIT(5) = -SPEED/SQRT(2.0D0)
C              PVINIT(6) =  SPEED/SQRT(2.0D0)
C
C        C
C        C     Calculate the state of the body at 0.5 period
C        C     after the epoch.
C        C
C              CALL PROP2B ( MU, PVINIT, T, STATE )
C
C        C
C        C     The `state' vector should equal -pvinit
C        C
C              WRITE(*,*) 'State at t0:'
C              WRITE(*,'(A,3F17.5)') '   R   (km):',
C             .                 PVINIT(1), PVINIT(2), PVINIT(3)
C              WRITE(*,'(A,3F17.5)') '   V (km/s):',
C             .                 PVINIT(4), PVINIT(5), PVINIT(6)
C
C              WRITE(*,*) ' '
C              WRITE(*,*) 'State at tau/2:'
C              WRITE(*,'(A,3F17.5)') '   R   (km):',
C             .                    STATE(1), STATE(2), STATE(3)
C              WRITE(*,'(A,3F17.5)') '   V (km/s):',
C             .                    STATE(4), STATE(5), STATE(6)
C
C              END
C
C
...
C
C         State at t0:
C           R   (km):          0.00000   70710678.11865   70710678.11865
C           V (km/s):          0.00000         -0.04464          0.04464
C
C         State at tau/2:
C           R   (km):         -0.00000  -70710678.11865  -70710678.11865
C           V (km/s):          0.00000          0.04464         -0.04464
...
C$ Restrictions
C
C     1)  Users should be sure that GM, PVINIT and DT are all in the
C         same system of units ( for example MKS ).
C
C$ Literature_References
C
C     [1]  J. Danby, "Fundamentals of Celestial Mechanics," 2nd Edition,
C          pp 168-180, Willman-Bell, 1988.
C
...
C     NEWEST(2)  contains the next most recently input initial
C                conditions etc.
C
...
C
     ...
     .           .AND.   LCOUNT .LT. MOSTC )
         ...
         END IF
C
C        As soon as the bracketing values move away from
