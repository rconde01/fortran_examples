...
C$ Abstract
C
C     Apply a rotation of ANGLE radians about axis IAXIS to a matrix.
C     This rotation is thought of as rotating the coordinate system.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     M1       is a 3x3 matrix to which a rotation is to be applied. In
C              matrix algebra, the components of the matrix are relevant
C              in one particular coordinate system. Applying ROTMAT
C              changes the components of M1 so that they are relevant to
C              a rotated coordinate system.
C
C     ANGLE    is the angle in radians through which the original
C              coordinate system is to be rotated.
C
C     IAXIS    is the index for the axis of the original coordinate
C              system about which the rotation by ANGLE is to be
C              performed. IAXIS = 1,2 or 3 designates the X-, Y- or
C              Z-axis, respectively.
C
C$ Detailed_Output
C
C     MOUT     is the matrix resulting from the application of the
C              specified rotation to the input matrix M1. If
C
C                 [ANGLE]
C                        IAXIS
C
C              denotes the rotation matrix by ANGLE radians about IAXIS,
C              (refer to the routine ROTATE) then MOUT is given by the
C              following matrix equation:
C
C                 MOUT = [ANGLE]      * M1
C                               IAXIS
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 27-MAY-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Reformatted
