...
C
C     [1]  "Sun FORTRAN Programmer's Guide", Sun Microsystems, Rev. A, 6
C          May 1988.
C
C     [2]  "Microsoft Fortran Optimizing Compiler User's Guide",
C          Microsoft Corporation, 1987.
C
C     [3]  "Lahey F77L EM/32 Programmers Reference Manual", version 4.0,
C          p 144.
C
C     [4]  "Language Systems FORTRAN Reference Manual", Language Systems
C          Corporation, p 12-7.
C
C     [5]  "FORTRAN/9000 Reference HP 9000 Series 700 Computers", 1st
C          Edition, June 1991, Hewlett Packard Company, page 5-110.
C
...
C$ Version
C
C-    SPICELIB Version 9.0.2, 25-NOV-2021 (JDR)
C
C        Edited the header of DAFAH umbrella routine and all its entry
C        points to comply with NAIF standard.
C
C        Added DAFOPR, DAFOPW, DAFCLS and DAFHSF header examples.
C
C        Corrected spelling mistakes.
C
...
C        tested after each write statement which is equivalent to the
...
C        file is closed when this error is signaled.
C
...
C        read access.
C
C-    SPICELIB Version 3.2.0, 06-OCT-1992 (HAN)
C
...
C
C-    SPICELIB Version 1.1.0, 05-NOV-1990 (HAN)
C
...
C         access, an error is signaled by a routine in the call
C         tree of this routine.
C
C     3)  If the specified file has already been opened by a non-DAF
C         routine, an error is signaled by a routine in the call
C         tree of this routine.
C
...
C         signaled by a routine in the call tree of this routine.
C
C     8)  If the file does not exist, an error is signaled by a routine
...
C         routine.
C
C     11) If the file was transferred improperly via FTP, an error is
C         signaled by a routine in the call tree of this routine.
C
C     12) If the file utilizes a binary file format that is not
C         currently supported on this platform, an error is signaled by
...
C
C
C     1) In the following code fragment, DAFOPR is used to open a file,
C        which is then searched for DAFs containing data for a
C        particular object.
C
...
C
C
C     2) Use a simple routine to output the double precision and integer
...
C
C
C        Note, the final entries in the integer array contain the
...
C$ Version
C
C-    SPICELIB Version 8.1.2, 25-NOV-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C        Updated code example with IMPLICIT NONE, and declarations of
C        SUM variable and MAXSUM parameter.
C
C        Corrected minor typos in header.
C
...
C-    SPICELIB Version 8.1.0, 02-APR-2002 (FST)
C
C        This routine was updated to accommodate changes to the
...
C        SPICE(NOTADAFFILE) is signaled to suggest that a common error
...
C$ Detailed_Input
C
C     FNAME    is the name of a DAF to be opened with write access.
C
C$ Detailed_Output
C
...
C$ Exceptions
C
C     1)  If the specified file has already been opened, either by the
C         DAF routines or by other code, an error is signaled by a
C         routine in the call tree of this routine. Note that this
C         response is not paralleled by DAFOPR, which allows you to open
C         a DAF for reading even if it is already open for reading.
C
C     2)  If the specified file cannot be opened without exceeding the
C         maximum number of files, the error SPICE(DAFFTFULL) is
C         signaled.
C
C     3)  If the attempt to read the file's file record fails, the error
C         SPICE(FILEREADFAILED) is signaled.
C
C     4)  If the specified file is not a DAF file, an error is signaled
...
C
C     6)  If the file does not exist, an error is signaled by a routine
C         in the call tree of this routine.
C
C     7)  If an I/O error occurs in the process of opening the file, the
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     8)  If the file name is blank or otherwise inappropriate, an error
C         is signaled by a routine in the call tree of this routine.
C
C     9)  If the file was transferred improperly via FTP, an error is
C         signaled by a routine in the call tree of this routine.
C
C     10) If the file utilizes a non-native binary file format, an error
C         is signaled by a routine in the call tree of this routine.
C
C$ Files
C
...
C$ Restrictions
C
C     1)  Only files of the native binary file format may be opened
C         with this routine.
C
...
C        code example. Updated $Index_Entries.
C
C-    SPICELIB Version 8.1.1, 10-OCT-2012 (EDW)
C
C        Corrected ordering of header section.
...
C        of the impact of the changes made for this version.
C
C-    SPICELIB Version 2.0.0, 24-JAN-1991 (NJB) (WLT)
C
C        DAFOPW now allows multiple files to be open for writing.
C
C        This routine was updated so that it now keeps current the set
...
C         SPICE(DAFINVALIDPARAMS) is signaled.
C
C     3)  If
C
C            ND + ( NI + 1 ) / 2   >  125
C
C         the error SPICE(DAFINVALIDPARAMS) is signaled.
C
C     4)  If the number of records to be reserved is not zero or
C         positive, the error SPICE(DAFNORESV) is signaled.
C
C     5)  If an I/O error occurs in the process of opening the file,
C         the error is signaled by a routine in the call tree of this
C         routine.
C
C     6)  If (for some reason) the initial records in the file cannot
C         be written, the error SPICE(DAFWRITEFAIL) is signaled.
C
C     7)  If no logical units are available, an error is
C         signaled by a routine in the call tree of this routine.
C
...
C     routine DAFONW. NAIF supports this routine only to provide
...
C
C     2)  If this routine is used to close a HANDLE not associated
C         with a DAF, an error is signaled by a routine in the call tree
C         of this routine.
C
C$ Files
C
...
C
C
C     1) In the following code fragment, the arrays in a file are
...
C        This makes it possible to examine files that have been opened
C        for use by other modules without interfering with the operation
C        of those routines.
C
C
C     2) Use a simple routine to output the double precision and integer
...
C
C
C         Number of d.p. words is      2098004
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 8.0.2, 25-NOV-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Changed example
C        code to hardcode the DAF file used as input.
C
...
C$ Exceptions
C
C     1)  If any error occurs while attempting to fetch a logical unit,
C         the error is signaled by a routine in the call tree of this
C         routine. The value of UNIT in this case is undefined.
C
C$ Files
C
C     None.
...
C$ Detailed_Output
C
C     FNAME    is the name of the file.
C
C$ Parameters
C
C     None.
...
C$ Exceptions
C
C     1)  If the specified name does not specify any file currently
C         known to be loaded as a DAF, the error SPICE(DAFNOSUCHFILE) is
C         signaled. The value of HANDLE is undefined in this case.
C
C     2)  If the file does not exist, an error is signaled by a routine
C         in the call tree of this routine. The value of HANDLE is
C         undefined in this case.
C
C     3)  If an I/O error is generated in the process of connecting the
C         specified name with a handle, the error is signaled by a
C         routine in the call tree of this routine. The value of
...
C
C        A RETURN statement was added after the error signaled when
...
C     DAF operations on handles cause errors to be signaled if
...
C     an open DAF, the error SPICE(DAFNOSUCHHANDLE) is signaled.
C
C     This routine allows subroutines to test file handles for
C     validity before performing operations on them, such as
C     finding the name of the file designated by a handle. Many
...
C         signaled an error (we're presuming that this code
