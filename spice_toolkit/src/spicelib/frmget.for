...
C         be set to .FALSE., OUTFRM will be set to zero and XFORM will
C         be set to the zero 6x6 matrix.
C
C     2)  If the class of the requested frame is not recognized the
C         exception, the error SPICE(UNKNOWNFRAMETYPE) is signaled.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 5.0.0, 15-MAR-2021 (NJB) (JDR)
C
C        **Updated shadow routines ZZFRMGT0 and ZZFRMGT1, as must be
C          done each time this routine is updated.**
C
C        Support for switch frames was added. VERSN is now
C        initialized via a DATA statement. Corrected long error
C        message to use the term "class" rather than "class id-code."
C
...
C
C     On the first pass, initialize the identity matrix.
C
      IF ( PASS1 ) then

         CALL CLEARD( 36, IDNT66 )
...
C
      ...
      ELSE IF ( TYPE .EQ. SWTCH ) THEN

         CALL ZZSWFXFM ( INFRM, ET, 6, XFORM, OUTFRM, FOUND )
      ...
      ELSE
         ...
         CALL SETMSG ( 'The reference frame # has class #. '
