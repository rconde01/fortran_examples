...
C              are included, and an error is signaled.
C
C$ Parameters
C
C     None.
...
C         sets, the error SPICE(ELEMENTSTOOSHORT) is signaled.
C
C$ Files
C
C     None.
...
C
C        Error signaled if output set elements are not long enough.
...
C
C        Otherwise, stop following the array, but continue to count the
