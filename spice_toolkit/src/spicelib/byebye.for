...
C$ Detailed_Input
C
C     STATUS   is a character string which indicates the status
...
C
C     In the case of Microsoft FORTRAN, stopping with an exit status
C     works only if the status is given as a literal value. A constant
C     parameter will not do the job. For example is you want to
C     issue a STOP with the status value 1, you cannot use a variable
C     or parameter as in
C
C            INTEGER               FAILUR
C            PARAMETER           ( FAILUR = 1 )
C
C            STOP FAILUR
C
C     instead you must code the STOP command as:
C
C            STOP 1
C
C
...
C$ Version
C
C-    SPICELIB Version 2.16.0, 13-AUG-2021 (JDR)
C
