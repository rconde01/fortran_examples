...
C$ Exceptions
C
C     1)  If an invalid value of the operation OP is supplied, the error
C         SPICE(INVALIDOPERATION) is signaled.
C
C     2)  If OP is 'SET' and the input argument ACTION does not indicate
C         a valid error handling action, the error SPICE(INVALIDACTION)
C         is signaled.
C
C$ Files
C
...
C
C        We've set the error action, or signaled an error.
C
...
C
C     We've performed the requested operation, or signaled an
C     error.
C
      CALL CHKOUT ( 'ERRACT' )
      RETURN
