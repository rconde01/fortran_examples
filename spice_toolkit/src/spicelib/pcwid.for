...
C
C     The function returns the index of the rightmost printable
C     character in the entire array of strings ARRAY.
C
C$ Detailed_Input
C
...
C
C     The function returns the index of the rightmost printable
C     character in the entire array. This is equivalent to the maximum
C     value of LASTPC for the array, but somewhat more efficient to
C     compute. If NELT is not greater than zero, PCWID is zero.
C
C     ASCII characters in the range [33,126] are considered printable.
C     Blanks are not considered printable. Thus, for character arrays
C     padded with blanks, PCWID is equivalent to NBWID.
C
C$ Parameters
C
C     None.
