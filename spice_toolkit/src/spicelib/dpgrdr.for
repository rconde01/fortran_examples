...
C     rectangular to planetographic coordinates.
C
C$ Disclaimer
C
...
C
C     5)  If the flattening coefficient is greater than or equal to
C         one, the error SPICE(VALUEOUTOFRANGE) is signaled.
C
C     6)  If the input point is on the Z-axis (X = 0 and Y = 0), the
C         Jacobian matrix is undefined, an error is signaled by a
