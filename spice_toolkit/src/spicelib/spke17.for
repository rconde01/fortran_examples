...
C$ Abstract
C
C     Evaluate a single SPK data record from a segment of type 17
...
C
C                    RECIN (3)  is the value of H at the specified
C                               epoch. ( E*SIN(ARGP+NODE) ).
C
C                    RECIN (4)  is the value of K at the specified epoch
C                               ( E*COS(ARGP+NODE) ).
C
C                    RECIN (5)  is the mean longitude (MEAN0+ARGP+NODE)
C                               at the epoch of the elements.
C
C                    RECIN (6)  is the value of P
C                               (TAN(INC/2)*SIN(NODE)) at the specified
C                               epoch.
C
C                    RECIN (7)  is the value of Q
C                               (TAN(INC/2)*COS(NODE)) at the specified
C                               epoch.
C
C                    RECIN (8)  is the rate of the longitude of periapse
C                               (dARGP/dt + dNODE/dt ) at the epoch of
C                               the elements. This rate is assumed to
C                               hold for all time.
C
...
C         SPICE(BADSEMIAXIS) is signaled.
C
C$ Files
C
C     None.
