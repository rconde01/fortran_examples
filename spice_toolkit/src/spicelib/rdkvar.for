...
C     TABSYM,
C     TABPTR,
C     TABVAL   are the components of a double precision symbol table. On
C              input, the table may or may not contain any variables.
C
C$ Detailed_Output
C
C     TABSYM,
...
C              directive, the values in the file may replace or augment
C              any existing values.
C
C      NAME    is the name of the variable. NAME is blank if no variable
C              is read.
C
...
C         error SPICE(DATEEXPECTED) is signaled.
C
C     2)  If an error occurs parsing a numeric value from the kernel
C         file, the error SPICE(NUMBEREXPECTED) is signaled.
C
C$ Files
C
...
C        the character string values placed into it.
C
