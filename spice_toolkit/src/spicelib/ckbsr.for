...
C         is signaled.
C
C     2)  See entry points CKLPF, CKUPF, CKBSS, and CKSNS for
C         exceptions specific to them.
C
C$ Files
C
...
C        CKHAVE      Determine whether or not any CKs are loaded.
C
...
C     segments in CK files matching certain criteria, which is
C     established based on CKBSS input arguments INST, SCLKDP, TOL and
C     NEEDAV. These two routines are used together to search through
C     all loaded CK files for segments.
C
...
C
C     1) Suppose that pointing data for the Viking Orbiter 2 scan
C        platform orientation for a certain interval of time are
C        contained in three separate files, one containing data for the
C        original SEDR (Supplemental Experiment Data Record) files,
C        contains the complete set of pointing data and another two
C        which contain two separate pointing updates based on
C        reconstruction, one of them containing discrete data, and the
C        other continuous pointing data.
C
C        In the following example, pointing from the C-kernel is
C        extracted in two different ways for the purpose of comparing
C        the two updates:
C
C        First, the original pointing file and one of the update files
C        are both loaded and pointing is retrieved for all of the
C        pictures. The update file is searched through first, and if
C        no data for the desired picture is located, then the original
C        file provides the requested pointing.
C
C        Then, the first update file is unloaded, the second update
C        file is loaded, and the same search is performed, as above.
C
C
C        Use the CK kernel below to load the Viking Orbiter 2 scan
C        platform orientation containing a combination of data from
C        SEDR files.
C
C           vo2_sedr_ck2.bc
C
C
C        Use the CK kernel below to load the Viking Orbiter 2 scan
C        platform orientation discrete data reconstructed during
C        cartographic image registration by S. Wu, USGS.
C
C           vo2_swu.bc
C
C
C        Use the CK kernel below to load the Viking Orbiter 2 scan
C        platform orientation discrete data reconstructed during
C        cartographic image registration by S. Wu, USGS, where each
C        discrete pointing instance was "expanded" into a 2 second
C        window.
C
C           vo2_swu_ck2.bc
C
...
C              ---------                     --------
C              vo2_fict.tsc                  Viking 2 SCLK
C              naif0012.tls                  Leapseconds
C
C
C           \begindata
C
C              KERNELS_TO_LOAD = ( 'vo2_fict.tsc',
C                                  'naif0012.tls'  )
C
C           \begintext
C
...
C
C              CHARACTER*(TIMLEN)    FDS      ( NPICS )
C              CHARACTER*(25)        FNAME
C              CHARACTER*(40)        SEGID
C              CHARACTER*(16)        OUTFDS
C              CHARACTER*(14)        TOLSTR
...
C
C        C
C        C     Set the times for the pictures.
C        C
C              DATA                  FDS /
C             .                 '1976 OCT 31 22:07:21.000',
C             .                 '1977-JAN-09 18:33:13.707',
C             .                 '1977 APR 24 11:48:05.000',
C             .                 '1977 JUN 07 00:13:15.000',
C             .                 '1977-AUG-07 14:55:12.019' /
C
C
C              UDFILE ( 1 ) = 'vo2_swu.bc'
C              UDFILE ( 2 ) = 'vo2_swu_ck2.bc'
C
C        C
C        C     The NAIF integer ID codes for the Viking Orbiter 2
C        C     spacecraft and scan platform on Viking Orbiter 2 are
C        C     -30 and -30000, respectively.
C        C
C              SC           = -30
C              INST         = -30000
C
C        C
C        C     Load the LSK and Viking 2 SCLK files.
C        C
C              CALL FURNSH ( 'ckbsr_ex1.tm' )
C
C        C
C        C     Allow a time tolerance of 500 milliseconds.  Convert
C        C     the tolerance to 'ticks', the units of encoded
C        C     spacecraft clock time.
C        C
C              TOLSTR  = '0.500'
...
C        C
C              CALL CKLPF ( 'vo2_sedr_ck2.bc', HNORIG )
C
C        C
C        C     Write banner.
C        C
C              WRITE(*,'(A)') '     Input UTC time       '
C             .            // 'Pointing found in      SCLK time'
C              WRITE(*,'(A)') '------------------------  '
C             .            // '-----------------  ----------------'
C
C              DO UPDATE = 1, 2
...
C
C                       CALL CKPFS ( HANDLE, DESCR,  SCLKDP,
C             .                      TOL,    NEEDAV, CMAT,
C             .                      AV,     CLKOUT, PFOUND )
C
C                       IF ( PFOUND ) THEN
...
C        C                 pointing instance came, decode the
C        C                 spacecraft clock time associated with the
C        C                 instance, and write the results to the
C        C                 table.
C        C
C                          CALL DAFHFN ( HANDLE, FNAME          )
C                          CALL SCDECD ( SC,     CLKOUT, OUTFDS )
C
C                          WRITE(*,'(A,2X,A17,2X,A)') FDS( I ), FNAME,
C             .                                       OUTFDS
C
C                       ELSE
C
...
C
C        C
C        C        Unload the update file. The original file stays
C        C        loaded.
C        C
C                 CALL CKUPF  ( HUPDT )
C
...
C
C
C             Input UTC time       Pointing found in      SCLK time
C        ------------------------  -----------------  ----------------
C        1976 OCT 31 22:07:21.000  vo2_sedr_ck2.bc    1/0026345241.000
C        1977-JAN-09 18:33:13.707  vo2_swu.bc         1/0032380394.707
C        1977 APR 24 11:48:05.000  vo2_sedr_ck2.bc    1/0041428086.000
C        1977 JUN 07 00:13:15.000  pointing not found in any file.
C        1977-AUG-07 14:55:12.019  vo2_sedr_ck2.bc    1/0050511313.019
C
C        1976 OCT 31 22:07:21.000  vo2_sedr_ck2.bc    1/0026345241.000
C        1977-JAN-09 18:33:13.707  vo2_swu_ck2.bc     1/0032380394.707
C        1977 APR 24 11:48:05.000  vo2_sedr_ck2.bc    1/0041428086.000
C        1977 JUN 07 00:13:15.000  pointing not found in any file.
C        1977-AUG-07 14:55:12.019  vo2_swu_ck2.bc     1/0050511313.019
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 5.1.0, 25-OCT-2021 (JDR) (BVS) (NJB)
C
C        Increased ITSIZE (from 100 to 5000).
C
C        Updated entry point CKSNS to always initialize FOUND.
C
C        Edited the header of umbrella routine CKBSR, and all entry
C        points to comply with NAIF standard. Created complete code
C        example from existing fragments in CKBSR $Examples section.
C
C        Added references to CKHAVE entry point in CKBSR header.
C
C        Moved details related to search criteria and conditions to meet
C        it from the $Detailed_Input to $Particulars.
C
...
C        identifiers ( STIDNT ) was changed from 56 to 40.
C
...
C        1) When loading a file, CKLPF now checks if the file table is
C           full only after determining that the file is not currently
C           loaded. Previously, if the file table was full and an
C           attempt was made to reload a file, an error was signaled. A
C           new exception was added as a result of this change.
C
C        2) A bug in the way that CKLPF and CKUPF clean up the
C           instrument tables after a file is unloaded was fixed.
C
C        3) Variable declarations were added to the example program
C           so that it can now be compiled.
C
...
C$Procedure CKLPF ( CK, load pointing file )
...
C$ Exceptions
C
C     1)  If an attempt is made to load more DAF files than is
C         specified by the parameter FTSIZE in DAF system, an error
...
C         by the parameter FTSIZE in the CK subsystem, and if the DAF
C         system has room to load another file, the error
C         SPICE(CKTOOMANYFILES) is signaled. The current setting of
C         FTSIZE does not allow this situation to arise: the DAF
C         system will trap the error before this routine has the
C         chance.
C
...
C$ Version
C
C-    SPICELIB Version 5.0.2, 25-OCT-2021 (JDR)
C
C        Updated the header to comply with NAIF standard. Extended
...
C-    SPICELIB Version 2.0.0, 25-NOV-1992 (JML)
C
C        When loading a file, CKLPF now checks if the file table is
C        full only after determining that the file is not currently
C        loaded. Previously, if the file table was full and an attempt
C        was made to reload a file, an error was signaled. A new
C        exception was added as a result of this change.
C
C        A bug in the way that CKLPF and CKUPF clean up the instrument
C        tables after a file is unloaded was fixed.
C
...
C$Procedure CKUPF ( CK, Unload pointing file )
...
C$ Detailed_Input
C
C     HANDLE   is the integer handle assigned to the CK file upon
C              loading.
C
C$ Detailed_Output
C
...
C$ Version
C
C-    SPICELIB Version 4.3.2, 16-JUL-2020 (JDR)
C
...
C-    SPICELIB Version 2.0.0, 25-NOV-1992 (JML)
C
C        1) A bug in the way that CKLPF and CKUPF clean up the
C           instrument tables after a file is unloaded was fixed.
C
...
C$Procedure CKBSS ( CK, begin search for segment )
...
C$ Version
C
C-    SPICELIB Version 4.1.2, 13-JAN-2021 (JDR)
C
...
C        it from the $Detailed_Input to $Particulars.
C
C        Fixed typo in the long error message ('needs must' -> 'must').
C
...
C
      IF ( NFT .EQ. 0 ) THEN
         ...
         CALL SETMSG ( 'At least one CK file must be loaded ' //
...
C     buffering segment descriptors, identifiers, and handles in the
...
C$ Version
C
C-    SPICELIB Version 4.6.0, 13-OCT-2021 (JDR) (NJB)
C
C        Edited the header to comply with NAIF standard.
C
C        Relocated initialization of FOUND so it is always
C        executed, even if an error state is indicated by RETURN().
C
...
C$Procedure CKHAVE ( CK --- Are there any loaded? )
...
C     specific arithmetic implementation.
C
C     1) Suppose you want to call one of the C-kernel readers, but wish
C        to handle the exceptional case of "no kernels loaded" so that
C        the SPICE exception handling mechanism is avoided in the case
C        of an empty set of loaded kernels. The code example below
C        shows how you might do this.
C
C
C        Example code begins here.
C
C
C              PROGRAM CKHAVE_EX1
C              IMPLICIT NONE
C
C        C
C        C     Local variables.
...
C
C        C
C        C        Here we could be calling CK readers. For this
C        C        example, just display a message indicating that
C        C        at least one CK has been loaded.
C        C
C                 WRITE(*,*) 'At least one CK has been loaded. '
C             .          //  'You may call CK readers.'
C
C              ELSE
C
C                 WRITE(*,*) 'No CK has been loaded.'
C                 WRITE(*,*) 'Calling CK readers will result in an '
C             .          //  'error.'
C
C              END IF
C
...
C
C
C         No CK has been loaded.
C         Calling CK readers will result in an error.
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 4.0.4, 16-JUL-2020 (JDR)
C
C        Edited the header to comply with NAIF standard. Added complete
C        code example based on existing example code fragment.
C
