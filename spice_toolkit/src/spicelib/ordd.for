...
C$ Detailed_Input
C
C     ITEM     is a double precision value to be located within a set.
C
...
C     Ordinal positions range from 1 to N, where N is the cardinality of
C     the set.
C
...
C         by a routine in the call tree of this routine. ORDD returns an
C         unspecified value in this case.
C
C     2)  If the input set has invalid size, an error is signaled by a
C         routine in the call tree of this routine. ORDD returns an
C         unspecified value in this case.
C
C$ Files
C
