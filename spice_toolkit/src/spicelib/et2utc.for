...
C
C     3)  If one of the ISO formats is specified (ISOC or ISOD) but the
C         year corresponding to ET is prior to 1 A.D. on the Gregorian
C         Calendar, the error SPICE(YEAROUTOFRANGE) is signaled.
C
...
C         Julian calendar see the SPICELIB routine GR2JUL.
C
...
C         this routine, an error is signaled by a routine in the
C         call tree of this routine.
C
C$ Files
C
C     A leapseconds kernel must be loaded via FURNSH prior to calling
C     this routine. The kernel need be loaded only once during a program
C     run.
C
C$ Particulars
C
...
C
C     [1]  J. Jespersen and J. Fitz-Randolph, "From Sundials to Atomic
C          Clocks, Understanding Time and Frequency," Dover
C          Publications, Inc. New York, 1982.
C
...
C$ Version
C
C-    SPICELIB Version 3.0.5, 24-AUG-2021 (JDR)
C
...
C-    SPICELIB Version 2.1.0, 11-JUL-1995 (KRG)
C
C        Removed some potential compile warnings that could be
C        caused by truncation of double precision values to
C        integers through a direct assignment. The direct
C        assignment has been replaced with a call to the
C        intrinsic function IDINT.
C
C-    SPICELIB Version 2.0.1, 10-MAR-1992 (WLT)
C
C        Comment section for permuted index source lines was
...
C        integer and fractional parts of the UTC value
C        corresponding to the input ET value.
C
C           In the case where rounding the double precision UTC
C           time corresponding to the input ET value to PREC
C           decimal places resulted in a carry (to the integer
C           portion of the UTC value), the integer portion of the
C           UTC value was treated correctly, but the fraction was
C           not always rounded correctly. The specific case where
C           the problem occurred was when the input ET value
C           mapped to a UTC time having a fractional part that
C           rounded up to 1.0 when rounded PREC decimal places,
C           but that did not round up to 1.0 when rounded to the
C           nearest PREC+1 decimal places. The set of such
C           fractions can be represented as
C
C             { 1 - EPSILON :      EPSILON  <  5 * ( 10 **
C             -(PREC+1) ) - and
C
C                                  EPSILON  >  5 * ( 10 **
C                                  -(PREC+2) )
...
C
C           then a call to this routine with PREC set to 3 would
C           result in the output
C
...
C
C           On the other hand, if the input ET mapped to the UTC
C           time
C
C              2 JAN 1991 00:34:12.99996,
C
C           then a call to this routine with PREC set to 3 would
C           result in the correct output.
C
C           This error was apparently difficult to generate: it
C           has never been reported by any SPICELIB users, and was
C           eventually discovered by NAIF staff.
C
C        2) The second bug is somewhat less severe, as far as the
C        magnitude of the error is concerned. However, it's
C        easier to generate this error. Namely, in some cases,
C        the fractional part of the input ET value is rounded to
C        PREC SIGNIFICANT DIGITS, rather than to PREC decimal
C        places. The effect of this is that the fraction is
C        occasionally truncated rather than rounded. For example,
C        the ET value equivalent to the UTC string
C
...
C        follows:
C
C           1)  The input ET value, after conversion to `UTC
C               seconds past 2000', is broken up into the sum of a
C               whole number of seconds and a non-negative,
C               fractional number of seconds. The fact that the
C               fractional part is non-negative simplifies the
C               conversion of the fraction.
C
...
C
C           3)  The whole number portion of the time value is
C           passed through the inverse Muller-Wimberly algorithm
C           to obtain years, months, days, hours, minutes, and
C           whole seconds. A small fraction is added to the whole
C           number to prevent round-off error from occurring when
C           divisions are performed.
C
