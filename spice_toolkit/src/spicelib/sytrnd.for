C$Procedure SYTRND (Transpose two values associated with a symbol)
      ...
      SUBROUTINE SYTRND ( NAME, IDX1, IDX2, TABSYM, TABPTR, TABVAL )
...
C              transposed. If this index is not valid (IDX1 < 1 or
C              IDX1 > the dimension of NAME) the symbol table is not
C              modified.
C
C     IDX2     is the index of the second associated value to be
C              transposed. If this index is not valid (IDX2 < 1 or
C              IDX2 > the dimension of NAME) the symbol table is not
C              modified.
C
C     TABSYM,
C     TABPTR,
...
C
            IF ( IDX1 .NE. IDX2 ) THEN
               ...
               CALL SWAPD ( TABVAL(LOCVAL+IDX1-1),
