...
C
C     V1,
C     V2       are two double precision 3-dimensional vectors. Either
C              V1 or V2, or both, may be the zero vector.
C
C              An implicit assumption exists that V1 and V2 are
C              specified in the same reference frame. If this is not
...
C     radians.
C
C     VSEP is strictly non-negative. If either V1 or V2 is the zero
C     vector, then VSEP is defined to be 0 radians.
C
C$ Parameters
C
C     None.
...
C
C           VSEP = 2.0 * ARCSIN ( LENGTH/2.0 )
C
C     This expression becomes increasingly unstable when VSEP gets
C     larger than PI/2 radians or 90 degrees. In this situation (which
C     is easily detected by determining the sign of the dot product of
...
C        C
C              DATA                  V1 / 1.D0,  0.D0,  0.D0,
C             .                           1.D0,  0.D0,  0.D0,
C             .                           3.D0,  0.D0,  0.D0  /
C
C              DATA                  V2 / 1.D0,  0.D0,  0.D0,
C             .                           0.D0,  1.D0,  0.D0,
C             .                          -5.D0,  0.D0,  0.D0  /
C
C        C
...
C
C
C        First vector            :    1.0   0.0   0.0
C        Second vector           :    1.0   0.0   0.0
C        Angular separation (rad):    0.0000000000
C
C        First vector            :    1.0   0.0   0.0
C        Second vector           :    0.0   1.0   0.0
C        Angular separation (rad):    1.5707963268
C
C        First vector            :    3.0   0.0   0.0
C        Second vector           :   -5.0   0.0   0.0
