...
C$ Exceptions
C
C     1)  If there is a problem opening or writing to the binary file,
C         an error is signaled by a routine in the call tree of this
C         routine.
C
C     2)  If there is a problem reading from the text file, the
C         error SPICE(FILEREADFAILED) is signaled.
C
C     3)  If there is a problem opening a scratch file, the error
C         SPICE(FILEOPENERROR) is signaled.
C
C     4)  If there is a problem writing to the scratch file, the
C         error SPICE(FILEWRITEFAILED) is signaled.
C
C$ Files
C
C     See arguments UNIT and BINARY above.
C
C     This routine uses a Fortran scratch file to temporarily store the
C     lines of comments if there are any.
C
C$ Particulars
C
...
C     that's better than signaling an error.
C
