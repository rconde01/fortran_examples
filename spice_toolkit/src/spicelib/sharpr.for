...
C$ Abstract
C
C     Adjust the columns of a matrix that is "nearly" a rotation
C     so that they are numerically unit length and orthogonal,
C     going from left to right in the usual printed presentation 
C     of a matrix.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     ROT      is a 3x3 matrix that is nearly a rotation matrix.
C
C$ Detailed_Output
C
C     ROT      is the input after sharpening the columns.
C
C$ Parameters
C
C     None.
...
C$ Restrictions
C
C     1)  See the $Exceptions section above.
C
...
C$ Index_Entries
C
C     Sharpen the orthogonality of the columns of a rotation
C
C-&
