C$Procedure PCKCOV ( PCK, coverage )
      ...
      SUBROUTINE PCKCOV ( PCKFNM, IDCODE, COVER )
...
C     --------  ---  --------------------------------------------------
C     PCKFNM     I   Name of PCK file.
C     IDCODE     I   Class ID code of PCK reference frame.
C     COVER     I-O  Window giving coverage in PCKFNM for IDCODE.
C
C$ Detailed_Input
C
C     PCKFNM   is the name of a binary PCK file.
C
C     IDCODE   is the integer frame class ID code of a PCK reference
C              frame for which data are expected to exist in the
...
C              coverage found for the reference frame designated by
C              IDCODE in the file PCKFNM.
C
...
C              merged coverage for the reference frame having frame
C              class ID IDCODE. This is the set of time intervals for
C              which data for IDCODE are present in the file PCKFNM,
C              merged with the set of time intervals present in COVER on
C              input. The merged coverage is represented as the union of
C              one or more disjoint time intervals. The window COVER
C              contains the pairs of endpoints of these intervals.
C
C              The interval endpoints contained in COVER are ephemeris
C              times, expressed as seconds past J2000 TDB.
C
...
C
C     3)  If the input file is a binary DAF file of type other than PCK,
C         the error SPICE(INVALIDFILETYPE) is signaled.
C
C     4)  If the PCK file cannot be opened or read, an error is signaled
...
C     the coverage a specified PCK file provides for a specified PCK
C     class reference frame.
C
C$ Examples
C
...
C        C
C              CALL PROMPT ( 'Name of PCK file           > ', PCKFNM )
C
C        C
...
C                 CALL PCKCOV ( PCKFNM, IDS(I), COVER )
C
C        C
...
C        platform, using the LSK file named naif0012.tls, and the PCK
C        file named earth_720101_070426.bpc, the output was:
C
C
C        Name of leapseconds kernel > naif0012.tls
C        Name of PCK file           > earth_720101_070426.bpc
C         ========================================
C         Coverage for reference frame         3000
C
C         Interval:            1
C         Start:    1962 JAN 20 00:00:41.184 (TDB)
C         Stop:     2007 APR 26 00:01:05.185 (TDB)
C
C         ========================================
C
C
...
C                                               binary PCK
C              earth_070425_370426_predict.bpc  Earth predicted
C                                               binary PCK
...
C           \begindata
C
C           KERNELS_TO_LOAD = ( 'earth_070425_370426_predict.bpc',
...
C
C        When this program was executed on a Mac/Intel/gfortran/64-bit
C        platform, using the meta-kernel named pckcov_ex2.tm provided
C        above to find the coverage window for the ITRF93 frame using
C        its ID code, '3000', the output was:
C
C
C        Enter name of metakernel > pckcov_ex2.tm
C        Enter ID code            > 3000
C
C         Coverage for frame         3000
C
C         Interval:            1
C         Start:    1962 JAN 20 00:00:41.184 (TDB)
C         Stop:     2037 JUL 17 00:01:05.183 (TDB)
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 08-OCT-2021 (JDR) (NJB)
C
C        Changed input argument name "PCK" to "PCKFNM" for consistency
C        with other routines.
C
C        Bug fix: added call to FAILED after call to GETFAT.
C
C        Edited the header to comply with NAIF standard.
C        Added examples' solution.
C
C        Corrected short error message in entries #2 and #3 in
C        $Exceptions section.
C
C-    SPICELIB Version 1.1.1, 03-JAN-2014 (NJB) (EDW)
C
...
C
      CALL GETFAT ( PCKFNM, ARCH, KERTYP )
