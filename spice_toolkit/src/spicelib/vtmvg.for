...
C     V2         I   M-dimensional double precision column vector.
C     NROW       I   Number of rows in MATRIX (number of rows in V1.)
C     NCOL       I   Number of columns in MATRIX (number of rows in V2.)
C
...
C$ Restrictions
C
C     1)  Since no error detection or recovery is implemented, the
C         programmer is required to insure that the inputs to this
C         routine are both valid and within the proper range.
C
...
C
C        Edited the header to comply with NAIF standard. Added comments
C        to the code and moved the declaration of each local variable to
C        a separate line.
C
