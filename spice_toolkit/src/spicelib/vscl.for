...
C$ Abstract
C
C     Multiply a scalar and a double precision 3-dimensional vector.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     S        is a double precision scalar used to multiply the vector
C              V1.
C
C     V1       is a double precision 3-dimensional vector, which is to
C              be scaled by S.
C
C$ Detailed_Output
C
C     VOUT     is a double precision 3-dimensional vector containing
...
C$ Particulars
C
C     For each value of the index I from 1 to 3, this subroutine
...
C        to scale a given 3-dimensional vector.
C
C
...
C
C
C              WRITE(*,'(A,3F6.1)') 'Input vector : ', V1
C              WRITE(*,*)
C
C        C
...
C              DO I=1, SETSIZ
C
C                 CALL VSCL ( S(I), V1, VOUT )
C
C                 WRITE(*,'(A,F6.1)')  'Scale factor : ', S(I)
C                 WRITE(*,'(A,3F6.1)') 'Output vector: ', VOUT
C                 WRITE(*,*)
C
C              END DO
C
...
C
C
C        Input vector :    1.0   2.0  -3.0
C
C        Scale factor :    3.0
C        Output vector:    3.0   6.0  -9.0
C
C        Scale factor :    0.0
C        Output vector:    0.0   0.0  -0.0
C
C        Scale factor :   -1.0
C        Output vector:   -1.0  -2.0   3.0
C
C
C$ Restrictions
C
C     1)  The user is responsible for insuring that no floating point
