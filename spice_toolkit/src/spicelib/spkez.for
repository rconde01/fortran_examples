...
C
C     2)  If the loaded kernels provide insufficient data to compute the
...
C         cannot be determined, the error SPICE(UNKNOWNFRAME2) is
C         signaled.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 5.1.1, 16-AUG-2021 (JDR)
C
...
C
C-    SPICELIB Version 2.1.1, 05-AUG-1994 (HAN) (MJS)
C
...
C        compute the state.
C
C        For example, suppose the file GLL.BSP contains segments of SPK
C        data for the Galileo spacecraft (body -77) relative to the
C        Jupiter Barycenter (body 5) over a period of time. If the
C        previous version of SPKEZ was called to compute the geometric
C        state of -77 relative to 5 (or vice versa), a routine that
C        SPKEZ calls, SPKSSB, would signal an error stating that there
C        is insufficient data for computing the state of body 5
C        (relative to 0). Version 1.0.0 of SPKEZ could not compute the
C        requested state even though sufficient data had been loaded.
C
...
C
C        Finally, the $Particulars section was updated. In the previous
C        version, it said that calling SPKEZ was equivalent to calling
C        SPKSSB and SPKAPP.
C
C-&
