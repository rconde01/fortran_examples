...
C$ Restrictions
C
C     1)  The input file must be opened and initialized by RDKNEW prior
...
C$ Version
C
C-    SPICELIB Version 3.7.0, 28-NOV-2021 (BVS)
C
C        Updated for MAC-OSX-M1-64BIT-CLANG_C.
C
C-    SPICELIB Version 3.6.1, 17-JUN-2021 (JDR)
C
C        Edited the header of the RDKER umbrella routine and all its
...
C
C-    SPICELIB Version 1.1.0, 07-DEC-1990 (HAN)
C
...
C         error message when notifying the user that it couldn't read
C         from FILE.
C
...
C     If RDKER is called, an error is signaled.
C
...
C$ Detailed_Input
C
C     KERNEL   is the name of the SPICE ASCII kernel file to be opened
C              and initialized.
C
C$ Detailed_Output
C
...
C$ Version
C
C-    SPICELIB Version 3.0.1, 17-JUN-2021 (JDR)
C
...
C$ Detailed_Output
C
C     LINE     is the next line of data from the kernel file most
C              recently opened by NEWKER. Data lines are non-blank lines
C              which lie between \begindata and \begintext markers.
C              Lines are returned left justified.
C
C     EOF      is .TRUE. when the end of the kernel file has been
C              reached, and is .FALSE. otherwise. The kernel file is
C              closed automatically when the end of the file is reached.
C
C$ Parameters
C
C     None.
...
C$ Restrictions
C
C     1)  The input file must be opened and initialized by NEWKER prior
C         to the first call to RDKDAT.
C
C$ Literature_References
C
...
C$ Detailed_Output
C
C     KERNEL   is the name of the last file supplied via a call to
C              RDKNEW. If no call to RDKNEW have been made KERNEL is
C              returned as a blank. If KERNEL is not sufficiently long
C              to hold th name of the file, the file name will be
C              truncated on the right.
C
C     NUMBER   is the number of the last line in KERNEL returned by a
C              call to RDKDAT. If no call to RDKNEW or RDKDAT have been
C              made NUMBER is returned with the value 0.
C
C$ Parameters
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 2.1.1, 17-JUN-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Corrected
C        output argument name in $Declarations section (changed FILE to
C        KERNEL)
C
C-    SPICELIB Version 2.1.0, 03-OCT-2005 (EDW)
C
