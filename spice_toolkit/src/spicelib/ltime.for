...
C$ Abstract
C
C     Compute the transmission (or reception) time of a signal at a
C     specified target, given the reception (or transmission) time at a
C     specified observer. Also return the elapsed time between
...
C$ Detailed_Input
C
C     ETOBS    is an epoch expressed as ephemeris seconds past J2000
C              TDB. This is the time at which an electromagnetic signal
C              is "at" the observer.
C
...
C              which way the electromagnetic signal is traveling.
C
C              If the argument list reads as below,
C
C                 ..., OBS, '->', TARG, ...
C
C              the signal is traveling from the observer to the
C              target.
C
...
C                 ..., OBS, '<-', TARG
C
C              the signal is traveling from the target to
C              the observer.
C
...
C$ Detailed_Output
C
C     ETTARG   is the epoch expressed as ephemeris seconds past J2000
C              TDB at which the electromagnetic signal is "at" the
C              target body.
C
...
C         SPICE(BADDIRECTION) is signaled. In this case
...
C         or target is not recognized, an error is signaled
...
C     the signal is traveling, and the time when the signal is
...
C
C
C           signal traveling to spacecraft
C       *  -._.-._.-._.-._.-._.-._.-._.-._.->  *
C
...
C$ Version
C
C-    SPICELIB Version 1.1.3, 26-OCT-2021 (JDR)
C
