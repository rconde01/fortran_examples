...
C              list and the containing list. Only DSK segments for
C              the body designated by TARGET and having surface
...
C
C     These parameters are discussed in the $Particulars section
C     below.
C
C     See the include file
C
...
C
C     2)  If NRAYS is less than 1, the error SPICE(INVALIDCOUNT)
C         is signaled.
C
C     3)  If NSURF is less than 0, the error SPICE(INVALIDCOUNT)
C         is signaled.
C
C     4)  If the input body name TARGET cannot be mapped to an
C         ID code, the error SPICE(IDCODENOTFOUND) is signaled.
C
C     5)  If the input frame name FIXREF cannot be mapped to an
C         ID code, the error SPICE(IDCODENOTFOUND) is signaled.
C
C     6)  If the frame center associated with FIXREF cannot be
C         retrieved, the error SPICE(NOFRAMEINFO) is signaled.
C
C     7)  If the frame center associated with FIXREF is not
...
C        C
C              CALL PROMPT ( 'Enter meta-kernel name >  ', FNAME )
C
C        C
...
C        platform, using as input the meta-kernel dskxv_ex1.tm, the
C        output was:
C
C
C        Enter meta-kernel name >  dskxv_ex1.tm
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 06-AUG-2021 (JDR) (BVS)
C
