...
C     WORK      I-O  Workspace.
C     SPAIXD     O   Double precision component of spatial index.
C     SPAIXI     O   Integer component of spatial index.
C
C$ Detailed_Input
C
...
C
C         the error SPICE(INTINDEXTOOSMALL) is signaled.
C
C     9)  If there is insufficient room to create any of the data
C         structures contained in the spatial index, an error is
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 03-JUN-2021 (JDR) (BVS)
C
C        Edited the header to comply with NAIF standard. Fixed I/O type
C        of arguments WORK, SPAIXD and SPAIXI in $Brief_I/O table.
C
