C$Procedure ZZTEME ( J2000 to TEME at epoch )
      ...
      SUBROUTINE ZZTEME( ET, J2TM, TM2J )

C$ Abstract
C
C     J2000 to TEME, and TEME to J2000, probably.
C
C$ Disclaimer
C
...
C     J2TM       O   The state transformation operator J2000 to TEME.
C     TM2J       O   The state transformation operator TEME to J2000.
C
C$ Detailed_Input
C
...
C$ Detailed_Output
C
C     J2TM       the 6x6 operator that transforms cartesian states from
C                the J2000 to TEME reference frame.
C
C     TM2J       the 6x6 operator that transforms cartesian states from
C                the TEME to J2000 reference frame.
C
C$ Parameters
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 2.0.0, 10-OCT-2021 (EDW)
C
C        Routine now returns TEME to J2000 mapping operator.
C
...
C$ Index_Entries
C
C   transformation J2000 frame to pseudo TEME
C   transformation pseudo TEME to J2000
C
C-&
...
C
C     Compute the TEME to J2000 state transformation, and the
C     inverse.
C
      CALL ZZTWOVXF ( ZJ2000, 3, XJ2000, 1, TM2J )
      CALL INVSTM   ( TM2J,  J2TM )
