...
C$ Detailed_Input
C
C     SIDE     indicates whether the left or right endpoints of the
C              intervals in the window are to be extracted.
C
C                 'L', 'l'       Left endpoints.
C                 'R', 'r'       Right endpoints.
C
C              If SIDE is not recognized, the input window is not
C              changed.
C
C     WINDOW   on input, is a window containing zero or more intervals.
C
C$ Detailed_Output
C
C     WINDOW   on output, is the collection of singleton intervals
C              containing either the left or the right endpoints of the
C              intervals in the original window.
C
C$ Parameters
C
C     None.
...
C         error SPICE(INVALIDENDPNTSPEC) is signaled.
C
C     2)  The cardinality of the input WINDOW must be even. Left
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 14-MAR-2021 (JDR) (NJB)
C
