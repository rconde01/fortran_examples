...
C
C     2)  If OP is 'SET' and an invalid word is detected within the list
C         of error message items LIST, the error SPICE(INVALIDLISTITEM)
C         is signaled.
C
C$ Files
C
...
C$ Restrictions
C
C     1)  The device to which the selected error messages will be
C         written must be selected via ERRDEV; otherwise, messages will
C         be written to the initial default device.
C
C     2)  Only the first 100 characters of LIST are saved.
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 19-APR-2021 (JDR)
C
