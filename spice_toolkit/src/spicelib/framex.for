...
C
C        CALL FRINFO ( FRAME2, CENT, FRCLSS, CLSSID, FOUND )
C
...
C$ Version
C
C-    SPICELIB Version 5.3.0, 26-AUG-2021 (JDR)
C
C        Changed the argument name CLASS to FRCLSS in FRINFO and CCIFRM
C        entry points for consistency with other routines.
C
C        Edited the header of the umbrella routine and all its entry
C        points to comply with NAIF standard. Updated the $Examples
C        section of CCIFRM entry point.
C
C        Updated frame name length from 26 to 32 in example code.
C
C        Increased MAXBFR from 127 to 149 to accommodate additional
C        built-in PCK based frames.
C
C-    SPICELIB Version 5.2.1, 02-FEB-2017 (BVS)
C
C        Shortened one of permuted index entries in CCIFRM.
C
...
C        using hashes instead of ordered array searches for built-in
C        frames.
C
C        Bug fix: CCIFRM entry point logic was corrected to examine the
...
C     provide the index in the kernel frame attribute arrays KNAME,
...
C
C     For the current N0067 NCOUNT equal to 145 (21 inertial + 124
C     non-inertial), such number is 149.
C
      INTEGER               MAXBFR
      PARAMETER           ( MAXBFR = 149 )
...
C     attributes in the built-in frame attributes arrays IDCODE, NAME,
...
C$ Version
C
C-    SPICELIB Version 3.1.1, 02-AUG-2021 (JDR)
C
...
C$ Detailed_Output
C
C     FRNAME   is the name associated with the reference frame. It will
C              be returned left justified.
C
C              If FRCODE is not recognized as the name of a known
C              reference frame, FRNAME will be returned as a blank.
C
C              If FRNAME is not sufficiently long to hold the name, it
C              will be truncated on the right.
C
C              All reference frame names are 26 or fewer characters in
C              length. Thus declaring FRNAME to be CHARACTER*(26) will
C              ensure that the returned name will not be truncated.
C
C$ Parameters
C
C     None.
...
C$ Exceptions
C
C     1)  If FRCODE is not recognized as the name of a known reference
C         frame, FRNAME will be returned as a blank.
C
C     2)  If FRNAME is not sufficiently long to hold the name, it will
...
C-    SPICELIB Version 3.1.1, 02-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Updated maximum
C        frame name length in detailed description of FRNAME argument
C        and changed frame name length from 26 to 32 in example code.
C
C-    SPICELIB Version 3.1.0, 08-AUG-2012 (BVS)
C
C        The routine was updated to be more efficient by using hashes
...
C     FRCLSS     O   the class (type) of the frame
...
C
C     FRCLSS   is the class or type of the frame. This identifies
...
C
C     FOUND    is .TRUE. if CENT, FRCLSS and CCODE are available.
...
C$ Exceptions
C
C     1)  If a frame definition is encountered that does not define a
C         central body for the frame, an error is signaled by a routine
C         in the call tree of this routine.
C
C     2)  If a frame definition is encountered that does not define
C         a class for the frame, an error is signaled by a routine
C         in the call tree of this routine.
C
C     3)  If a frame definition is encountered that does not define a
C         class ID for the frame, an error is signaled by a routine in
...
C
C     1) Given a frame ID, retrieve the SPICE body ID associated with
C        the frame's center, the frame class (or type of the frame),
C        and the ID used for the frame within its class.
C
C          Example code begins here.
C
C
C                PROGRAM FRINFO_EX1
C                IMPLICIT NONE
C
C          C
...
C
C          C
C          C     Retrieve the information for frame ID 13000.
C          C
C                CALL FRINFO ( FRCODE, CENTER, FRCLSS, CLSSID, FOUND )
C
C                IF ( FOUND ) THEN
C
C                   WRITE(*,'(A,I6)') 'Frame center  : ', CENTER
C                   WRITE(*,'(A,I6)') 'Frame class   : ', FRCLSS
C                   WRITE(*,'(A,I6)') 'Frame class ID: ', CLSSID
C
C                ELSE
C
C                   WRITE (*,'(A,I6)') 'There is insufficient data '
C               .                   // 'for frame ', FRCODE
C
C                END IF
C
...
C$ Version
C
C-    SPICELIB Version 4.2.0, 02-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C        Added complete code example from existing fragments.
C
C        Changed the output argument name CLASS to FRCLSS for
...
C              with the object specified by CENT. FRNAME should be
...
C$ Exceptions
C
C     1)  If FRNAME does not have enough room to contain the frame name,
C         the name will be truncated on the right. (Declaring FRNAME to
C         be CHARACTER*(32) will ensure that the name will not be
C         truncated).
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 3.2.1, 13-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Updated maximum frame name length from 26 to 32 in
C        $Detailed_Output and $Exceptions section.
C
...
C$ Version
C
C-    SPICELIB Version 3.2.1, 02-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Updated maximum frame name length from 26 to 32 in
...
C     VARIABLE  I/O  DESCRIPTION
C     --------  ---  --------------------------------------------------
C     FRCLSS     I   Class of frame.
C     CLSSID     I   Class ID of frame.
C     FRCODE     O   ID code of the frame identified by FRCLSS, CLSSID.
C     FRNAME     O   Name of the frame identified by FRCLSS, CLSSID.
C     CENT       O   Center of the frame identified by FRCLSS, CLSSID.
C     FOUND      O   .TRUE. if the requested information is available.
C
C$ Detailed_Input
C
...
C              identified by FRCLSS and CLSSID.
C
C     FRNAME   is the name of the frame identified by FRCLSS and
...
C
C     CENT     is the body ID code for the center of the reference
C              frame identified by FRCLSS and CLSSID.
C
...
C         keyword, the error SPICE(INVALIDFRAMEDEF) is signaled.
C
...
C     specific arithmetic implementation.
C
C     1) The following code example demonstrates how to find the frame
C        information about a frame by its ID using FRINFO and
C        by its class and class ID using CCIFRM.
C
C
C        Example code begins here.
C
C
C              PROGRAM CCIFRM_EX1
C              IMPLICIT NONE
C
C        C
C        C     Local parameters.
C        C
C              INTEGER               FRNLEN
C              PARAMETER           ( FRNLEN = 32 )
C
C        C
C        C     Local variables.
C        C
C              CHARACTER*(FRNLEN)    FRNAME
C
C              INTEGER               CENTR1
C              INTEGER               CENTR2
C              INTEGER               CLSS
...
C
C        C
C        C     Find the frame code associated with ITRF93
C        C
C              FRNAME = 'ITRF93'
C              CALL NAMFRM ( FRNAME, FRCOD1 )
C
C        C
C        C     Get the frame information.
C        C
C              CALL FRINFO ( FRCOD1, CENTR1, CLSS, CLSSID, FOUND )
C
C              IF ( .NOT. FOUND ) THEN
C
C                    WRITE(*,*) 'No info found for frame ', FRCOD1
C                    STOP
C
C              END IF
C
C              WRITE(*,'(A)')    'Frame ITRF93 info:'
C              WRITE(*,'(A,I6)') '   Frame Code: ', FRCOD1
C              WRITE(*,'(A,I6)') '   Center ID : ', CENTR1
C              WRITE(*,'(A,I6)') '   Class     : ', CLSS
C              WRITE(*,'(A,I6)') '   Class ID  : ', CLSSID
C
C        C
C        C     Return the frame name, frame ID, and center associated
C        C     with the frame CLSS and CLSSID.
C        C
C              CALL CCIFRM ( CLSS,   CLSSID, FRCOD2,
C             .              FRNAME, CENTR2, FOUND  )
C
C              IF ( .NOT. FOUND ) THEN
C
C                    WRITE(*,*) 'No info found for type ', CLSS,
C             .                 ' frame ', CLSSID
C                    STOP
C
C              END IF
C
C
C              WRITE(*,'(A,I3,A,I6,A)') 'Type', CLSS, ' frame', CLSSID,
C             .                         ' info:'
C              WRITE(*,'(2A)')   '   Frame name: ', FRNAME
C              WRITE(*,'(A,I6)') '   Frame Code: ', FRCOD2
C              WRITE(*,'(A,I6)') '   Center ID : ', CENTR2
C
C              END
C
C
...
C$ Restrictions
C
C     1)  See item (1) in the $Exceptions section above.
C
C$ Literature_References
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 01-OCT-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C        Added complete code example from existing fragments.
C
C        Changed the input argument name CLASS to FRCLSS for
C        consistency with other routines.
C
C        Updated maximum frame name length from 26 to 32 in
C        $Detailed_Output and $Exceptions section.
...
C
C        Shortened one of permuted index entries.
C
...
C        frames.
C
C        Bug fix: CCIFRM logic was corrected to examine the built-in
