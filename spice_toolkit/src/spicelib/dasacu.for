...
C$ Detailed_Input
C
C     COMLUN   is the Fortran logical unit of a previously opened text
C              file which contains comments that are to be added to
C              the comment area of a binary E-Kernel file.
C
C     BEGMRK   is a marker which identifies the beginning of the
C              comments in the comment text file. This marker must
C              appear on a line by itself, and leading and trailing
...
C
C     ENDMRK   is a marker which identifies the end of the comments in
C              the comment text file. This marker must appear on a line
C              by itself, and leading and trailing blanks are not
C              significant.
C
...
C              text file.
C
C     INSBLN   is a logical flag which indicates whether a blank line is
...
C
C     HANDLE   is the file handle for a binary DAS file that has been
...
C         culled from the text file cannot be opened, the
C         error SPICE(FILEOPENFAILED) is signaled.
C
...
C         error SPICE(MARKERNOTFOUND) is signaled.
C
...
C$ Files
C
C     See parameters COMLUN and HANDLE in the $Detailed_Inputs
C     section.
C
C     A scratch file is used to temporarily hold the comments culled
C     from the comment text file. This is so we do not have to find the
C     place where we started searching for comments in the original
C     file.
C
C$ Particulars
C
...
C
C     'wndrland.dat' is a binary DAS file with an empty comment area
...
C$ Version
C
C-    SPICELIB Version 1.2.2, 02-JUN-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C-    SPICELIB Version 1.2.1, 15-MAR-2017 (NJB)
C
C        Added description of parameter LNSIZE. Fixed typos
C        throughout the comments.
C
C-    SPICELIB Version 1.2.0, 07-JUL-1996 (NJB) (KRG)
C
...
C
C     Beta Version 1.0.0, 04-JAN-1993 (KRG)
C
C-&
...
C$ Revisions
C
C-    SPICELIB Version 1.2.0, 07-JUL-1996 (NJB)
C
C        Removed declaration, DATA and SAVE statements for unused
C        variable FIRST.
C
