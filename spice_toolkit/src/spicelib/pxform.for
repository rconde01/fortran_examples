...
C$ Detailed_Input
C
C     FROM     is the name of a reference frame in which a position
C              vector is known.
C
C     TO       is the name of a reference frame in which it is desired
C              to represent a position vector.
C
C     ET       is the epoch in ephemeris seconds past the epoch of J2000
C              (TDB) at which the position transformation matrix ROTATE
...
C         two frames, an error is signaled by a routine
...
C
C     A common type of reference frame transformation is one from one
C     time-dependent frame to another, where the orientations of the
C     frames are computed at different epochs. For example, a remote
C     sensing application may compute the transformation from a target
C     body-fixed frame, with its orientation evaluated at the epoch of
C     photon emission, to a spacecraft instrument frame, with its
C     orientation evaluated at the epoch of photon reception. The
C     SPICELIB routine PXFRM2 computes this type of frame
C     transformation.
C
C$ Examples
C
...
C
C     The position JPOS is the desired J2000 position of the station.
C
C$ Restrictions
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 09-AUG-2021 (NJB) (JDR)
C
C        Edited the header to comply with NAIF standard. Updated the
C        $Particulars header section to mention PXFRM2.
C
