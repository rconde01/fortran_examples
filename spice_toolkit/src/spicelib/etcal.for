...
C     CALSTR     O   A standard calendar representation of ET.
C
C$ Detailed_Input
C
...
C$ Detailed_Output
C
C     CALSTR   is a calendar string representing the input ephemeris
...
C              CALSTR, it should be declared to have length at
...
C           WRITE (*,*) '   ', CALSTR(1:E1), ' (', ETSTR(1:E2), ')'
C
C$ Restrictions
C
C     1)  One must keep in mind when using this routine that
...
C
C        Added IMPLICIT NONE statement. Changed output argument name
C        STRING to CALSTR for consistency with other routines.
C
C        Edited the header to comply with NAIF standard. Removed
C        unnecessary $Revisions section. Added TIME required reading.
C
...
C     and transfer the results to the output CALSTR when we are
C     done.
C
