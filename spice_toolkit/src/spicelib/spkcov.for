...
C     --------  ---  --------------------------------------------------
C     SPKFNM     I   Name of SPK file.
C     IDCODE     I   ID code of ephemeris object.
C     COVER     I-O  Window giving coverage in SPKFNM for IDCODE.
C
C$ Detailed_Input
C
C     SPKFNM   is the name of an SPK file.
C
C     IDCODE   is the integer ID code of an object for which ephemeris
C              data are expected to exist in the specified SPK file.
C
...
C              merged coverage for IDCODE. This is the set of time
C              intervals for which data for IDCODE are present in the
C              file SPKFNM, merged with the set of time intervals
C              present in COVER on input. The merged coverage is
C              represented as the union of one or more disjoint time
C              intervals. The window COVER contains the pairs of
C              endpoints of these intervals.
C
...
C
C     3)  If the input file is a binary DAF file of type other than SPK,
C         the error SPICE(INVALIDFILETYPE) is signaled.
C
C     4)  If the SPK file cannot be opened or read, an error is signaled
...
C        C
C              CALL PROMPT ( 'Name of SPK file           > ', SPKFNM )
C
C        C
...
C                 CALL SPKCOV ( SPKFNM, IDS(I), COVER )
C
C        C
...
C        platform, using the LSK file named naif0012.tls and the SPK
C        file named mar097.bsp, the output was:
C
C
C        Name of leapseconds kernel > naif0012.tls
C        Name of SPK file           > mar097.bsp
C         ========================================
C         Coverage for object            3
C
C         Interval:            1
C         Start:    1900 JAN 04 00:00:41.184 (TDB)
C         Stop:     2100 JAN 01 00:01:07.183 (TDB)
C
C         ========================================
C         ========================================
C         Coverage for object            4
C
C         Interval:            1
C         Start:    1900 JAN 04 00:00:41.184 (TDB)
C         Stop:     2100 JAN 01 00:01:07.183 (TDB)
...
C
C        When this program was executed on a Mac/Intel/gfortran/64-bit
C        platform, using the meta-kernel named spkcov_ex2.tm provided
C        above to find the coverage window for the Earth using its ID
C        code, '399', the output was:
C
C
C        Enter name of metakernel > spkcov_ex2.tm
C        Enter ID code            > 399
C
C         Coverage for object          399
C
C         Interval:            1
C         Start:    1549 DEC 31 00:00:00.000 (TDB)
C         Stop:     2650 JAN 25 00:00:00.000 (TDB)
C
C
C$ Restrictions
C
...
C
C        Changed input argument name "SPK" to "SPKFNM" for consistency
...
C        examples' solution and meta-kernel for example #2.
C
...
C
      CALL GETFAT ( SPKFNM, ARCH, KERTYP )
