...
C
C     DVEC     is a pointing vector emanating from the observer. The
...
C
C     FOUND    is a logical flag indicating whether or not the ray
...
C     left unchanged.
C
C     1)  If the input argument METHOD is not recognized, an error
...
C
C     3)  If OBSRVR cannot be mapped to an ID code, an error is signaled
C         by a routine in the call tree of this routine.
C
C     4)  If the input argument ABCORR is invalid, an error
C         is signaled by a routine in the call tree of this
C         routine.
C
C     5)  If a body-fixed reference frame associated with the
C         target cannot be found, the error SPICE(NOFRAME) is signaled.
C
C     6)  If OBSRVR and TARGET map to the same NAIF integer ID codes, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     7)  If frame definition data enabling the evaluation of the state
C         of the target relative to the observer in target body-fixed
C         coordinates have not been loaded prior to calling SRFXPT, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     8)  If the specified aberration correction is not recognized, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     9)  If insufficient ephemeris data have been loaded prior to
C         calling SRFXPT, an error is signaled by a
...
C
C     10) If the computation method has been specified as "Ellipsoid"
C         and triaxial radii of the target body have not been loaded
C         into the kernel pool prior to calling SRFXPT, an error is
C         signaled by a routine in the call tree of this routine.
C
C     11) If PCK data needed to define the target body-fixed frame have
C         not been loaded prior to calling SRFXPT, an error is signaled
C         by a routine in the call tree of this routine.
C
C     12) If the reference frame designated by DREF is not recognized
C         by the SPICE frame subsystem, an error is signaled
C         by a routine in the call tree of this routine.
C
C     13) If the direction vector DVEC is the zero vector, an error
...
C
C     1) The following program computes surface intercept points on
C        Mars for the boresight and FOV boundary vectors of the MGS MOC
...
C              ---------                        --------
C              de405s.bsp                       Planetary ephemeris
C              mars_iau2000_v0.tpc              Planet orientation and
C                                               radii
...
C           \begindata
C
C              KERNELS_TO_LOAD = ( 'de405s.bsp',
C                                  'mars_iau2000_v0.tpc',
C                                  'naif0011.tls',
C                                  'mgs_moc_v20.ti',
...
C              DATA                  TARGET / 'Mars'      /
C              DATA                  UTC    /
C             .                '2003 OCT 13 06:00:00 UTC' /
C
C        C
C        C     Load kernel files.
C        C
C              CALL FURNSH ( 'srfxpt_ex1.tm' )
C
C        C
...
C
C           Intercept:
C              Radius                   (km)  =    3384.9411359391133
C              Planetocentric Latitude  (deg) =   -48.477481851561002
C              Planetocentric Longitude (deg) =   -123.47407882634886
C              Range                    (km)  =    388.98310724844424
C
C
C         Corner vector 2
C
...
C
C           Intercept:
C              Radius                   (km)  =    3384.9396987514451
C              Planetocentric Latitude  (deg) =   -48.481636266908055
C              Planetocentric Longitude (deg) =   -123.39882275183645
C              Range                    (km)  =    388.97512489721356
C
C
C         Corner vector 3
C
...
C
C           Intercept:
C              Radius                   (km)  =    3384.9396899058052
C              Planetocentric Latitude  (deg) =   -48.481661836856034
C              Planetocentric Longitude (deg) =   -123.39882595816586
C              Range                    (km)  =    388.97466598000682
C
C
C         Corner vector 4
C
...
C
C           Intercept:
C              Radius                   (km)  =    3384.9411270910964
C              Planetocentric Latitude  (deg) =   -48.477507427894842
C              Planetocentric Longitude (deg) =   -123.47408199055646
C              Range                    (km)  =    388.98264816551176
C
C
C         Boresight vector
C
...
C
C           Intercept:
C              Radius                   (km)  =    3384.9404101835457
C              Planetocentric Latitude  (deg) =   -48.479579751487201
C              Planetocentric Longitude (deg) =   -123.43645374920047
C              Range                    (km)  =    388.97573917648396
C
C
C     2) SRFXPT will signal an error if required kernel data are
...
C
C        We could modify the code of Example 1 as shown below to test
C        for the availability of C-kernel data. We would add the
C        declarations shown, and we'd call the C-kernel reader CKGP to
C        find whether the desired pointing was available. Depending on
C        the value of the FOUND flag returned by CKGP, we'd go on to
C        compute the surface intercept point or respond to the error
C        condition.
C
C
C                        .
...
C        C     MGS spacecraft frame. To do this, we'll need to
C        C     represent our observation epoch in terms of MGS encoded
C        C     SCLK.
C        C
...
C$ Restrictions
C
C     1)  A cautionary note: if aberration corrections are used, and if
...
C         the observer and the *center* of the target body. This light
C         time normally will differ from the light time between the
C         observer and intercept point. Consequently the orientation of
C         the target body-fixed frame at TRGEPC will not match that of
C         the target body-fixed frame at the epoch associated with DREF.
C         As a result, various derived quantities may not be as
C         expected: for example, OBSPOS would not be the inverse of the
C         aberration-corrected position of the target as seen by the
C         observer.
C
C         In many applications the errors arising from this frame
C         discrepancy may be insignificant; however a safe approach is
C         to always use as DREF a frame other than the target body-fixed
C         frame.
C
...
C$ Version
C
C-    SPICELIB Version 1.6.0, 01-NOV-2021 (EDW) (JDR)
C
...
C
C        Updated example #1 to use a meta-kernel to load the required
C        kernels and modified its output format to comply with the
C        maximum line length for header comments.
C
