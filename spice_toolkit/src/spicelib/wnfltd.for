...
C     --------  ---  --------------------------------------------------
C     SMLINT     I   Limiting measure of small intervals.
C     WINDOW    I-O  Window to be filtered.
C
C$ Detailed_Input
C
C     SMLINT   is the limiting measure of the small intervals to be
C              filtered. Intervals of measure less than or equal to
C              SMLINT are removed from the window. The measure SMLINT is
C              signed, and is used as is---the absolute value of SMLINT
...
C
C     2)  If SMLINT is less than or equal to zero, this routine has
...
C     measure less than or equal to the limiting measure (SMLINT).
C
C$ Examples
C
...
C
C        remove any intervals equal to or less than 3.0.
C
C
C        Example code begins here.
C
C
C              PROGRAM WNFLTD_EX1
C              IMPLICIT NONE
C
C        C
...
C
C        C
C        C     Filter the intervals smaller than or equal to 3.0
C        C
C              CALL WNFLTD ( 3.0D0, WINDOW )
C
C        C
C        C     Output the intervals.
C        C
C              WRITE(*,*) ' '
C              WRITE(*,*) 'Window after filtering intervals <= 3.0:'
C
C              DO I= 1, WNCARD( WINDOW )
C
C                 CALL WNFETD ( WINDOW, I, LEFT, RIGHT )
...
C
C         Window after filtering intervals <= 3.0:
C           Interval  1: [  7.000, 11.000 ]
C           Interval  2: [ 23.000, 27.000 ]
C
C
...
C
C           CALL WNFLTD (  0.D0, WINDOW )                           (1)
C           CALL WNFLTD (  2.D0, WINDOW )                           (2)
C           CALL WNFLTD (  3.D0, WINDOW )                           (3)
C
C        produces the following series of windows
C
C           [ 1.0, 3.0 ]   [ 7.0, 11.0 ]  [ 23.0, 27.0 ]            (1)
C                          [ 7.0, 11.0 ]  [ 23.0, 27.0 ]            (2)
C                          [ 7.0, 11.0 ]  [ 23.0, 27.0 ]            (3)
C
C$ Restrictions
C
C     None.
...
C
C        Changed the argument name SMALL to SMLINT for consistency with
C        other routines.
C
...
C        $Exceptions section. Extended SMLINT description in
C        $Detailed_Input.
C
...
C
      ...
      DO WHILE ( J .LE. CARD )
         ...
         IF ( ( WINDOW(J) - WINDOW(J-1) ) .GT. SMLINT ) THEN
