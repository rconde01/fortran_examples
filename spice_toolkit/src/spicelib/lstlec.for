...
C$ Abstract
C
C     Find the index of the largest array element less than or equal to 
C     a given character string in an ordered array of character strings.
C
C$ Disclaimer
C
...
C     is lexically less than or equal to STRING.
C
C$ Detailed_Input
C
C     STRING   is a string acting as an upper bound: the element of
C              ARRAY that is lexically the greatest element less than or
C              equal to STRING is to be found. Trailing blanks in this
C              bound value are not significant.
C
...
C     the input array that is lexically less than or equal to STRING.
C     The routine assumes the array elements are sorted in
C     non-decreasing order.
C
C     Indices range from 1 to N.
C
C     If all elements of ARRAY are lexically greater than STRING, the
...
C           2
C
C     steps to compute the value of LSTLEC.
C
C     Note: If you need to find the first element of the array that is
C     lexically greater than STRING, simply add 1 to the result returned
C     by this function and check to see if the result is within the
C     array bounds given by N.
C
C$ Examples
C
