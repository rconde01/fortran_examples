...
C     (NORAD two-line element sets.). This evaluator uses algorithms 
C     as described in Vallado 2006 [4].
C
C$ Disclaimer
C
...
C              Units are km and km/sec relative to the J2000 
C              reference frame.
C
C$ Parameters
C
C     NGEO     is the number of geophysical constants for SGP4 SPK
C              records.
C
...
C
C     IMPRVD   set the SGP4 propagator to use the improved GST 
C              algorithm as defined in Vallado [4]; value defined in
C              zzsgp4.inc. 
C
C$ Exceptions
C
C     1)  If a problem occurs when evaluating the two-line elements, an
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 10-OCT-2021 (JDR) (EDW)
C
C        Use of modified ZZTEME to eliminate a matrix inversion.
C
...
C     Local variables
C
      DOUBLE PRECISION      J2TM     ( 6, 6 )
      DOUBLE PRECISION      TM2J     ( 6, 6 )
...
C
C     Finally, convert the TEME state to J2000.  First 
C     calculate the mapping from J2000 to TEME (J2TM), and from 
C     TEME to J2000 (TM2J) at time ET...
C
C                         -1
C     Note that J2TM = TM2J
C
      CALL ZZTEME( ET, J2TM, TM2J )

C
C     ...now convert the TEME state to a J2000 state. 
C
      CALL MXVG   ( TM2J, STATE, 6, 6, TMPSTA )
