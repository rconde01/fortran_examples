...
C
C     2)  See the entry points for discussions of their exceptions.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 07-OCT-2021 (NJB) (JDR)
C
C        Added initializers for record buffers.
C
C        Edited the headers of DASRWR and all its entry points to comply
C        with NAIF standard.
C
C        Cleaned up the $Revisions history.
C
...
C
      ...
      INTEGER               BUFNVD
      PARAMETER           ( BUFNVD  = BUFSZD * NWD )

      INTEGER               BUFNVI
      PARAMETER           ( BUFNVI  = BUFSZI * NWI )
...
C
      ...
      DATA                  RCBUFC  / BUFSZC * ' '     /
      DATA                  RCBUFD  / BUFNVD * 0.D0    /
      DATA                  RCBUFI  / BUFNVI * 0       /
...
C         routine in the call tree of this routine. The output argument
C         DATAD will not be modified.
C
C     2)  If a read operation attempted by this routine fails, an error
C         is signaled by a routine in the call tree of this routine. The
C         output argument DATAD will not be modified.
C
C     3)  If a write operation attempted by this routine fails, an error
C         is signaled by a routine in the call tree of this routine. The
C         output argument DATAD will not be modified. This routine may
C         write out updated, buffered records in order to make room in
C         the double precision buffer for a newly read record. Note that
C         the file written to may be different than the file designated
C         by HANDLE if multiple DAS files are open for writing.
C
C     4)  If FIRST or LAST is not in the range [1, NWD], the error
C         SPICE(INDEXOUTOFRANGE) is signaled. The output argument
C         DATAD will not be modified.
C
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 22-FEB-2021 (JDR)
C
...
C         by a routine in the call tree of this routine. The
...
C
C     3)  If a write operation attempted by this routine fails, an
C         error is signaled by a routine in the call tree of this
C         routine. The output argument DATAI will not be modified. This
C         routine may write out updated, buffered records in order to
C         make room in the integer buffer for a newly read record. Note
C         that the file written to may be different than the file
...
C-    SPICELIB Version 2.0.1, 22-FEB-2021 (JDR)
C
C        Updated the header to comply with NAIF standard. Cleaned up
C        the $Revisions history.
C
C-    SPICELIB Version 2.0.0, 05-FEB-2015 (NJB)
C
C        Upgraded to support handle manager integration and
...
C         signaled by a routine in the call tree of this routine. The
...
C     3)  If a write operation attempted by this routine fails, an
C         error is signaled by a routine in the call tree of this
C         routine. The output argument DATAC will not be modified. This
C         routine may write out updated, buffered records in order to
C         make room in the character buffer for a newly read record.
...
C
C     1)  If the input file handle is invalid, an error is signaled by a
C         routine in the call tree of this routine. The DAS file
C         designated by HANDLE will not be modified.
C
C     2)  If a write operation attempted by this routine fails, an error
C         is signaled by a routine in the call tree of this routine. The
C         status of the DAS file written to is uncertain in this case.
C         Note that the file written to may be different than the file
C         designated by HANDLE if multiple DAS files are open for
C         writing.
C
...
C         or any other reason, an error is signaled by a routine in the
C         call tree of this routine. The indicated record will not be
C         modified.
C
C     2)  If a write operation attempted by this routine fails, an error
C         is signaled by a routine in the call tree of this routine. The
...
C         writing.
C
C     3)  If the input file handle is invalid, an error is signaled by a
C         routine in the call tree of this routine. The indicated record
C         will not be modified.
C
C     4)  If FIRST or LAST is not in the range [1, NWD], the error
C         SPICE(INDEXOUTOFRANGE) is signaled. The indicated
...
C$ Abstract
C
C     Write out all buffered records of a specified DAS file.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C     None.
C
C     See $Particulars for a description of the action of this routine.
C
C$ Parameters
C
C     None.
...
C
C     1)  If the input file handle is invalid, an error is signaled
C         by a routine in the call tree of this routine. The indicated
C         file will not be modified.
C
C     2)  If a write operation attempted by this routine fails, an
C         error is signaled by a routine in the call tree of this
C         routine. The status of the DAS file written to is uncertain
C         in this case.
C
C$ Files
C
...
C     they correspond.
C
C     Because the DAS system buffers records that are written as well as
C     those that are read, data supplied to the DAS add data (DASADC,
C     DASADD, DASADI) and DAS update data (DASUDC, DASUDD, DASUDI)
...
C     been opened for writing, the DAS system must write out to the file
C     any updated records present in the DAS buffers. The SPICELIB
C     routine DASCLS uses this routine to perform this function. The
C     routines DASAC and DASDC, through the use of the SPICELIB routines
C     DASACR and DASRCR, which respectively add comment records to or
C     delete comment records from a DAS file, use this routine to ensure
C     that the SPICELIB routine DASRWR record buffers don't become out
C     of sync with the file they operate upon.
C
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 19-MAY-2021 (NJB) (JDR)
C
C        Edited the header to comply with NAIF standard. Added complete
C        code example.
C
C        Updated $Particulars in order to provide information about the
C        high level APIs that actually use this routine.
C
C-    SPICELIB Version 2.0.0, 30-JUL-2014 (NJB)
C
C        Upgraded to support handle manager integration.
