...
C
C     5)  If FIRST is greater than LAST, the error
...
C         SPICE(COVERAGEGAP) is signaled.
C
C     8)  If DLSIZE is greater than the limit
C
C            ( 4 * MAXTRM ) + 11
C
C         the error SPICE(DIFFLINETOOLARGE) is signaled. If
...
C         error SPICE(ZEROSTEP) is signaled. The checked portion
