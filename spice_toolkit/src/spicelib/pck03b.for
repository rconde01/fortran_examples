...
C     None.
C
C     The data are stored in the PCK segment in the DAF
...
C         signaled.
C
C     2)  If there is any error in the structure or content of the
C         inputs other than the degree of the Chebyshev Polynomial, the
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     3)  If a file access error occurs while this routine begins a new
C         type 03 segment, the error is signaled by a routine in the
...
C$ Restrictions
C
C     1)  The binary PCK file must be open with write access.
C
C     2)  Only one segment may be written to a particular PCK file at a
...
C$ Version
C
C-    SPICELIB Version 1.0.2, 03-JUN-2021 (JDR)
C
