...
C              with ID code set found for the file CKFNM.
C
...
C              each object for which pointing data are present in the
C              indicated CK file. The elements of SPICE sets are
C              unique; hence each ID code in IDS appears only once, even
C              if the CK file contains multiple segments for that ID
C              code.
C
...
C         CK, the error SPICE(INVALIDFILETYPE) is signaled.
C
...
C         the indicated CK file, an error is signaled by a routine in
...
C        C     time conversion.  Note that we assume a single
C        C     spacecraft clock is associated with all of the objects
C        C     in the CK.
C        C
...
C        solution using CASSINI data. Corrected short error message in
C        entries #2 and #3 in $Exceptions section.
C
