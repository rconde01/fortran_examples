...
C              cases. The current value for MINCOS is 1.0D-15.
C
C$ Exceptions
C
...
C         signaled.
C
C     3)  If the FOV_SHAPE specified by the instrument kernel is not
C         one of the four values: 'CIRCLE', 'POLYGON', 'ELLIPSE', or
C         'RECTANGLE', the error SPICE(SHAPENOTSUPPORTED) is signaled.
C         If the 'ANGLES' specification is used, FOV_SHAPE must be
...
C         kernel pool is not 3, or they are not numeric, the error
C         SPICE(BADBORESIGHTSPEC) is signaled.
C
C     6)  If the boresight vector is the zero vector, the error 
C         SPICE(ZEROBORESIGHT) is signaled.
C
C     7)  If the 'ANGLES' specification is not present in the kernel
C         pool and the boundary vectors for the edge of the field of
C         view cannot be found in the kernel pool, the error
C         SPICE(BOUNDARYMISSING) is signaled.
C
C     8)  If there is insufficient room (as specified by the argument
...
C
C     11) If the reference vector for the 'ANGLES' specification can not
C         be found in the kernel pool, the error SPICE(REFVECTORMISSING)
C         is signaled.
C
C     12) If the reference vector stored in the kernel pool to support
...
C
C     13) If the 'ANGLES' specification is present in the kernel pool
C         and the reference angle stored in the kernel pool to support
C         the 'ANGLES' specification is absent from the kernel pool, the
C         error SPICE(REFANGLEMISSING) is signaled.
C
C     14) If the keyword that stores the angular units for the angles
C         used in the 'ANGLES' specification is absent from the kernel
C         pool, the error SPICE(UNITSMISSING) is signaled.
C
C     15) If the value used for the units in the 'ANGLES' specification
...
C
C     16) If the keyword that stores the cross angle for the 'ANGLES'
C         specification is needed and is absent from the kernel pool,
C         the error SPICE(CROSSANGLEMISSING) is signaled.
C
C     17) If the angles for the 'RECTANGLE'/'ANGLES' specification case
C         have cosines that are less than those stored in the parameter
C         MINCOS, the error SPICE(BADBOUNDARY) is signaled.
C
C     18) If the class specification contains something other than
C         'ANGLES' or 'CORNERS', the error SPICE(UNSUPPORTEDSPEC) is
C         signaled.
C
C     19) In the event that the CLASS_SPEC keyword is absent from the
...
C        various shapes and sizes, to load the FOV definitions.
C
C
C           KPL/IK
C
C           File name: getfov_ex1.ti
C
...
C        C
C              DATA                  INSIDS  /  -999001, -999002,
C             .                                 -999003, -999004  /
C
C        C
C        C     Load the IK file.
C        C
C              CALL FURNSH( 'getfov_ex1.ti' )
C
C        C
...
C        C
C                 WRITE (*,'(A,I7)')     'Instrument ID: ', INSIDS(I)
...
C$ Restrictions
C
C     1)  This routine will not operate unless an I-kernel for the
...
C$ Version
C
C-    SPICELIB Version 2.3.0, 17-DEC-2021 (JDR) (ML) (BVS)
C
C        Bug fix: added missing exception for the boresight vector
C        being the zero vector.
C
C        Updated long error message for 'BADBOUNDARY' exception to
C        correctly describe the check that's actually done.
C
C        Edited the header to comply with NAIF standard. Removed
C        unnecessary $Revisions section.
C
C        Updated entry #5 and added entries #14 and #18 to $Exceptions
C        section. Updated $Restrictions section.
C
C        Updated $Particulars to describe the actual condition that
C        reference and cross angles values must satisfy.
C
C        Updated to check FAILED() after calls to CONVRT to prevent
C        use of uninitialized values in subsequent calls.
C
C-    SPICELIB Version 2.2.0, 22-MAR-2017 (JDR) (BVS)
C
...
C
C        Updated code to remove unnecessary lines in the SPICE
...
C
C     Check whether boresight is a zero vector; complain if it is.
C
      IF ( VZERO(BSIGHT) ) THEN

         CALL SETMSG ( 'The boresight vector, stored in the ''#'' '
     .   //            'variable, is the zero vector.'               )
         CALL ERRCH  ( '#', KWORD(1:RTRIM(KWORD))                    )
         CALL SIGERR ( 'SPICE(ZEROBORESIGHT)'                        )
         CALL CHKOUT ( 'GETFOV'                                      )
         RETURN
...
C
     ...
     .         //            'vectors. This usually happens '
     .         //            'when one (or both) of the angles '
     .         //            'results in the angular separation '
     .         //            'between the boresight and the FOV '
     .         //            'side plane that it defines being '
