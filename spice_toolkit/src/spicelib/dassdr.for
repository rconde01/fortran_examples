...
C
C     2)  If a Fortran read attempted by this routine fails, an error is
C         signaled by a routine in the call tree of this routine. The
C         state of the DAS file undergoing re-ordering will be
C         indeterminate.
C
C     3)  If a Fortran write attempted by this routine fails, an error
C         is signaled by a routine in the call tree of this routine. The
C         state of the DAS file undergoing re-ordering will be
C         indeterminate.
C
C     4)  If any other I/O error occurs during the re-arrangement of the
C         records in the indicated DAS file, the error is signaled by a
C         routine in the call tree of this routine.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 12-AUG-2021 (JDR)
C
...
C
C-    SPICELIB Version 2.0.0, 17-NOV-1993 (KRG)
C
...
C        change.
C
C-    SPICELIB Version 1.2.0, 07-OCT-1993 (NJB) (HAN) (MJS)
C
C        Bug fix: call to CLEARD replaced with call to
C        CLEARI.
C
C-    SPICELIB Version 1.1.0, 08-JUL-1993 (NJB) (MJS)
C
