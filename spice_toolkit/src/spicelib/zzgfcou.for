...
C     METHOD     I   COIN
C     TARGET     I   COIN
C     ET         I   COIN, CODC, COG, COCD, COCG, COSD, COSG, COEX
C     REF        I   COIN
C     ABCORR     I   COIN
C     OBSRVR     I   COIN
C     DREF       I   COIN
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 17-OCT-2021 (EDW) (JDR)
C
C        ZZGFCOIN, body radii accessed from kernel pool
C        using ZZGFTREB.
C
...
C                 zzgf.inc.
C
C              Note that when geodetic or planetographic coordinates
...
C         kernel pool, an error is signaled by a routine in the
...
C
C     17) If a body RADII vector has other than exactly thee elements,
C         an error is signaled by a routine in the call tree of this
C         routine.
C
C     18) If a body RADII vector has any element less-than or equal to
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 11-SEP-2021 (EDW) (JDR)
C
...
C         defined by ZZGFCOIN, the error is signaled by a routine
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 11-SEP-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C-    SPICELIB Version 1.0.0, 05-MAR-2009 (NJB)
...
C
C     2)  If an error occurs while this routine computes the coordinate
C         defined by ZZGFCOIN, the error is signaled by a routine in the
C         call tree of this routine.
C
C     3)  If an error occurs while this routine computes the derivative
C         with respect to time of the coordinate defined by ZZGFCOIN,
...
C         coordinate defined by ZZGFCOIN, the error is signaled by a
