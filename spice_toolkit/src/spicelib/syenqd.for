...
C     TABVAL   are the components of a double precision symbol table.
C
C              On output, the value table contains the new value in
C              addition to the old values associated with the symbol
C              NAME. The pointer table is updated to reflect the change
C              in the dimension of the symbol.
C
C$ Parameters
C
C     None.
...
C$ Exceptions
C
C     1)  If the addition of the new value to the symbol table causes an
C         overflow in the value table, the error SPICE(VALUETABLEFULL)
C         is signaled.
C
C$ Files
C
...
C      The call,
C
C         CALL SYENQD ( 'BODY399_POLE_RA', 0.0D0,
