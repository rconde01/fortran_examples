...
C
C              Use the SPICELIB routine DAFOPR to open the file for read
C              access and get HANDLE unless SPKLEF or CKLPF has already
C              been called and returned the handle. This file is
C              unchanged by calling SPCEC.
C
...
C
C              Use the SPICELIB routine TXTOPN to open the file and get
C              UNIT. Upon exit, this file will contain the text from the
C              comment area of the binary SPK or CK file, beginning at
C              the line that was the position of the file pointer when
C              SPCEC was called. In other words, SPCEC does not rewind
C              or backspace this file before writing the text to it.
C
C$ Detailed_Output
C
C     None.
...
C         SPICE(FILEWRITEFAILED) is signaled.
C
C$ Files
C
C     See argument HANDLE and UNIT.
C
C$ Particulars
C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 03-JUN-2021 (JDR)
C
