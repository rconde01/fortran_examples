...
C         error is signaled by a routine in the call
...
C         are parallel, an error is signaled by a
...
C
C     -  A PCK file containing rotational elements for the central body
...
C
C
C              CALL PROMPT ( 'Enter name of LSK file > ', LSK )
...
C        platform, using the LSK file named naif0012.tls, the PCK file
C        named pck00010.tpc, the SPK file named de421.bsp; the 'EARTH'
C        as central body and the time string '2018 Mar 8  17:59 UTC',
C        the output was:
C
C
C        Enter name of LSK file > naif0012.tls
C        Enter name of PCK file > pck00010.tpc
C        Enter name of SPK file > de421.bsp
C
C         Kernels have been loaded.
C
C        Enter name of central body       > EARTH
C        Enter calendar, JD, or DOY time  > 2018 Mar 8  17:59 UTC
C
C         Central body              = EARTH
C         Time                      = 2018 Mar 8  17:59 UTC
C         Planetocentric L_s (deg.) =    348.11593978317080
C
C
C$ Restrictions
C
...
C
C     [1]  "The Astronomical Almanac for the Year 2005," page L9,
C          United States Naval Observatory, U.S. Government Printing
C          Office, Washington, D.C., 2004.
C
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 25-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Removed WHILE
C        loop from example code, and added solution.
C
