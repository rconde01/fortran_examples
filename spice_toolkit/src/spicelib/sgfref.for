...
C$ Abstract
C
C     Fetch from the references partition of a generic segment 
C     the double precision numbers from FIRST to LAST. The 
C     segment is identified by a DAF file handle and segment 
C     descriptor.
C
C$ Disclaimer
C
...
C$ Exceptions
C
C     1)  If FIRST is less than 1 or LAST is greater than the number of
C         reference values, the error SPICE(REQUESTOUTOFBOUNDS) is
C         signaled.
C
C     2)  If LAST is less than FIRST, the error SPICE(REQUESTOUTOFORDER)
C         is signaled.
C
C     3)  If the reference directory structure is unrecognized, the
...
C
     ...
     .   //            ' likely cause of this anomaly is that'
