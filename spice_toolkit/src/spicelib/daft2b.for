...
C     routine DAFTB. NAIF supports this routine only to provide backward
C     compatibility.
C
...
C$ Restrictions
C
C     1)  DAFT2B cannot be executed while any other DAF is open
C         for writing.
C
...
C
C        Edited the header to comply with NAIF standard. Moved DAF
...
C
C-    SPICELIB Version 2.0.1, 06-AUG-1990 (HAN)
C
...
C
C-    SPICELIB Version 2.0.0, 02-AUG-1990 (JEM)
C
C        The previous version of this routine always failed and
C        signaled the error SPICE(DAFNOIDWORD) because of a faulty
C        logical expression in an error-checking IF statement.
C        The error SPICE(DAFNOIDWORD) should be signaled if the
...
C        signaled every time no matter what the word was. The
...
C$ Index_Entries
C
C     DEPRECATED text DAF to binary
C
C-&
...
C     routine, and an error will be signaled.
C
