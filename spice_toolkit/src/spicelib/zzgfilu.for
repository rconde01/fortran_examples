...
C$ Exceptions
C
C     1)  See individual entry points.
C
C$ Files
C
...
C         user applications.
C
C     2)  ZZGFILIN must be called prior to use of any of the other
C         entry points.
C
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 30-MAY-2021 (NJB) (JDR)
C
C        Edited the umbrella routine and all its entry points' headers
C        to comply with NAIF standard.
C
C        Updated header of entry point ZZGFILIN: added missing
...
C     ILLUM      I   Illumination source name.
...
C
C     4)  If the aberration correction string is invalid, an error
...
C
C        Updated header: added missing argument documentation.
C
C-    SPICELIB Version 1.1.0, 08-MAR-2017 (NJB)
C
...
C$ Exceptions
C
C     1)  If the state of the target relative to the observer at ET can
C         not be found due to an SPK lookup failure, an error is
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 29-MAY-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C-    SPICELIB Version 1.1.0, 08-MAR-2017 (NJB)
...
C$ Exceptions
C
C     1)  If the position of the target relative to the observer at ET
C         can not be found due to an SPK lookup failure, an error is
C         signaled by a routine in the call tree of this routine.
C
C$ Files
C
