...
C     routines DAFGDA and DAFGSR. This routine is supported for purposes
C     of backward compatibility only.
C
...
C         is signaled.
C
C     2)  If BEGIN > END, the error SPICE(DAFBEGGTEND) is signaled.
C
C     3)  If the file associated with HANDLE is not of the native
C         binary file format, the error SPICE(UNSUPPORTEDBFF) is
C         signaled.
C
C     4)  If HANDLE is invalid, an error is signaled by a routine in
