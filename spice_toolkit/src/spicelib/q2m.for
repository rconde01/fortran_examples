...
C     Error free.
C
C     1)  If Q is not a unit quaternion, the output matrix R is
C         the rotation matrix that is the result of converting
C         normalized Q to a rotation matrix.
C
C     2)  If Q is the zero quaternion, the output matrix R is
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 12-APR-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Corrected the
C        output argument name in $Exceptions section.
C
