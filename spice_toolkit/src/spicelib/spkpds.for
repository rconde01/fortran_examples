...
C$ Exceptions
C
C     1)  If the value of BODY is 0, the error SPICE(BARYCENTEREPHEM) is
C         signaled.
C
C     2)  If the values of BODY and CENTER are the same, the error
C         SPICE(BODYANDCENTERSAME) is signaled.
C
C     3)  If FRAME is not one of the known SPICE reference frames, the
C         error SPICE(INVALIDREFFRAME) is signaled.
C
C     4)  If FIRST is greater than or equal to LAST, the error
C         SPICE(BADDESCRTIMES) is signaled.
C
C     5)  If the value of TYPE is outside the range 1 to 1000
C         (inclusive), the error SPICE(UNKNOWNSPKTYPE) is signaled. This
C         does not ensure that the TYPE is a legitimate SPK segment
