...
C              and the elements of VALUES become the values associated
C              with NAME. If NAME is not in the symbol table, a new
C              symbol is created, provided there is room in the symbol
C              table.
C
C     VALUES   are the new values associated with the symbol NAME.
C
C     N        is the number of elements in the VALUES array. If N < 1,
C              the symbol table is not modified.
C
C     TABSYM,
C     TABPTR,
...
C
C     4)  If N < 1, the error SPICE(INVALIDARGUMENT) is signaled.
C
C$ Files
C
C     None.
...
C$ Particulars
C
C     This subroutine is like SYSETC, but SYPUTC allows **several**
C     values to be associated with a symbol.
C
...
C     the call
C
C        CALL SYPUTC ( 'BODY4_POLE_RA', VALUES, 3,
