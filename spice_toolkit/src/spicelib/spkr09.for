...
C
C     1)  If the input HANDLE does not designate a loaded SPK file, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     2)  If the segment specified by DESCR is not of data types 9 or
C         13, the error SPICE(WRONGSPKTYPE) is signaled.
C
...
C$ Version
C
C-    SPICELIB Version 2.1.1, 12-AUG-2021 (JDR)
C
