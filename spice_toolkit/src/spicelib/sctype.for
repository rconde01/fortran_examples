...
C
C     The function returns the spacecraft clock type associated with
C     the spacecraft specified by SC.
C
C$ Detailed_Input
C
...
C         spacecraft specified by SC is not found in the kernel pool, an
C         error is signaled by a routine in the call tree of this
C         routine. SCTYPE returns the value 0 in this case.
C
C$ Files
C
C     None.
...
C         spacecraft specified by SC has been loaded into the kernel
C         pool.
C
...
C$ Version
C
C-    SPICELIB Version 1.4.0, 12-AUG-2021 (NJB) (JDR)
C
C        Re-implemented using the type 1 SCLK database in order to
C        improve performance. The routine still works as before for
C        clock types other than 1.
C
...
C
      ...
      CALL CHKIN ( 'SCTYPE' )
C
C     Get the type from the type 1 subsystem.
C
      CALL SCTY01 ( SC, SCTYPE )
