...
C$ Detailed_Input
C
C     LINE     is the text line which is to be written to UNIT.
C
C              The value of this variable is not modified.
C
C     UNIT     is the Fortran unit number for the output. This may be
...
C         file attached to UNIT, the error SPICE(FILEWRITEFAILED) is
C         signaled.
C
C$ Files
C
...
C
C-    SPICELIB Version 2.26.0, 16-AUG-2021 (JDR)
C
...
C
C        Removed implementation detail (discovery check-in) from
...
C
C-    SPICELIB Version 1.0.0, 20-DEC-1995 (KRG) (HAN)
C
...
C
C     Some environments make a distinction between a text file and the
C     terminal screen when writing to a logical unit using list directed
C     writes of character strings or the FMT='(A)' format specifier. The
C     difference has to do with the interpretation of the first
C     character in the output string which is supposed to control the
C     vertical spacing. This control character is required for writing
C     to the standard output device, but not for writing to a text file.
C     This is a pretty dumb thing to do, but we have to live with it.
C
C     So, we check to see if we are writing to the standard output unit
C     and perform a list directed write, otherwise we use the format
C     specifier FMT='(A)', which does not use any vertical carriage
C     control characters.
C
       IF ( UNIT .EQ. STDOUT ) THEN
          WRITE ( UNIT, *, IOSTAT=IOSTAT ) LINE(:RTRIM(LINE))
       ELSE
