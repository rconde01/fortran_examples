...
C$ Exceptions
C
C     1)  If the number of constraints NCNSTR is out of range, the
C         error SPICE(INVALIDCOUNT) is signaled.
C
C     2)  If the table count in either input join row set is out of
C         range, the error SPICE(INVALIDCOUNT) is signaled.
C
C     3)  If the sum of the table counts of the input join row sets is
C         too large, the error SPICE(INVALIDCOUNT) is signaled.
C
...
C     procedure is the typical application envisioned for this routine.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.2, 02-OCT-2021 (NJB)
C
C        Corrected typos in comments.
C
