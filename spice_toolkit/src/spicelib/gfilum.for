...
C              computation method to be used. Parameters include, but
C              are not limited to, the shape model used to represent the
C              surface of the target body.
C
...
C
C              Neither case nor whitespaces are significant in METHOD.
C              For example, the string ' eLLipsoid ' is valid.
C
C     ANGTYP   is a string specifying the type of illumination angle for
C              which a search is to be performed. The possible values of
C              ANGTYP are
C
C                 'PHASE'
C                 'INCIDENCE'
C                 'EMISSION'
C
C              When the illumination source is the sun, the incidence
C              angle is commonly called the "solar incidence angle."
C
C              See the $Particulars section below for a detailed
C              description of these angles.
C
C              Neither case nor whitespaces are significant in ANGTYP.
C              For example, the string ' Incidence ' is valid.
C
C     TARGET   is the name of a target body. The point at which the
C              illumination angles are defined is located on the surface
C              of this body.
C
...
C              ' Incidence ' is valid. Sequences of embedded blanks are
C              treated as a single blank.
C
C     ILLMN    is the name of the illumination source. This source may
C              be any ephemeris object. Case, blanks, and numeric values
C              are treated in the same way as for the input TARGET.
C
C     FIXREF   is the name of the body-fixed, body-centered reference
C              frame associated with the target body. The input surface
C              point SPOINT is expressed relative to this reference
C              frame, and this frame is used to define the orientation
C              of the target body as a function of time.
C
...
C              observer-surface point vector, the surface point-
C              illumination source vector, and the target body
C              orientation to account for one-way light time and stellar
C              aberration.
C
C              Any "reception" correction accepted by SPKEZR can be used
C              here. See the header of SPKEZR for a detailed description
C              of the aberration correction options. For convenience,
...
C
C              Case and blanks are not significant in the string ABCORR.
C
...
C              Cartesian coordinates, relative to the body-fixed target
C              frame designated by FIXREF.
C
C              SPOINT need not be visible from the observer's location
C              in order for the constraint specified by RELATE and
C              REFVAL (see descriptions below) to be satisfied.
C
C              The components of SPOINT have units of km.
C
C     RELATE   is a relational operator used to define a constraint on a
C              specified illumination angle. The result window found by
C              this routine indicates the time intervals where the
C              constraint is satisfied. Supported values of RELATE and
...
C              argument ADJUST (described below) is used to specify this
C              separation.
C
...
C              ADJUST is set to a positive value, GFILUM will find times
C              when the specified illumination angle is within ADJUST
C              radians of the specified extreme value.
C
C              If ADJUST is non-zero and a search for an absolute
C              minimum is performed, the result window contains time
C              intervals when the specified illumination angle has
C              values between the absolute minimum ABSMIN and
C              ABSMIN + ADJUST radians.
C
C              If ADJUST is non-zero and the search is for an absolute
C              maximum, the corresponding angle is between the absolute
C              maximum ABSMAX and ABSMAX - ADJUST radians.
C
...
C              short enough for a search using this step size to locate
C              the time intervals where the specified illumination angle
C              is monotone increasing or decreasing. However, STEP must
C              not be *too* short, or the search will take an
C              unreasonable amount of time.
C
C              The choice of STEP affects the completeness but not the
C              precision of solutions found by this routine; the
C              precision is controlled by the convergence tolerance. See
C              the discussion of the parameter CNVTOL for details.
C
...
C              greatly reduce the time window that must be searched for
C              the desired solution. See the $Particulars section below
C              for further discussion.
C
C              See the $Examples section below for a code example that
C              shows how to create a confinement window.
C
C              CNFINE must be initialized by the caller via the SPICELIB
C              routine SSIZED.
C
...
C              array WORK (see description below) used by this routine.
C              MW should be at least as large as TWICE the number of
C              intervals within the search window on which the specified
C              illumination angle is monotone increasing or decreasing.
C              It does no harm to pick a value of MW larger than the
C              minimum required to execute the specified search, but if
C              MW is too small, the search will fail.
C
...
C              discarded before GFILUM conducts its search.
C
C$ Detailed_Output
C
...
C
C              where MW is a constant declared by the caller and NWILUM
C              is a constant defined in the SPICELIB INCLUDE file
C              gf.inc.
C
...
C
C     6)  If the output SPICE window RESULT has size less than 2, the
C         error SPICE(INVALIDDIMENSION) is signaled.
C
C     7)  If the output SPICE window RESULT has insufficient capacity to
C         hold the set of intervals on which the specified illumination
C         angle condition is met, an error is signaled by a routine in
C         the call tree of this routine.
C
C     8)  If the input target body-fixed frame FIXREF is not
...
C
C     9)  If the input frame FIXREF is not centered at the target body,
C         an error is signaled by a routine in the call tree of this
C         routine.
C
C     10) If the input argument METHOD is not recognized, an error is
C         signaled by a routine in the call tree of this routine.
C
C     11) If the illumination angle type ANGTYP is not recognized,
C         an error is signaled by a routine in the call tree
C         of this routine.
C
C     12) If the relational operator RELATE is not recognized, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     13) If the aberration correction specifier contains an
C         unrecognized value, an error is signaled by a routine in the
C         call tree of this routine.
C
C     14) If ADJUST is negative, an error is signaled by a routine in
C         the call tree of this routine.
C
C     15) If any of the input body names do not map to NAIF ID
C         codes, an error is signaled by a routine in the call tree of
C         this routine.
C
C     16) If the target coincides with the observer or the illumination
C         source, an error is signaled by a routine in the call tree
C         of this routine.
C
C     17) If required ephemerides or other kernel data are not
...
C        Reconnaissance Orbiter (MRO) spacecraft.
C
...
C        C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'gfilum_ex1.tm' )
C
C        C
...
C        C     WORK, WNSOLR and RESULT are saved because this practice
...
C
C        C
C        C     Search over the previous result window for times when
C        C     the emission angle is less than the reference value.
C        C
C              CALL GFILUM ( METHOD, 'EMISSION', TARGET, ILLMN,
C             .              FIXREF, ABCORR,     OBSRVR, ROVPOS,
C             .              '<',    REFVAL,     ADJUST, STEP,
C             .              WNSOLR, MAXWIN,     NWILUM, WORK,
C             .              RESULT                             )
C
C        C
...
C                                                  (deg)         (deg)
C
C        Start: 2006 OCT 03 12:43:46.949 UTC   56.10415019   20.00000019
C        Stop:  2006 OCT 03 12:44:42.288 UTC   56.29996181   20.00000015
C
C        Start: 2006 OCT 08 16:03:33.956 UTC   56.48955485   20.00000021
C        Stop:  2006 OCT 08 16:04:29.495 UTC   56.68754510   19.99999997
C
C        Start: 2006 OCT 13 19:23:24.634 UTC   56.88741059   19.99999988
C        Stop:  2006 OCT 13 19:24:12.492 UTC   57.05931857   20.00000017
C
C        Start: 2006 OCT 18 22:43:21.631 UTC   57.30924467   20.00000012
C        Stop:  2006 OCT 18 22:43:47.966 UTC   57.40457272   20.00000004
C
C        Start: 2006 NOV 14 15:39:44.153 UTC   54.32875839   19.99999994
C        Stop:  2006 NOV 14 15:40:10.446 UTC   54.42668077   19.99999990
C
C        Start: 2006 NOV 19 18:59:10.190 UTC   54.63096111   20.00000007
C        Stop:  2006 NOV 19 18:59:54.776 UTC   54.79840753   19.99999985
C
C        Start: 2006 NOV 24 22:18:38.342 UTC   54.94960000   19.99999982
C        Stop:  2006 NOV 24 22:19:30.964 UTC   55.14883883   20.00000003
C
C        Start: 2006 NOV 30 01:38:07.309 UTC   55.28054784   19.99999983
C        Stop:  2006 NOV 30 01:39:03.296 UTC   55.49418925   19.99999999
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 27-OCT-2021 (JDR) (NJB)
C
C        Edited the header to comply with NAIF standard.
C
C        Changed the code example for the solution to fit within the
C        $Examples section without modifications. Added SAVE statements
C        for CNFINE, WNSOLR, WORK and RESULT variables in code example.
C
C        Added initialization of QCPARS(8) to pacify Valgrind.
C
...
C
C     Initialize the corresponding character parameter, even though
C     we don't need it, since GFEVNT will left-justify it and convert
C     it to upper case. Memory usage checkers such as Valgrind dislike
C     code that operates on uninitialized variables.
C
      QCPARS(8) = ' '
