...
C$ Exceptions
C
C     In the case of any SPICELIB error occurring, ERROR is set to
C     .TRUE. and MSG to 'SPICELIB error detected.'.
C
...
C         routine. The output argument TICKS will not be modified. and
C         MSG
C
...
C               offset value
C
C         If any of these conditions is detected, no error is signaled.
C         ERROR is set to .TRUE. and MSG contains the specific issue.
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 20-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Added introductory paragraph to $Exceptions section, split
C        entry #1 into two, and added description of behavior in entry
C        #4.
C
