...
C     --------  ---  -------------------------------------------------
C     NDIM       I   Dimension of function to be differentiated.
...
C$ Detailed_Input
C
C     NDIM     is the dimension of the function to be
C              differentiated. The derivative of each
C              function component will be found.
C
C     F0       is an array of NDIM function values at a point on
C              the real line; we'll refer to this point as X0.
C
C     F2       is an array of NDIM function values at a second
C              point on the real line; we'll refer to this point
C              as X2. The points X0 and X2 must satisfy
C
C                 X2 = X0 + 2 * DELTA
C
C
...
C
C     2)  If NDIM is less than 1, this routine will fail in a
C         system-dependent manner.
C
C$ Files
C
C     None.
...
C
C
C         4 - DFDT(1) =    0.4547473508864641E-12
C
C
C        Note that the difference displayed is platform-dependent, but
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 05-AUG-2020 (JDR)
C
C        Changed input argument name "N" to "NDIM" for consistency with
C        other routines.
C
...
C
C
      CALL VLCOMG (  NDIM, 0.5D0/DELTA, F2, -0.5D0/DELTA, F0, DFDT )
