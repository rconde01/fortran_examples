...
C              EPOCH may be either UTC or ephemeris seconds past
C              J2000, as specified by EPTYPE.
C
C     EPTYPE   is the type of input epoch. It may be either
C              of the following:
C
C                 'UTC'    UTC seconds past J2000 UTC.
C
C                 'ET'     Ephemeris seconds past J2000 TDB,
C                          also known as barycentric dynamical
C                          time (TDB).
C
C$ Detailed_Output
C
...
C        UTCSEC = ET - DELTA
C
C     See the Time required reading time.req for further examples.
C
C$ Restrictions
C
C     1)  The routines STR2ET and ET2UTC are preferred for conversions
C         between UTC and ET. This routine is provided mainly as a
C         utility for STR2ET and ET2UTC.
C
C     2)  A leapseconds kernel containing leapseconds and relativistic
C         terms MUST be loaded prior to calling this subroutine.
C         Examples demonstrating how to load a kernel pool are included
C         in the Required Reading file time.req and in the $Examples
...
C
C     [1]  "The Astronomical Almanac for the Year 1990," United States
C          Naval Observatory, U.S. Government Printing Office,
C          Washington, D.C., 1989.
C
...
C$ Version
C
C-    SPICELIB Version 1.3.0, 24-AUG-2021 (JDR)
C
...
C
C        Replaced reference to UTC2ET with STR2ET in $Restrictions
C        section.
C
...
C        and TPARSE differed by one second. Upon closer
C        inspection, crack NAIF staff members deduced that in fact
C        Mr. Colvin had not loaded the kernel pool, and were
C        surprised to learn that no error had occurred.
C
C        Multiple FOUND flags and a bevy of new error messages
C        were implemented to cope with this unfortunate oversight.
C
C-&
