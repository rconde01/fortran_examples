...
C              including the zero vector.
C
C     NDIM     is the dimension of V1 and VOUT.
C
C$ Detailed_Output
C
C     VOUT     is the double precision n-dimensional unit vector in the
C              direction of V1. If V1 is the zero vector, then VOUT
C              will also be the zero vector.
C
...
C
C     1) Define a set of n-dimensional vectors and compute their
C        corresponding unit vectors and magnitudes.
C
C
C        Example code begins here.
C
C
C              PROGRAM UNORMG_EX1
C              IMPLICIT NONE
C
C        C
...
C        C
C              DATA                  V1    /
C             .                         5.D0,  12.D0,  0.D0,  4.D0,
C             .                         1.D-6,  2.D-6, 2.D-6, 0.D0  /
C
C        C
...
C              DO I=1, SETSIZ
C
C                 CALL UNORMG ( V1(1,I), NDIM, VOUT, VMAG )
C
C                 WRITE(*,'(A,4F12.7)') 'Vector     :',
C             .                         ( V1(J,I), J=1,NDIM )
C                 WRITE(*,'(A,4F12.7)') 'Unit vector:', VOUT
C                 WRITE(*,'(A,F12.7)')  'Magnitude  :', VMAG
C                 WRITE(*,*)
C
C              END DO
C
...
C
C
C        Vector     :   5.0000000  12.0000000   0.0000000   4.0000000
C        Unit vector:   0.3676073   0.8822575   0.0000000   0.2940858
C        Magnitude  :  13.6014705
C
C        Vector     :   0.0000010   0.0000020   0.0000020   0.0000000
C        Unit vector:   0.3333333   0.6666667   0.6666667   0.0000000
C        Magnitude  :   0.0000030
C
C
C$ Restrictions
C
C     1)  No error checking is implemented in this subroutine to guard
...
C        unnecessary $Revisions section. Updated code comments.
C
