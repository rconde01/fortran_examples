C$Procedure DPSPCE ( Evaluate "two-line" element data, deep-space )

      SUBROUTINE DPSPCE ( ET, GEOPHS, ELEMS, STATE )

C$ Abstract
C
C     Deprecated: This routine has been superseded by the SPICELIB
C     routine EVSGP4. *ALL* TLE evaluations should use that routine.
C     NAIF supports DPSPCE only for backward compatibility.
C
C     Evaluate NORAD two-line element data for deep-space, earth
C     orbiting spacecraft (that is spacecraft with orbital periods
C     greater-than 225 minutes). This evaluator uses algorithms as
C     described in Hoots 1980 [1].
C
C$ Disclaimer
C
...
C     --------  ---  --------------------------------------------------
C     ET         I   Time for state evaluation in seconds past ephemeris
C                    epoch J2000.
C     GEOPHS     I   The array of geophysical constants.
C     ELEMS      I   Array of orbit elements.
C     STATE      O   State vector at ET.
C
C$ Detailed_Input
C
C     ET       is the epoch in TDB seconds past ephemeris epoch J2000
...
C        for the TDRS 4 geosynchronous satellite. The following example
C        code demonstrates how you obtain its state at an epoch of
C        interest.
C
...
C
C              INTEGER               TIMSLN
C              PARAMETER           ( TIMSLN = 25 )
C
C              INTEGER               TLELLN
C              PARAMETER           ( TLELLN = 69 )
C
C        C
C        C     The TDRS-4 satellite is an Earth orbiting object; set
...
C        C     required by DPSPCE. These constants are available from
...
C
C        C
C        C     Define the Two-Line Element set for TDRS-4.
C        C
C              TLE(1)  = '1 19883U 89021B   97133.05943164 -.00000277'
C             .      //                   '  00000-0  10000-3 0  3315'
C              TLE(2)  = '2 19883   0.5548  86.7278 0001786 312.2904 '
...
C        C     GETELM to perform time conversions. Use a meta-kernel for
C        C     convenience.
C        C
C              CALL FURNSH ( 'dpspce_ex1.tm' )
C
C        C
...
C        C     Set the lower bound for the years to be the earliest
C        C     first year for the elements.
C        C
C              CALL GETELM ( 1988, TLE, EPOCH, ELEMS )
C
C        C
C        C     Define the final time past epoch, 1400 mins (in seconds),
C        C     the step size for elements output, 360 mins (in seconds),
C        C     and the start time keyed off epoch.
C        C
C              TF   = 1440.D0 * 60.D0
C              DELT = 360.D0  * 60.D0
C              ET   = EPOCH - 4.D0 * DELT
C
C        C
C        C     Display the reference epoch for the elements.
C        C
C              CALL ET2UTC ( EPOCH, 'C', 3, TIMSTR )
C              WRITE(*,'(2A)') 'Reference epoch: ', TIMSTR
C
C        C
C        C     Obtain the state at different epochs around the
C        C     reference epoch.
C        C
C              DO WHILE ( DABS(TIME - EPOCH) .LE. DABS(TF) )
C
C                 CALL DPSPCE ( ET, GEOPHS, ELEMS, STATE  )
C                 CALL ET2UTC ( ET, 'C',    3,     TIMSTR )
C
C                 WRITE(*,*)
C                 WRITE(*,'(2A)')       'Time    : ', TIMSTR
...
C
C
C        Reference epoch: 1997 MAY 13 01:25:34.894
C
C        Time    : 1997 MAY 12 01:25:34.894
C        Position:  -36415.06027922 -21268.27678598    324.63292275
C        Velocity:       1.55019424     -2.65462811     -0.01618361
C
C        Time    : 1997 MAY 12 07:25:34.894
C        Position:   21403.92800065 -36329.04571718   -223.49047558
C        Velocity:       2.64861127      1.56125103     -0.02363616
C
C        Time    : 1997 MAY 12 13:25:34.894
C        Position:   36222.44396185  21565.83478418   -323.53517502
C        Velocity:      -1.57313376      2.64235356      0.01642601
C
C        Time    : 1997 MAY 12 19:25:34.894
C        Position:  -21736.44940088  36128.44906628    226.92953877
C        Velocity:      -2.63481828     -1.58473777      0.02355544
C
C        Time    : 1997 MAY 13 01:25:34.894
C        Position:  -36048.19799634 -21884.57697027    322.54133293
C        Velocity:       1.59511867     -2.62787056     -0.01665374
C
C        Time    : 1997 MAY 13 07:25:34.894
C        Position:   22018.52968309 -35959.79405656   -229.86590206
C        Velocity:       2.62168498      1.60606852     -0.02347795
C
C        Time    : 1997 MAY 13 13:25:34.894
C        Position:   35850.58313589  22178.69474099   -321.27968318
C        Velocity:      -1.61782447      2.61522408      0.01688604
C
C        Time    : 1997 MAY 13 19:25:34.894
C        Position:  -22347.26148339  35754.07411618    233.17456026
C        Velocity:      -2.60750172     -1.62927167      0.02338424
C
C        Time    : 1997 MAY 14 01:25:34.894
C        Position:  -35671.56713967 -22493.50518477    320.10323846
C        Velocity:       1.63950543     -2.60040084     -0.01710490
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 01-NOV-2021 (JDR) (EDW)
C
C        Declared routine as deprecated.
C
C        Changed input argument name "TIME" to "ET" for consistency
C        with other routines.
C
C        Corrected the description of QO and SO constants in the
C        detailed description of the input argument GEOPHS and the input
C        element names in ELEMS.
C
...
C        to FURNSH.
C
C-    SPICELIB Version 1.2.1, 27-DEC-2000 (EDW)
C
C        Corrected error in header documentation. Horner's Rule
C        not Butcher's.
C
C-    SPICELIB Version 1.2.0, 24-MAR-1999 (EDW)
C
...
C$ Index_Entries
C
C     DEPRECATED NORAD two line elements deep space evaluator
C
C-&
