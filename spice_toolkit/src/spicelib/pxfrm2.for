...
C     time-dependent reference frames.
C
...
C
C              CHARACTER*(*)         METAKR
C              PARAMETER           ( METAKR = 'pxfrm2_ex1.tm' )
C
C              INTEGER               FRNMLN
...
C        C
C        C     Now we will transform one of the FOV corner vectors into
C        C     the IAU_MARS frame so the surface intercept point can be
...
C        C     and finding the position of MGS with respect to Mars
C        C     could be done once and used for every pixel.
C        C
...
C            Latitude  (deg):    -48.477481877892430
...
C            Radius    (km) :     3384.9411359139667
C            Latitude  (deg):    -48.477481924252700
...
C         SURFPT and SINCPT:
C            Distance (mm):        32.139880286899256
C
C
C$ Restrictions
C
...
C
C        Edited the header to comply with NAIF standard. Updated code
C        example comments and format.
C
