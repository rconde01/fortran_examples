...
C$ Abstract
C
C     Read a single line of text from the Fortran logical unit UNIT,
C     reporting the end of file if it occurs.
C
C$ Disclaimer
C
...
C$ Detailed_Output
C
C     LINE     is the next text line encountered when reading from UNIT.
C
...
C              is not guaranteed.
C
C     EOF      is .TRUE. if the end of file ( IOSTAT < 0 ) is
C              encountered during the attempt to read from unit UNIT.
C              Otherwise, this variable will be set to .FALSE.
C
C$ Parameters
C
C     None.
...
C
C     1)  If an error occurs while attempting to read from the text file
C         attached to UNIT, the error SPICE(FILEREADFAILED) is signaled.
C
C$ Files
C
C     None.
...
C        unnecessary $Version history entries for Beta versions.
C
