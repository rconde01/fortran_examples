...
C         call tree of this routine. The output arguments will not be
C         modified.
C
C     2)  If SEGNO is not the index of an existing segment in the
C         specified file, the error SPICE(INDEXOUTOFRANGE) is signaled.
C         The output arguments will not be modified.
C
C     3)  If an I/O error occurs while attempting to obtain summary
C         information for the specified segment, the error is signaled
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 02-JUL-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Fixed previous
C        version number.
C
