...
C
C     The function returns the time derivative of the separation angle
C     between the two input states, S1 and S2.
C
C$ Detailed_Input
C
C     S1,
C     S2       are, respectively, the state vector of the first and
C              second target bodies as seen from the observer.
C
C              An implicit assumption exists that both states lie in the
C              same reference frame with the same observer for the same
C              epoch. If this is not the case, the numerical result has
C              no meaning.
C
C$ Detailed_Output
C
...
C$ Exceptions
C
C     1)  If numeric overflow and underflow cases are detected, an error
C         is signaled by a routine in the call tree of this routine.
C
C     2)  If called in 'RETURN' mode, the function returns 0.
C
...
C
C     1) Calculate the time derivative of the angular separation of
C        the Earth and Moon as seen from the Sun at a given time.
C
C
...
C
C        C
C        C     Calculate the state vectors Sun to Moon, and
C        C     Sun to Earth at ET.
C        C
C        C
...
C
C        C
C        C     Calculate the time derivative of the angular separation
C        C     of the Earth and Moon as seen from the Sun at ET.
C        C
C              DSEPT = DVSEP( STATEE, STATEM )
C              WRITE(*,*) 'Time derivative of angular'
C              WRITE(*,*) '   separation (rad/sec): ', DSEPT
C
C              END
C
C
...
C
C
C         Time derivative of angular
C            separation (rad/sec):    3.8121193666132696E-009
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 06-JUL-2021 (JDR)
C
