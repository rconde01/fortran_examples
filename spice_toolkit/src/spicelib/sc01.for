...
C$ Files
C
C     An SCLK kernel appropriate to the spacecraft clock identified
C     by SC must be loaded at the time any entry point of this
C     routine is called.
C
C     If the SCLK kernel used with this routine does not map SCLK
...
C     called.
C
C     Normally kernels are loaded by user applications once at the
C     start of execution. It is not necessary to load them repeatedly.
C
C$ Particulars
C
...
C        SCTY01 ( Return type of SCLK )
C        SCPR01 ( Return partition start and stop times, type 1 )
C
C$ Examples
C
...
C$ Restrictions
C
C     1)  Efficiency of the conversion routines in this package
C         depends on the assumption that the kernel pool is updated
C         relatively infrequently. Any modification of kernel pool data,
C         including setting watches, will cause entry points of this
C         package to re-buffer saved SCLK data.
C
...
C$ Version
C
C-    SPICELIB Version 4.0.0, 01-DEC-2021 (NJB) (JDR)
C
C        Added entry points SCPR01 (return partition data) and
C        SCTY01 (indicate whether clock is type 1).
C
C        Updated all entry points to support buffering of data for
C        multiple clocks.
C
C        Added error checks for invalid time system codes.
C
C        Edited headers of umbrella routine and all entry points to
C        comply with NAIF standard.
C
...
C     SCLK pool:
C
      INTEGER               HDSCLK ( MXNCLK )
      INTEGER               SCPOOL ( LBSNGL : MXNCLK )
      INTEGER               CLKLST ( MXNCLK )
      INTEGER               SCBASE ( MXNCLK )
...
C
      ...
      DOUBLE PRECISION      DPBUFF ( DBUFSZ )
...
C
C     Declarations of statement functions
C
      DOUBLE PRECISION      COEFFS
      DOUBLE PRECISION      MODULI
      DOUBLE PRECISION      OFFSET
      DOUBLE PRECISION      PREND
      DOUBLE PRECISION      PRSTRT
...
C
C     NEW: Statement functions
C

C
C     Function COEFFS returns the Ith element of the Jth coefficient
C     record for the record set starting at index COFBAS+1 in the
C     double precision SCLK data buffer.

      COEFFS( I, J ) = DPBUFF( COFBAS + I + (J-1)*3 )

C
C     Function PREND returns the Ith partition end time for the
C     partition end time set starting at index ENDBAS+1 in the double
C     precision SCLK data buffer.
C
      PREND( I ) = DPBUFF( ENDBAS + I )

C
C     Function PRSTRT returns the Ith partition start time for the
C     partition start time set starting at index STRBAS+1 in the double
C     precision SCLK data buffer.
C
      PRSTRT( I ) = DPBUFF( STRBAS + I )

C
C     Function MODULI returns the Ith field modulus for the modulus
C     array starting at index MODBAS+1 in the double
C     precision SCLK data buffer.
C
      MODULI( I ) = DPBUFF( MODBAS + I )

C
C     Function OFFSET returns the Ith field offset for the offset
C     array starting at index OFFBAS+1 in the double
C     precision SCLK data buffer.
C
      OFFSET( I ) = DPBUFF( OFFBAS + I )

C
C     For later implementation:
C
C     Function CMPWID returns the width in characters required to
C     represent as a string the maximum count of the Ith field.
C
C      CMPWID( I ) = INTBUF( OFFCMP + I )
...
C$ Exceptions
C
C     1)  This routine assumes that that an SCLK kernel appropriate to
C         the spacecraft clock identified by the input argument SC has
C         been loaded. If an SCLK kernel has not been loaded, does not
C         contain all of the required data, or contains invalid data, an
C         error is signaled by a routine in the call tree of this
C         routine. The output argument TICKS will not be modified.
C
...
C         for conversion between SCLK and ET but is not loaded, an error
C         is signaled by a routine in the call tree of this routine. The
C         output argument TICKS will not be modified.
C
...
C
C
C     3)  If any of the following kernel variables have invalid values,
...
C
C
C     4)  The input argument CLKSTR may be invalid for a variety of
C         reasons:
C
...
C$ Version
C
C-    SPICELIB Version 3.0.0, 01-DEC-2021 (NJB) (JDR)
C
C        Updated to support buffering of data for multiple clocks.
C        This entry point tracks kernel pool changes but no longer
C        sets or uses watches.
C
C        Updated long error message.
C
...
C
      ...
      IF ( PASS1 ) THEN
C
C        Initialize local data structures. This routine is error-free.
C
...
C
C     Get parameters and data for this clock from the type 1 database.
C     Update the database if necessary.
C
      CALL ZZSCUP01 ( SC,     POLCTR, HDSCLK, SCPOOL, CLKLST, DPFREE,
     .                DPBUFF, IFREE,  INTBUF, SCBASE, PRVSC,  NFIELD,
     .                DELCDE, TIMSYS, NCOEFF, NPART,  COFBAS, STRBAS,
     .                ENDBAS, MODBAS, OFFBAS )

      IF ( FAILED() ) THEN
C
C        For safety, initialize the database on the next call to
C        any entry point of SC01. Setting PASS1 to .TRUE.
C        accomplishes this.
C
         PASS1 = .TRUE.
...
C
     ...
     .      //            'counting left to right, is invalid: '
     .      //            'component # is less than field offset #.' )
            CALL ERRINT ( '#', I                                     )
            CALL ERRDP  ( '#', SAVCMP                                )
            CALL ERRDP  ( '#', OFFSET(I)                             )
            CALL SIGERR ( 'SPICE(INVALIDSCLKSTRING)'                 )
...
C         contain all of the required data, or contains invalid data, an
C         error is signaled by a routine in the call tree of this
C         routine. The output argument CLKSTR will not be modified.
C
C         The variables that must be set by the SCLK kernel are:
C
C            -  The number of fields in an (unabridged) SCLK string
...
C         for conversion between SCLK and ET but is not loaded, an error
C         is signaled by a routine in the call tree of this routine. The
C         output argument CLKSTR will not be modified.
C
C         The time system that an SCLK kernel maps SCLK to is indicated
C         by the variable SCLK_TIME_SYSTEM_nn in the kernel, where nn
C         is the negative of the NAIF integer code for the spacecraft.
...
C         diagnosed by this routine.
C
C     4)  If the input value for TICKS is negative, the error
C         SPICE(VALUEOUTOFRANGE) is signaled. The output argument
C         CLKSTR will not be modified.
C
C     5)  If the output argument CLKSTR is too short to accommodate
C         the output string produced by this routine, the error
C         SPICE(SCLKTRUNCATED) is signaled. The output string
C         CLKSTR will not be modified.
C
C$ Files
C
...
C-    SPICELIB Version 2.0.0, 17-APR-1992 (NJB)
C
C        Error is now signaled if truncation of output string occurs.
C        Header was updated, particularly $Exceptions and $Restrictions
C        sections. Kernel pool watch is now set on required kernel
C        variables. Comment section for permuted index source lines
C        was added following the header.
...
C
C        An error is now signaled if truncation of output string
C        occurs.
C
...
C         contain all of the required data, or contains invalid data, an
C         error is signaled by a routine in the call tree of this
C         routine. The output argument ET will not be modified.
C
C         The variables that must be set by the SCLK kernel are:
C
C            -  The number of fields in an (unabridged) SCLK string
...
C         for conversion between SCLK and ET but is not loaded, an error
C         is signaled by a routine in the call tree of this routine. The
C         output argument ET will not be modified.
C
C         The time system that an SCLK kernel maps SCLK to is indicated
C         by the variable SCLK_TIME_SYSTEM_nn in the kernel, where nn
C         is the negative of the NAIF integer code for the spacecraft.
...
C         diagnosed by this routine.
C
C     4)  If the time system code is not recognized, the error
C         SPICE(VALUEOUTOFRANGE) is signaled.
C
C     5)  If the input SCLK value SCLKDP is out of range, the error
C         SPICE(VALUEOUTOFRANGE) is signaled. The output argument ET
C         will not be modified.
C
C     6)  If the SCLK rate used to interpolate SCLK values is
C         nonpositive, the error SPICE(VALUEOUTOFRANGE) is signaled.
C         The output argument SCLKDP will not be modified.
C
C     7)  If the partition times or SCLK coefficients themselves
...
C        sets or uses watches.
C
C        A check for invalid time system code was added.
C
C        A check for nonpositive clock rate was added.
C
C        Edited the header to comply with NAIF standard.
C
...
C
      ...
      ELSE
C
C        This code should be unreachable. It's present for safety.
C
         PASS1 = .TRUE.

         CALL SETMSG ( 'Invalid time system code # was found for '
     .   //            'SCLK #.'                                   )
         CALL ERRINT ( '#', TIMSYS                                 )
         CALL ERRINT ( '#', SC                                     )
         CALL SIGERR ( 'SPICE(VALUEOUTOFRANGE)'                    )
...
C         for conversion between SCLK and ET but is not loaded, an error
C         is signaled by a routine in the call tree of this routine. The
C         output argument SCLKDP will not be modified.
C
C         The time system that an SCLK kernel maps SCLK to is indicated
C         by the variable SCLK_TIME_SYSTEM_nn in the kernel, where nn
C         is the negative of the NAIF integer code for the spacecraft.
...
C         SPICE(VALUEOUTOFRANGE) is signaled.
C
C     5)  If the input ephemeris time value ET is out of range, the
C         error SPICE(VALUEOUTOFRANGE) is signaled. The output argument
C         SCLKDP will not be modified.
C
C     6)  If the SCLK rate used to interpolate SCLK values is
C         nonpositive, the error SPICE(VALUEOUTOFRANGE) is signaled.
C         The output argument SCLKDP will not be modified.
...
C        Edited the header to comply with NAIF standard.
C
C        Corrected description of invalid clock rate exception.
C
C-    SPICELIB Version 2.3.0, 09-SEP-2013 (BVS)
C
C        Updated to keep track of the POOL counter and call ZZCVPOOL.
...
C$ Version
C
C-    SPICELIB Version 2.0.0, 01-DEC-2021 (NJB) (JDR)
C
C        Updated to support buffering of data for multiple clocks.
C        This entry point tracks kernel pool changes but no longer
C        sets or uses watches.
...
C$Procedure SCTY01 ( SCLK type, using type 1 SCLK database )

      ENTRY SCTY01 ( SC, CLKTYP )

C$ Abstract
C
C     Return the type of a specified clock. The clock need not be
C     type 1.
C
C$ Disclaimer
C
...
C     CLKTYP     O   NAIF spacecraft clock type.
C
C$ Detailed_Input
C
C     SC       is a NAIF ID code for a spacecraft clock.
C
C$ Detailed_Output
C
C     CLKTYP   is the integer spacecraft clock type. The type
C              need not be 1.
C
C$ Parameters
C
...
C$ Exceptions
C
C     1)  If a kernel variable identifying requested SCLK type is not
C         found, the error SPICE(KERNELVARNOTFOUND) is signaled.
C
C     2)  If an error occurs in the course of updating the type 1 SCLK
C         database, the error is signaled by a routine in the call tree
C         of this routine.
C
C     3)  It is not an error to request the type of a clock that is not
C         type 1.
C
C$ Files
C
...
C$ Particulars
C
C     SCLK "type" is a SPICE-specific concept. It indicates which
C     mechanism is used by SPICE for representing time as measured
C     by a specified clock, and for converting those time measurements
C     to standard time systems such as TDB.
C
C     This routine speeds up determination of a spacecraft clock's type
C     by querying the type 1 SCLK database for information about that
C     clock. If the clock is not found, the kernel pool is searched,
C     and if the clock is determined to be type 1, information about it
C     is stored in the type 1 database. If the clock is not type 1,
C     the type is returned, but the type 1 database is not modified.
C
C     This is the only entry point of SC01 for which it is valid to
C     supply the ID of a clock not of type 1.
C
C$ Examples
C
C     See usage in the SPICELIB function SCTYPE.
C
C$ Restrictions
C
C     1)  This is essentially a SPICE-private routine. It should not
...
C$ Version
C
C-    SPICELIB Version 1.0.0, 01-DEC-2021 (NJB) (JDR)
C
C        New entry point.
C
C-&
...
C$ Index_Entries
C
C     return SCLK type, using type 1 database
C
C-&
...
C
C     At this point, PRVSC is either 0 or the ID of the last type 1
C     clock for which a successful call to an entry point of SC01
C     was made. PRVSC is never assigned the code of a clock that isn't
C     type 1.
C
      IF ( PASS1 ) THEN
C
...
C
C     The clock type is unknown at this point.
C
      CLKTYP = 0

C
C     Reinitialize the SC01 database if the kernel pool has been
...
C
      ...
      ELSE
C
C        We never indicate the clock is the "same" as the previous
C        one unless the clock is type 1.
C
      ...
      IF ( .NOT. SAMCLK )  THEN
C
C        Get the SCLK type of the input SCLK.
C
         KVNAME = 'SCLK_DATA_TYPE_#'
         CALL REPMI  ( KVNAME,  '#',  -SC,  KVNAME )

         CALL GIPOOL ( KVNAME, 1, 1, N, CLKTYP, FOUND )
         ...
         IF ( FAILED() .OR. .NOT. FOUND ) THEN
            ...
            IF ( .NOT. FOUND ) THEN
C
C              The GIPOOL call succeeded but the kernel variable was
C              not found.
C
               PASS1 = .TRUE.

               CALL SETMSG ( 'Kernel variable # was not '
...
C
C        At this point, the clock type is set.
C
         IF ( CLKTYP .NE. 1 ) THEN
C
C           The clock is not type 1, so we don't add it to the
C           SC01 database.
C
            PRVSC = 0
...
C
C        The SCLK is type 1 but may not be in the SC01 database, and
C        in any case is not the last type 1 clock seen on a successful
C        call to any SC01 entry point. Update the SCLK parameters and,
C        if necessary, the database.
C
         CALL ZZSCUP01( SC,     POLCTR, HDSCLK, SCPOOL, CLKLST, DPFREE,
     .                  DPBUFF, IFREE,  INTBUF, SCBASE, PRVSC,  NFIELD,
     .                  DELCDE, TIMSYS, NCOEFF, NPART,  COFBAS, STRBAS,
...
C
C     We can arrive here only if the clock type is 1.
C
      CLKTYP = 1
      PRVSC  = SC
...
C$Procedure SCPR01 ( SCLK partition bounds, type 1  )

      ENTRY SCPR01 ( SC, NPARTN, PARBEG, PAREND )

C$ Abstract
C
C     Return partition data for a specified type 1 clock.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C     INTEGER               SC
C     INTEGER               NPARTN
C     DOUBLE PRECISION      PARBEG ( * )
C     DOUBLE PRECISION      PAREND ( * )
C
C$ Brief_I/O
C
C     VARIABLE  I/O  DESCRIPTION
C     --------  ---  --------------------------------------------------
C     SC         I   NAIF spacecraft clock ID code.
C     NPARTN     O   Number of partitions.
C     PARBEG     O   Partition begin times.
C     PAREND     O   Partition end times.
C
C$ Detailed_Input
C
C     SC       is a NAIF ID code for a spacecraft clock.
...
C$ Detailed_Output
C
C     NPARTN   is the number of partitions for the clock
C              designated by SC.
C
C     PARBEG,
C     PAREND   are, respectively, arrays of partition begin and
C              end times for the clock designated by SC. The
C              times are expressed as encoded SCLK.
C
C$ Parameters
C
C     None.
...
C$ Exceptions
C
C     1)  If an error occurs in the course of updating the type 1 SCLK
C         database, the error is signaled by a
...
C$ Examples
C
C     See usage in the SPICELIB function SCPART.
C
C$ Restrictions
C
C     1)  This is essentially a SPICE-private routine. It should not
...
C$ Index_Entries
C
C     return partition data for a type 1 SCLK
C
C-&
...
C
C     Get partition data for this clock. Update the type 1 database
C     if necessary.
C
      CALL ZZSCUP01 ( SC,     POLCTR, HDSCLK, SCPOOL, CLKLST, DPFREE,
     .                DPBUFF, IFREE,  INTBUF, SCBASE, PRVSC,  NFIELD,
...
C
C     Set the output partition arguments.
C
      CALL MOVED ( DPBUFF(STRBAS+1), NPART, PARBEG )
      CALL MOVED ( DPBUFF(ENDBAS+1), NPART, PAREND )
