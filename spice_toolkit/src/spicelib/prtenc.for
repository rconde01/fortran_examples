...
C     portably, using 128 as the base for encoding.
C
C$ Disclaimer
C
...
C         the error SPICE(INSUFFLEN) is signaled.
C
C     2)  If the number to be encoded is negative, the error
C         SPICE(OUTOFRANGE) is signaled.
C
C                                                       MINLEN
C     3)  If the number to be encoded is larger than 128       - 1,
C         the error SPICE(OUTOFRANGE) is signaled.
C
C$ Files
C
C     None.
