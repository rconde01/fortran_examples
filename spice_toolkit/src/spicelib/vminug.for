C$Procedure VMINUG ( Negate vector, "-V", general dimension )
...
C     --------  ---  --------------------------------------------------
C     VIN        I   n-dimensional vector to be negated.
C     NDIM       I   Dimension of VIN (and also VOUT).
C     VOUT       O   Negated vector -V1.
C
C$ Detailed_Input
C
C     VIN      is any double precision vector of arbitrary size.
C
C     NDIM     is the dimension of VIN and VOUT.
C
C$ Detailed_Output
C
C     VOUT     is a n-dimensional double precision vector which
C              contains the negation of VIN.
C
C$ Parameters
C
C     None.
...
C     AND at least one component of VIN falls outside the common range.
...
C
C     1) Define a set of n-dimensional vectors and negate each of them.
C
C
C        Example code begins here.
C
C
C              PROGRAM VMINUG_EX1
C              IMPLICIT NONE
C
C        C
...
C        C     Local variables.
C        C
C              DOUBLE PRECISION      VIN  ( NDIM, SETSIZ )
...
C
C        C
C        C     Define a set of n-dimensional vectors.
C        C
C              DATA                  VIN  /
C             .                    -10.D0, 15.D0, -5.D0, 20.D0,
...
C              DO I=1, SETSIZ
C
C                 CALL VMINUG ( VIN(1,I), NDIM, VOUT )
C
C                 WRITE(*,'(A,4F7.1)') 'Input vector  : ',
C             .                        ( VIN(J,I), J=1,NDIM )
C                 WRITE(*,'(A,4F7.1)') 'Negated vector: ', VOUT
C                 WRITE(*,*)
C
C              END DO
C
...
C
C
C        Input vector  :   -10.0   15.0   -5.0   20.0
C        Negated vector:    10.0  -15.0    5.0  -20.0
C
C        Input vector  :     0.0    0.0    0.0    0.0
C        Negated vector:    -0.0   -0.0   -0.0   -0.0
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 1.0.3, 02-OCT-2020 (JDR)
C
...
C
C        Extended $Particulars section.
C
