...
C$ Exceptions
C
C     1)  If there is an issue while reading the components of a double
C         precision symbol table, an error is signaled by a routine in
C         the call tree of this routine. This normally indicates that
C         the double precision symbol table is corrupted.
C
C$ Files
C
C     None.
...
C                                            1.99096871D-7
C
C        BODY4_POLE_RA     1       3         3.17681D2           .TRUE.
C                                            1.08D-1
C                                            0.0D0
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 16-AUG-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Moved entry
C        from $Exceptions to $Restrictions and added entry #1 in
C        $Exceptions.
C
