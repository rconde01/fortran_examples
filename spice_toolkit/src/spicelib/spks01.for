...
C$ Exceptions
C
C     1)  If an error occurs while reading data from the source SPK
C         file, the error is signaled by a routine in the call tree of
C         this routine.
C
C     2)  If an error occurs while writing data to the output SPK file,
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 14-APR-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Completed
