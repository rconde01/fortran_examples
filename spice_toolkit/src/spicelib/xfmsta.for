...
C              geodetic or planetographic, BODY is ignored. It may
C              be a blank string.
C
...
C
C     7)  If radii for BODY are not found in the kernel pool, an error
C         is signaled by a routine in the call tree of this routine.
C
C     8)  If the size of the BODY radii kernel variable is not three,
C         an error is signaled by a routine in the call tree of this
C         routine.
C
C     9)  If any of the three BODY radii is less-than or equal to zero,
C         an error is signaled by a routine in the call tree of this
C         routine.
C
C     10) If body's equatorial radii are not equal and either the
...
C           Position [km]:
C            -2059.271283     -942.128329      -95.837672
C           Velocity [km/s]:
C                3.910113       -4.228139       -1.526561
C
C         Phoebe as seen by CASSINI - latitudinal
C           Position [km, rad, rad]:
C             2266.580876       -2.712515       -0.042296
C           Velocity [km/s, rad/s, rad/s]:
C               -1.730462        0.002416       -0.000706
C
C         Verification:
C         Phoebe as seen by CASSINI - rectangular
C           Position [km]:
...
C        C     planetographic for verification. The result should be
C        C     very close to STACYL.
C        C
...
C                0.500000        1.547722    -6356.240364
C           Velocity [rad/s, rad/s, km/s]:
C                0.100000       -0.004722       -0.195332
C
...
C
C        Edited he header to comply with NAIF standard.
C        Added missing begintext tag to the meta-kernel of example #1.
C        Modified example #2 to furnish a single kernel.
C
C        Updated Examples' kernels set to use PDS archived data.
C
...
C
C     BODYID is only used when the input or output coordinate
C     systems are geodetic or planetographic. The BODYID is the NAID ID
C     associated with the input body name.
C
      INTEGER               BODYID
