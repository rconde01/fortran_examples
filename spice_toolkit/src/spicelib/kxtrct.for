...
C     WORDSQ    I-O  String containing a sequence of words.
...
C
C     TERMS    is a set of words, any one of which may signal the end of
C              text of interest.
C
C     NTERMS   is the number of TERMS.
C
C     WORDSQ   is a character string made up of words, that may contain
C              the keyword in KEYWD.
C
C$ Detailed_Output
C
C     WORDSQ   is the input string stripped of all words from the
C              beginning of the keyword KEYWD to the end of the last
C              word preceding one of the words in TERMS (or the end of
C              the string if none of the TERMS follows KEYWD in the
C              string).
C
C     FOUND    is .TRUE. if KEYWD is present in the input WORDSQ.
C
C     SUBSTR   is the substring that begins with the first word
C              following KEYWD up to the beginning of any of the words
C              in TERM or the end of the string.
C
C$ Parameters
C
C     None.
...
C     Example 1.
C     ----------
C       Input:  WORDSQ  'FROM 1 October 1984 12:00:00 TO 1 January 1987'
C               KEYWD   'TO'
C               TERMS   'FROM'
C                       'TO'
C                       'BEGINNING'
C                       'ENDING'
C
C       Output: WORDSQ  'FROM 1 October 1984 12:00:00 '
C               FOUND   .TRUE.
...
C                       'ENDING'
C
C       Output: WORDSQ  ' TO 1 January 1987'
C               FOUND   .TRUE.
...
C     Example 3.
C     ----------
C       Input:  WORDSQ  'ADDRESS: 4800 OAK GROVE DRIVE PHONE: 354-4321 '
C               KEYWD   'ADDRESS:'
C               TERMS   'ADDRESS:'
C                       'PHONE:'
C                       'NAME:'
C
C       Output: WORDSQ  ' PHONE: 354-4321 '
C               FOUND   .TRUE.
...
C                       'NAME:'
C
C       Output: WORDSQ  'ADDRESS: 4800 OAK GROVE DRIVE PHONE: 354-4321 '
C               FOUND   .FALSE.
C               SUBSTR  ' '
C
C$ Restrictions
C
C     1)  It is the user's responsibility to make sure there is adequate
C         room in SUBSTR to contain the substring.
C
C     2)  SUBSTR cannot overwrite WORDSQ.
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 17-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement. Changed the input argument
C        STRING to WORDSQ for consistency with other routines.
C
...
C
      IF ( E .NE. 0 ) THEN
         DELIMS = ISRCHC( WORDSQ(B:E), NTERMS, TERMS )
      END IF
...
C
     ...
     .     .OR. ( DELIMS .NE. 0 ) ) THEN
         ...
         CALL SHIFTL ( WORDSQ(BERASE:), EERASE-BERASE+1, ' ',
     .                 WORDSQ(BERASE:)                        )
         SUBSTR = ' '
         RETURN
