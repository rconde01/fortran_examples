...
C$ Version
C
C-    SPICELIB Version 10.2.0, 27-AUG-2021 (JDR) (BVS) (NJB)
C
C        Changed input argument name KERNEL to FNAME in entry point
C        LDPOOL for consistency with other routines.
C
C        Edited the header of the umbrella routine and all its entry
C        points.
C
C        Updated SZPOOL $Detailed_Input section.
C
...
C
C-    SPICELIB Version 1.2.0, 09-MAR-1989 (HAN)
C
...
C
C-    SPICELIB Version 1.0.0, 08-JAN-1989 (IMU)
C
C-&
...
C     Error free.
C
C     1)  All known agents (those established through the SPICELIB
C         routine SWPOOL) will be "notified" that their watched
C         variables have been updated whenever CLPOOL is called.
C
C$ Files
C
C     None.
...
C     the kernel POOL subsystem. All the variables in the pool are
C     deleted. However, all watcher information is retained.
C
...
C     updated.
C
C     Application programs can delete watches by calling the SPICELIB
C     routine DWPOOL. See the header of that routine for details.
C
C$ Examples
C
...
C
C     1) This code example demonstrates how to assign values to kernel
C        pool variables, how to check for the existence of kernel pool
C        variables and how to clear the kernel pool, i.e. how to delete
C        all variable assignments loaded into the kernel pool.
C
C        Place a value into the kernel pool and check for the variable
C        to which the value has been assigned. Clear the kernel pool
C        and check for that variable again.
C
C        Example code begins here.
C
C
C              PROGRAM CLPOOL_EX1
C              IMPLICIT NONE
C
C        C
C        C     Local variables
C        C
C              DOUBLE PRECISION      DVALS ( 1 )
C
C              INTEGER               N
C
C              LOGICAL               FOUND
...
C
C        C
C        C     Place a value into the kernel pool. Recall the routines
C        C     for direct insertion of pool assignments have arrays for
C        C     input.
C        C
C              DVALS( 1 ) = -666.D0
C              CALL PDPOOL ( 'TEST_VAR', 1, DVALS )
C
C        C
C        C     Check for the variable assignment to TEST_VAR.
C        C
C              CALL GDPOOL ( 'TEST_VAR', 1, 1, N, DVALS, FOUND )
C
C              WRITE(*,'(A)') 'First call to GDPOOL:'
C
C              IF ( FOUND ) THEN
C
C                 WRITE(*,'(A,F8.2)') '   TEST_VAR value:', DVALS(1)
C
C              ELSE
C
C                 WRITE(*,'(A)') '   TEST_VAR not in kernel pool.'
C
C              END IF
C
C        C
C        C     Now clear the kernel pool.
C        C
C              CALL CLPOOL ()
C
C        C
C        C     Again, check for the TEST_VAR assignment.
C        C
C              CALL GDPOOL ( 'TEST_VAR', 1, 1, N, DVALS, FOUND )
C
C              WRITE(*,'(A)') 'Second call to GDPOOL:'
C
C              IF ( FOUND ) THEN
C
C                 WRITE(*,'(A,F6.2)') '   TEST_VAR value:', DVALS(1)
C
C              ELSE
C
C                 WRITE(*,'(A)') '   TEST_VAR not in kernel pool.'
...
C
C
C        First call to GDPOOL:
C           TEST_VAR value: -666.00
C        Second call to GDPOOL:
C           TEST_VAR not in kernel pool.
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 8.2.1, 17-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Added complete
C        code example.
C
C        Routine declared error free.
C
...
C     --------  ---  --------------------------------------------------
C     FNAME      I   Name of the text kernel file.
C
C$ Detailed_Input
C
C     FNAME    is the name of the text kernel file whose variables will
C              be loaded into the pool.
C
C$ Detailed_Output
C
C     None.
...
C$ Exceptions
C
C     1)  If an I/O error occurs while opening or reading a text kernel,
C         the error is signaled by a routine in the call tree of this
C         routine.
C
C     2)  If any text kernel parsing error occurs, the error is signaled
C         by a routine in the call tree of this routine.
C
C     3)  If a kernel pool overflow is detected, an error is signaled by
C         a routine in the call tree of this routine.
C
C$ Files
C
C     See FNAME in $Detailed_Input.
C
C$ Particulars
C
C     None.
...
C     specific arithmetic implementation.
C
C     1) The following program demonstrates how to load the variables
C        contained in a NAIF ASCII kernel file into the kernel pool
C        and how to determine the properties of a stored kernel
C        variable.
C
C        The program prompts for text kernel name and for the name of
C        a kernel variable. If the variable is present in the kernel
C        pool, the dimension and type of the variable are displayed.
C
C
C        Example code begins here.
C
C
C              PROGRAM LDPOOL_EX1
C              IMPLICIT NONE
C
C        C
...
C
C              INTEGER               KVNMLN
C              PARAMETER           ( KVNMLN = 33 )
C
C        C
C        C     Local variables
C        C
C              CHARACTER*(FILSIZ)    FNAME
C              CHARACTER*(KVNMLN)    VARNAM
C              CHARACTER*(1)         VTYPE
C
C              INTEGER               N
C
...
C
C        C
C        C     Prompt for the name of a text-kernel file.
C        C
C              CALL PROMPT ( 'Enter text-kernel name        > ', FNAME )
C
C        C
C        C     Load the kernel. The same operation could be done using
C        C     a FURNSH call.
C        C
C              CALL LDPOOL ( FNAME )
C
C              CALL PROMPT ( 'Enter name of kernel variable > ',
C             .               VARNAM )
C
C              CALL DTPOOL ( VARNAM, FOUND, N, VTYPE )
C
C              IF ( FOUND ) THEN
C                 WRITE(*,*) ' '
C                 WRITE(*,*) 'Properties of variable ',
C             .               VARNAM(:RTRIM(VARNAM)), ':'
C                 WRITE(*,*) ' '
C                 WRITE(*,*) '   Size:   ', N
C
C                 IF ( VTYPE .EQ. 'C' ) THEN
C
...
C
C                 WRITE(*,*) VARNAM,
C             .              ' is not present in the kernel pool.'
C
C              END IF
C
C              END
...
C
C        When this program was executed on a Mac/Intel/gfortran/64-bit
C        platform, using the PCK file gm_de431.tpc to ask for the
C        variable 'BODY000_GMLIST', the output was:
C
C
C        Enter text-kernel name        > gm_de431.tpc
C        Enter name of kernel variable > BODY000_GMLIST
C
C         Properties of variable BODY000_GMLIST:
C
C            Size:             65
C            Type:   Numeric
C
...
C$ Version
C
C-    SPICELIB Version 8.3.0, 17-AUG-2021 (JDR)
C
C        Changed input argument name KERNEL to FNAME for consistency
...
C-    SPICELIB Version 2.0.0, 18-OCT-1989 (RET)
C
C        A FAILED test was inserted into the control of the DO-loop
C        which reads in each kernel variable in LDPOOL.
C
C-    SPICELIB Version 1.2.0, 09-MAR-1989 (HAN)
C
C        Parameters BEGDAT and BEGTXT have been moved into the
...
C$ Exceptions
C
C     1)  If the output argument VALUES is not large enough to hold all
C         of the values of the kernel variable designated by NAME,
C         then memory will be corrupted. RTPOOL cannot detect this
C         error.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 8.1.1, 17-AUG-2021 (JDR) (NJB)
C
C        Edited the header to comply with NAIF standard.
C
C-    SPICELIB Version 8.1.0, 19-MAR-2009 (NJB)
...
C$ Abstract
C
C     Confirm the existence of a numeric kernel variable in the kernel
C     pool.
C
C$ Disclaimer
C
...
C     VARIABLE  I/O  DESCRIPTION
C     --------  ---  --------------------------------------------------
C     NAME       I   Name of a numeric kernel variable.
C     FOUND      O   .TRUE. when the variable is in the pool.
C
C$ Detailed_Input
C
C     NAME     is the name of the numeric kernel variable whose
C              existence in the kernel pool is to be checked.
C
C$ Detailed_Output
C
...
C
C     A better routine for determining the existence of numeric kernel
C     pool variables is the routine DTPOOL which determines the
...
C     specific arithmetic implementation.
C
C     1) The following code example demonstrates how to use EXPOOL
C        to confirm the existence of numeric kernel pool variables.
C        In the example, we will look for different variables;
C        some of them numeric, some string valued and some not
C        present in the kernel pool.
C
C        Use the kernel shown below; an IK defining two keywords
C        used to provide data for an instrument with NAIF ID -999001.
C
C
C           KPL/IK
C
C           File name: expool_ex1.ti
C
...
C        C
C              CHARACTER*(*)         IKNAME
C              PARAMETER           ( IKNAME = 'expool_ex1.ti' )
C
C              INTEGER               KPVNLN
C              PARAMETER           ( KPVNLN = 32 )
C
C              INTEGER               NKPVNM
C              PARAMETER           ( NKPVNM = 3  )
C
C        C
C        C     Local variables.
C        C
C              CHARACTER*(KPVNLN)    KEYWRD ( NKPVNM )
C
C              INTEGER               I
C
C              LOGICAL               FOUND
C
C        C
C        C     Define the variable names
C        C
C              DATA                  KEYWRD   /
C             .                             'INS-999001_FREQ_RGB',
C             .                             'NOT_IN_THE_POOL',
C             .                             'INS-999001_FREQ_UNITS' /
C
C        C
...
C
C        C
C        C        Check if the variable is numeric and present
C        C        in the kernel pool.
C        C
C                 CALL EXPOOL ( KEYWRD(I), FOUND )
C
C                 WRITE(*,*) 'Variable name: ', KEYWRD(I)
C
C                 IF ( FOUND ) THEN
C
C                    WRITE(*,*) '   It is numeric and exists in the '
C             .              // 'kernel pool.'
C
C                 ELSE
C
C                    WRITE(*,*) '   Either it is not numeric or it is '
C             .              // 'not in the kernel pool.'
C
C                 END IF
C
C              END DO
...
C
C
C         Variable name: INS-999001_FREQ_RGB
C            It is numeric and exists in the kernel pool.
C         Variable name: NOT_IN_THE_POOL
C            Either it is not numeric or it is not in the kernel pool.
C         Variable name: INS-999001_FREQ_UNITS
C            Either it is not numeric or it is not in the kernel pool.
C
C
C$ Restrictions
...
C$ Version
C
C-    SPICELIB Version 8.1.1, 17-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Added complete
C        code example.
C
C        Updated the header to reflect that only numeric variables
C        present in the kernel pool will cause the routine to return
C        .TRUE.
C
C-    SPICELIB Version 8.1.0, 19-MAR-2009 (NJB)
C
...
C$ Index_Entries
C
C     CONFIRM the existence of a pooled numeric kernel variable
C
C-&
...
C$ Exceptions
C
C     1)  If writing a variable to the output file fails due to an I/O
C         error, the error SPICE(WRITEERROR) is signaled.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 8.1.2, 17-AUG-2021 (JDR) (BVS)
C
C        Edited the header to comply with NAIF standard.
C
C        Updated the $Exceptions section to document the actual error
C        handling of the routine.
C
...
C         variable name, the error SPICE(KERVARSETOVERFLOW) is signaled.
C
C     2)  If sufficient room is not available to hold a new agent
C         name, the error SPICE(TOOMANYWATCHES) is signaled.
C
...
C         that kernel variable, the error SPICE(AGENTLISTOVERFLOW) is
C         signaled.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 8.1.2, 17-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C-    SPICELIB Version 8.1.1, 30-JUN-2014 (NJB)
...
C     MAXCHR   is the maximum number of characters that can be
C              stored in a component of a string valued kernel
C              variable. This value is currently 80.
C
C$ Exceptions
C
...
C-    SPICELIB Version 8.1.1, 17-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Added MAXCHR
C        description in $Parameters.
C
C-    SPICELIB Version 8.1.0, 19-MAR-2009 (NJB)
C
C        ZZPINI call was updated for compatibility
...
C     specific arithmetic implementation.
C
C     1) The following program demonstrates how to determine the
C        properties of a stored kernel variable. The program prompts
C        for text kernel name and for the name of a kernel variable.
C        If the variable is present in the kernel pool, the dimension
C        and type of the variable are displayed.
C
C
C        Example code begins here.
C
C
C              PROGRAM DTPOOL_EX1
C              IMPLICIT NONE
C
C        C
C        C     SPICELIB functions
...
C
C        When this program was executed on a Mac/Intel/gfortran/64-bit
C        platform, using the FK file named cas_v40.tf to ask for the
C        variable 'FRAME_-82104_NAME', the output was:
C
C
C        Enter text-kernel name        > cas_v40.tf
C        Enter name of kernel variable > FRAME_-82104_NAME
C
C         Properties of variable FRAME_-82104_NAME:
C
C            Size:              1
C            Type:   Character
C
...
C$ Abstract
C
C     Provide toolkit programmers a method for programmatically
C     inserting character data into the kernel pool.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C
C     2)  If there is not sufficient room to insert a new variable into
C         the kernel pool and NAME is not already present in the kernel
C         pool, an error is signaled by a routine in the call tree of
C         this routine.
C
C     3)  If there is not sufficient room to insert the values
C         associated with NAME, the error SPICE(NOMOREROOM) is signaled.
C
C     4)  If the kernel pool variable name length exceeds its maximum
C         allowed length (see Kernel Required Reading, kernel.req), the
C         error SPICE(BADVARNAME) is signaled.
C
C$ Files
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 9.1.1, 27-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
...
C
C     Provide toolkit programmers a method for programmatically
C     inserting double precision data into the kernel pool.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C
C     Provide toolkit programmers a method for programmatically
C     inserting integer data into the kernel pool.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C
C     N        is the number of entries in CVALS.
C
C$ Detailed_Output
C
C     None.
...
C$ Exceptions
C
C     1)  If any of the kernel pool variables names or their values, as
C         provided in the input CVALS array, cannot be parsed, an error
C         is signaled by a routine in the call tree of this routine.
C
C     2)  If there is no room left in the kernel pool to store all
C         variables present in the input CVALS array, an error is
C         signaled by a routine in the call tree of this routine.
C
C     3)  If the length of any kernel pool variable name present in the
C         input CVALS array exceeds its maximum allowed length (see
C         Kernel Required Reading, kernel.req), an error is signaled by
C         a routine in the call tree of this routine.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 8.3.1, 17-AUG-2021 (JDR) (BVS)
C
C        Edited the header to comply with NAIF standard.
C
C        Updated $Exceptions section to better describe the error
C        handling of this routine.
C
...
C
C                 'MAXVAR'    is the maximum number of variables that
C                             the kernel pool may contain at any one
C                             time. MAXVAR should be a prime number.
C
C                 'MAXLEN'    is the maximum length of the variable
C                             names that can be stored in the kernel
C                             pool.
C
C                 'MAXVAL'    is the maximum number of distinct values
C                             that may belong to the variables in the
C                             kernel pool. Each variable must have at
C                             least one value, and may have any number,
C                             so long as the total number does not
C                             exceed MAXVAL. MAXVAL must be at least as
C                             large as MAXVAR.
C
C                 'MXNOTE'    is the maximum number of distinct
C                             variable-agents pairs that can be
C                             maintained by the kernel pool. (A variable
C                             is "paired" with an agent, if that agent
C                             is to be notified whenever the variable is
C                             updated.)
C
C                 'MAXAGT'    is the maximum number of agents that can
C                             be kept on the distribution list for
C                             notification of updates to kernel
C                             variables.
C
C                 'MAXCHR'    is the maximum number of characters that
C                             can be stored in a component of a string
C                             valued kernel variable.
C
C                 'MAXLIN'    is the maximum number of character strings
C                             that can be stored as data for kernel pool
C                             variables.
C
C              Note that the case of NAME is insignificant.
C
C$ Detailed_Output
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 17-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Updated NAME
C        description to not refer to the main routine.
C
...
C$ Version
C
C-    SPICELIB Version 8.3.1, 17-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Moved example from $Restrictions to $Examples section.
C
C-    SPICELIB Version 8.3.0, 30-JUL-2013 (BVS)
C
C        Updated to increment POOL state counter.
...
C
C     MAXLEN   is the maximum length of the variable names that
C              can be stored in the kernel pool. This value is
C              currently 32.
C
C$ Exceptions
C
...
C         SPICE(BADARRAYSIZE) is signaled.
C
C     2)  If CVALS has declared length less than the size of a variable
C         name to be returned, the name will be truncated on the right.
C         See MAXLEN for the maximum size of variable names.
C
C$ Files
C
C     None.
...
C-    SPICELIB Version 8.1.1, 17-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Added MAXLEN
C        description in $Parameters.
C
C-    SPICELIB Version 8.1.0, 19-MAR-2009 (NJB)
C
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 17-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
...
C
C     Return copies of certain POOL variables that are used in the
C     implementation of the watcher mechanism. These variables
C     comprise a data structure that maps kernel variable names to sets
C     of agents watching those kernel variables.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C         set WTVARS, an error is signaled by a routine
...
C$ Version
C
C-    SPICELIB Version 1.0.2, 17-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
