...
C$ Version
C
C-    SPICELIB Version 2.0.0, 14-JUN-2021 (NJB)
C
C        Updated logic for limb grazing geometry to reduce chance of
C        failing to find an intercept.
C
...
C        failing to find an intercept.
C
C        In the main solution loop, if the iteration count has not
C        reached the termination limit minus one, and if a call to
C        UDRAYX does produce an intercept, NPEDLN and NPLNPT are used
C        to find the tangent point. That point is used to produce a new
C        light time estimate. In some cases, this allows an intercept
C        to be found again in a later pass through the loop, and a
C        solution intercept to be found at that point or later.
C-&
...
C
C     Making it to this point means we've got an intersection, given
C     our current light time estimate. It's possible that a better
C     light time estimate will yield no intersection.
C
...
C
C        If there's no intercept, try to get an estimate of the
C        intercept location and the light time by using the nearest
C        point to the ellipsoid on the line containing the ray. This is
C        useful only if the iteration count has not reached its maximum
C        value (the termination value minus one), since the point of
C        this is to make it possible to find an intercept.
C
C        Note that an intercept was already found using the initial
C        aberration corrections, so we can't get to this case unless
C        we have near-intercept geometry.
C
         IF ( .NOT. FOUND ) THEN

            IF ( I .LT. ( NITR-1 ) ) THEN
C
C              SPOINT is the nearest point to the ellipsoid on the
C              line containing the ray.
C
               CALL UDNEAR ( OBSPOS, TRGDIR, ET, PNEAR, RAYALT )

               CALL NPLNPT ( OBSPOS, TRGDIR, PNEAR, SPOINT, RAYALT )

            ELSE
C
C              We're not going to find an intercept.
C
               CALL CHKOUT ( RNAME )
               RETURN
