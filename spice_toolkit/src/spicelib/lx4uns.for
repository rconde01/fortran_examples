...
C     Error free.
C
C     1)  If FIRST is beyond either end of the string, then LAST will be
C         returned with the value FIRST-1 and NCHAR will be returned
C         with the value 0.
C
C     2)  If STRING(FIRST:FIRST) is not part of an unsigned integer then
C         LAST will be returned with the value FIRST-1 and NCHAR will be
C         returned with the value 0.
C
C$ Files
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 04-AUG-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard.
C
C        Fixed return value for LAST in $Exceptions section entry #1.
C
...
C
C        If this character is a digit, move the LAST pointer one
