...
C
C         the error SPICE(VALUEOUTOFRANGE) is signaled. Longitudes
...
C         + ANGMRG, the error SPICE(INVALIDLONEXTENT) is signaled.
...
C
C         the error SPICE(VALUEOUTOFRANGE) is signaled. Latitudes
C         outside of the range by a smaller amount than ANGMRG will be
...
C         out of order, the error SPICE(BOUNDSOUTOFORDER) is signaled.
C
...
C         SPICE(ZEROBOUNDSEXTENT) is signaled. If the lower
...
C         SPICE(DEGENERATESURFACE) is signaled.
C
...
C         upper bound, the error SPICE(BOUNDSOUTOFORDER) is signaled.
C
C     13) If the coordinate system code is not recognized, the error
C         SPICE(NOTSUPPORTED) is signaled.
C
C     14) If any vertex index belonging to an input plate is outside of
C         the range 1:NV, the error SPICE(BADVERTEXINDEX) is signaled.
C
...
C         DSK file, the error is signaled by a routine in the call
...
C
C        Use the DSK kernel below to provide, for simplicity, the
...
C        C
C        C        Create a three-segment DSK file using plate model
C        C        data for Phobos. Use latitudinal, rectangular, and
C        C        planetodetic coordinates in the respective segments.
C        C
C        C        For simplicity, use an existing DSK file to provide
C        C        the input plate and vertex data. The selected input
C        C        file has one segment.
C        C
C        C           Version 1.0.0 22-JAN-2016 (NJB)
C        C
C              PROGRAM DSKW02_EX1
C              IMPLICIT NONE
C
C              INCLUDE 'dla.inc'
...
C         Done.
C         Creating segment            1
C         Creating spatial index...
C         Done.
C         Computing radius bounds of plate set...
C         Done.
C         Writing segment...
C         Done.
C         Creating segment            2
C         Creating spatial index...
C         Done.
C         Computing Z-coordinate bounds of plate set...
C         Done.
C         Writing segment...
C         Done.
C         Creating segment            3
C         Creating spatial index...
C         Done.
C         Computing altitude bounds of plate set...
C         Done.
C         Writing segment...
C         Done.
C         Segregating and closing DSK file...
C         Done.
C
C
C        Note that after run completion, a new DSK exists in the output
C        directory.
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 28-AUG-2021 (JDR) (NJB)
C
C        Edited the header to comply with NAIF standard.
C        Added solution to code example.
C
C        Corrected description of coverage requirements for class 1
C        segments.
C
C        Deleted paragraph saying that, except for changes made to
C        move longitude values into range, the values are stored in
C        segment "as is."
C
