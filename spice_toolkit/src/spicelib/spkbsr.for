...
C$ Version
C
C-    SPICELIB Version 6.1.0, 13-OCT-2021 (JDR) (BVS) (NJB)
C
C        Increased BTSIZE (from 200 to 10000).
C
C        Updated entry point SPKSFS to always initialize FOUND.
C
C        Edited the header of SPKBSR umbrella routine and its entry
C        points SPKLEF, SPKUEF and SPKSFS.
C
...
C
C-    SPICELIB Version 1.0.2, 09-SEP-1991 (HAN)
C
...
C$ Abstract
C
C     Load an ephemeris file for use by the readers. Return that file's
C     handle, to be used by other SPK routines to refer to the file.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C$ Detailed_Input
C
C     FNAME    is a string containing the name of the file to be loaded.
C
C$ Detailed_Output
C
C     HANDLE   is an integer handle assigned to the file upon loading.
C              Almost every other SPK routine will subsequently use this
C              number to refer to the file.
C
C$ Parameters
C
...
C
C     2)  If an attempt is made to load more files than is specified by
C         the local parameter FTSIZE, and if the DAF system has room to
C         load another file, the error SPICE(SPKFILETABLEFULL) is
C         signaled. The current setting of FTSIZE does not allow this
C         situation to arise: the DAF system will trap the error before
C         this routine has the chance.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 5.0.2, 17-JUN-2021 (JDR)
C
...
C-    SPICELIB Version 2.0.0, 25-NOV-1992 (JML)
C
C        When loading a file, SPKLEF now checks if the file table is
C        full only after determining that the file is not currently
...
C
C        A bug in the way that SPKLEF and SPKUEF clean up the body
C        tables after a file is unloaded was fixed.
C
...
C$Procedure SPKUEF ( SPK Kernel, Unload ephemeris file )
...
C     HANDLE, is removed. Any segment table entry which came from the
C     specified file is also deleted.
C
...
C$ Version
C
C-    SPICELIB Version 4.1.2, 17-JUN-2021 (JDR)
C
C        Updated the header to comply with NAIF standard.
C
C        Removed the reference to SPK required reading from
C        $Literature_References section. Extended $Particulars
C        section.
C
...
C        following the header.
C
C-    SPICELIB Version 1.1.0, 02-MAY-1990 (RET)
C
...
C$ Abstract
C
C     Search through loaded SPK files to find the highest-priority
C     segment applicable to the body and time specified and buffer
C     searched segments in the process, to attempt to avoid re-reading
C     files.
C
C$ Disclaimer
C
...
C$ Detailed_Output
C
C     HANDLE   is the handle of the SPK file containing a located
C              segment.
C
C     DESCR    is the descriptor of a located segment.
C
...
C
C     2)  If an error occurs while this routine attempts to extract
C         segment descriptors from loaded SPK files, the error is
C         signaled by a routine in the call tree of this routine.
C
C         Note however that I/O errors occurring during reads of DAF
C         double precision records are NOT treated as SPICE errors
C         and are not signaled.
C
C$ Files
C
C     All SPK files loaded by FURNSH or SPKLEF are potential search
C     targets for SPKSFS.
C
C$ Particulars
C
...
C
C        Added entry #2 to $Exceptions section. Added reference to
C        FURNSH routine in $Files section.
C
