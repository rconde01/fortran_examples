...
C         routine.
C
C     2)  If transmission corrections are commanded, the error
C         SPICE(INVALIDOPTION) is signaled.
C
C     3)  If either the target or observer input strings cannot be
...
C
C     8)  If METHOD contains an invalid terminator type, the error
C         SPICE(INVALIDTERMTYPE) is signaled.
C
C     9)  If the target and observer have distinct identities but are
C         at the same location, the error SPICE(NOSEPARATION) is
C         signaled.
C
C     10) If insufficient ephemeris data have been loaded prior to
C         calling TERMPT, an error is signaled by a routine in
...
C         system barycenter.
C
C     11) If the computation method requires an ellipsoidal target shape
C         and triaxial radii of the target body have not been loaded
C         into the kernel pool prior to calling TERMPT, an error is
...
C         have not been loaded prior to calling TERMPT, an error is
C         signaled by a routine in the call tree of this routine.
C
...
C
C     18) If the GUIDED terminator type is used with the
C         ELLIPSOID TERMINATOR aberration correction locus, the
C         error SPICE(BADTERMLOCUSMIX) is signaled.
C
...
C
C     21) If the terminator points cannot all be stored in the output
C         POINTS array, the error SPICE(OUTOFROOM) is signaled.
C
C     22) If NCUTS is greater than 1, the roll step ROLSTP must be
C         positive. Otherwise, the error SPICE(INVALIDROLLSTEP) is
C         signaled.
C
C$ Files
C
...
C                    WRITE(*,FM3) '    Number of terminator points  '
...
C            Number of terminator points  at this roll angle:  1
C               Terminator points:
C                -11.186660113        -0.142367244        -4.646136750
C
C          Roll angle (deg) =         240.000000000
C             Target epoch  =   271684865.152078211
C            Number of terminator points  at this roll angle:  1
...
C
C        C
C        C              Get illumination angles for this terminator
C        C              point.
C        C
C                       M = K+START
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 12-SEP-2021 (NJB) (JDR)
C
...
C        Corrected the filename of code example #1's meta-kernel.
C        Removed unnecessary SAVE statement from code example #2.
C
