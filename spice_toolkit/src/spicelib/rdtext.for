...
C                 simultaneously open one file for each available
...
C         SPICE(TOOMANYFILESOPEN) is signaled.
C
...
C         the error SPICE(INQUIREFAILED) is signaled.
C
C$ Files
C
C     See input FILE.
...
C          Operations", Section 6.1.7, p 6-6, Digital Equipment
C          Corporation, September 1984.
C
C     [2]  "Microsoft FORTRAN Reference", Section C.3, p 404, Microsoft
C          Corporation, 1989.
C
C     [3]  "Sun FORTRAN Programmer's Guide", Section 7.2, p 73, Sun
C          Microsystems, Revision A, 6 May 1988.
C
C     [4]  "The Unix Man Pages," for limits on the HP and Silicon
C          Graphics. The value of OPEN_MAX refers to the number of files
C          a process can have open.
C
...
C$ Version
C
C-    SPICELIB Version 6.27.0, 28-NOV-2021 (BVS)
C
C        Updated for MAC-OSX-M1-64BIT-CLANG_C.
C
C-    SPICELIB Version 6.26.0, 13-AUG-2021 (JDR)
C
...
C
C-    SPICELIB Version 5.0.0, 09-NOV-1993 (HAN)
C
...
C        occurred while reading the text file, LINE was returned with
...
C$ Version
C
C-    SPICELIB Version 6.1.0, 03-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard.
