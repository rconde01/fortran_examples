...
C$ Abstract
C
C     Create a formatted string that represents a double precision
C     number, using a format picture.
C
C$ Disclaimer
C
...
C$ Exceptions
C
C     1)  If PICTUR begins with a blank, the error SPICE(NOPICTURE) is
C         signaled.
C
C     2)  If PICTUR consists only of '+', '-', '.', '+.' or '-.' are
C         regarded as invalid (there's no significant component to the
C         picture.) therefore, the error SPICE(BADPICTURE) is signaled.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.3, 27-OCT-2021 (JDR) (EDW) (NJB)
C
...
C
     ...
     .   //            'to the supplied format picture. The '
