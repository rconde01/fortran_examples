C$Procedure EV2LIN ( Evaluate "two-line" element data, near-earth )
...
C     NAIF supports EV2LIN only for backward compatibility.
C
C     Evaluate NORAD two-line element data for near-earth, earth
C     orbiting spacecraft (that is spacecraft with orbital periods
C     less-than 225 minutes). This evaluator uses algorithms as
...
C              with the NMODL. Empirically, 6 seems to be a good
...
C
C     2)  If the number of iterations for the calculation exceeds the
C         limit for this value, the error SPICE(ITERATIONEXCEEDED) is
C         signaled. This error should signal only for bad (nonphysical)
C         TLEs.
C
C$ Files
C
...
C     Hoots 1980 [1]. This code is an adaptation of
C     the NORAD routine SGP4 to eliminate common blocks, allow
C     buffering of models and intermediate parameters and double
C     precision calculations.
C
C     Near earth is as an orbital period of less than 225
C     minutes.
C
C$ Examples
C
...
C        Position:  -4644.60403398  -5038.95025540   -337.27141117
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 01-NOV-2021 (JDR) (EDW)
C
...
C
C        Added Spacetrack Report #3 to literature references.
C
...
C$ Index_Entries
C
C     DEPRECATED Evaluate NORAD two-line element data.
C
C-&
...
C
C        HEAD           points to the most recently used ephemeris
C                       model.
C
C
         HEAD            = 1
...
C     of x, we can use Newton's method to achieve faster
C     convergence.
C
