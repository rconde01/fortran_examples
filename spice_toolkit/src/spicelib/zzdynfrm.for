...
C         call tree of this routine.
C
C     23) If a body RADII vector has other than exactly thee elements,
C         the error SPICE(INVALIDCOUNT) is signaled by a routine in
C         the call tree of this routine.
C
C     24) If a body RADII vector has any element less-than or
...
C           -------------------
C           Array of "from" frames
C           Array of "to" frames
C
C
C
C
...
C$ Version
C
C-    SPICELIB Version 2.0.0, 09-OCT-2021 (NJB) (EDW)
C
C        Support for product frames was added. Re-ordered header
C        sections.
C
C        Body radii accessed from kernel pool using ZZGFTREB.
C
C        Bug fix: aberration correction strings for observer-target
C        velocity vectors are now parsed using ZZVALCOR. This improves
C        error checking. Note that aberration correction strings for
C        constant vectors used in two-vector frame specifications are
C        still parsed using ZZPRSCOR.
C
C        Bug fix: calls to FAILED after calls to ZZDYNVAC were added.
C
C        Added some FAILED() calls.
C
...
C
      ...
      CHARACTER*(FRNMLN)    FFRAMS ( MXNFAC )
      ...
      CHARACTER*(FRNMLN)    TFRAMS ( MXNFAC )
...
C
C           We expect to see specifications of an axis sequence, units,
...
C
         ...
         ELSE IF ( DYNFAM .EQ. KVPROD ) THEN
C
C           The frame belongs to the product family.
C
C           We expect to see specifications of "from" and "to" frames
C           that make up the product.
C
C           Look up the "from" and "to" frames that define the product.
C
            CALL ZZDYNVAC ( INNAME, INFRAM, KWFFRM, MXNFAC, M, FFRAMS )
            CALL ZZDYNVAC ( INNAME, INFRAM, KWTFRM, MXNFAC, N, TFRAMS )
            ...
            IF ( N .NE. M ) THEN
               ...
               CALL SETMSG ( 'Definition of product '
     .         //            'parameterized dynamic frame # '
     .         //            'has # "from" frames and # "to" '
     .         //            'frames. These counts must match.' )
               CALL ERRCH  ( '#',  INNAME                       )
               CALL ERRINT ( '#',  M                            )
               CALL ERRINT ( '#',  N                            )
               CALL SIGERR ( 'SPICE(BADFRAMECOUNT)'             )
               CALL CHKOUT ( RNAME                              )
               RETURN
...
C
C           The product of the factors is the transformation from the
C           base frame to the output frame. We need the inverse of
C           this transformation.
C
C           Compute the inverse of the product of the factors; place
C           the result in XFORM.
C
            CALL NAMFRM ( FFRAMS(1), FROMID )
            CALL NAMFRM ( TFRAMS(1), TOID   )

            CALL ZZFRMCH0 ( TOID, FROMID, T0, XFORM )
            ...
            DO I = 2, N
               ...
               CALL NAMFRM ( FFRAMS(I), FROMID )
               CALL NAMFRM ( TFRAMS(I), TOID   )
               ...
               CALL MXMG  ( XFORM, XFTEMP, 6, 6, 6, XOUT  )
               CALL MOVED ( XOUT,  36,              XFORM )
