...
C$ Detailed_Input
C
C     UTCSTR   is an input time string, containing a Calendar or Julian
C              Date, UTC. Any input string acceptable to the SPICELIB
C              routine TPARTV are acceptable to UTC2ET. The length of
C              UTCSTR should not exceed 80 characters.
C
C$ Detailed_Output
C
...
C         SPICE(INVALIDTIMESTRING) is signaled.
C
...
C$ Particulars
C
C     Note: NAIF recommends the use of STR2ET instead of UTC2ET.
C
...
C
C     As noted in the $Exceptions section, this routine does not attempt
C     to account for variations in the length of the second that were
C     used prior to Jan 1, 1972. Instead each "UTC" day prior to
C     Jan 1, 1972 is assumed to have exactly 86400 TDT seconds.
C
C     Ancient Epochs
C     --------------
C
...
C     you make use of the SPICELIB routine STR2ET.
C
C$ Examples
C
...
C           abbreviated with a leading quote as in 'xy (such as '92) is
C           to treat the year as 19xy if xy > 68 and to treat it is
C           20xy otherwise. Thus '69 is interpreted as 1969 and '68 is
C           treated as 2068. However, you may change the "split point"
C           and centuries through use of the SPICELIB routine TSETYR
C           which is an entry point in the SPICELIB module TEXPYR. See
C           that routine for a discussion of how you may reset the
C           split point.
C
C        ** All epochs are regarded as belonging to the Gregorian
C           calendar. We formally extend the Gregorian calendar
C           backward and forward in time for all epochs. If you have
C           epochs belonging to the Julian Calendar, consult the
C           SPICELIB routines TPARTV and JUL2GR for a discussion
C           concerning conversions to the Gregorian calendar and ET.
C
...
C           the year.
C
C     2) Convert a calendar date in UTC string format to Julian
C        Ephemeris date.
C
...
C
C
C              PROGRAM UTC2ET_EX1
C              IMPLICIT NONE
C
C        C
C        C     SPICELIB functions
C        C
C              DOUBLE PRECISION      UNITIM
C
C        C
C        C     Local constants.
C        C
C              CHARACTER*(*)         UTCSTR
C              PARAMETER           ( UTCSTR = 'Dec 19 2003  16:48:00' )
C
C        C
C        C     Local variables.
C        C
C              DOUBLE PRECISION      ET
C              DOUBLE PRECISION      JED
C
C        C
C        C     Load the LSK file.
C        C
C              CALL FURNSH ( 'naif0012.tls' )
C
C        C
C        C     Convert input UTC string to Ephemeris Time.
C        C
C              CALL UTC2ET ( UTCSTR, ET )
C              WRITE(*,'(2A)') 'UTC time             : ', UTCSTR
C              WRITE(*,'(A,F22.9)') 'Ephemeris time       :', ET
C
C        C
C        C     Convert the Ephemeris Time to Julian ephemeris date,
C        C     i.e. Julian date relative to TDB time scale.
C        C
C              JED = UNITIM( ET, 'ET', 'JED' )
C              WRITE(*,'(A,F22.9)') 'Julian Ephemeris Date:', JED
C
C              END
C
C
...
C
C
C        UTC time             : Dec 19 2003  16:48:00
C        Ephemeris time       :   125124544.183560610
C        Julian Ephemeris Date:     2452993.200742865
C
C
C$ Restrictions
C
C     1)  The conversion between ET and UTC depends on the values in the
C         input kernel pool. The kernel pool should be loaded prior to
C         calling this routine.
C
C     2)  Before using this routine for epochs prior to Jan 1, 1972
...
C$ Version
C
C-    SPICELIB Version 2.1.1, 13-AUG-2021 (JDR) (EDW)
C
