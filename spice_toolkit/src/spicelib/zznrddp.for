...
C$ Version
C
C-    SPICELIB Version 2.2.0, 14-OCT-2021 (NJB)
C
C        Bug fix: fixed routine name in CHKIN/CHKOUT calls (NRDDP 
C        -> ZZNRDDP).
C
...
C
C        Eliminated bug in ZZDPINIT that allowed NaN contamination
...
C
C        Eliminated bug that allowed NaN contamination for zero
...
C
C        Check for, and adjust SH, at inclinations near 0 and 180 degs.
C
