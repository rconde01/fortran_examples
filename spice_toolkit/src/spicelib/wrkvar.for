...
C     TABSYM,
C     TABPTR,
C     TABVAL     I   Double precision symbol table.
C
C$ Detailed_Input
C
C     UNIT     is the logical unit to which the variable will be
C              written. This is usually the logical unit to which the
C              output kernel file is connected.
C
C     NAME     is the name of the variable to be written to UNIT.
C
C     DIRCTV   is the directive linking NAME and its associated values
C              in the kernel file. This may be any of the directives
C              recognized by RDKVAR.
C
C     TABSYM,
C     TABPTR,
C     TABVAL   are the components of a double precision symbol table.
C
C              On input, the table may or may not contain any variables.
C
C$ Detailed_Output
C
C     None.
...
C$ Exceptions
C
C     1)  If an error occurs writing the variable to UNIT, the error
C         SPICE(WRITEERROR) is signaled.
C
C$ Files
C
...
C
C        Updated $Brief_I/O entry for symbol table (TABSYM, TABPTR,
C        TABVAR) to indicate that these arguments only are an input to
C        the routine, and not an output. Removed $Detailed_Output
C        documentation, as the routine operates by side-effects and has
...
C
C-    SPICELIB Version 1.0.0, 31-JAN-1990 (IMU) (NJB)
C
C-&
