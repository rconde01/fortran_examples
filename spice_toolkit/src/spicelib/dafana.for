...
C
C     1) The following example illustrates one possible way to copy
C        an array from one DAF to another, N words at a time.
C
C        Use the CK kernel below as the original DAF file.
C
C           vo2_swu_ck2.bc
C
C
...
C        C     Local variables.
C        C
C              CHARACTER*(MAXNSZ)    NAME
C
C              DOUBLE PRECISION      DATA   ( NWORDS )
...
C
C        C
C        C     Open the origin DAF file for reading.
C        C
C              CALL DAFOPR ( 'vo2_swu_ck2.bc', ORIGIN )
C
C        C
C        C     Start forward search in origin DAF.
C        C
C              CALL DAFBFS ( ORIGIN )
C
C        C
C        C     Find the first array in origin DAF.
C        C
C              CALL DAFFNA ( FOUND  )
C
C        C
C        C     Get the summary and name of the current array in the
C        C     ORIGIN DAF file
C        C
C              CALL DAFGS  ( SUM  )
C              CALL DAFGN  ( NAME )
C
C        C
C        C     Unpack the summary.
C        C
C              CALL DAFHSF ( ORIGIN, ND, NI )
C              CALL DAFUS  ( SUM,    ND, NI, DC, IC )
C
C        C
C        C     Open the target DAF file for writing. Use 'CK' as
C        C     data type, and reserve no records for comments.
C        C
C              CALL DAFONW ( 'dafana_ex1.bc', 'CK', ND, NI,
C             .              'CK file created for example 1 DAFANA', 0,
C             .              TARGET )
C
C        C
C        C     Begin a new array in the target DAF file, using the
C        C     origin SUM and NAME.
C        C
C              CALL DAFBNA ( TARGET, SUM, NAME )
C
C        C
C        C     Copy the complete array for the first segment of the
C        C     origin DAF file.
C        C
C              BIDX = IC(NI-1)
C              EIDX = IC(NI  )
C
C              DO WHILE ( BIDX .LE. EIDX )
C
C                 CHUNK = MIN ( BIDX + NWORDS - 1, EIDX )
C
C                 CALL DAFGDA ( ORIGIN, BIDX, CHUNK, DATA )
C                 CALL DAFADA ( DATA,   NWORDS )
C
C                 BIDX = BIDX + NWORDS
C
C              END DO
C
C        C
C        C     End the new array in the target DAF.
C        C
C              CALL DAFENA
C
C        C
C        C     Close the DAF files.
C        C
C              CALL DAFCLS ( ORIGIN )
C              CALL DAFCLS ( TARGET )
C
C              END
C
C
...
C         of data to multiple DAFs.
C
C         Assume we have data from a text file containing three
C         columns of numbers. We will write the data from each
C         column out to a separate DAF.
C
C         To confirm that the DAFs created by this program contain the
C         correct contents, we will read the data from each DAF and
C         combine it to create a matrix. This matrix should contain
C         the same data as the file we assumed to be the source for
C         our dataset.
C
C         The format of the output text should be as follows:
C
C            .-                   -.
C            |  n11    n12    n13  |
C            |  n21    n22    n23  |
...
C            `-                   -'
C
C         where the symbol nij indicates the jth number on the ith line
C         of the source data file.
C
C
C        Example code begins here.
C
C
C              PROGRAM DAFANA_EX2
C              IMPLICIT NONE
C
C        C
C        C     Assume we have read columns of d.p. numbers
C        C     from a text file. Write the data from each
C        C     column into a separate DAF.  Read these DAFs
C        C     and create a matrix containing the same data
C        C     as assumed input text file.
C        C
...
C        C     Initial values
C        C
C              DATA                  DAFNAM   /  'COLUMN1.DAF',
C             .                                  'COLUMN2.DAF',
C             .                                  'COLUMN3.DAF'  /
C
C              DATA                  NUMBER  /
C             .               11.D0, 21.D0, 31.D0, 41.D0, 51.D0,
C             .               61.D0, 71.D0, 81.D0, 91.D0,
C             .               12.D0, 22.D0, 32.D0, 42.D0, 52.D0,
C             .               62.D0, 72.D0, 82.D0, 92.D0,
C             .               13.D0, 23.D0, 33.D0, 43.D0, 52.D0,
C             .               63.D0, 73.D0, 83.D0, 93.D0        /
C
C        C
...
C              DO I = 1, NDAF
C
C                 CALL DAFOPN ( DAFNAM(I), ND, NI,
C             .                 DAFNAM(I), 0,  HAN(I) )
C
C                 CALL DAFBNA ( HAN(I), SUMMRY, DAFNAM(I) )
C
C              END DO
C
C        C
C        C     At this point, we assume that we have read the
C        C     file line by line. Add the numbers from each column
C        C     to the corresponding DAF.
C        C
C              DO I = 1, MAXLNS
C
C        C
...
C        C     print a matrix containing the same data as
C        C     the original input file.  The format of the numbers,
C        C     the delimiters separating the numbers, spacing, and
C        C     non-printing characters may differ.
C        C
C        C     Open the DAFs for reading.
C        C
C              DO I = 1, NDAF
C                 CALL DAFOPR ( DAFNAM(I), HAN(I) )
C              END DO
C
C        C
...
C        C     lines of text.  Print these lines out.
C        C
C              DO I = 0,  LENGTH - 1
C
C                 LINE = ' '
...
C                 DO J = 1, NDAF
C
C                    CALL DAFRDA ( HAN(J), IA+I, IA+I, NUMDP )
C
C        C
C        C           Convert the double precision number to a
C        C           character string, and append it to the current
C        C           line.
C        C
C                    CALL DPSTR  ( NUMDP,  SIG,        NUMCH )
C                    CALL SUFFIX ( NUMCH,  3,          LINE  )
C
C                 END DO
C
C                 WRITE(*,*) LINE
...
C
C
C             1.100000000E+01    1.200000000E+01    1.300000000E+01
C             2.100000000E+01    2.200000000E+01    2.300000000E+01
C             3.100000000E+01    3.200000000E+01    3.300000000E+01
C             4.100000000E+01    4.200000000E+01    4.300000000E+01
C             5.100000000E+01    5.200000000E+01    5.200000000E+01
C             6.100000000E+01    6.200000000E+01    6.300000000E+01
C             7.100000000E+01    7.200000000E+01    7.300000000E+01
C             8.100000000E+01    8.200000000E+01    8.300000000E+01
C             9.100000000E+01    9.200000000E+01    9.300000000E+01
C
C
C        Note that after run completion, three new DAF files exist in
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 13-AUG-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header of DAFANA and all entry points to comply with
C        NAIF standard. Added complete code examples to DAFANA $Examples
C        section based on the existing fragments.
C
...
C$ Exceptions
C
C     1)  If the input handle is not that of a DAF that is open for
C         writing, an error is signaled by a routine in the call tree of
C         this routine. These files are implicitly of the native binary
C         file format.
C
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 06-JUL-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
...
C        and initializes the state pool.
C
C-&
...
C         the error SPICE(DAFNOWRITE) is signaled.
C
C     2)  If a new array has not been started in the current DAF (by a
C         call to DAFBNA), the error SPICE(DAFNEWCONFLICT) is signaled.
C
...
C
C     1)  If there are no DAFs to which data is currently being added,
C         the error SPICE(DAFNOWRITE) is signaled, or the error will
...
C         by HANDLE, the error SPICE(NOARRAYSTARTED) is signaled.
C
C$ Files
C
C     None.
...
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Edited entry #2
C        in $Exceptions section: No write in progress is detected by
C        this routine.
C
C-    SPICELIB Version 3.0.0, 16-NOV-2001 (FST)
C
