...
C              square centered on the celestial equator that
...
C$ Exceptions
C
C     1)  If no star catalog has been loaded, an error is signaled by a
C         routine in the call tree of this routine.
C
C     2)  If the catalog query fails for any reason, the error
C         SPICE(QUERYFAILURE) is signaled.
C
C$ Files
C
