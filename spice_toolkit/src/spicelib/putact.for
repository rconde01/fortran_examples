...
C$ Abstract
C
C     Store the error response action.
C
C     PUTACT is a low-level data structure access routine.
C     DO NOT CALL THIS ROUTINE. USE ERRACT, NOT PUTACT, TO SET THE
C     CURRENT ERROR RESPONSE ACTION.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     ACTION   is the new integer code for the error response action.
...
C
C     2)  Calls to this routine by routines other than the SPICELIB
C         error handling routines may interfere with error processing.
C
C     3)  See the subroutine ERRACT for the definitions of the error
C         action codes.
C
...
C     Error free.
C
C     1)  However, this routine is part of the SPICELIB error
...
C$ Index_Entries
C
C     get current error response action
C
C-&
