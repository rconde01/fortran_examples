...
C$ Version
C
C-    SPICELIB Version 2.1.0, 12-AUG-2021 (NJB) (JDR)
C
C        Bug fixes: now NREAD is not incremented once it reaches
C        INTMAX(), if it does.
C
C        Re-ordered header sections in all entry points. Corrected
C        typos in comments.
C
C        Edited the header of the DAFRWD umbrella routine and all its
C        entry entry points to comply with NAIF standard.
C
C        Removed DAF required reading from $Literature_References.
C
...
C     any one time by setting the value of the parameter RBSIZE before
...
C              inclusive) of the specified record from the specified
C              file, specifically.
C
C     FOUND    is .TRUE. when the specified record is found, and is
C              .FALSE. otherwise.
...
C         END < BEGIN ) are not signaled as errors. The effects of
...
C-    SPICELIB Version 2.1.0, 12-AUG-2021 (NJB) (JDR)
C
C        Bug fix: now NREAD is not incremented once it reaches
C        INTMAX(), if it does.
C
C        Re-ordered header sections. Corrected typos in comments.
...
C$ Abstract
C
C     Read a portion of the contents of a summary record in a DAF file.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C
C     RECNO    is the record number of a particular double precision
C              record within the DAF, whose contents are to be read. DAF
C              record numbers start at 1.
C
C     BEGIN    is the first word in the specified record to be returned.
C              Word numbers range from 1 to 128.
C
C     END      is the final word in the specified record to be returned.
C              Word numbers range from 1 to 128.
C
C$ Detailed_Output
C
...
C$ Exceptions
C
C     1)  If HANDLE does not belong to any file that is currently open,
C         an error is signaled by a routine in the call tree of this
C         routine.
C
C     2)  If an error occurs while reading the record, the error is
C         signaled by a routine in the call tree of this routine.
C
C     3)  Bad values for BEGIN and END ( BEGIN < 1, END > 128,
C         END < BEGIN ) are not diagnosed. See $Particulars and
C         $Restrictions for the effect of this routine in this case.
C
C$ Files
C
C     The input HANDLE must refer to a DAF file that is open for read
C     or write access.
C
C$ Particulars
C
C     DAFGSR checks the DAF record buffer to see if the requested
C     record can be returned without actually reading it from
C     external storage. If not, it reads the record and stores
C     it in the buffer, typically removing another record from
C     the buffer as a result.
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 09-AUG-2021 (NJB) (JDR)
C
C        Bug fix: now NREAD is not incremented once it reaches
C        INTMAX(), if it does.
C
C        Corrected typos in comments. Edited the header to comply with
C        NAIF standard. Added $Exceptions and $Files sections and
C        extended $Detailed_Input.
C
C        Removed DAF required reading from $Literature_References.
C
C-    SPICELIB Version 2.0.0, 16-NOV-2001 (FST)
...
C     Read a portion of the contents of a double precision record in a
C     DAF file.
C     Obsolete: This routine has been superseded by DAFGDR, and it is
...
C         binary file format, the error SPICE(UNSUPPORTEDBFF) is
C         signaled.
C
C$ Files
C
...
C        INTMAX(), if it does.
C
C        Tweaked one line of code to suppress ftnchek warning.
C
C        Re-ordered header sections. Corrected typos in comments.
C        Edited the header to comply with NAIF standard.
C
...
C         SPICE(DAFILLEGWRITE) is signaled.
C
C     2)  If (for some reason) the record cannot be written, the
C         error SPICE(DAFDPWRITEFAIL) is signaled.
C
C$ Files
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 09-AUG-2021 (NJB) (JDR)
C
C        Re-ordered header sections. Corrected typos in comments.
C        Edited the header to comply with NAIF standard.
C
...
C$ Version
C
C-    SPICELIB Version 1.3.1, 09-AUG-2021 (NJB)
C
C        Re-ordered header sections. Corrected typos in comments.
C        Edited the header to comply with NAIF standard.
C
