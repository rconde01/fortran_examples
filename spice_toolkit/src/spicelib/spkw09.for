...
C
C     6)  If FIRST is greater than or equal to LAST, the error
...
C         signaled.
C
C     8)  If the first epoch, EPOCHS(1), is greater than FIRST, the
...
C        of type 09 in an SPK file. Create a new SPK file with this
...
C        C
C              CHARACTER*(*)         SPKNAM
C              PARAMETER           ( SPKNAM = 'spkw09_ex1.bsp' )
C
C              INTEGER               DEGREE
...
C        C     for comments.
C        C
C              IFNAME = 'Test SPK type 9 internal filename.'
...
C        C
C              CALL SPKW09 ( HANDLE,    NOBJ,           MARS,  'J2000',
...
C$ Version
C
C-    SPICELIB Version 3.0.1, 05-JUL-2021 (JDR)
C
