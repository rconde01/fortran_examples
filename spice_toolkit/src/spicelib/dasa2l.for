...
C$ Exceptions
C
C     If any of the following exceptions occur, the output arguments may
C     contain bogus information.
C
C     1)  If TYPE is not recognized, the error SPICE(DASINVALIDTYPE)
C         is signaled.
C
C     2)  ADDRSS must be between 1 and LAST inclusive, where LAST is
C         last address in the DAS for a word of the specified type. If
C         ADDRSS is out of range, the error SPICE(DASNOSUCHADDRESS) is
C         signaled.
C
...
C         signaled.
C
C     4)  If the input handle is invalid, an error is signaled by a
