...
C
C     2)  If COLUMN is not the name of a declared column, an error is
...
C
C     4)  If the specified column already contains ANY entries, an error
...
C        named 'ekaclc_ex1.bes' which contains records of orders for
...
C
C
C        The file "ekaclc_ex1.bdb" will contain two segments, the first
...
C
C              CALL EKOPN ( 'ekaclc_ex1.bes', IFNAME, NRESVC, HANDLE )
C
C        C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 12-AUG-2021 (JDR) (BVS)
C
