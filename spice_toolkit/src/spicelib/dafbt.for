...
C$ Detailed_Input
C
C     BINFIL   is the name of a binary DAF file which is to be converted
C              to an equivalent DAF transfer file.
C
C     XFRLUN   is the Fortran logical unit number of a previously opened
...
C$ Exceptions
C
C     1)  If the binary DAF file specified by the filename BINFIL cannot
C         be opened for read access, an error is signaled by a routine
...
C
C     3)  If, for any reason, the DAF file cannot be read, an error is
...
C
C     This routine will check the SPICELIB function FAILED() after
C     each DAF file access call, and if an error was signaled it will
...
C              will have already been set by the routine which signaled
C              the error.
C
               CALL CHKOUT ( 'DAFBT' )
               RETURN
