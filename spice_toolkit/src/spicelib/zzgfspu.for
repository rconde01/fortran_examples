...
C$ Exceptions
C
C     1)  If a direct call to ZZGFSPU occurs, the error
...
C$ Restrictions
C
C     1)  ZZGFSPIN must be called prior to use of any of the
...
C$ Version
C
C-    SPICELIB Version 3.0.0, 31-MAY-2021 (EDW) (JDR)
C
C        Edited umbrella routine and all its entry points' headers to
C        comply with NAIF standard. Extended $Brief_I/O to list all
C        arguments and refer to corresponding entry points.
C
C        Moved UDFUNC declaration to $Declarations section.
C
C        Routine ZZGFSPGQ now calls ZZSEPQ, a copy of ZZGFSPQ cast as
C        a function.
C
...
C$ Exceptions
C
C     1)  If the object name for target 1, OF(1), is not a recognized
C         name, the error SPICE(IDCODENOTFOUND) is signaled.
C
C     2)  If the object name for target 2, OF(2), is not a recognized
C         name, the error SPICE(IDCODENOTFOUND) is signaled.
C
C     3)  If the object name for the observer, FROM, is not a recognized
C         name, the error SPICE(IDCODENOTFOUND) is signaled.
C
C     4)  If the three objects associated with an ANGULAR SEPARATION
C         search are not distinct, the error SPICE(BODIESNOTDISTINCT) is
C         signaled.
C
C     5)  If the body shape for target 1, SHAPE(1), is not recognized,
C         the error SPICE(NOTRECOGNIZED) is signaled.
C
C     6)  If the SHAPE(1) value lacks a corresponding case block, the
C         error SPICE(BUG) is signaled. This indicates a programming
C         error.
C
C     7)  If the body shape for target 2, SHAPE(2), is not recognized,
C         the error SPICE(NOTRECOGNIZED) is signaled.
C
C     8)  If the SHAPE(2) value lacks a corresponding case block, the
C         error SPICE(BUG) is signaled. This indicates a programming
C         error.
C
C     9)  If frame subsystem did not recognize frame name FRAME(1), the
C         error SPICE(NOFRAME) is signaled.
C
C     10) If the reference frame associated with target body 1, OF(1),
C         is not centered on target body 1, the error
C         SPICE(INVALIDFRAME) is signaled.
C
C     11) If frame subsystem did not recognize frame name FRAME(2), the
C         error SPICE(NOFRAME) is signaled.
C
C     12) If the reference frame associated with target body 2, OF(2),
C         is not centered on target body 2, the error
C         SPICE(INVALIDFRAME) is signaled.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 31-MAY-2021 (JDR)
C
...
C$ Exceptions
C
C     1)  If the observer is inside one of the objects, the object will
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 30-MAY-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
...
C$ Version
C
C-    SPICELIB Version 3.0.0, 30-MAY-2021 (EDW) (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Routine now calls ZZSEPQ, a copy of ZZGFSPQ cast as
C        a function.
C
...
C     VARIABLE  I/O  DESCRIPTION
C     --------  ---  --------------------------------------------------
C     XABCR      O   Saved value SVABCR.
C     XBOD       O   Saved values SVBOD1 and SVBOD2.
C     YREF       O   Saved value SVREF.
C     XREF       O   Saved values SVREF1 and SVREF2.
C     XOBS       O   Saved value SVOBS.
C     XRAD       O   Saved values SVRAD1 and SVRAD2.
C     XSHP       O   Saved values SVSHP1 and SVSHP2.
C
C$ Detailed_Input
C
C     None.
...
C
C     YREF     initialized via ZZGFSPIN.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 30-MAY-2021 (EDW) (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Completed descriptions for $Brief_I/O.
C
