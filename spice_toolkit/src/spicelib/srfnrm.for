...
C         loaded into the kernel pool prior to calling SRFNRM, an error
...
C         not available, an error is signaled by a routine in the
...
C
C     11) If the radii are not available in the kernel pool, an error is
...
C             .                 SRFPTS(1,1,I)              )
C
C                 CALL SRFNRM ( METHOD(I),    TARGET, ET,
C             .                 FIXREF,       N,      SRFPTS(1,1,I),
C             .                 NORMLS(1,1,I)                      )
C              END DO
C
C
C              WRITE (*,*) 'Number of grid points: ', N
C
C        C
C        C     Print out the surface points in latitudinal
C        C     coordinates and compare the derived lon/lat values
C        C     to those of the input grid for the first 3 points.
C        C
C              DO I = 1, 3
C        C
...
C
C
C        Note that only the first 3 points of the grid are
C        presented in the output (the rest of the points are not
C        shown due to their large number).
C
C$ Restrictions
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 08-JUL-2020 (JDR)
C
C        Edited the header to comply with NAIF standard.
C        Limited the number of grid points presented in the output of
C        the code example to three.
C
