...
C$ Abstract
C
C     Compute the outward-pointing, unit normal vector at a point on
C     the surface of an ellipsoid.
C
C$ Disclaimer
C
...
C
C     POINT    is a 3-vector giving the bodyfixed coordinates of a point
C              on the ellipsoid. In bodyfixed coordinates, the semi-axes
C              of the ellipsoid are aligned with the X, Y, and Z-axes of
C              the reference frame.
C
C$ Detailed_Output
C
C     NORMAL   is the unit vector pointing away from the ellipsoid and
...
C$ Restrictions
C
C     1)  It is assumed that the input POINT is indeed on the ellipsoid.
...
C$ Version
C
C-    SPICELIB Version 1.4.0, 26-OCT-2021 (JDR)
C
...
C
C        Updated the documentation to refer to "reference frame" instead
C        of "coordinate system" as per NAIF conventions.
C
