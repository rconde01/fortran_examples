...
C$ Detailed_Input
C
C     NAME     is the name of the symbol to be duplicated.
C
C              The components associated with NAME will be given to the
C              new symbol COPY. If NAME is not in the symbol table, no
C              duplicate symbol can be made.
C
C     COPY     is the name of the new symbol. If a symbol with the name
C              COPY already exists in the symbol table, its components
...
C              whose components are the same as the components of NAME.
C
C$ Parameters
C
C     None.
...
C         SPICE(NOSUCHSYMBOL) is signaled.
C
