...
C              discarded before GFRFOV conducts its search.
C
C$ Detailed_Output
C
C     RESULT   is a SPICE window representing the set of time intervals,
C              within the confinement period, when the input ray is
C              "visible"; that is, when the ray is contained in the
...
C
C     3)  If the observer's name cannot be mapped to an ID code, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     4)  If the aberration correction flag calls for light time
C         correction, an error is signaled by a routine in the call tree
C         of this routine.
C
C     5)  If the ray's direction vector is zero, an error is signaled by
...
C
C     8)  If the FOV boundary has more than MAXVRT vertices, an error
...
C         of this routine.
C
C     12) If the output SPICE window RESULT has size less than 2, the
C         error SPICE(WINDOWTOOSMALL) is signaled.
C
C     13) If the output SPICE window RESULT has insufficient capacity
...
C        C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'gfrfov_ex1.tm' )
C
C              CHARACTER*(*)         TIMFMT
C              PARAMETER           ( TIMFMT =
...
C
C
C        Note that the star is visible throughout the confinement
C        window.
C
C$ Restrictions
C
C     1)  The kernel files to be used by GFRFOV must be loaded (normally
C         via the SPICELIB routine FURNSH) before GFRFOV is called.
C
