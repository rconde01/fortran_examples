...
C$ Abstract
C
C     Return a list of voxels that a given ray intersects in a given
C     voxel grid.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     VERTEX   is the voxel grid coordinates of ray's vertex. These
C              coordinates are zero-based, double precision offsets from
C              the grid's origin. The units of the coordinates are
C              voxels, that is, voxel edge lengths.
C
C     RAYDIR   is the direction vector of ray from VERTEX.
C
C     GRDEXT   is the integer 3-vector containing the voxel grid
C              extents. These are the dimensions of the voxel grid in
C              voxel units, in the X, Y, and Z directions respectively.
C
C     MAXNVX   is the maximum number of voxel coordinate sets that can
C              be stored in VOXLST.
C
C$ Detailed_Output
C
C     NVX      is the number of voxel coordinate sets contained in
C              VOXLST.
C
C     VOXLST   is the list of coordinate sets of voxels intersected by
C              ray. Elements
C
C                 VOXLST(J,I), J = 1, 3
C
C              are the coordinates of the Ith voxel in the list. These
C              coordinates are 1-based integer values.
C
C              The voxels in the output list are ordered by increasing
C              distance from the ray's vertex.
C
C$ Parameters
C
...
C$ Exceptions
C
C     1)  If the input RAYDIR has all zero components, the error
C         SPICE(ZEROVECTOR) is signaled.
C
C     2)  If the maximum output list size MAXNVX is non-positive, the
C         error SPICE(INVALIDSIZE) is signaled.
C
C     3)  If any element of the grid extents array GRDEXT is
C         non-positive, the error SPICE(BADDIMENSIONS) is signaled.
C
C     4)  If the ray's vertex is neither inside, nor within a small
C         distance from, the voxel grid, the error
C         SPICE(VERTEXNOTINGRID) is signaled. See the description of the
C         parameter GRDTOL.
C
C     5)  If the value of the NVX counter (number of intersected voxels)
C         exceeds the size of the VOXLST input vector, the error
...
C$ Version
C
C-    SPICELIB Version 3.1.1, 26-OCT-2021 (NJB) (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Typo correction: the description of the error denoted
C        by the short message SPICE(VERTEXNOTINGRID) had been
C        the negative of what was intended.
C
...
C
C-    SPICELIB Version 3.0.0, 11-JUL-2014 (NJB) (EDW) (BVS) (JAB)
C
C        Previously released as DSKLIB:
C
...
C           the return list. Extended programming comments.
C
