...
C$ Version
C
C-    SPICELIB Version 1.1.0, 14-OCT-2021 (JDR) (BVS) (NJB)
C
C        Changed entry point ZZHSIINI to not check RETURN()
C        on entry. That entry point now initializes the hash
C        as long as the size parameter is greater than zero.
C
C        Fixed short error message in umbrella routine.
C
C        Bug fix: fixed routine name in CHKIN/CHKOUT calls 
C        in the ZZHSICHK entry (ZZHSIADD -> ZZHSICHK).
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 07-OCT-2020 (JDR) (BVS) (NJB)
C
C        This entry point no longer checks RETURN() on entry. It
C        initializes the hash as long as the size parameter is greater
C        than zero.
C
C-    SPICELIB Version 1.0.0, 01-AUG-2013 (BVS)
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 14-OCT-2021 (BVS)
C
C        Bug fix: fixed routine name in CHKIN/CHKOUT calls 
C        (ZZHSIADD -> ZZHSICHK).
C
