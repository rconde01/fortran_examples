...
C              routine in order to be valid.
C
C$ Detailed_Output
C
C     None.
...
C        the data from it.
C
C        The example shows the effect of the EKCLS calls when the EK
C        file has been opened for write or read access.
C
C
...
C        C
C              CHARACTER*(*)         EKNAME
C              PARAMETER           ( EKNAME = 'ekcls_ex1.bdb' )
C
C              CHARACTER*(*)         IFNAME
...
C        code example and removed non-applicable restriction.
C
