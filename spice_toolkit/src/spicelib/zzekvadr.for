...
C     a specified index, given a union of EK join row sets and a row
C     vector index. Also return the base address of the row
...
C$ Version
C
C-    SPICELIB Version 1.0.2, 01-OCT-2021 (NJB)
C
C        Updated header Abstract section. Corrected typos in comments.
C
...
C         error SPICE(BADADDRESS) is signaled.
C
...
C         the error SPICE(BADADDRESS) is signaled.
C
C$ Files
C
C     1)  This routine uses the EK scratch area, which employs a scratch
...
C     centralizing the activities that need be performed only once
