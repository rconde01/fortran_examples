...
C              numeric words that follow the first occurrence of the
...
C     Definitions:
C
C        WORD           is a set of consecutive non-blank characters
C                       delimited by blanks or the end of the string
C                       that contains them.
C
C        NUMERIC WORD   a word that can be parsed by the
...
C
C                       See NPARSD FOR A a full description of
C                       legitimate numeric words.
C
C     Given a string and a keyword this routine locates the first
C     occurrence of the keyword in the string and returns the double
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 05-JUN-2021 (JDR)
C
