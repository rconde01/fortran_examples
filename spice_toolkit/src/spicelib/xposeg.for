...
C     NROW       I   Number of rows of input matrix MATRIX.
C     NCOL       I   Number of columns of input matrix MATRIX.
C     XPOSEM     O   Transposed matrix.
C
C$ Detailed_Input
C
C     MATRIX   is a matrix to be transposed.
C
C     NROW     is the number of rows of input matrix MATRIX.
C
C     NCOL     is the number of columns of input matrix MATRIX.
C
C$ Detailed_Output
C
C     XPOSEM   is the transposed matrix.
C
C$ Parameters
C
C     None.
...
C     Error free.
C
C     1)  If either NROW or NCOL is less than or equal to zero, no
C         action is taken. The routine simply returns.
C
C$ Files
C
C     None.
...
C            specifically it assures that the elements of the matrix
C            to be transformed are stored in contiguous memory locations
...
C$ Version
C
C-    SPICELIB Version 1.3.0, 13-AUG-2021 (JDR)
C
...
C
C-    SPICELIB Version 1.2.2, 04-MAY-1993 (HAN)
C
...
C
C-    SPICELIB Version 1.2.0, 06-AUG-1990 (WLT)
C
