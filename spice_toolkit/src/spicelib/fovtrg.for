...
C
C     4)  If the target and observer bodies are the same, an error is
C         signaled by a routine in the call tree of this routine.
C
C     5)  If the body model specifier TSHAPE is invalid, an error is
...
C
C     7)  If a target body-fixed reference frame is not centered at the
...
C         in the call tree of this routine.
C
C     10) If the FOV boundary has more than MAXVRT vertices, an error
C         is signaled by a routine in the call tree of this
C         routine.
C
C     11) If the instrument FOV shape is a polygon or rectangle, and
C         this routine cannot find a ray R emanating from the FOV vertex
C         such that maximum angular separation of R and any FOV boundary
C         vector is within the limit (pi/2)-MARGIN radians, an error is
C         signaled by a routine in the call tree of this routine. If the
C         FOV is any other shape, the same error check will be applied
C         with the instrument boresight vector serving the role of R.
C
...
C           \begintext
C
C           For project meta-kernels similar to the one shown
C           here, please see the CASSINI SPICE PDS archive.
C
C
C        Example code begins here.
C
C
C              PROGRAM FOVTRG_EX1
C              IMPLICIT NONE
C
C        C
...
C        C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   =  'fovtrg_ex1.tm' )
C
C              CHARACTER*(*)         TIMFMT
C              PARAMETER           ( TIMFMT =
...
C                 IF ( FOUND ) THEN
C        C
C        C           Check to see if a body-fixed frame for this ID
C        C           exists.  If the frame is not in the kernel pool,
C        C           we cannot perform the visibility test.  The main
C        C           cause of a failure is a missing kernel.
C        C
...
C$ Restrictions
C
C     1)  The reference frame associated with INST must be centered at
C         the observer or must be inertial. No check is done to ensure
C         this.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 13-AUG-2021 (JDR)
C
C        Edited header to comply with NAIF standard. Corrected the
C        value of MARGIN in the $Parameters section.
C
