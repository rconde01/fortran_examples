...
C$ Detailed_Input
C
C     POSITN   is a 3-vector giving the position of an observer with
C              respect to the center of an ellipsoid. The vector is
C              expressed in a body-fixed reference frame. The semi-axes
C              of the ellipsoid are aligned with the X, Y, and Z-axes of
C              the body-fixed frame.
C
C     U        is a pointing 3-vector emanating from the observer.
C
...
C$ Detailed_Output
C
C     POINT    is the position of the intercept of the input ray,
C              defined by the direction vector U emanating from POSITN,
C              on the surface of the input ellipsoid.
C
C              If the ray intersects the ellipsoid, POINT will be
...
C
C     FOUND    is a logical flag indicating whether or not the ray from
C              POSITN with direction U actually intersects the
C              ellipsoid. If the ray does intersect the ellipsoid, FOUND
C              will be returned as .TRUE. If the ray misses the
...
C$ Version
C
C-    SPICELIB Version 1.4.0, 25-MAY-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Improved
C        "POINT" argument description.
C
