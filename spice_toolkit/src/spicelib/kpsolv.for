C$Procedure KPSOLV ( Solve Kepler's Equation --- Vector Form )

      DOUBLE PRECISION FUNCTION KPSOLV ( EVEC )

C$ Abstract
C
C     Solve the equation X = < EVEC, U(X) > where U(X) is the unit
C     vector [ COS(X), SIN(X) ] and  < , > denotes the two-dimensional
C     dot product.
C
C$ Disclaimer
C
...
C         the error SPICE(EVECOUTOFRANGE) is signaled.
C
C$ Files
C
C     None.
