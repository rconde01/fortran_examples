...
C              with write access.
C
C     N        is the number of comments in BUFFER that are to be added
C              to the comment area of the binary DAF attached to HANDLE.
C
C     BUFFER   is a buffer containing comments which are to be added
C              to the comment area of the binary DAF attached to HANDLE.
C
C$ Detailed_Output
C
...
C         error SPICE(INVALIDARGUMENT) is signaled.
C
C     2)  If a non printing ASCII character is encountered in the
C         comments, the error SPICE(ILLEGALCHARACTER) is signaled.
C
C     3)  If the binary DAF file attached to HANDLE is not open with
C         write access, an error is signaled by a routine in the call
...
C
C     1) This example demonstrates how to append new comments to the
C        comment area of a DAF file.
C
...
C
C              INTEGER               BUFSIZ
C              PARAMETER           ( BUFSIZ = 25 )
C
C              INTEGER               CMTSIZ
C              PARAMETER           ( CMTSIZ = 7  )
C
C              INTEGER               LINLEN
...
C              CHARACTER*(LINLEN)    NEWCMT ( CMTSIZ )
C
C              INTEGER               HANDLE
C              INTEGER               I
C              INTEGER               N
...
C
C        C
C        C     Set the new comments to be added to the DAF file.
C        C
C              DATA                  NEWCMT /
C             .  '================== NEW COMMENTS ==================',
C             .  '',
C             .  '   New comments can be appended to the end of the',
C             .  '   comment area of a DAF file, with a single',
C             .  '   operation.',
C             .  '',
C             .  '================ END NEW COMMENTS ================' /
C
...
C
C        C
C        C     Print the end of comment area from the DAF file.
C        C     (Maximum 15 lines.)
C        C
C              DONE = .FALSE.
C
C              DO WHILE ( .NOT. DONE )
C
C                 CALL DAFEC  ( HANDLE, 15, N, BUFFER, DONE )
C
C                 IF ( DONE ) THEN
C
C                    WRITE(*,'(A)') 'End of comment area of input '
C             .                  // 'DAF file (max. 15 lines): '
C                    WRITE(*,'(A)') '-------------------------------'
C             .                  // '-------------------------------'
C
C                    DO I = 1, N
...
C
C        C
C        C     Append the new comments to the DAF file.
C        C
C              CALL DAFAC ( HANDLE, CMTSIZ, NEWCMT )
C
C        C
...
C
C        C
C        C     Check if the comments have indeed appended.
C        C
C        C     Open a DAF for read.
C        C
...
C
C
C        End of comment area of input DAF file (max. 15 lines):
C        --------------------------------------------------------------
C            DSS-65_DXYZ       =    (    -0.0100          0.0242     ***
C            DSS-65_TOPO_EPOCH =       @2020-OCT-23/00:00
C            DSS-65_UP         =       'Z'
C            DSS-65_NORTH      =       'X'
C
C         \begintext
C        --------------------------------------------------------------
C        End of comment area of input DAF file (max. 25 lines):
C        --------------------------------------------------------------
C            DSS-65_DXYZ       =    (    -0.0100          0.0242     ***
C            DSS-65_TOPO_EPOCH =       @2020-OCT-23/00:00
C            DSS-65_UP         =       'Z'
...
C
C
C        Warning: incomplete output. 2 lines extended past the right
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 25-NOV-2021 (JDR)
C
...
C        interfaces.
C
C-    SPICELIB Version 1.0.0, 26-JUL-1994 (KRG)
C
C-&
...
C        is the record immediately after the file record, so we set
