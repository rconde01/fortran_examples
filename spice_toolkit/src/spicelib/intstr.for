...
C$ Detailed_Input
C
C     NUMBER   is the integer to be converted into a character string.
C
C$ Detailed_Output
C
C     STRING   is the character string representing the integer NUMBER.
C
C$ Parameters
C
C     None.
...
C$ Restrictions
C
C     1)  This routine assumes that all signed integers will fit into a
C         character string with LINLEN or fewer digits. See the
C         parameter LINLEN below for the current value.
C
...
C$ Version
C
C-    SPICELIB Version 2.2.0, 12-AUG-2021 (JDR)
C
...
C
C-    SPICELIB Version 1.0.1, 07-DEC-1990 (WLT)
C
