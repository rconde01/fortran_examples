...
C$ Abstract
C
C     Determine whether the components of a time vector are in the
C     "usual" range for the components, if component checking is
C     enabled.
C
C     If component checking is not enabled, this routine simply
C     returns after setting the outputs.
C
C$ Disclaimer
C
...
C     Error free.
C
C     1)  All problems with TVEC are reported via the logical OK
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 01-NOV-2021 (JDR) (EDW)
C
C        Added logic to prevent evaluation of MODIFY when MODS false.
C
C        Added text listing routines affected and not affected by
C        explicit assignments to TPARCH.
C
...
C$ Abstract
C
C     Restrict the set of strings that are recognized by SPICE time
C     parsing routines to those that have standard values for all time
C     components.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     TYPE     is a character string that is used to adjust the set of
C              strings that will be regarded as valid time strings by
C              SPICE time parsing routines.
C
C              The default behavior of SPICE time software is to allow
C              an extended range of values for the various components
C              (tokens) of a time string. For example, using its default
C              behavior, TPARSE would regard 1993 JAN 367 as a valid
C              time string and return the UTC seconds past the J2000
C              epoch value that corresponds to Jan 2, 1994.
C
C              While this is a "reasonable" interpretation of such a
C              string, there may be occasions when such a string should
C              be regarded as an error.
C
C              By calling TPARCH with a value of 'YES', the action of
C              the time software will be modified. Strings that have
C              components that are out of the range of values used in
C              most English discourse will be regarded as errors. Thus
C              the numeric values of MONTH, DAY, HOUR, MINUTE, and
C              SECOND must satisfy the following conditions to be
...
C              action, set TYPE to a value that is not equivalent to
C              'YES' when case and spaces are ignored.
C
C$ Detailed_Output
C
C     None.
...
C
C     All time parsing routines --including the top-level APIs TPARSE
C     and UTC2ET-- respect the setting assigned by TPARCH, except the
C     SPICELIB routine STR2ET.
C
C$ Examples
C
...
C
C     1) When accepting times as input interactively, you usually
C        read a string typed at a keyboard and then pass that string
C        to the SPICE time system to convert it to an ephemeris time.
C        The default behavior of SPICE software is to accept strings
C        such as FEB 34, 1993 and to interpret these in a "natural way"
C        (FEB 34, 1993 is regarded as MARCH 6, 1993.) The following
C        example code demonstrates how to modify this behavior.
C
C
C        Example code begins here.
C
C
C              PROGRAM TPARCH_EX1
C              IMPLICIT NONE
C
C        C
...
C        C
C              CHARACTER*(*)         TIMSTR
C              PARAMETER           ( TIMSTR = 'FEB 34, 1993' )
C
C              INTEGER               ERRMLN
C              PARAMETER           ( ERRMLN = 1000 )
C
C        C
C        C     Local variables.
C        C
...
C
C        C
C        C     First, demonstrate the default behavior of SPICE.
C        C     Let's get the number of UTC seconds past J2000 epoch.
C        C
C              CALL TPARSE ( TIMSTR, SP2000, ERRMSG )
C
C              IF ( ERRMSG .EQ. ' ' ) THEN
C
C                 WRITE(*,'(A,F18.6)') 'UTC (s): ', SP2000
C
C              ELSE
C
C                 WRITE(*,'(2A)') 'Error  : ', ERRMSG
C
C              END IF
C
C        C
C        C     Now, turn error checking on and parse the time string
C        C     again.
C        C
C              CALL TPARCH ( 'YES' )
C              CALL TPARSE ( TIMSTR, SP2000, ERRMSG )
...
C
C
C        UTC (s):  -215352000.000000
C        Error  : The day of the month specified for the month of Feb***
C
C
C        Warning: incomplete output. 1 line extended past the right
C        margin of the header and has been truncated. This line is
C        marked by "***" at the end of the line.
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 1.0.2, 01-NOV-2021 (JDR) (EDW)
C
C        Added text listing routines affected and not affected by
C        explicit assignments to TPARCH.
C
...
C
C-    SPICELIB Version 1.0.0, 07-APR-1996 (WLT)
C
