...
C$ Exceptions
C
C     1)  If CHGIRF is called directly, the signal, the error
...
C
C     [1]  J. Lieske, "Precession Matrix Based on IAU (1976) System of
C          Astronomical Constants," Astron. Astrophys. 73, 282-284,
C          1979.
C
C     [2]  E. M. Standish, Jr., "Orientation of the JPL Ephemerides,
C          DE 200/LE 200, to the Dynamical Equinox of J2000," Astron.
C          Astrophys. 114, 297-302, 1982.
C
...
C
C     [5]  J. Lieske, "Expressions for the Precession Quantities Based
C          upon the IAU (1976) System of Astronomical Constants,"
C          Astron. Astrophys. 58, 1-16, 1977.
C
C     [6]  L. Bass and R. Cesarone, "Mars Observer Planetary Constants
C          and Models," JPL D-3444, November 1990.
C
C     [7]  P. Kenneth Seidelmann (Ed.), "Explanatory Supplement to the
C          Astronomical Almanac," University Science Books, 1992.
C
...
C$ Version
C
C-    SPICELIB Version 4.5.0, 06-NOV-2021 (JDR) (NJB)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard.
C
C        Corrected sign error in comments describing the angles used to
C        convert between the J2000 and B1950 reference frames. Also
C        corrected a typo in the comments.
C
C-    SPICELIB Version 4.4.0, 24-SEP-2013 (BVS)
C
C        Updated entry point IRFNUM to treat J2000 as a special case
...
C        IRFROT and IRFDEF. Before they simply signaled the error
...
C                3           2      3
C
C     The values for z, theta, and zeta are computed from the formulas
C     given in table 5 of [5].
C
...
C     ROTAB      O   Rotation from frame A to frame B.
C
C$ Detailed_Input
C
C     REFA,
...
C$ Version
C
C-    SPICELIB Version 4.4.0, 17-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Corrected
C        argument name in $Brief_I/O section: ROTAB was MATRIX. Added
C        $Literature_References.
C
...
C$ Version
C
C-    SPICELIB Version 4.3.0, 17-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Added
