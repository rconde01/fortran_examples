...
C$ Abstract
C
C     Fetch the data packets indexed from FIRST to LAST from the 
C     packet partition of a generic segment. The segment is 
C     identified by a DAF file handle and segment descriptor.
C
C$ Disclaimer
C
...
C         packets, the error SPICE(REQUESTOUTOFBOUNDS) is signaled.
C
C     2)  If LAST is less than FIRST, the error SPICE(REQUESTOUTOFORDER)
C         is signaled.
C
C     3)  If the packet directory structure is unrecognized, the error
C         SPICE(UNKNOWNPACKETDIR) is signaled. The most likely cause of
C         this error is that an upgrade to your version of the SPICE
C         toolkit is needed.
C
C$ Files
C
