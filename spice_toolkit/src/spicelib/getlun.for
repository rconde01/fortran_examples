...
C$ Exceptions
C
C     1)  If there are no free logical units available, the error
C         SPICE(NOFREELOGICALUNIT) is signaled and UNIT is assigned the
C         value 0.
C
C     2)  This routine obtains a logical unit number from FNDLUN, which
C         executes a Fortran INQUIRE statement. If that statement fails
C         to execute properly, FNDLUN returns a negative unit number,
C         GETLUN assigns the value 0 to UNIT, and, the error
C         SPICE(INQUIREFAILED) is signaled.
C
C$ Files
C
C     None.
...
C$ Restrictions
C
C     1)  This routine never returns a logical unit number of 0. The
...
C         is assigned the value 0, and an error is signaled.
C
