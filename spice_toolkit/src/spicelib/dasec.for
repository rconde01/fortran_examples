...
C$ Detailed_Input
C
C     HANDLE   is the file handle of a binary DAS file which has been
...
C
C     BUFFER   is a list of at most BUFSIZ comments which have been
...
C
C     2)  If a comment line in a DAS file is longer than the length
...
C         expected, the error SPICE(BADDASCOMMENTAREA) is signaled.
C
C     4)  If the binary DAS file attached to HANDLE is not open for
C         reading, an error is signaled by a routine in the call tree of
C         this routine.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 1.4.1, 20-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Removed
C        reference to non-described parameters from entry #1 in
C        $Restrictions section.
C
