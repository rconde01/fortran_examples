...
C         call tree of this routine.
C
C     22) If a body RADII vector has other than exactly thee elements,
C         the error SPICE(INVALIDCOUNT) is signaled by a routine in
C         the call tree of this routine.
C
C     23) If a body RADII vector has any element less-than or
...
C
C        Support for product frames was added.
C
...
C
C        Code clean-up: unnecessary call to ZZPRSCOR was removed.
C
C        Added and removed some FAILED() calls.
C
...
C           the result in ROTATE.
C
            CALL NAMFRM ( FFRAMS(1), FROMID )
            CALL NAMFRM ( TFRAMS(1), TOID   )

            CALL ZZREFCH0 ( TOID, FROMID, T0, ROTATE )
            ...
            DO I = 2, N
               ...
               CALL MXM   ( ROTATE, RTEMP, ROUT   )
               CALL MOVED ( ROUT,  9,      ROTATE )
