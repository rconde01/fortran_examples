C$Procedure REPMOT  ( Replace marker with ordinal text )
      ...
      SUBROUTINE REPMOT ( IN, MARKER, VALUE, RTCASE, OUT )

C$ Abstract
C
C     Replace a marker with the text representation of an ordinal
C     number.
C
C$ Disclaimer
C
...
C              representation of the ordinal number VALUE.
C
...
C              representation of the ordinal number VALUE for the first
...
C$ Parameters
C
C     MAXLON   is the maximum expected length of any ordinal text.
C              147 characters are sufficient to hold the text
C              representing any ordinal value whose corresponding
C              ordinal value is in the range
C
C                ( -10**12, 10**12 )
C
C              An example of a number whose ordinal text representation
...
C     by the calls
C
C        CALL REPMOT ( STRING, '#1',  51,           'L', STRING )
...
C
C     1) The following example illustrate the use of REPMOT to
...
C
C              CALL REPMOT ( INSTR, MARKER, 5, 'U', OUTSTR )
C
...
C        C
C              MARKER = ' XX '
C              INSTR  = 'The XX word of the XX sentence was ...'
C
C              CALL REPMOT ( INSTR, MARKER, 5, 'L', OUTSTR )
C
...
C              CALL REPMOT ( OUTSTR, MARKER, 5,    'C', OUTSTR )
C
...
C            Output: INVALID COMMAND. FIFTH WORD NOT RECOGNIZED.
C
C         Case 2: Replacement text in lowercase.
C            Input : The XX word of the XX sentence was ...
C            Output: The fifth word of the XX sentence was ...
C
C         Case 3: Replacement text capitalized.
C            Input : Name:  YY.  Rank:  XX.
C            Output: Name:  Moriarty.  Rank:  Fifth.
C
C
C$ Restrictions
C
C     1)  VALUE must be in the range accepted by the SPICELIB routine
C         INTORD. This range is currently
C
C            ( -10**12, 10**12 )
C
...
C        code example from existing fragments. Improved example in
