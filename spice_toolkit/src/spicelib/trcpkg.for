...
C$ Version
C
C-    SPICELIB Version 4.28.0, 28-NOV-2021 (BVS)
C
C        Updated for MAC-OSX-M1-64BIT-CLANG_C.
C
C-    SPICELIB Version 4.27.0, 03-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Added entry in
C        $Index_Entries.
C
...
C$ Restrictions
C
C     1)  Routines that call this routine must call CHKOUT immediately
C         prior to any RETURN or END statement.
C
C     2)  Module names are assumed to have no embedded blanks.
C
C$ Literature_References
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 4.2.0, 03-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
...
C$ Restrictions
C
C     1)  Routines that call this routine must call CHKIN as the second
...
C$ Detailed_Output
C
C     DEPTH    is the number of module names in the traceback
C              representation.
C
...
C$ Version
C
C-    SPICELIB Version 4.1.0, 03-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard.
...
C$ Detailed_Output
C
C     DEPTH    is the maximum number of module names encountered in the
C              traceback stack. This would be the longest call chain
C              that occurred during the run of a program.
C
C$ Parameters
C
C     None.
...
C$ Restrictions
C
C     1)  It is assumed no module names exceed NAMLEN characters in
C         length.
C
C$ Literature_References
C
...
C$ Restrictions
C
C     1)  For SPICELIB error handling only.
C
C$ Literature_References
C
C     None.
...
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Added entry to
C        $Index_Entries.
C
C-    SPICELIB Version 4.0.5, 17-JUL-2002 (BVS)
C
...
C$ Index_Entries
C
C     get frozen copy of traceback
C
C-&
