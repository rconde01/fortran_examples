...
C$ Detailed_Input
C
C     HANDLE   is the DAF handle of an SPK file to which a type 3
...
C              coefficients for coordinates and their derivatives
C              (number of logical records) to be stored in the segment.
C              There is one set of Chebyshev coefficients for each time
C              period.
C
C     POLYDG   is the degree of each set of Chebyshev polynomials used
C              to represent the ephemeris information. POLYDG must not
C              exceed MAXDEG (see $Parameters below).
C
...
C              Each set has size SETSZ = 6*(POLYDG+1). The coefficients
...
C
C
C     The ephemeris data supplied to the type 3 SPK writer is packed
...
C        +--------------------------------------+
C
C     Where T_n represents the Chebyshev polynomial of the first kind of
C     degree n.
C
C$ Examples
C
...
C
C     1) This example demonstrates how to create an SPK type 3 kernel
C        containing only one segment, given a set of Chebychev
...
C        C
C              CHARACTER*(*)         SPK3
C              PARAMETER           ( SPK3  = 'spkw03_ex1.bsp' )
C
C              CHARACTER*(*)         REF
...
C        C
C              IFNAME = 'Type 3 SPK internal file name.'
C              SEGID  = 'SPK type 3 test segment'
C
C        C
C        C     Open a new SPK file.
C        C
C              CALL SPKOPN( SPK3, IFNAME, NCOMCH, HANDLE )
C
C        C
C        C     Write the segment.
C        C
C              CALL SPKW03 ( HANDLE, BODY,   CENTER, REF,
...
C        screen. After run completion, a new SPK type 3 exists in
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 17-JUN-2021 (JDR)
C
