...
C$ Abstract
C
C     Deprecated: This routine is deprecated because all high-level
C     time conversion routines (STR2ET, UTC2ET, TPARSE) were updated
C     to accept ISO formatted times on input.  This routine is 
C     supported for purposes of backward compatibility only.
C
C     Convert date-time strings represented in the format adopted by the
C     International Standards Organization (ISO) to equivalent UTC time
C     strings recognized by the SPICELIB routine TPARSE.
C
C$ Disclaimer
C
...
C         error SPICE(YEAROUTOFBOUNDS) is signaled. UTCSTR is
C         not changed.
C
C     2)  If the string does not clearly match the ISO format,
C         the error SPICE(NOTISOFORMAT) is signaled. UTCSTR is not
C         changed.
C
C$ Files
C
...
C          Publications, Inc. New York, 1877.
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 23-DEC-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Remove obsolete
C        reference to NAIF document 268 from $Literature_References.
C
...
C
C-    SPICELIB Version 1.0.0, 11-JUL-1995 (KRG) (JML)
C
C        Based on
C
...
C           The output variable was listed as ET when it should have
C           been UTCSTR.
C
C           Changed the length of ASCII to be 100 rather than 128. This
C           removes possible compiler warning messages for truncating
C           character variables on assignments. The maximum nonblank
C           length for an input time ISO string is 80 characters, so
C           placing it into a temporary array of 100 characters should
C           pose no difficulties.
C
...
C$ Index_Entries
C
C     DEPRECATED Transform ISO time strings to UTC strings
C
C-&
