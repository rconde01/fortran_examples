...
C
C     AVFLAG   is a logical flag that indicates whether segment will
...
C     None.
C
C     The input data is used to create the segment summary for the
C     segment being started in the DAF file associated with HANDLE.
C
...
C$ Exceptions
C
C     1)  If a file access error occurs while this routine begins a new
C         type 04 segment, the error is signaled by a routine in the
C         call tree of this routine.
C
C     2)  If numeric ID for given reference frame cannot be resolved
C         from it's name, the error SPICE(INVALIDREFFRAME) is signaled.
C
C     3)  If SEGID is more than 40 characters long, the error
