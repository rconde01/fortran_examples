...
C     POINT,
C     SPAN1,
C     SPAN2    are, respectively, a point and two orthogonal spanning
C              vectors that generate the geometric plane represented by
C              PLANE. The geometric plane is the set of vectors
C
C                 POINT   +   s * SPAN1   +   t * SPAN2
C
C              where `s' and `t' are real numbers. POINT is the closest
C              point in the plane to the origin; this point is always a
C              multiple of the plane's normal vector. SPAN1 and SPAN2
C              are an orthonormal pair of vectors. POINT, SPAN1, and
C              SPAN2 are mutually orthogonal.
C
C$ Parameters
C
C     None.
...
C            C     by the SPICE plane PLANE in this example.
C            C
...
C
C            C
C            C     Make a SPICE plane from N and C, and then find a
