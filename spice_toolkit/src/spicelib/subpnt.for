...
C         routine.
C
C     7)  If the sub-observer point type is not specified or is not
...
C         calling SUBPNT, an error is signaled by a
...
C
C     10) If the computation method specifies an ellipsoidal target
C         shape and triaxial radii of the target body have not been
C         loaded into the kernel pool prior to calling SUBPNT, an error
C         is signaled by a routine in the call tree of this routine.
C
C     11) The target must be an extended body, and must have a shape
...
C
C     12) If PCK data specifying the target body-fixed frame orientation
C         have not been loaded prior to calling SUBPNT, an error is
...
C         of this routine.
C
C     14) If METHOD specifies that the target surface is represented
C         by DSK data, and the ray from the observer to the
C         sub-observer point doesn't intersect the target body's
C         surface, the error SPICE(SUBPOINTNOTFOUND) is signaled.
C
C     15) If the surface intercept on the target body's reference
C         ellipsoid of the observer to target center vector cannot not
C         be computed, the error SPICE(DEGENERATECASE) is signaled. Note
C         that this is a very rare case.
C
C     16) If radii for TARGET are not found in the kernel pool, an error
C         is signaled by a routine in the call tree of this routine.
C
C     17) If the size of the TARGET body radii kernel variable is not
C         three, an error is signaled by a routine in the call tree of
C         this routine.
C
C     18) If any of the three TARGET body radii is less-than or equal to
...
C
C        C
C        C        Convert the sub-observer point's rectangular
C        C        coordinates to planetographic longitude, latitude
C        C        and altitude. Convert radians to degrees.
C        C
...
C
C        C
C        C        Convert sub-observer point's rectangular coordinates
C        C        to planetocentric radius, longitude, and latitude.
...
C                 WRITE(*,FM) ' '
C                 WRITE(*,FM)
C             .   '  Observer ALT relative to spheroid (km) =', OPGALT
C                 WRITE(*,FM)
C             .   '  Length of SRFVEC                  (km) =', ODIST
C                 WRITE(*,FM)
C             .   '  Sub-observer point ALT            (km) =', SPGALT
C                 WRITE(*,FM)
C             .   '  Sub-observer planetographic LON  (deg) =', SPGLON
C                 WRITE(*,FM)
C             .   '  Observer planetographic LON      (deg) =', OPGLON
C                 WRITE(*,FM)
C             .   '  Sub-observer planetographic LAT  (deg) =', SPGLAT
C                 WRITE(*,FM)
C             .   '  Observer planetographic LAT      (deg) =', OPGLAT
C                 WRITE(*,FM)
C             .   '  Sub-observer planetocentric LON  (deg) =', SPCLON
C                 WRITE(*,FM)
C             .   '  Observer planetocentric LON      (deg) =', OPCLON
C                 WRITE(*,FM)
C             .   '  Sub-observer planetocentric LAT  (deg) =', SPCLAT
C                 WRITE(*,FM)
C             .   '  Observer planetocentric LAT      (deg) =', OPCLAT
C                 WRITE(*,FM) ' '
C
C              END DO
C
...
C
C          Observer ALT relative to spheroid (km) =  349199089.540947020
C          Length of SRFVEC                  (km) =  349199089.577642620
C          Sub-observer point ALT            (km) =          0.000000000
C          Sub-observer planetographic LON  (deg) =        199.302305028
C          Observer planetographic LON      (deg) =        199.302305028
C          Sub-observer planetographic LAT  (deg) =         26.262401237
C          Observer planetographic LAT      (deg) =         25.994936751
C          Sub-observer planetocentric LON  (deg) =        160.697694972
C          Observer planetocentric LON      (deg) =        160.697694972
C          Sub-observer planetocentric LAT  (deg) =         25.994934171
C          Observer planetocentric LAT      (deg) =         25.994934171
C
C
C         Computation method = Near point/ellipsoid
C
C          Observer ALT relative to spheroid (km) =  349199089.540938556
C          Length of SRFVEC                  (km) =  349199089.540938556
C          Sub-observer point ALT            (km) =          0.000000000
C          Sub-observer planetographic LON  (deg) =        199.302305029
C          Observer planetographic LON      (deg) =        199.302305029
C          Sub-observer planetographic LAT  (deg) =         25.994936751
C          Observer planetographic LAT      (deg) =         25.994936751
C          Sub-observer planetocentric LON  (deg) =        160.697694971
C          Observer planetocentric LON      (deg) =        160.697694971
C          Sub-observer planetocentric LAT  (deg) =         25.729407227
C          Observer planetocentric LAT      (deg) =         25.994934171
C
C
C         Computation method = Intercept/DSK/Unprioritized
C
C          Observer ALT relative to spheroid (km) =  349199089.541017115
C          Length of SRFVEC                  (km) =  349199091.785406590
C          Sub-observer point ALT            (km) =         -2.207669751
C          Sub-observer planetographic LON  (deg) =        199.302304999
C          Observer planetographic LON      (deg) =        199.302304999
C          Sub-observer planetographic LAT  (deg) =         26.262576677
C          Observer planetographic LAT      (deg) =         25.994936751
C          Sub-observer planetocentric LON  (deg) =        160.697695001
C          Observer planetocentric LON      (deg) =        160.697695001
C          Sub-observer planetocentric LAT  (deg) =         25.994934171
C          Observer planetocentric LAT      (deg) =         25.994934171
C
C
...
C
C          Observer ALT relative to spheroid (km) =  349199089.541007578
C          Length of SRFVEC                  (km) =  349199091.707172215
C          Sub-observer point ALT            (km) =         -2.166164622
C          Sub-observer planetographic LON  (deg) =        199.302305000
C          Observer planetographic LON      (deg) =        199.302305000
C          Sub-observer planetographic LAT  (deg) =         25.994936751
C          Observer planetographic LAT      (deg) =         25.994936751
C          Sub-observer planetocentric LON  (deg) =        160.697695000
C          Observer planetocentric LON      (deg) =        160.697695000
C          Sub-observer planetocentric LAT  (deg) =         25.729237570
C          Observer planetocentric LAT      (deg) =         25.994934171
C
C
...
C        C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'subpnt_ex2.tm' )
C
C              CHARACTER*(*)         F1
...
C        C           transformation matrix from IAU_MARS at TRGEPC to
C        C           the MRO_HIRISE_LOOK_DIRECTION frame at time ET.
C        C
...
C        C
C                    WRITE(*,F2) ' '
C                    WRITE(*,F2) '   Aberration correction = '
C             .      // ABCORR(J)
C                    WRITE(*,F1) ' '
C                    WRITE(*,F2) '      MRO-to-sub-observer vector in'
C                    WRITE(*,F2) '      MRO HIRISE look direction frame'
C                    WRITE(*,F1) '        X-component             '
C             .      // '(km) = ', MROVEC(1)
C                    WRITE(*,F1) '        Y-component             '
C             .      // '(km) = ', MROVEC(2)
C                    WRITE(*,F1) '        Z-component             '
C             .      // '(km) = ', MROVEC(3)
C                    WRITE(*,F1) '      Sub-observer point radius '
C             .      // '(km) = ', RADIUS
C                    WRITE(*,F1) '      Planetocentric latitude  '
C             .      // '(deg) = ', LAT
C                    WRITE(*,F1) '      Planetocentric longitude '
C             .      // '(deg) = ', LON
C                    WRITE(*,F1) '      Observer altitude         '
C             .      // '(km) = ', ALT
C
C        C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 01-NOV-2021 (JDR) (EDW) (NJB)
C
...
C        Changed code examples' output to comply with maximum
C        line length for header comments.
C
C        Bug fix: TRGEPC is now initialized prior to first use.
C        Previously the lack of initialization could cause this routine
C        to fail to find DSK data within the time bounds of a DSK
C        segment.
C
