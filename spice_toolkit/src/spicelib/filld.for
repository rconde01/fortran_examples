...
C
C     1) Initialize all members of a double precision array to the same
C        value.
C
C
...
C
C        C
C        C     Initialize all members of the array ARRAY to 1, and
C        C     print out its contents.
C        C
C              CALL FILLD ( 1.D0, NDIM, ARRAY )
C
C              WRITE(*,'(A)') 'Contents of ARRAY:'
C              DO I=1, NDIM
C
C                 WRITE(*,'(A,I2,A,F4.1)') '   Index:', I,
C             .                            '; value:',  ARRAY(I)
C
C              END DO
C
