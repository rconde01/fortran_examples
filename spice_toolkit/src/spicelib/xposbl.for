...
C$ Exceptions
C
C     1)  If the number of rows input is not positive, the error
C         SPICE(BADROWCOUNT) is signaled.
C
C     2)  If the number of columns input is not positive, the error
C         SPICE(BADCOLUMNCOUNT) is signaled.
C
C     3)  If the block size input is not positive, the error
C         SPICE(BADBLOCKSIZE) is signaled.
C
C     4)  If BMAT cannot be partitioned into an integer number of
C         blocks, the error SPICE(BLOCKSNOTEVEN) is signaled.
C
C$ Files
C
C     None.
