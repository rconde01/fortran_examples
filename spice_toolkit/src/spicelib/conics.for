...
C         SPICE(BADECCENTRICITY) is signaled.
C
C     2)  If a non-positive periapse distance is supplied, the error
C         SPICE(BADPERIAPSEVALUE) is signaled.
C
C     3)  If a non-positive value for the attracting mass is supplied,
C         the error SPICE(BADGM) is signaled.
C
C     4)  If ELTS is such that the resulting orbit at periapsis has
C         either its position or velocity equal to zero, or the square
C         of the resulting specific angular momentum's magnitude is
C         zero, an error is signaled by a routine in the call tree of
C         this routine. This is an indication of invalid ELTS elements.
C
C     5)  If ET is such that the offset in time from periapsis, at which
C         the state is to be determined, is so large that there is a
C         danger of floating point overflow during computation, an error
...
C$ Version
C
C-    SPICELIB Version 4.1.0, 13-AUG-2021 (JDR)
C
...
C
C        Added entry #4 and updated entry #5 in $Exceptions section.
C
...
C        a result, several problems were simultaneously corrected.
C
C        A major bug was fixed that caused improper state
C        evaluations for ET's that precede the epoch of the
C        elements in the elliptic case.
C
C        A danger of non-convergence in the solution of Kepler's
C        equation has been eliminated.
C
...
C        installed that ensure the elements supplied are physically
C        meaningful. Eccentricity must be non-negative. The
C        distance at periapse and central mass must be positive.
C        If not errors are signaled.
C
C        These changes were prompted by the discovery that the old
C        formulation had a severe bug for elliptic orbits and
C        epochs prior to the epoch of the input elements, and by
C        the discovery that the time of flight routines had
