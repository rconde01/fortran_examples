...
C         routine.
C
C     2)  If the specified descriptor does not belong to a segment data
C         format organized in accordance with generic segment
C         architecture, an error is signaled by a routine in the call
...
C         third integer component of the segment descriptor,
C         the error SPICE(WRONGDATATYPE) is signaled.
C
...
C     contain the set of Chebyshev polynomial coefficients for the
...
C
C     The following code fragment searches backwards through all of the
C     segments in a file applicable to the Mars Global Surveyor
C     spacecraft bus that are of data type 4, for a particular
C     spacecraft clock time. It then evaluates the pointing for that
C     epoch and prints the result.
C
C     The search performed here does not mimic the behavior of the CK
C     reader APIs CKGP and CKGPAV, which consider data from multiple CK
C     files, when available. See the CK Required reading for details.
C
C     C
...
C
C     C
C     C     Search backwards from the end of the CK file through all
C     C     of the segments.
C     C
...
C$ Version
C
C-    SPICELIB Version 1.0.3, 17-AUG-2021 (NJB) (JDR)
C
C        Updated code example to use backwards search. Added
C        note regarding difference between this search and those
C        performed by the CK reader APIs CKGP and CKGPAV.
C
