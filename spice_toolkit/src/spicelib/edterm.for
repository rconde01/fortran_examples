...
C         signaled.
C
C     4)  If the terminator type is not recognized, an error
C         is signaled by a routine in the call tree of
C         this routine.
C
C     5)  If the terminator point count NPTS is not at least 1, an error
C         is signaled by a routine in the call tree of this routine.
C
C     6)  If the light source has non-positive radius, an error
C         is signaled by a routine in the call tree of
C         this routine.
C
C     7)  If the light source intersects the smallest sphere centered at
C         the origin and containing the ellipsoid, an error is signaled
C         by a routine in the call tree of this routine.
C
C     8)  If radii for the target body or light source are not
C         available in the kernel pool, an error is signaled by
C         a routine in the call tree of this routine.
C
C     9)  If radii are available but either body does not have three
C         radii, an error is signaled by a routine in the call tree of
C         this routine.
C
C     10) If any of the radii is less-than or equal to zero, an error is
C         signaled by a routine in the call tree of this routine.
C
C     11) If any SPK look-up fails, an error is signaled by
C         a routine in the call tree of this routine.
C
C$ Files
C
...
C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'edterm_ex1.tm' )
C
C              INTEGER               NPTS
C              PARAMETER           ( NPTS   = 3 )
C
...
C             Solar incidence angle      (deg):       90.269765815
C             Solar incidence angle adjusted for
C             sun's angular radius (deg):             90.000000125
C
C           Terminator point  2:
C             Radius                     (km):      1737.400000000
C             Planetocentric longitude   (deg):       84.228091534
C             Planetocentric latitude    (deg):       59.995755519
C             Solar incidence angle      (deg):       90.269765709
C             Solar incidence angle adjusted for
C             sun's angular radius (deg):             90.000000019
C
C           Terminator point  3:
C             Radius                     (km):      1737.400000000
C             Planetocentric longitude   (deg):       87.216417974
C             Planetocentric latitude    (deg):      -59.979550515
C             Solar incidence angle      (deg):       90.269765733
C             Solar incidence angle adjusted for
C             sun's angular radius (deg):             90.000000043
C
...
C             Solar incidence angle      (deg):       89.730234402
C             Solar incidence angle adjusted for
C             sun's angular radius (deg):             90.000000122
C
C           Terminator point  2:
C             Radius                     (km):      1737.400000000
C             Planetocentric longitude   (deg):      -95.769215814
C             Planetocentric latitude    (deg):      -59.995785101
C             Solar incidence angle      (deg):       89.730234301
C             Solar incidence angle adjusted for
C             sun's angular radius (deg):             90.000000021
C
C           Terminator point  3:
C             Radius                     (km):      1737.400000000
C             Planetocentric longitude   (deg):      -92.780892017
C             Planetocentric latitude    (deg):       59.979498997
C             Solar incidence angle      (deg):       89.730234325
C             Solar incidence angle adjusted for
C             sun's angular radius (deg):             90.000000044
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 01-NOV-2021 (EDW) (JDR)
C
