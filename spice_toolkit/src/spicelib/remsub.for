...
C$ Exceptions
C
C     1)  If LEFT > RIGHT, RIGHT < 1, LEFT < 1, RIGHT > LEN(IN), or
C         LEFT > LEN(IN), the error SPICE(INVALIDINDEX) is signaled.
C
C$ Files
C
C     None.
...
C     'An Apple a day'      5      2         An error is signaled.
C     'An Apple a day'      0      0         An error is signaled.
C     'An Apple a day'     -3      3         An error is signaled.
C
C     Whenever an error has been signaled, the contents of OUT are
C     unpredictable.
C
C$ Restrictions
C
