...
C$ Abstract
C
C     Calculate the first derivative of a caller-specified scalar
C     function using a three-point estimation.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     UDFUNC   is the routine that returns the value of the scalar
...
C                 VALUE = UDFUNC ( X )
C
C     X        is a scalar double precision value at which to determine
C              the derivative of UDFUNC.
C
C              For many SPICE uses, X will represent ephemeris time,
C              expressed as seconds past J2000 TDB.
C
C     DX       is a scalar double precision value representing half the
...
C$ Detailed_Output
C
C     DERIV    is the scalar double precision approximate value of the
...
C$ Exceptions
C
C     1)  If DX has a value of zero, an error is signaled by a routine
...
C     point estimation. See the header of the SPICE routine QDERIV for
C     details of the discrete derivative computation performed by this
C     routine.
C
C$ Examples
C
...
C
C     1) Calculate the time derivative of the light time corresponding
C        to the apparent position of Mercury relative to the Moon at
C        time "JAN 1 2009."
C
...
C
C
C          -1.3567094055133566E-004
C
C
C$ Restrictions
C
C     1)  The function UDFUNC must exist everywhere within
C         [X - DX, X + DX].
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 05-JUL-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Included
C        required meta-kernel and added IMPICIT NONE statement to code
C        example.
C
C        Moved reference to QDERIV header from $Literature_References to
C        $Particulars with description of what it is expected from that
C        header.
C
C-    SPICELIB Version 1.0.0, 31-MAR-2010 (EDW) (NJB)
C
C-&
