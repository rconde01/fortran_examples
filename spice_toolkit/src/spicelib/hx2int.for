...
C$ Detailed_Input
C
C     STRING   is the hexadecimal string to be converted to an integer.
C
...
C             embedded within the significant portion of the character
...
C$ Detailed_Output
C
C     NUMBER   is the integer value to be returned. The value of this
...
C              occurred while attempting to parse NUMBER from the
C              hexadecimal character string STRING. ERROR will have the
C              value .TRUE. if an error occurs. It will have the value
C              .FALSE. otherwise.
C
