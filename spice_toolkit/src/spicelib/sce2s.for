...
C$ Exceptions
C
C     1)  If an SCLK kernel has not been loaded, does not contain all of
C         the required data, or contains invalid data, an error is
C         signaled by a routine in the call tree of this routine. The
C         output argument SCLKCH will not be modified. This routine
C         assumes that an SCLK kernel appropriate to the spacecraft
C         clock identified by the input argument SC has been loaded.
C
C     2)  If a leapseconds kernel is required for conversion between
C         SCLK and ET but is not loaded, an error is signaled by a
C         routine in the call tree of this routine. The output argument
C         SCLKCH will not be modified. When using an SCLK kernel that
C         maps SCLK to a time system other than ET (also called
C         barycentric dynamical time---`TDB'), it is necessary to have a
C         leapseconds kernel loaded at the time this routine is called.
C
...
C         clock string format for the spacecraft clock identified by SC,
...
C         output spacecraft clock string produced by this routine, an
C         error is signaled by a routine in the call tree of this
C         routine. The output argument SCLKCH may contain a portion of
C         the truncated string.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 1.3.0, 05-JUL-2021 (JDR)
C
