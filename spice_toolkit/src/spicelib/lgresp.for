...
C$ Detailed_Input
C
C     N        is the number of points defining the polynomial. The
C              array YVALS contains N elements.
C
C     FIRST,
...
C
C     WORK     is a work space array of the same dimension as YVALS.
C              It is used by this routine as a scratch area to hold
...
C
C        The returned value of LGRESP should be 1.D0, since the
...
C
C
C         ANSWER =    1.0000000000000000
C
C
C     2) Solve the same problem using a negative step. In order to
C        find the solution, set the elements of YVALS in reverse order.
C
C        The returned value of LGRESP would still be 1.D0.
C
C
C        Example code begins here.
C
C
C              PROGRAM LGRESP_EX2
C              IMPLICIT NONE
C
C              DOUBLE PRECISION      LGRESP
C              DOUBLE PRECISION      ANSWER
...
C
C        Update "N" and "WORK" detailed descriptions to remove
C        references to nonexistent arguments.
C
C        Updated the header to comply with NAIF standard.
C        Added IMPLICIT NONE to code example. Added second example
C        providing code to solve the problem of example #1 using a
C        negative step.
C
