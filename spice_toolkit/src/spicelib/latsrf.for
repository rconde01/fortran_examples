...
C$ Exceptions
C
C     1)  If the target body name input string cannot be converted to an
C         integer ID code, the error SPICE(IDCODENOTFOUND) is signaled.
C
...
C         FIXREF and the frame of a DSK segment of interest, an error
C         is signaled by a routine in the call tree of this
C         routine.
C
C     5)  If the input argument METHOD cannot be parsed, an error
C         is signaled by either this routine or a routine in
...
C         available, an error is signaled by a routine in the call
...
C
C     12) If the radii are not available in the kernel pool, an error is
C         signaled by a routine in the call tree of this routine.
C
C     13) If the target shape is "ellipsoid" and not all radii of the
...
C
C
C     1) In the following example program, a DSK file containing a
C        type 2 segment is used to provide a plate model representation
C        of the surface of Phobos.
C
C        Find the surface points on a target body corresponding to a
C        given planetocentric longitude/latitude grid.
C
C
C        Example code begins here.
C
C
C              PROGRAM LATSRF_EX1
C              IMPLICIT NONE
C        C
...
C        C     to range from +30 to -30 degrees.  Longitude
C        C     ranges from 0 to 240 degrees.  The increment
C        C     is 90 degrees for latitude and 120 degrees for
C        C     longitude.
C        C
C              N = 0
C
...
C           Cartesian coordinates: (9.5706817, 0.00000000, 5.5256356)
C           Latitudinal Coordinates:
C            Longitude (deg):    0.000000
C            Latitude  (deg):   30.000000
C            Radius     (km):   11.051271
C
...
C           Cartesian coordinates: (-4.7586430, 8.2422114, 5.4948076)
C           Latitudinal Coordinates:
C            Longitude (deg):  120.000000
C            Latitude  (deg):   30.000000
C            Radius     (km):   10.989615
C
C           Original Grid Coordinates:
C            Longitude (deg):  120.000000
C            Latitude  (deg):   30.000000
...
C           Cartesian coordinates: (-4.5704268, -7.9162115, 5.2774743)
C           Latitudinal Coordinates:
C            Longitude (deg):  240.000000
C            Latitude  (deg):   30.000000
C            Radius     (km):   10.554949
C
C           Original Grid Coordinates:
C            Longitude (deg):  240.000000
C            Latitude  (deg):   30.000000
C
C         Intercept for grid point 4:
C           Cartesian coordinates: (10.959385, 0.00000000, -6.3274040)
C           Latitudinal Coordinates:
C            Longitude (deg):    0.000000
C            Latitude  (deg):  -30.000000
C            Radius     (km):   12.654808
C
C           Original Grid Coordinates:
C            Longitude (deg):    0.000000
C            Latitude  (deg):  -30.000000
C
C         Intercept for grid point 5:
C           Cartesian coordinates: (-4.8830077, 8.4576174, -5.6384116)
C           Latitudinal Coordinates:
C            Longitude (deg):  120.000000
C            Latitude  (deg):  -30.000000
C            Radius     (km):   11.276823
C
C           Original Grid Coordinates:
C            Longitude (deg):  120.000000
C            Latitude  (deg):  -30.000000
C
C         Intercept for grid point 6:
C           Cartesian coordinates: (-4.5322568, -7.8500991, -5.2333994)
C           Latitudinal Coordinates:
C            Longitude (deg):  240.000000
C            Latitude  (deg):  -30.000000
C            Radius     (km):   10.466799
C
C           Original Grid Coordinates:
C            Longitude (deg):  240.000000
C            Latitude  (deg):  -30.000000
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 06-JUL-2021 (NJB) (JDR)
C
C        Bug fix: removed spurious blank from long error
C        message.
C
C        Edited the header to comply with NAIF standard. Modified
C        the grid dimensions and output format in the code example to
C        reduce the number of lines and their length in the solution.
C
