...
C     INCDNC     O   Solar incidence angle at the surface point.
...
C                            equations for SPOINT and the Sun, the
...
C              SRFVEC and the SPOINT-Sun vector at TRGEPC. Units are
C              radians. The range of PHASE is [0, pi]. See $Particulars
C              below for a detailed discussion of the definition.
C
C     INCDNC   is the solar incidence angle at SPOINT, as seen from
C              OBSRVR at time ET. This is the angle between the surface
C              normal vector at SPOINT and the SPOINT-Sun vector at
C              TRGEPC. Units are radians. The range of INCDNC is
C              [0, pi]. See $Particulars below for a detailed discussion
C              of the definition.
C
...
C         converted to an integer ID code, an error is signaled
C         by a routine in the call tree of this routine.
C
C     3)  If OBSRVR and TARGET map to the same NAIF integer ID code, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     4)  If the input target body-fixed frame FIXREF is not
C         recognized, an error is signaled by a routine in the
...
C         loaded into the kernel pool prior to calling ILUMIN, an error
C         is signaled by a routine in the call tree of this routine.
C
C     9)  If any of the radii of the target body are non-positive, an
C         error is signaled by a routine in the call tree of this
C         routine. The target must be an extended body.
C
C     10) If PCK data specifying the target body-fixed frame orientation
C         have not been loaded prior to calling ILUMIN, an error is
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 20-NOV-2021 (NJB) (JDR)
C
C        Changed the name of the argument SOLAR to INCDNC
C        for consistency with other illumination angle
C        routines.
C
