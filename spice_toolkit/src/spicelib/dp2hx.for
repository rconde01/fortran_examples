...
C     --------  ---  --------------------------------------------------
C     STRLEN     P   Max number of characters allowed in output string.
C     NUMBER     I   D.p. number to be converted.
C     HXSTR      O   Equivalent character string, left justified.
C     HXSSIZ     O   Length of the character string produced.
C
C$ Detailed_Input
C
C     NUMBER   is the double precision number to be converted to a
C              character string representation.
C
C$ Detailed_Output
C
C     HXSTR    is the character string produced by this routine that
...
C                 Size Mantissa Exponent   Minimum output string
C                 bits   bits     bits     length
C                 ---- -------- --------   ----------------------
...
C              the right if HXSSIZ < LEN( HXSTR ).
C
C     HXSSIZ   is the length of the base 16 "scientific notation"
C              character string produced by this routine.
C
C$ Parameters
C
C     STRLEN   is the maximum number of characters permitted in the
C              output string. The value of STRLEN is 255.
C
C$ Exceptions
C
C     Error free.
...
C
C     2)  If LEN( HXSTR ) > HXSSIZ, the output character string will
...
C
C     1) Convert a set of double precision numbers to their equivalent
C        character string using a base 16 "scientific notation."
C
C
...
C        C
C              INTEGER               HXSLEN
C              PARAMETER           ( HXSLEN = 40 )
C
C        C
C        C     Local variables.
C        C
C              CHARACTER*(HXSLEN)    STRVAL
C
C              DOUBLE PRECISION      NUMBER (8)
C
C              INTEGER               I
C              INTEGER               SVALLN
C
C        C
C        C     Assign an array of double precision numbers.
C        C
C              DATA                  NUMBER /    2.0D-9,       1.0D0,
C             .                                  -1.0D0,    1024.0D0,
C             .                               -1024.0D0,  521707.0D0,
C             .                                  27.0D0,       0.0D0 /
C
C        C
C        C     Loop over the NUMBER array, call DP2HX for each
C        C     element of NUMBER.
C        C
C              WRITE(*,*) 'number       string             length'
C              WRITE(*,*) '-----------  -----------------  ------'
C
C              DO I= 1, 8
C
C                 CALL DP2HX ( NUMBER(I), STRVAL, SVALLN )
C                 WRITE(*,'(E12.4,2X,A17,2X,I5)') NUMBER(I), STRVAL,
C             .                                              SVALLN
C
C              END DO
C
...
C         -----------  -----------------  ------
C          0.2000E-08  89705F4136B4A8^-7     17
C          0.1000E+01  1^1                    3
C         -0.1000E+01  -1^1                   4
C          0.1024E+04  4^3                    3
C         -0.1024E+04  -4^3                   4
C          0.5217E+06  7F5EB^5                7
C          0.2700E+02  1B^2                   4
C          0.0000E+00  0^0                    3
C
C
...
C$ Restrictions
C
C     1)  The maximum number of characters permitted in the output
C         string is specified by the parameter STRLEN.
C
...
C
C        Changed the argument names "STRING" and "LENGTH" to "HXSTR"
C        and "HXSSIZ" for consistency with other routines.
C
C        Added IMPLICIT NONE statement.
C
C        The declaration of STRLEN has been promoted to the
...
C
C        Updated $Brief_I/O, $Parameters and $Restrictions sections to
C        properly describe STRLEN.
C
...
C     the truncation on the right if HXSTR is not long enough to
