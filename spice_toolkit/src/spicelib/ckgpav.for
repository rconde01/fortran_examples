...
C
C     1) The following example program uses CKGPAV to get C-matrices
C        and associated angular velocity vectors for a set of images
C        whose SCLK counts (un-encoded character string versions) are
C        known.
C
C        For each C-matrix, a unit pointing vector is constructed and
C        printed along with the angular velocity vector.
C
C        Note: if the C-kernels of interest do not contain angular
C        velocity data, then the SPICELIB routine CKGP should be used
C        to read the pointing data. An example program in the header
C        of the SPICELIB routine CKGP demonstrates this.
C
C        We need to load also an SCLK kernel to convert from clock
C        string to 'ticks.' Although not required for older spacecraft
C        clocks, most modern spacecraft ones require a leapseconds
C        kernel to be loaded in addition to an SCLK kernel.
C
C
...
C        C        seconds. This is an acceptable tolerance, and must
C        C        be converted to "ticks" (units of encoded SCLK) for
C        C        input to CKGPAV.
C        C
C        C     -- The reference frame we want is J2000.
C        C
C        C     -- The CASSINI ISS camera boresight in the spacecraft
C        C        frame is (0.0005760, -0.99999982, -0.0001710).
C        C
C              INTEGER               NPICS
C              PARAMETER           ( NPICS  = 2 )
C
...
C            Angular velocity vector:  0.0000000  0.0000000  0.0000000
C
...
C$ Restrictions
C
C     1)  Only loaded C-kernel segments containing both pointing and
C         angular velocity data will be searched by this reader.
C         Segments containing only pointing data will be skipped over.
C
...
C$ Version
C
C-    SPICELIB Version 5.3.1, 02-JUL-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Updated the
C        example code, input times and kernel set to work with
C        PDS archived CASSINI data.
C
C        Added missing entry in $Versions section.
C
...
C        be non-inertial.
C
C-    SPICELIB Version 3.0.0, 05-OCT-1994 (WLT)
C
