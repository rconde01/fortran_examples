...
C
C     1)  If the input HANDLE does not designate a loaded CK file, an
...
C         is set to .TRUE., the error SPICE(NOAVDATA) is signaled.
C
C$ Files
C
...
C     reader APIs CKGP and CKGPAV, which continue searching when an
C     applicable segment doesn't satisfy a pointing request. See the CK
C     Required reading for details.
C
C     C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 06-JUL-2021 (NJB) (JDR)
C
C        Corrected code example: removed comment character preceding
C        CKBSS call. Added note regarding difference between this
C        search and those performed by the CK reader APIs CKGP and
C        CKGPAV.
C
