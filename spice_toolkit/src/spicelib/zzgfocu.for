...
C$ Restrictions
C
C     1)  This is a SPICELIB private routine; it should not be called by
C         user applications.
C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 17-OCT-2021 (EDW) (JDR)
C
C        Body radii accessed from kernel pool using ZZGFTREB in
C        ZZGFOCIN.
C
C        Edited the header of the umbrella routine and all its entry
C        points to comply with NAIF standard. Added $Index_Entries.
C
...
C$ Index_Entries
C
C     solving for occultation states in GF routines
C
C-&
...
C         is not recognized, the error SPICE(INVALIDSHAPE) is signaled.
C
...
C         SPICE(INVALIDFRAME) is signaled.
C
C     10) If the aberration correction string is invalid, an error
C         is signaled by a routine in the call tree of this
C         routine.
C
C     11) If either FSHAPE or BSHAPE specifies that the target surface
...
C
C     12) If either FSHAPE or BSHAPE specifies that the target surface
C         is represented by DSK data, but the shape specification is
C         invalid, an error is signaled by a routine in the call tree
C         of this routine.
C
C     13) If a body RADII vector has other than exactly thee elements,
C         an error is signaled by a routine in the call tree of this
C         routine.
C
C     14) If a body RADII vector has any element less-than or equal to
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 07-SEP-2021 (EDW) (JDR)
C
...
C$ Index_Entries
C
C     initialization for occultation state determination
C
C-&
...
C$ Exceptions
C
C     1)  If any SPK lookup fails, an error is signaled by a routine in
C         the call tree of this routine.
C
C     2)  If any frame transformation lookup fails, an error is signaled
C         by a routine in the call tree of this routine.
C
C     3)  If any occultation computation is done for ellipsoidal
C         targets, and if either semi-axis matrix is invalid, an error
C         is signaled by a routine in the call tree of this
C         routine.
C
C     4)  If any two of the bodies defining the occultation geometry
C         intersect, either error, the error SPICE(NOTDISJOINT) is
C         signaled by this routine, or, the error is signaled by a
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 11-SEP-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C-    SPICELIB Version 2.0.0, 21-FEB-2017 (NJB)
