...
C$ Exceptions
C
C     This routine is meant to signal no errors under normal
C     circumstances. The only errors it can signal would be caused by
C     bugs.
C
...
C
C        Made minor formatting changes to code.
C        Updated inline comments to clarify logic.
C
...
C
C     If multiple surface ID codes are associated with a name, only
C     the highest priority ID is associated with the name in the
C     mapping data structures.
C
      DO I = NVALS, 1, -1
C
C        Register this normalized surface name and body ID, but only if
C        the name from this pair is not already in the hash.
C
...
C        matching name.
C
C        Use hash function to get index of the head node. We apply the
C        hash function to a version of the normalized name that has
C        all blanks compressed out. (A normalized name may have single
C        blanks separating words comprising the name.)
C
...
C
C        ITEMAT is the node at which a name match was found by the
C        above loop, if a match was found. ITEMAT is the tail node of
C        the list if no match was found. It is 0 if the list is empty.
C
C        ITEMAT will be used below only if the list is non-empty and
C        no match was found, in which case ITEMAT is non-zero.
C
         IF ( NAMNEW ) THEN
C
...
C           attempting to register any more surface IDs if the name was
C           already registered due to a higher-indexed assignment.
C
...
C
C           ITEMAT is the node at which a surface ID code match was
C           found by the above loop, if a match was found. ITEMAT is
C           the tail node of the list if no match was found. It is 0 if
C           the list is empty.
C
C           ITEMAT will be used below only if the list is non-empty and
C           no match was found, in which case ITEMAT is non-zero.
