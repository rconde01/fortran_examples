...
C     RASIG      O   Right ascension uncertainty in radians.
C     DECSIG     O   Declination uncertainty in radians.
...
C$ Exceptions
C
C     1)  If fetching of any of output values fails, the error
C         SPICE(BADSTARINDEX) is signaled.
C
C     2)  If no star catalog has been loaded, an error is signaled by a
C         routine in the call tree of this routine.
C
C     3)  If a star catalog type 1 was not queried by calling STCF01
C         before calling this routine, an error is signaled by a routine
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 16-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Updated header to comply with NAIF standard. Corrected argument
C        names "RASIG" and "DECSIG" in $Brief_I/O.
C
C        Updated entry #3 in $Exceptions section.
C
