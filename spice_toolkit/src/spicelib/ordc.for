...
C$ Abstract
C
C     Return the ordinal position of a given item in a set. If the
C     item does not appear in the set, return zero.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     ITEM     is a string to be located within a character set.
C              Trailing blanks are not significant in the comparison.
C
C     SET      is a properly validated SPICE set that is to be
C              searched for the occurrence of ITEM. Trailing blanks are
C              not significant in the comparison.
C
C$ Detailed_Output
C
C     The function returns the ordinal position of ITEM within SET.
C     Ordinal positions range from 1 to N, where N is the cardinality
C     of the set.
C
C     If ITEM is not an element of SET, the function returns 0.
C
C$ Parameters
C
C     None.
...
C$ Exceptions
C
C     1)  If the input set has invalid cardinality, an error is signaled
C         by a routine in the call tree of this routine. ORDC returns an
C         unspecified value in this case.
C
C     2)  If the input set has invalid size, an error is signaled by a
C         routine in the call tree of this routine. ORDC returns an
C         unspecified value in this case.
C
C$ Files
C
...
C     SPICE set, be it INTEGER, CHARACTER or DOUBLE PRECISION. For
...
C$ Restrictions
C
C     1)  SET must be a validated or empty set.
C
...
C        $Detailed_Input, $Detailed_Output and $Exceptions sections.
C
