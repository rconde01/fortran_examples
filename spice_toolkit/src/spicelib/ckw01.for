...
C     AVFLAG     I   .TRUE. if the segment will contain angular
C                    velocity.
...
C
C     4)  If the first encoded SCLK time is negative, the error
C         SPICE(INVALIDSCLKTIME) is signaled.
C
C     5)  If the second encoded SCLK or any subsequent times, or if the
C         encoded SCLK times are not strictly increasing, the error
C         SPICE(TIMESOUTOFORDER) is signaled.
C
C     6)  If BEGTIM is greater than SCLKDP(1) or ENDTIM is less than
C         SCLKDP(NREC), the error SPICE(INVALIDDESCRTIME) is signaled.
C
...
C
C     9)  If any quaternion has magnitude zero, the error
...
C
C     1) The following example creates a CK file with a type 1 segment
C        from a series of pointing instances that represent a structure
C        initially aligned with the J2000 frame, and which is rotating
C        about the J2000 Z-axis. There will be one pointing instance
C        per 10-tick interval.
C
C
...
C        C
C              SEGID  = 'Test type 1 CK segment'
C              IFNAME = 'Test CK type 1 segment created by CKW01'
C
C        C
C        C     Open a new kernel.
C        C
C              CALL CKOPN ( CK1, IFNAME, NCOMCH, HANDLE )
C
C        C
...
C$ Version
C
C-    SPICELIB Version 3.0.1, 13-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Created
C        complete code example from existing fragment.
C
C        Updated $Exceptions entry #9 to reflect change implemented in
C        version 3.0.0. Removed unnecessary $Revisions section.
C
