...
C$ Detailed_Output
C
C     C        is the output SPICE window, containing the union of A
C              and B --- every point contained in A, or in B, or in
C              both.
C
...
C         elements, the error SPICE(WINDOWEXCESS) is signaled.
C
...
C        of argument C in $Brief_I/O table.
C
C        Added entry #2 in $Exceptions section. Removed unnecessary
C        entries in $Revisions section.
C
