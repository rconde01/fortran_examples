...
C     ray-surface intercept algorithm.
C
C     See the include file
C
C        dla.inc
...
C         of the shape model, the error is signaled by a routine in the
C         call tree of this routine.
C
C     5)  If the input ray direction is the zero vector, the error
C         SPICE(ZEROVECTOR) is signaled.
C
C     6)  If the coarse voxel grid scale of the shape model is less
C         than 1, the error SPICE(VALUEOUTOFRANGE) is signaled.
C
C     7)  If the coarse voxel grid of the shape model contains more
C         than MAXCGR (see dsk02.inc) voxels, the error
...
C        target body.
C
C        This simple program assumes the shape model for the target
C        body is stored in a single type 2 DSK segment, and that this
C        segment is the first one in the DSK file to which it belongs.
C
C
...
C        platform, using the DSK file named phobos_3_3.bds, the output
C        was:
C
C
...
C         Intercept found:
C            Plate ID:                   306238
C            Cartesian coordinates:  1.52087789  0.00000000  8.62532711
C            Latitudinal coordinates:
C            Longitude (deg):    0.0000000000000000
C            Latitude  (deg):    80.000000000000014
C            Range      (km):    8.7583866856211490
C
C            Ray vertex:
C            Longitude (deg):    0.0000000000000000
...
C         Intercept found:
C            Plate ID:                   317112
C            Cartesian coordinates:  1.18970365  0.99827989  8.80777185
C            Latitudinal coordinates:
C            Longitude (deg):    40.000000000000000
C            Latitude  (deg):    80.000000000000000
C            Range      (km):    8.9436459265318629
C
C            Ray vertex:
C            Longitude (deg):    40.000000000000000
C            Latitude  (deg):    80.000000000000000
...
C         Intercept found:
C            Plate ID:                   324141
C            Cartesian coordinates:  0.27777518  1.57534131  9.07202903
C            Latitudinal coordinates:
C            Longitude (deg):    80.000000000000028
C            Latitude  (deg):    80.000000000000014
C            Range      (km):    9.2119797003191017
C
C            Ray vertex:
...
C         Intercept found:
C            Plate ID:                   327994
C            Cartesian coordinates: -0.81082405  1.40438846  9.19682344
C            Latitudinal coordinates:
C            Longitude (deg):    120.00000000000001
C            Latitude  (deg):    80.000000000000000
C            Range      (km):    9.3386992651610452
C
C            Ray vertex:
C            Longitude (deg):    119.99999999999999
C            Latitude  (deg):    80.000000000000000
C            Range      (km):    28.023536291251524
C
C
C         Intercept found:
C            Plate ID:                   329431
C            Cartesian coordinates: -1.47820193  0.53802150  8.92132122
C            Latitudinal coordinates:
C            Longitude (deg):    160.00000000000006
C            Latitude  (deg):    80.000000000000014
C            Range      (km):    9.0589469760393797
C
C            Ray vertex:
C            Longitude (deg):    160.00000000000000
C            Latitude  (deg):    80.000000000000000
C            Range      (km):    28.023536291251524
C
C
C         Intercept found:
C            Plate ID:                   196042
C            Cartesian coordinates: -1.49854761 -0.54542673  9.04411256
C            Latitudinal coordinates:
C            Longitude (deg):    200.00000000000000
C            Latitude  (deg):    80.000000000000000
C            Range      (km):    9.1836325764960041
C
C            Ray vertex:
C            Longitude (deg):    200.00000000000000
C            Latitude  (deg):    80.000000000000000
...
C         Intercept found:
C            Plate ID:                   235899
C            Cartesian coordinates: -0.78240454 -1.35516441  8.87447325
C            Latitudinal coordinates:
C            Longitude (deg):    239.99999999999991
C            Latitude  (deg):    80.000000000000000
C            Range      (km):    9.0113763066160804
C
C            Ray vertex:
C            Longitude (deg):    239.99999999999997
C            Latitude  (deg):    80.000000000000000
C            Range      (km):    28.023536291251524
C
C
...
C
C
C        Warning: incomplete output. Only 100 out of 1135 lines have
C        been provided.
C
C
C$ Restrictions
...
C
C     [1]  A. Woo, "Fast Ray-Box Intersection", Graphic Gems I,
C          395-396, Aug. 1990
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 13-JAN-2021 (NJB) (JDR) (BVS)
C
C        Bug fix: in some cases the previous version of this routine
C        could still return an intercept outside of the segment
C        boundaries by more than the allowed margin. In those cases,
C        the returned plate ID was invalid. Both problems have been
C        corrected.
C
C        See $Revisions for details.
C
C        Edited the header to comply with NAIF standard. Updated the
C        example code to retrieve directly from the DSK descriptor the
C        upper bound for the target radius. Added record in
C        $Index_Entries.
C
C-    SPICELIB Version 1.0.0, 04-APR-2017 (NJB) (EDW) (JAB)
C
...
C           voxel---by more than a small tolerance---is found, the
C           search rejects that intercept and continues until a
C           valid intercept is found and all plates in the voxel
C           containing that intercept have been checked for an
C           intersection. The rejected intercept may later be
C           determined to be a valid solution during a check of
C           plates associated with a voxel that contains that
C           intercept; in fact it is the correct solution if no
C           other plates contain a solution closer to the ray's
C           vertex. (Usually there is a unique voxel containing the
C           intercept, but this is not so if the intercept lies on
C           a voxel boundary not on an edge of the voxel grid.)
C
C           Note that there's no need to look for intersections in
C           voxels further out in the voxel list than the first one
C           that contains an intercept.
C
C           The previous version of the routine terminated the
C           search after checking all plates in the current voxel,
C           after an intercept was found on any plate associated
C           with that voxel. The intercept was not required to be
C           contained in the voxel.
C
...
C           call.
C
C           Some precautionary measures were added: a backstop
C           check for an empty voxel list was added after the XDDA
C           call. A backstop initialization of PNTR was added
C           before the plate collection loop. This initialization
C           is needed only if the voxel list returned by XDDA is
C           empty. The list should never be empty.
C
C        25-MAR-2014 (NJB)
C
...
C
C           The example program was updated for compatibility with
C           the final DSK descriptor layout. The output format was
C           adjusted. Sample output from the program is now shown
C           in the header.
C
C        13-MAY-2010 (NJB)
C
...
C     intersection of ray and surface
C
C-&
...
C$ Revisions
C
C-    SPICELIB Version 1.1.0, 13-JAN-2021 (NJB)
C
C        Bug fix: in some cases the previous version of this
C        routine could return an intercept outside of the segment
C        boundaries (the "outside intercept") by more than the
C        allowed margin. In those cases, the returned plate ID was
C        invalid.
C
C        Both problems have been corrected.
C
C        Description of the bug
C        ----------------------
C
C        In the case where all of the follow conditions hold:
C
C           - the input ray's intercepts exist both within and
C             outside the segment's boundaries
C
C           - the outside intercept is considered the nearest
C             solution to the vertex at the time the intercept
C             is found
C
C           - the intercept that should have been selected was
C             found before the outside intercept
C
C           - both intercepts lie on plates belonging to the same voxel
C
C        the outside intercept will overwrite the correct intercept.
C
C        In the situation described above, the plate ID returned
C        will not be that of the outside plate.
C
C        Solution
C        --------
C
C        Each intercept that passes the test for being closest, of
C        all intercepts seen so far, to the ray's vertex is stored
C        in a temporary variable. The output XPT is updated only
C        when the intercept is found to lie within the segment's
C        coordinate bounds, or outside the bounds by no more than
C        the allowed margin.
C
C-    SPICELIB Version 1.0.0, 04-APR-2017 (NJB)
C
C        10-SEP-2014 (NJB)
C
C           Bug fix: during an intercept search over the voxel
C           list returned by XDDA, if an intercept outside the
C           current voxel---by more than a small tolerance---is
C           found, the search rejects that intercept and continues
C           until a valid intercept is found and all plates in the
C           voxel containing that intercept have been checked for
C           an intersection. The rejected intercept may later be
C           determined to be a valid solution during a check of
C           plates associated with a voxel that contains that
C           intercept; in fact it is the correct solution if no
C           other plates contain a solution closer to the ray's
C           vertex. (Usually there is a unique voxel containing
C           the intercept, but this is not so if the intercept
C           lies on a voxel boundary not on an edge of the voxel
C           grid.)
C
C           Note that there's no need to look for intersections in
C           voxels further out in the voxel list than the first
C           one that contains an intercept.
C
C           The previous version of the routine terminated the
C           search after checking all plates in the current voxel,
C           after an intercept was found on any plate associated
...
C           contained in the voxel.
C
C           In the previous version of the routine, an intercept
C           found outside of the current voxel could effectively
C           mask an intercept closer to the ray's vertex, as shown
...
C           Here the ray hits voxel 1 and finds an intercept on
C           plate P* at the point marked by "@." No other
C           intercepts on plates in voxel 1 exist, so the search
C           terminates. The intercept marked by "$" is closer to
C           the vertex but is not seen.
C
C                                   V
C                                  /
C                                 /
...
C
C
C           The updated algorithm, when presented with the
C           situation shown above, will check all plates in voxel
C           2 before terminating.
C
C           Note that the problem could occur in cases where
C           voxels 1 and 2 are not adjacent.
C
C-&
...
C                 members of the edge rays. Finally see if the ray
C                 lies inside the tetrahedron.
C
...
C
     ...
     .                             POINTS(1,3), NORMAL      )
                  ...
                  IF ( HITS ) THEN
C
C                    The ray intersects this plate.
C
...
C
C                       Note that we don't yet know that this solution
C                       is valid.
C
C                          ___    ____   __________
C                          XPT2 = VTX2 + SCALE*UDIR
C
                        CALL VLCOM ( 1.D0, VTX2, SCALE, UDIR, XPT2 )
...
C                       are zero-based.
C
                        CALL ZZTOGRID ( XPT2, VOXORI, VOXSIZ, HITCOR )
C
