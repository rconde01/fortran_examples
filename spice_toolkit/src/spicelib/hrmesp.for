...
C$ Abstract
C
C     Evaluate, at a specified point, a Hermite interpolating polynomial
C     for a specified set of equally spaced abscissa values and
C     corresponding pairs of function and function derivative values.
C
C$ Disclaimer
C
...
C              polynomial of degree 2*N-1 that fits the points and
...
C$ Exceptions
C
C     1)  If STEP is zero, the error SPICE(INVALIDSTEPSIZE) is
C         signaled.
C
...
C
C        The returned value of ANSWER should be 141.D0, and the
C        returned derivative value should be 456.D0, since the unique
C        7th degree polynomial that fits these constraints is
C
C                     7       2
C           f(x)  =  x   +  2x  + 5
C
...
C
C
C         ANSWER =    141.00000000000000
C         DERIV  =    456.00000000000000
C
C
C$ Restrictions
C
...
C          chapters 3.0 and 3.1, Cambridge University Press, 1986.
C
C     [2]  S. Conte and C. de Boor, "Elementary Numerical Analysis -- An
C          Algorithmic Approach," 3rd Edition, p 64, McGraw-Hill, 1980.
C
...
C$ Version
C
C-    SPICELIB Version 1.2.2, 01-OCT-2021 (JDR) (NJB)
C
C        Edited the header to comply with NAIF standard. Added code
C        example's solution.
C
C        Fixed formula in the description of YVALS argument in
C        $Detailed_Input.
C
...
C           there are 2*N abscissa values, since each input abscissa
