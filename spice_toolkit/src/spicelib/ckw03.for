...
C
C     BEGTIM,
C     ENDTIM   are the beginning and ending encoded SCLK times for
C              which the segment provides pointing information.
...
C         supported by the SPICELIB routine NAMFRM, the error
C         SPICE(INVALIDREFFRAME) is signaled.
C
C     8)  If NREC, the number of pointing records, is less than or equal
C         to 0, the error SPICE(INVALIDNUMREC) is signaled.
C
...
C         which there is an actual pointing instance in the segment, the
C         error SPICE(INVALIDSTARTTIME) is signaled.
C
...
C            THETA   +   2 * k * pi
C
C         radians, where k is any integer. These `large' rotations
C         will yield invalid results when interpolated. You must
C         ensure that the data stored in the segment will not be
...
C        dependent rotation and angular velocity.
C
...
C$ Version
C
C-    SPICELIB Version 3.0.1, 08-JUL-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Removed
C        unnecessary $Revisions section. Changed, in $Exceptions
C        entry #7, reference to FRAMEX by NAMFRM.
C
