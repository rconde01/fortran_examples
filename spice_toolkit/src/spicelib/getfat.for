...
C              specified by FILE. If the architecture cannot be
...
C$ Exceptions
C
C     1)  If the filename specified is blank, the error
C         SPICE(BLANKFILENAME) is signaled.
C
C     2)  If any inquire on the filename specified by FILE, required to
C         obtain information about the physical file, fails for some
C         reason, the error SPICE(INQUIREERROR) is signaled.
C
...
C
C     7)  If an issue is detected during the opening the input file or
C         the process to determine its architecture and type, an error
...
C     type. Filenames of open files should not be passed to this
C     routine.
C
C$ Particulars
C
...
C
C     1) Determine the file architecture and file type of all the
C        kernels loaded through a meta-kernel, and of a kernel in
C        transfer format.
C
C        Use the SPK kernel below to provide an example of a kernel in
C        transfer format.
C
C           earthstns_itrf93_050714.xsp
C
C
C        Use the meta-kernel shown below to load the other types of
C        SPICE kernels.
C
C
C           KPL/MK
C
C           File: getfat_ex1.tm
C
...
C        C
C              INTEGER               ARCHLN
C              PARAMETER           ( ARCHLN = 4 )
C
C              INTEGER               FILSIZ
C              PARAMETER           ( FILSIZ = 256 )
C
C              INTEGER               KTYPLN
C              PARAMETER           ( KTYPLN = 5 )
C
C        C
C        C     Local variables.
C        C
C              CHARACTER*(ARCHLN)    ARCH
C              CHARACTER*(FILSIZ)    FNAME
C              CHARACTER*(28)        FNAME1
C              CHARACTER*(KTYPLN)    KTYPE
...
C
C        C
C        C     Check the file architecture and type of an SPK
C        C     in transfer format.
C        C
C              FNAME1 = 'earthstns_itrf93_050714.xsp'
C
C              CALL GETFAT ( FNAME1, ARCH, KTYPE )
C
C              WRITE(*,*) 'File name     : ', FNAME1
C              WRITE(*,*) '  Architecture: ', ARCH
C              WRITE(*,*) '  Kernel type : ', KTYPE
C              WRITE(*,*) ' '
C
C        C
C        C     Load the kernels.
C        C
C              CALL FURNSH ( 'getfat_ex1.tm' )
C
C        C
C        C     Get the file architecture and kernel type for each of
C        C     the kernels in the kernel pool.
C        C
C              CALL KTOTAL ( 'ALL', COUNT )
C
C              DO I= 1, COUNT
C
C                 CALL KDATA ( I,     'ALL', FNAME, KTYPE, SOURCE,
C             .                HANDLE, FOUND                      )
C
C                 CALL GETFAT ( FNAME, ARCH, KTYPE )
C
C                 WRITE(*,*) 'File name     : ', FNAME
...
C
C
C         File name     : earthstns_itrf93_050714.xsp
C           Architecture: XFR
C           Kernel type : DAF
C
C         File name     : getfat_ex1.tm
C           Source      :
C           Architecture: KPL
C           Kernel type : MK
C
C         File name     : de430.bsp
C           Source      : getfat_ex1.tm
C           Architecture: DAF
C           Kernel type : SPK
C
C         File name     : mar097.bsp
C           Source      : getfat_ex1.tm
C           Architecture: DAF
C           Kernel type : SPK
C
C         File name     : pck00010.tpc
C           Source      : getfat_ex1.tm
C           Architecture: KPL
C           Kernel type : PCK
C
C         File name     : naif0011.tls
C           Source      : getfat_ex1.tm
C           Architecture: KPL
C           Kernel type : LSK
C
C         File name     : mgs_moc_v20.ti
C           Source      : getfat_ex1.tm
C           Architecture: KPL
C           Kernel type : IK
C
C         File name     : mgs_sclkscet_00061.tsc
C           Source      : getfat_ex1.tm
C           Architecture: KPL
C           Kernel type : SCLK
C
C         File name     : mgs_sc_ext12.bc
C           Source      : getfat_ex1.tm
C           Architecture: DAF
C           Kernel type : CK
C
C         File name     : mgs_ext12_ipng_mgs95j.bsp
C           Source      : getfat_ex1.tm
C           Architecture: DAF
C           Kernel type : SPK
C
C         File name     : megr90n000cb_plate.bds
C           Source      : getfat_ex1.tm
C           Architecture: DAS
C           Kernel type : DSK
C
...
C$ Version
C
C-    SPICELIB Version 5.1.0, 28-NOV-2021 (BVS)
C
C        Updated for MAC-OSX-M1-64BIT-CLANG_C.
C
C-    SPICELIB Version 5.0.1, 07-AUG-2020 (JDR)
C
C        Edited the header to comply with NAIF standard. Added complete
C        code example to $Examples section.
C
...
C
C-    SPICELIB Version 1.4.0, 05-JAN-1995 (HAN)
C
