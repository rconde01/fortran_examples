...
C$ Version
C
C-    SPICELIB Version 1.2.0, 24-NOV-2021 (NJB) (EDW)
C
C        Body radii accessed from kernel pool using ZZGFTREB.
C
C        Bug fix: boresight is now scaled to unit length.
C
C        Bug fix: in the case of a circular or elliptical FOV, where
C        the target shape is an ellipsoid, this routine creates an
C        ellipsoid such that its limb coincides with the FOV.
C        Previously this ellipsoid was allowed to be very large when
C        the FOV itself was large. Now the sum of the maximum ellipsoid
C        radius plus the distance between the ellipsoid center and the
C        observer is limited to FVEMAX km. See entry point ZZGFFVIN for
C        details. These changes were made on 09-JUN-2017. Minor updates
C        were made on the date given by the version line.
C
...
C
      ...
      DOUBLE PRECISION      FVEMAX
      PARAMETER           ( FVEMAX = 1.D-5 )
...
C$ Version
C
C-    SPICELIB version 1.3.0, 25-AUG-2021 (EDW)
C
C        Body radii accessed from kernel pool using ZZGFTREB.
C
C-    SPICELIB Version 1.2.0, 01-NOV-2020 (NJB)
C
C        Bug fix: boresight is now scaled to unit length.
C
C        Bug fix: in the case of a circular or elliptical FOV, where
...
C        the FOV itself was large. Now the sum of the maximum ellipsoid
C        radius plus the distance between the ellipsoid center and the
C        observer is limited to FVEMAX km. These changes were made on
C        09-JUN-2017. Minor updates were made on the date given by the
C        version line.
C
C-    SPICELIB Version 1.1.0, 04-JUN-2013 (EDW)
C
...
C$ Revisions
C
C-    SPICELIB Version 1.2.0, 29-OCT-2020 (NJB)
C
C        Bug fix: in the case of a circular or elliptical FOV, where
C        the target shape is an ellipsoid, this routine creates an
C        ellipsoid such that its limb coincides with the FOV. Previously
C        this ellipsoid was allowed to be very large when the FOV itself
C        was large. Now the sum of the maximum ellipsoid radius plus
C        the distance between the ellipsoid center and the observer is
C        limited to FVEMAX km.
C
C        Renamed local LOGICAL variable SVXTRG (save extended target)
C        to SVETRG (save ellipsoidal target), since it does not apply
C        to DSK targets. Note that DSK targets currently are not
C        supported.
C
C-&
...
C
C     Indicate whether we have an ellipsoidal target. SVETRG is .TRUE.
C     if and only we have one.
C
      SVETRG = SVTSHP .EQ. EDSHAP
...
C
     ...
     .      //            'error.'                                  )
         ...
         IF ( SVETRG ) THEN
C
C           We have an ellipsoidal target. Check the target frame's
C           center.
C
...
C
      ELSE
C
C        We don't have an ellipsoidal target body: zero out radius
C        values for this target.
C
         CALL CLEARD ( 3, SVTRAD )
...
C
C     Scale boresight vector to unit length.
C
      CALL VHATIP( BSITE )

C
C     Make sure the instrument shape specifier is left-justified
C     and in upper case.
C
...
C     a circular FOV when the target is an ellipsoid. Note that
C     these computations are not needed for the combination of a
C     circular FOV and a point or ray target.
C
      IF (        ( SVISHP .EQ. ELLFOV )
     .     .OR. ( ( SVISHP .EQ. CIRFOV ) .AND. SVETRG )  ) THEN
C
...
C
      IF (   (      ( SVISHP .EQ. CIRFOV )
     .         .OR. ( SVISHP .EQ. ELLFOV ) )  .AND.  SVETRG  ) THEN
C
C
C        Create an ellipsoid whose semi-axes are consistent with the
C        ellipse in SVPLAN defined by SEMIPT. Caution: after we
C        create this ellipsoid, we'll scale and shift it so it doesn't
C        extend too far from the observer.
C
C        Recall the origin is that of the instrument frame. The plane
C        SVPLAN is normal to the FOV axis and has distance 1 km from
C        the origin.
C
C        To start out, select the center of the ellipsoid. We place the
C        center along the direction defined by the FOV axis, at a
C        distance beyond SVPLAN (that is, on the side of the plane
C        opposite the observer), such that a sphere centered at this
C        point would have a limb consisting of a circle of radius
C        SVXMAG(1). If CTREXT is the distance of the ellipsoid center
C        from SVFVCT, then the limb geometry requires
C
...
C        length.
C
         ELLRAD(3) = SVXMAG(1) * SQRT ( 1.D0 + SVXMAG(1)**2.D0 )
         ELLRAD(1) = ELLRAD(3)

C
C        Compute the corresponding columns of the FOV semi-axis matrix.
C
C        The ellipsoid's third axis points along the FOV axis. Note
C        that SVFVCT is a unit vector pointing in the desired
C        direction.
C
         CALL VSCL ( ELLRAD(3), SVFVCT, SVFSMX(1,3) )

C
C        The first ellipsoid semi-axis is associated with SEMIPT(*,1)
C        and also has length ELLRAD(3):
C
         CALL VHAT ( SVSEMI(1,1), VTEMP )
         CALL VSCL ( ELLRAD(1),   VTEMP,  SVFSMX(1,1) )
...
C
         ELLRAD(2) = ( SVXMAG(2)/SVXMAG(1) ) * ELLRAD(3)
...
C
         ...
         CALL VSCL   ( ELLRAD(2),   VTEMP,       SVFSMX(1,2) )

C
C        Scale the ellipsoid and the distance of its center from
C        the observer so that the ellipsoid is of reasonable size
C        and doesn't extend too far from the observer. Caution: this
C        modification means the ellipsoid no longer intersects the FOV
C        plane at the FOV boundary. The ellipsoid is still usable with
C        ZZOCCED, which is the ellipsoid's raison d'etre.
C
         ESCALE = FVEMAX/ ( VNORM(SVEDCT) + MAX(ELLRAD(1), ELLRAD(2)) )

         DO I = 1, 3
            CALL VSCLIP ( ESCALE, SVFSMX(1,I) )
         END DO

         CALL VSCLIP ( ESCALE, SVEDCT )

      END IF


      IF ( ( SVISHP .EQ. CIRFOV ) .AND. ( .NOT. SVETRG ) ) THEN
C
...
C        diagnosed by routines in the call tree of this routine.
C
C     6)  If a body RADII vector has other than exactly thee elements,
C         the error SPICE(INVALIDCOUNT) is signaled by a routine in
C         the call tree of this routine.
C
C     7)  If a body RADII vector has any element less-than or
C         equal to zero, the error SPICE(BADAXISLENGTH) is signaled by
...
C
      IF ( SVETRG ) THEN
C
C        The target is an ephemeris object modeled as an ellipsoid.
C        There are two branches here: one for a rectangular/
