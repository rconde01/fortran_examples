...
C$ Detailed_Input
C
C     ITEM     is an integer value to be located within a set.
C
...
C     the set.
C
C     If ITEM is not an element of SET, the function return 0.
C
C$ Parameters
C
C     None.
...
C         by a routine in the call tree of this routine. ORDI returns an
C         unspecified value in this case.
C
C     2)  If the input set has invalid size, an error is signaled by a
C         routine in the call tree of this routine. ORDI returns an
C         unspecified value in this case.
C
C$ Files
C
