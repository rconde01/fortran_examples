...
C
C     The function returns .TRUE. if and only if M is a rotation matrix.
C
C$ Detailed_Input
C
...
C$ Detailed_Output
C
C     The function returns .TRUE. if and only if M is found to be a
C     rotation matrix. The criteria that M must meet are:
C
...
C         SPICE(VALUEOUTOFRANGE) is signaled. ISROT returns the
...
C
      IF ( NTOL .LT. 0.D0 ) THEN
         ...
         CALL SETMSG ( 'NTOL should be non-negative; it is #.' )
         CALL ERRDP  ( '#',  NTOL                              )
      ...
      ELSE IF ( DTOL .LT. 0.D0 ) THEN
         ...
         CALL SETMSG ( 'DTOL should be non-negative; it is #.' )
         CALL ERRDP  ( '#',  DTOL                              )
         CALL SIGERR ( 'SPICE(VALUEOUTOFRANGE)' )
         CALL CHKOUT ( 'ISROT'                  )
         RETURN
