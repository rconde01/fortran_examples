...
C$ Exceptions
C
C     See the entry points for descriptions of the exceptions specific
...
C
C        will produce an identifier specification IDSPEC that,
...
C$Procedure LXIDNT ( Lex identifier )

      ENTRY LXIDNT ( IDSPEC, STRING, FIRST, LAST, NCHAR )

C$ Abstract
C
C     Scan an identifier, starting from a specified character
C     position.
C
C$ Disclaimer
C
...
C$ Exceptions
C
C     1)  If IDSPEC is not properly initialized on input, or if its size
C         is too small, an error is signaled by a routine in the call
C         tree of this routine. IDSPEC is undefined on output in this
C         case.
C
C$ Files
C
