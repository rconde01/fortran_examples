...
C$ Detailed_Output
C
C     ROTATE   is a 3 x 3 rotation matrix that can be used to
C              transform positions relative to the frame
C              corresponding to frame FRAME2 to positions relative
C              to the frame FRAME2. More explicitly, if POS is
...
C         frame is not available, an error is signaled
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 08-OCT-2021 (JDR) (NJB)
C
C        Bug fix: added calls to FAILED after each call to
C        FRINFO and to ROTGET.
C
