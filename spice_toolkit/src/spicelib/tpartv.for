...
C$ Abstract
C
C     Parse the components of a time string and return a vector of the
C     components of that string. Also return an array of any modifiers
C     present in the input string; these may alter the interpretation
C     of the components.
C
C$ Disclaimer
C
...
C              daily time usage, and the system (UTC, TDB, TT, or TDT).
C
...
C
C                                        Possible
C                 Component   Meaning    Non-blank Modifier Values
C                 ---------   ---------  -------------------------
C                 1           ERA        'A.D.', 'B.C.'
C                 2           Weekday    'SUN', 'MON', ... etc.
C                 3           Time Zone  'UTC+i:i', 'UTC-i:i'
C                 4           AM/PM      'A.M.', 'P.M.'
C                 5           System     'UTC', 'TDB', 'TT', 'TDT'
C
...
C     Error free.
C
C     1)  All problems detected by this routine are reported via the
C         variables SUCCES and ERROR.
C
C$ Files
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 3.2.0, 23-DEC-2021 (EDW) (BVS) (JDR)
C
C        ISO format logic recognizes/evaluates ISO time strings
C        with or without trailing 'Z'.
C
C        Reordered header sections. Edited the header to comply with
C        NAIF standard.
C
C        Updated $Exceptions entry #1 wording.
C
...
C           o        stands for the symbol UTC-
C           s  ---   stands for a time system (UTC, TDB, TT, TDT)
...
C
C           If you tag an ISO as A.D., other logic in the Time 
C           subsystem will not intepret the two digit year value as a 
C           two digit abbreviation.
C
C            MODIFY(ERA) = 'A.D.'
C
