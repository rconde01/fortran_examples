...
C     --------  ---  --------------------------------------------------
C     ACTION     I   Kind of action to take 'SET' or 'GET'.
C     ITEM       I   Default item of interest.
C     VALUE     I-O  Value associated with the default item.
C
C$ Detailed_Input
C
...
C              is signaled. The case of VALUE is not significant.
C
C$ Detailed_Output
C
...
C     TDB (barycentric Dynamical Time), TDT (Terrestrial Dynamical
C     Time), or TT (Terrestrial Time). TDT and TT represent the same
C     time system. Both TDB and TT (TDT) have no leapseconds. As such
C     the time elapsed between any two epochs on these calendars does
C     not depend upon when leapseconds occur.
C
C     To set the default time system, select TDT, TT, TDB or UTC for
C     VALUE and make the following call.
C
...
C     followed by a + for time zones east of Greenwich and - for time
C     zones west of Greenwich. This is followed by the number of hours
C     to add or subtract from UTC. This is optionally followed by a
C     colon ':' and the number of minutes to add or subtract (based on
C     the sign that follows 'UTC') to get the local time zone. Thus to
C     specify the time zone of Calcutta you would specify the time zone
C     to be UTC+5:30. To specify the time zone of Newfoundland use the
C     time zone UTC-3:30.
C
...
C$ Version
C
C-    SPICELIB Version 1.3.0, 14-OCT-2021 (EDW) (JDR)
C
C        UCASE and LJUST called on VALUE only in 'SET' block.
C
C        Add time system name 'TT' (Terrestrial Time) as alternate
...
C
C           Recognize only the know time systems. Note, 'UTC' not
C           actually a time system, more a time representation.
C
            IF (       (MYVAL .EQ. 'TDB')
     .            .OR. (MYVAL .EQ. 'TDT')
     .            .OR. (MYVAL .EQ. 'TT')
...
C
     ...
     .      //            '''SYSTEM'', or ''ZONE'' ' )
      ...
      ELSE
         ...
         CALL SETMSG ( 'The action specified to TIMDEF was '
