...
C$ Restrictions
C
C     1)  CAUTION must be exercised when comparing floating point
C         numbers for equality. If the numbers in ARRAY or the number in
C         VALUE are the result of computations, then it is likely that
C         strict equality between VALUE and some element of ARRAY will
C         NOT hold (even if the two numbers are very close) unless the
C         numbers are the result of exactly the same computations.
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 03-JUL-2021 (JDR)
C
