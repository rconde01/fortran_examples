...
C$ Abstract
C
C     Compute, for a given observer and a ray emanating from the
C     observer, the surface intercept of the ray on a target body at 
C     a specified epoch, optionally corrected for light time and 
...
C              earth or on another extended object.
C
C              The observer must be outside the target body.
C
C              OBSRVR is case-insensitive, and leading and
...
C              The intercept with the target body's surface of the ray
...
C
C     6)  If the input argument METHOD cannot be parsed, an error
...
C         calling SINCPT, an error is signaled by a
...
C         loaded into the kernel pool prior to calling SINCPT, an error
...
C
C     11) If PCK data specifying the target body-fixed frame orientation
C         have not been loaded prior to calling SINCPT, an error is
...
C             Surface representation: Ellipsoid
C
C              Radius                   (km)  =    3384.9411269137695
C              Planetocentric Latitude  (deg) =   -48.477507940479093
C              Planetocentric Longitude (deg) =   -123.47407797517749
...
C              Planetocentric Longitude (deg) =   -123.47541506563023
C              Range                    (km)  =    386.14464664863726
C
C         Boresight vector
C
C           Vector in MGS_MOC_NA frame =
...
C
C
C        Warning: incomplete output. Only 100 out of 112 lines have been
C        provided.
C
C
...
C$ Restrictions
C
C     1)  A cautionary note: if aberration corrections are used, and
C         if DREF is the target body-fixed frame, the epoch at which
C         that frame is evaluated is offset from ET by the light time
C         between the observer and the *center* of the target body.
C         This light time normally will differ from the light time
C         between the observer and intercept point. Consequently the
C         orientation of the target body-fixed frame at TRGEPC will
C         not match that of the target body-fixed frame at the epoch
C         associated with DREF. As a result, various derived quantities
C         may not be as expected: for example, SRFVEC would not be
C         parallel to DVEC.
C
...
C         body-fixed frame.
C
C     2)  This routine must not be used for cases where the observer
C         is inside the target body. This routine does not attempt to
C         detect this condition.
C
C         If the observer is a point on a target surface described
C         by DSK data, care must be taken to ensure the observer is
C         sufficiently far outside the target. The routine should
C         not be used for surfaces for which "outside" cannot be
C         defined.
C
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 26-OCT-2021 (JDR) (NJB)
C
C        Bug fix: PRVCOR is no longer set to blank before
C        ABCORR is parsed.
C
C        ZZVALCOR is now used instead of ZZPRSCOR. This provides
C        better error handling.
C
C        Edits to $Examples section to comply with NAIF standard.
C
C        The header's $Detailed_Input and $Restrictions sections
C        were updated to state that the observer must be
C        outside the target body.
C
