...
C
C     ELLIPS   is a SPICE ellipse that represents an ellipse
...
C$ Exceptions
C
C     1)  If the input ellipse ELLIPS has one or both semi-axis lengths
C         equal to zero, the error SPICE(DEGENERATECASE) is signaled.
C
C     2)  If the geometric ellipse represented by ELLIPS does not
C         have a unique point nearest to the input point, any point
C         at which the minimum distance is attained may be returned
C         in PNEAR.
C
C     3)  If a ratio of non-zero ellipse radii violates the constraints
C         imposed by NEARPT, an error is signaled by a routine in the
C         call tree of this routine.
C
C     4)  The routine does not check for overflow when scaling or
C         translating the input point.
C
C$ Files
C
C     None.
...
C$ Examples
C
C     1)  For planetary rings that can be modeled as flat disks with
...
C
C            C
C            C     Make a SPICE ellipse representing the ring,
...
C$ Version
C
C-    SPICELIB Version 1.3.0, 24-AUG-2021 (JDR) (NJB)
C
C        Added IMPLICT NONE statement.
C
C        Edited the header to comply with NAIF standard.
C
C        Added entries #3 and #4 to $Exceptions section.
C
...
C
C     Find the nearest point to PRJPNT on the ellipsoid, PNEAR.
C
