...
C              designated by RFRAME (see description below).
C
...
C              FOVs. Such FOVs must satisfy the following
C              constraints:
C
...
C                     this plane (the normal plane contains the
C                     angle bisector defined by U and V) such that
C                     the projections have angular separation of at
C                     least 2*MARGIN radians from the plane spanned
C                     by U and V.
C
...
C
C     6)  If the FOV boundary has more than MAXVRT vertices, an error
...
C        C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   =  'fovray_ex1.tm' )
C
C              CHARACTER*(*)         TIMFMT
C              PARAMETER           ( TIMFMT =
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 03-JUL-2021 (JDR)
C
