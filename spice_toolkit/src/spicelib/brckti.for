...
C$ Abstract
C
C     Bracket an integer number. That is, given a number and an
...
C        C     Local variables.
C        C
C              INTEGER                 END1    ( LISTSZ )
C              INTEGER                 END2    ( LISTSZ )
C              INTEGER                 I
C              INTEGER                 NUMBER  ( LISTSZ )
C
C        C
C        C     Set the values for the example.
C        C
C              DATA                    END1   /  1,  1,  10, -10 /
C              DATA                    END2   / 10, 10, -10,  -1 /
C              DATA                    NUMBER / -1, 29,   3,   3 /
C
C
...
C              DO I = 1, LISTSZ
C
C                 WRITE(*,'(3I6,I11)') NUMBER(I), END1(I), END2(I),
C             .             BRCKTI ( NUMBER(I), END1(I), END2(I) )
C
C              END DO
C
C
...
C        BRCKTI: force an identifier to be within a range. Note that
C        this code assumes that the user provided value is a valid
C        integer number.
C
C
C        Example code begins here.
...
C
C        C
C        C     Prompt the user for the code identifier.
C        C
C              CALL PROMPT ( 'Enter object code: ', USRIN )
C
C        C
C        C     Convert the user input to integer.
C        C
C              CALL PRSINT ( USRIN, CODEIN )
C
C        C
C        C     Object code must be in the range 701-705.
C        C
C              CODEOK = BRCKTI ( CODEIN, 701, 705 )
C
C        C
C        C     Display confirmation message.
C        C
C              IF ( CODEIN .NE. CODEOK ) THEN
C
C                 WRITE(*,'(A,I3,A)') 'Provided object code ', CODEIN,
C             .                       ' is out of range (701-705).'
C
C              ELSE
C
C                 WRITE(*,'(A,I3,A)') 'Provided object code ', CODEIN,
C             .                       ' is in range (701-705).'
C
C              END IF
C
C              END
...
C
C        When this program was executed on a Mac/Intel/gfortran/64-bit
C        platform, using '710' as user provided input, the output was:
C
C
C        Enter object code: 710
C        Provided object code 710 is out of range (701-705).
C
C
C$ Restrictions
C
...
C        code examples based on existing code fragment.
C
