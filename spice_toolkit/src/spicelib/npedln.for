...
C     UBEL       P   Upper bound of array containing SPICE ellipse.
C     UBPL       P   Upper bound of array containing SPICE plane.
C
C$ Detailed_Input
C
C     A,
...
C              a SPICE ellipse. See the ELLIPSES Required
C              Reading for details.
C
C     UBPL     is the upper bound of the array used to contain
C              a SPICE plane. See the PLANES Required Reading
C              for details.
C
C$ Exceptions
C
C     If this routine detects an error, the output arguments PNEAR and
...
C          and the surface of a body modeled as a tri-axial ellipsoid
...
C             C     on the ellipsoid to the line containing the
C             C     boresight ray and the boresight ray's vertex. Find
C             C     the angular separation between this difference
C             C     vector and the boresight ray. If the angular
C             C     separation does not exceed pi/2, we have the nominal
C             C     geometry. Otherwise, we have an error.
C             C
...
C$ Version
C
C-    SPICELIB Version 1.4.0, 24-AUG-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Corrected
C        argument name in $Exceptions section.
C
