...
C              years in the pre-Christian era (B.C.) A year, Y B.C.,
C              is be represented as -(Y-1).  For example the year
...
C     in the Gregorian Calendar. However, the Gregorian calendar
...
C$ Version
C
C-    SPICELIB Version 1.2.1, 02-OCT-2021 (JDR)
C
...
C$ Revisions
C
C-    SPICELIB Version 1.1.0, 23-FEB-1998 (NJB)
C
C        Updated to remove non-standard use of duplicate arguments
C        in RMAINI calls.
C
...
C
C     The number of days elapsed since Gregorian Jan 1, of year 1 A.D.
...
C
C     The number of days elapsed since Julian Jan 1, of year 1 A.D.
C     to Jan 1 of YR is given by:
C
      JYDAYS(YR) = 365*(YR-1)
     .             +  ((YR-1)/4)
...
C     J2000 epoch) of some ancient epoch that occurred during
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 02-OCT-2021 (JDR)
C
