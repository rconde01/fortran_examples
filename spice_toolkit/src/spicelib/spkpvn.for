...
C$ Abstract
C
C     Return, for a specified SPK segment and time, the state (position
C     and velocity) of the segment's target body relative to its center
C     of motion.
C
C$ Disclaimer
C
...
C
C     STATE    is a 6-dimensional vector that contains the position and
C              velocity, at epoch ET, for whatever body is covered by
C              the specified segment. STATE has six elements: the first
C              three contain the body's position; the last three contain
C              the body's velocity. These vectors are rotated into the
C              specified  reference frame, the origin of which is
C              located at the center of motion for the body (see CENTER,
C              below). Units are always km and km/sec.
C
...
C$ Version
C
C-    SPICELIB Version 4.0.1, 27-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Removed
C        unnecessary $Revisions section. Improved $Abstract section.
C
...
C
C-    SPICELIB Version 1.1.0, 07-JAN-1997 (WLT)
C
