...
C$ Detailed_Output
C
C     ELLIPS   is the SPICE ellipse formed by the intersection
...
C        that solution.
C
C        We assume LOC is outside of the body. The body is modeled as
...
C
C        This example is equivalent to the one in EDLIMB, but it uses
C        INEDPL to compute the limb.
C
C
...
C
C        C
C        C     Normalize the position to factors of the radii.
C        C
C              SCPOS(1) = SCPOS(1) / RAD(1)**2
C              SCPOS(2) = SCPOS(2) / RAD(2)**2
C              SCPOS(3) = SCPOS(3) / RAD(3)**2
C
C        C
C        C     Find the apparent limb.  LIMB is a SPICE ellipse
C        C     representing the limb.
C        C
C              CALL NVC2PL ( SCPOS,  1.0D0,  PLANE  )
C              CALL INEDPL ( RAD(1), RAD(2), RAD(3),
C             .              PLANE,  LIMB,   FOUND  )
C
C        C
...
C        computation. The value 1.0 was replaced with the value 1.0D0
C        in the following three lines:
C
C           DSTORT(1) = 1.0 / A
C           DSTORT(2) = 1.0 / B
C           DSTORT(3) = 1.0 / C
