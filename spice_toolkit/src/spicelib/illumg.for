...
C              except within double-quoted strings representing surface
C              names. For example, the string ' eLLipsoid ' is valid.
C
C              Within double-quoted strings representing surface names,
C              blank characters are significant, but multiple
C              consecutive blanks are considered equivalent to a single
C              blank. Case is not significant. So
C
...
C              the surface normal vector at SPOINT and the
C              SPOINT-illumination source vector at TRGEPC. Units are
C              radians. The range of INCDNC is [0, pi]. See $Particulars
C              below for a detailed discussion of the definition.
C
...
C
C     7)  If the input argument METHOD is not recognized, the error
C         SPICE(INVALIDMETHOD) is signaled.
C
C     8)  If insufficient ephemeris data have been loaded prior to
C         calling ILLUMG, an error is signaled by a
...
C         loaded into the kernel pool prior to calling ILLUMG, an error
...
C         have not been loaded prior to calling ILLUMG, an error is
