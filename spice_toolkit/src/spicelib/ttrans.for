...
C
C     FROM,
C     TO       are two strings used to describe the type of time vector
C              TVEC. FROM is the type of the input vector TVEC and
...
C$ Version
C
C-    SPICELIB Version 1.6.0, 05-SEP-2021 (EDW) (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Removed INT casts in HMSSEC calls. The casts prevent
C        correct calculation of TDB time for non integer hour
C        and minute values in time strings from STR2ET.
C
C        Removed reference to FURNSH from the "LSK variable
...
C
C-    SPICELIB Version 1.1.0, 09-JUN-1999 (WLT)
C
...
C
C        Calculate seconds from midnight, 00:00:00.
C
         SECS   = HMSSEC( TVEC(4), TVEC(5), TVEC(6) )
...
C        Calculate seconds from midnight, 00:00:00.
C
         SECS   = HMSSEC ( TVEC(3), TVEC(4), TVEC(5) )
