...
C$ Detailed_Input
C
C     LIST     is a list of items delimited by any one of the characters
C              in the string DELIMS. Consecutive delimiters, and
C              delimiters at the beginning and end of the list, are
C              considered to delimit blank items. A blank list is
C              considered to contain a single, blank item. Leading and
C              trailing blanks in list are ignored.
C
C     DELIMS   contains the individual characters which delimit the
C              items in the list. These may be any ASCII characters,
C              including blanks.
C
C              However, by definition, consecutive blanks are NOT
C              considered to be consecutive delimiters. Nor are a blank
C              and any other delimiter considered to be consecutive
C              delimiters. In addition, leading and trailing blanks are
C              ignored.
C
C$ Detailed_Output
C
C     SET      is a SPICE set containing the items in the list, left
C              justified. Any item in the list too long to fit into an
C              element of SET is truncated on the right.
C
C              The strings in SET will be sorted in increasing order,
C              and duplicates will be removed. Trailing blanks are
C              ignored in string comparisons.
C
C              The size of the set must be initialized prior to calling
C              LPARSS.
C
C$ Parameters
C
...
C         of the items in the set, an error is signaled by a routine in
C         the call tree of this routine.
C
C     2)  If the string length of SET is too short to accommodate an
C         item, the item will be truncated on the right.
C
C     3)  If the string length of SET is too short to permit encoding of
C         integers via the SPICELIB routine ENCHAR, an error is signaled
...
C        documentation of arguments LIST, DELIM and SET.
C
C        Updated entries #2 and #3 in $Exceptions section: changed
C        wrong argument name, and indicated that the routine used
C        for encoding is part of SPICELIB.
C
