...
C
C     2)  If the reference frame OUTREF is unrecognized, an error is
C         signaled by a routine in the call tree of this routine.
C
C     3)  If the reference frame OBSREF is unrecognized, an error is
C         signaled by a routine in the call tree of this routine.
C
C     4)  If the frame evaluation locus REFLOC is not recognized, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     5)  If the loaded kernels provide insufficient data to compute
C         the requested state vector, an error is signaled
C         by a routine in the call tree of this routine.
C
C     6)  If an error occurs while reading an SPK or other kernel file,
C         the error  is signaled by a routine in the call tree of
C         this routine.
C
C     7)  If the aberration correction ABCORR is not recognized, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C$ Files
...
C        C
C              CHARACTER*(*)         FMT0
C              PARAMETER           ( FMT0   = '(A,3F20.8)' )
C
C              CHARACTER*(*)         FMT1
C              PARAMETER           ( FMT1   = '(1X,A, F20.8)' )
C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'spkcpo_ex1.tm' )
C
C              CHARACTER*(*)         TIMFMT
C              PARAMETER           ( TIMFMT =
...
C              62512272.82074845   58967494.42513601 -122059095.46751881
...
C
C        Corrected meta-kernel name in code examples. Modified code
C        examples output format for the solutions to fit within the
C        $Examples section without modifications.
C
