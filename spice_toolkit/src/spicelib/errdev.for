...
C
C                 A file name     Note that the name must not
...
C$ Parameters
C
C     FILEN    is the maximum length of a file name that can be
C              processed by this routine. FILEN is currently set to 255
C              characters.
C
C$ Exceptions
C
C     1)  If an invalid value of the argument OP is supplied, the error
C         SPICE(INVALIDOPERATION) is signaled.
C
C     2)  If OP is 'SET' and the device name DEVICE exceeds the maximum
C         length FILEN, the error SPICE(DEVICENAMETOOLONG) is signaled.
C
C$ Files
C
C     None.
...
C$ Restrictions
C
C     1)  This routine has no capability of determining the validity of
C         the name of an output device. Care must be taken to ensure
C         that the file named is the correct one.
C
C         The device name is assumed to be no longer than FILEN
C         characters.
C
...
C
C-    SPICELIB Version 2.26.0, 07-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. $Exceptions
C        section has been completely updated to provide only the list
C        of exceptions. Additional information provided there has been
C        moved to $Particulars.
C
...
C
C-    SPICELIB Version 1.2.0, 03-NOV-1993 (HAN)
C
...
C
C-    SPICELIB Version 1.1.0, 09-OCT-1992 (HAN)
C
