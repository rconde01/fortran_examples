...
C     than signaling errors via SIGERR and setting the long
...
C         on a system where logical file names are supported, if
...
C$ Files
C
C     If DEVICE specifies a device other than 'SCREEN' or 'NULL',
...
C$ Version
C
C-    SPICELIB Version 4.27.0, 28-NOV-2021 (BVS)
C
C        Updated for MAC-OSX-M1-64BIT-CLANG_C.
C
C-    SPICELIB Version 4.26.0, 13-AUG-2021 (JDR)
C
...
C
      ...
      ELSE IF ( TMPNAM .EQ. 'SCREEN' ) THEN
         ...
         WRITE ( *, *, IOSTAT=IOSTAT ) LINE(:RTRIM(LINE))
         RETURN
...
C     it cannot use the normal SPICELIB error signaling mechanism.
...
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Fixed $Files
C        section and added entry to $Index_Entries.
C
C-    SPICELIB Version 4.0.5, 17-JUL-2002 (BVS)
C
C        Added MAC-OSX environments.
