...
C        short and long messages, providing the value of two double
C        precision variables within the long message, and signal the
C        error.
C
C
...
C        C     strings where the value of the double precision
C        C     variables will go.  Our markers are '#' and 'XX'.
C        C
...
C
C        C
C        C     Insert a double precision where the # is now.
C        C
C              CALL ERRDP ( '#',  5.D0  )
C
C        C
C        C     Insert another double precision number in the long
C        C     message where the first XX is now.
C        C
C              CALL ERRDP ( 'XX', 73.4567D0 )
C
C        C
...
C        SPICE(USERDEFINED) --
C
C        LONG MESSAGE. Invalid operation value. The value was
C        5.0000000000000E+00. Left endpoint exceeded right endpoint. ***
C        endpoint was: 7.3456700000000E+01. The right endpoint was: XX.
C
...
C$ Version
C
C-    SPICELIB Version 2.3.0, 13-AUG-2021 (JDR)
C
