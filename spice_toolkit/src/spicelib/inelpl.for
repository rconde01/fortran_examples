...
C$ Detailed_Input
C
C     ELLIPS   is a SPICE ellipse. The ellipse is allowed to
...
C$ Exceptions
C
C     1)  If the input plane is invalid, the error SPICE(INVALIDPLANE)
C         is signaled. The input plane must be a SPICE plane: the normal
C         vector must be non-zero and the constant must be non-negative.
C
C     2)  If the input ellipse has non-orthogonal axes, the error
C         SPICE(INVALIDELLIPSE) is signaled.
C
...
C            C     this plane by a SPICE plane.
C            C
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 24-AUG-2021 (JDR)
C
