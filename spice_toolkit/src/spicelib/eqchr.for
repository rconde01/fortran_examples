...
C$ Abstract
C
C     Return .TRUE. if two given characters are equivalent when the
C     case of the characters is ignored.
C
C$ Disclaimer
C
...
C
C     A,
C     B        are two characters that are to be compared to see
C              if they are the same letter (although possibly
...
C$ Abstract
C
C     Return .TRUE. if two given characters are not equivalent if the
C     case of the characters is ignored.
C
C$ Disclaimer
C
...
C     A,
C     B        are two characters that are to be compared to see
C              if they are different letters. Letters that have
...
C        Edited the header to comply with NAIF standard.
C
C-    SPICELIB Version 1.0.0, 17-SEP-1998 (EDW)
C
C        Replace the UVALUE data statement with a loop to fill
C        UVALUE. The Absoft Mac compiler failed to compile the
C        data statement correctly, and so this function failed
