...
C$ Detailed_Input
C
C     HANDLE   is the file handle of a binary DAF which has been opened
C              with read access.
C
C     BUFSIZ   is the maximum number of comments that may be placed into
...
C$ Detailed_Output
C
C     N        is the number of comment lines extracted from the comment
...
C
C     BUFFER   is an array of at most BUFSIZ comments which have been
C              extracted from the comment area of the binary DAF
C              attached to HANDLE.
C
C     DONE     is a logical flag indicating whether or not all of the
...
C
C     1) The following example will extract a maximum of 30 lines
C        from the comment area of a binary DAF, displaying the
C        comments on the terminal screen.
C
C        Although it would be possible to read the 30 lines in one
C        go, for this example, use only a buffer size of 10,
C        demonstrating the use of the DONE logical flag.
C
C
...
C        C     Local variables.
C        C
C              CHARACTER*(LINLEN)    BUFFER ( BUFSIZ )
C
C              INTEGER               HANDLE
C              INTEGER               I
C              INTEGER               J
...
C
C        C
C        C     Read a maximum of 30 lines of comments.
C        C
...
C
C        C
C        C        Write the N lines to the terminal screen.
C        C
C                 DO J = 1, N
C
...
C
C        C
C        C     Have all the comments been read?
C        C
C              IF ( .NOT. DONE ) THEN
C
C                 WRITE(*,*) ' '
C                 WRITE(*,*) 'Warning: Not all comments have been read!'
C
C              END IF
C
C        C
...
C
C
C        Comment area of input DAF file (max. 30 lines):
C        --------------------------------------------------------------
C
...
C            ========================================================***
C
C            This file provides geocentric states---locations and vel***
C            set of DSN stations cited in the list below under "Posit***
C            position vectors point from the earth's barycenter to th***
C            velocities are estimates of the derivatives with respect***
C            vectors; in this file, velocities are constant. Station ***
C            magnitudes on the order of a few cm/year.
C
C            The states in this file are given relative to the terres***
C            frame ITRF93.
C
C            This SPK file has a companion file
C
C               earthstns_fx_201023.bsp
C
C            which differs from this one only in that it uses the ref***
C            frame alias 'EARTH_FIXED'. See the comment area of that ***
C            and the Frames Required Reading for details.
C
C        --------------------------------------------------------------
C
C         Warning: Not all comments have been read!
C
C
C        Warning: incomplete output. 12 lines extended past the right
C        margin of the header and have been truncated. These lines are
C        marked by "***" at the end of each line.
C
C
C     2) The following example demonstrates the use of this routine to
C        simultaneously read the comment areas of multiple DAFs. For
C        each file, the comments will be displayed on the screen as
C        they are extracted.
C
C        Use the SPK kernel below as the first DAF file for the
C        program.
C
C           earthstns_itrf93_201023.bsp
C
C
C        Use the CK kernel below as the second DAF file for the
C        program.
C
C           vo2_swu_ck2.bc
C
...
C
C              INTEGER               FNAMLN
C              PARAMETER           ( FNAMLN = 255  )
C
C              INTEGER               LINLEN
C              PARAMETER           ( LINLEN = 1000 )
C
C              INTEGER               NUMFIL
C              PARAMETER           ( NUMFIL = 2    )
C
C        C
C        C     Local variables.
C        C
C              CHARACTER*(FNAMLN)    DAFNAM ( NUMFIL )
C              CHARACTER*(LINLEN)    BUFFER ( BUFSIZ )
C
C              INTEGER               HANDLE ( NUMFIL )
C              INTEGER               I
C              INTEGER               J
C              INTEGER               N
C
C              LOGICAL               DONE   ( NUMFIL )
C
C        C
C        C     Set the DAF file names.
C        C
C              DATA                  DAFNAM /
C             .                          'earthstns_itrf93_201023.bsp',
C             .                          'vo2_swu_ck2.bc'          /
C
C        C
C        C     Set the initial values for DONE and HANDLE.
C        C
C              DO I = 1, NUMFIL
C
C                 DONE(I)   = .FALSE.
...
C                       WRITE (*,*)
C                       WRITE (*,*) 'File: ',
C             .                     DAFNAM(I)(:RTRIM(DAFNAM(I)))
C                       WRITE (*,*) '---------------------------------'
C             .                 //  '-----------------------------'
C                       N = 0
C
C                       CALL DAFEC ( HANDLE(I), BUFSIZ, N,
C             .                      BUFFER,    DONE(I)   )
C
C                       DO J = 1, N
C
...
C
C        C
C        C     Safely close the DAF files.
C        C
C              DO I = 1, NUMFIL
C
C                 CALL DAFCLS ( HANDLE(I) )
C
C              END DO
C
C              END
...
C
C
C         File: earthstns_itrf93_201023.bsp
C         --------------------------------------------------------------
C
C            SPK for DSN Station Locations
C            ========================================================***
...
C            Introduction
C
C         File: vo2_swu_ck2.bc
C         --------------------------------------------------------------
C         \beginlabel
C         PDS_VERSION_ID               = PDS3
C         RECORD_TYPE                  = FIXED_LENGTH
C         RECORD_BYTES                 = 1024
C         ^SPICE_KERNEL                = "vo2_swu_ck2.bc"
C         MISSION_NAME                 = VIKING
C         SPACECRAFT_NAME              = "VIKING ORBITER 2"
C         DATA_SET_ID                  = "VO1/VO2-M-SPICE-6-V1.0"
C         KERNEL_TYPE_ID               = CK
C         PRODUCT_ID                   = "vo2_swu_ck2.bc"
C
C         File: earthstns_itrf93_201023.bsp
C         --------------------------------------------------------------
C            ========================================================***
...
C         File: vo2_swu_ck2.bc
C         --------------------------------------------------------------
C         PRODUCT_CREATION_TIME        = 2008-12-03T14:03:35
C         PRODUCER_ID                  = "NAIF/JPL"
C         MISSION_PHASE_NAME           = {
C                                        PRIMARY_MISSION,
C                                        EXTENDED_MISSION,
C                                        CONTINUATION_MISSION
C                                        }
C         PRODUCT_VERSION_TYPE         = ACTUAL
C         PLATFORM_OR_MOUNTING_NAME    = "SPACECRAFT BUS"
C         START_TIME                   = 1977-01-09T18:33:12.707
C
C         File: earthstns_itrf93_201023.bsp
C         --------------------------------------------------------------
C            frame ITRF93.
...
C         File: vo2_swu_ck2.bc
C         --------------------------------------------------------------
C         STOP_TIME                    = 1977-11-27T05:55:16.772
C         SPACECRAFT_CLOCK_START_COUNT = "1/0032380393.707"
C         SPACECRAFT_CLOCK_STOP_COUNT  = "1/0060155717.772"
C         TARGET_NAME                  = MARS
C         INSTRUMENT_NAME              = "SCAN PLATFORM"
C         NAIF_INSTRUMENT_ID           = -30000
C         SOURCE_PRODUCT_ID            = "N/A"
C         NOTE                         = "See comments in the file fo***
C         OBJECT                       = SPICE_KERNEL
C           INTERCHANGE_FORMAT         = BINARY
C
C         File: earthstns_itrf93_201023.bsp
C         --------------------------------------------------------------
C
...
C            --------------------
C
C            This kernel contains data from a single, current source:***
C
C            This kernel supersedes the kernels
C
C               earthstns_itrf93_050714.bsp
C               dss_53_prelim_itrf93_201018.bsp (data in this kernel ***
C
C         File: vo2_swu_ck2.bc
C         --------------------------------------------------------------
C           KERNEL_TYPE                = POINTING
C           DESCRIPTION                = "VO2 instrument platform ori***
C         reconstructed during cartographic image registration by S. ***
C         Type 2 CK file supersedes the less usable Type 1 CK vo2_swu***
C         END_OBJECT                   = SPICE_KERNEL
C         \endlabel
C
C        [...]
C
C
C        Warning: incomplete output. Only 100 out of 1049 lines have
C        been provided. 18 lines extended past the right margin of the
C        header and have been truncated. These lines are marked by "***"
C        at the end of each line.
C
C
C$ Restrictions
C
C     1)  The comment area may consist only of printing ASCII
...
C
C     4)  This routine is intended to be used on DAF files whose comment
C         area does not change while this routine is called to extract
C         comments, between the start and end of the extraction process.
C         If the comment area does change (gets updated, reduced,
C         extended, or deleted) between calls to this routine on the
C         same DAF file, the routine's outputs are undefined and
C         subsequent calls to it are likely to trigger an exception.
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 25-NOV-2021 (JDR) (BVS)
C
...
C
C        Added complete code examples from existing code fragments.
C
