...
C
C     A,
C     B        are SPICE windows, each of which contains zero or more
C              intervals.
C
C     OP       is a comparison operator, indicating the way in which the
C              input sets are to be compared. OP may be any of the
C              following:
C
C                 Operator             Meaning
C                 --------  -------------------------------------------
...
C
C                   '<>'    A <> B is .TRUE. if A and B are not equal.
C
...
C$ Exceptions
C
C     1)  If the relational operator is not recognized, the error
C         SPICE(INVALIDOPERATION) is signaled.
C
C     2)  The cardinality of the input windows must be even. Left
...
C$ Particulars
C
C     This function returns .TRUE. whenever the specified relationship
...
C     Thus, B and C are proper subsets of A as well, and
C
C           WNRELD ( B, '<', A )
C           WNRELD ( A, '>', B )
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 24-AUG-2021 (JDR) (NJB)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Added entry #2
C        in $Exceptions section.
C
...
C$ Revisions
C
C-    Beta Version 2.0.0, 02-FEB-1989 (HAN)
C
C        If the relational operator is not recognized, an error is
C        signaled. The previous version returned .FALSE. as the
C        function value, and no error was signaled.
C
