...
C                            'LT' option uses two iterations.
C
...
C              "transmission" case in which photons depart from the
C              observer's location at ET and arrive at the aberration
C              correction locus at the light-time corrected epoch
C              ET+LT:
C
C                 'XLT'      Correct for one-way light time (also
C                            called "planetary aberration") using a
C                            Newtonian formulation. This correction
C                            yields the locus at the moment it
C                            receives photons departing from the
C                            observer at ET.
C
C                            The light time correction uses an
C                            iterative solution of the light time
...
C                            body, are corrected for light time.
C
C                 'XLT+S'    Correct for one-way transmission light
C                            time and stellar aberration using a
C                            Newtonian formulation. This option
C                            modifies the locus obtained with the 'XLT'
C                            option to account for the observer's
C                            velocity relative to the solar system
C                            barycenter. These corrections yield points
C                            on the apparent limb.
C
C                 'XCN'      Converged transmission Newtonian light
C                            time correction. In solving the light time
C                            equation, the 'XCN' correction iterates
C                            until the solution converges. Both the
C                            position and rotation of the target body
C                            are corrected for light time.
C
C                 'XCN+S'    Converged transmission Newtonian light
C                            time and stellar aberration corrections.
C                            This option produces a solution that is at
C                            least as accurate at that obtainable with
C                            the `XLT+S' option. Whether the 'XCN+S'
C                            solution is substantially more accurate
C                            depends on the geometry of the
C                            participating objects and on the accuracy
C                            of the input data. In all cases this
C                            routine will execute more slowly when a
...
C         SPICE(INVALIDMETHOD) is signaled by either this routine or a
C         routine in the call tree of this routine.
C
C     7)  If METHOD contains an invalid limb type, the error
C         SPICE(INVALIDLIMBTYPE) is signaled.
C
...
C         calling LIMBPT, an error is signaled by a routine in
...
C         system barycenter.
C
C     10) If the computation method requires an ellipsoidal target shape
C         and triaxial radii of the target body have not been loaded
C         into the kernel pool prior to calling LIMBPT, an error is
...
C
C     11) If the radii are available in the kernel pool but the count
C         of radii values is not three, the error SPICE(BADRADIUSCOUNT)
C         is signaled.
C
C     12) If the target body's shape is modeled as an ellipsoid, and if
C         any of the radii of the target body are non-positive, an error
C         is signaled by a routine in the call tree of this routine. The
C         target must be an extended body.
C
C     13) If PCK data specifying the target body-fixed frame orientation
C         have not been loaded prior to calling LIMBPT, an error is
C         signaled by a routine in the call tree of this routine.
C
...
C         target, an error is signaled by a routine in the call tree
C         of this routine.
C
C     15) If the array bound MAXN is less than 1, the error
C         SPICE(INVALIDSIZE) is signaled.
C
C     16) If the number of cutting half-planes specified by NCUTS
C         is negative or greater than MAXN, the error
C         SPICE(INVALIDCOUNT) is signaled.
C
C     17) If the aberration correction locus is not recognized, the
C         error SPICE(INVALIDLOCUS) is signaled.
C
C     18) If the aberration correction locus is 'ELLIPSOID LIMB'
C         but limb type is not 'TANGENT', the error
C         SPICE(BADLIMBLOCUSMIX) is signaled.
C
C     19) If the reference vector REFVEC is the zero vector, the
C         error SPICE(ZEROVECTOR) is signaled.
C
C     20) If the reference vector REFVEC and the observer target
C         vector are linearly dependent, the error
C         SPICE(DEGENERATECASE) is signaled.
C
C     21) If the limb computation uses the target ellipsoid limb
C         plane, and the limb plane normal and reference vector
C         REFVEC are linearly dependent, the error
C         SPICE(DEGENERATECASE) is signaled.
C
C     22) If the limb points cannot all be stored in the output POINTS
C         array, the error SPICE(OUTOFROOM) is signaled.
C
C     23) If the surface is represented by DSK data, and if the search
C         step is non-positive, the error SPICE(INVALIDSEARCHSTEP) is
C         signaled.
C
C     24) If the surface is represented by DSK data, and if the search
C         tolerance is non-positive, the error SPICE(INVALIDTOLERANCE)
C         is signaled.
C
C     25) If the roll step is non-positive and NCUTS is greater
C         than 1, the error SPICE(INVALIDROLLSTEP) is signaled.
C
C$ Files
C
...
C
C              CHARACTER*(*)         FM1
C              PARAMETER           ( FM1     =  '(A,F20.9)' )
C
C              CHARACTER*(*)         FM2
C              PARAMETER           ( FM2     =  '(1X,3F20.9)' )
C
C              INTEGER               BDNMLN
...
C             Number of limb points at this roll angle:            1
C             Limb point planetodetic coordinates:
C              Longitude      (deg):         85.434644188
C              Latitude       (deg):        -26.705411228
C              Altitude        (km):         -0.044832392
C              Emission angle (deg):         89.583080105
C
C           Roll angle (deg) =        240.000000000
C             Target epoch   =  271683700.368949771
C             Number of limb points at this roll angle:            1
...
C              Distance error  (km):         24.686473322
C
C           Roll angle (deg) =        240.000000000
C             Target epoch   =  271683700.368922532
C             Number of limb points at this roll angle:            1
...
C              NAIF0011.TLS                      Leapseconds
C              ROS_CG_M004_NSPCESA_N_V1.BDS      DSK plate model based
C                                                on Rosetta NAVCAM data
...
C                \begindata
C
C             KERNELS_TO_LOAD = ( 'DE405.BSP'
C                                 'NAIF0011.TLS',
C                                 'RORB_DV_145_01_______00216.BSP',
C                                 'CORB_DV_145_01_______00216.BSP',
C                                 'ROS_CG_RAD_V10.TPC',
C                                 'ROS_V25.TF',
C                                 'CATT_DV_145_01_______00216.BC',
C                                 'ROS_CG_M004_NSPCESA_N_V1.BDS'   )
C                \begintext
C
C                End of meta-kernel
C
...
C        C     Compute 6 limb points, starting with the
...
C
C          Roll angle (deg) =          1.080000000
C             Target epoch  =  484488067.184933782
C             Number of limb points at this roll angle:            3
C               Limb points
C                  1.346729151        -0.365488231         1.423051540
C                  0.960760394         0.183652804         0.924323093
C                  0.464582286         0.084076587         0.447930141
C
C          Roll angle (deg) =          1.440000000
C             Target epoch  =  484488067.184933782
C             Number of limb points at this roll angle:            3
C               Limb points
C                  1.351235771        -0.380664224         1.413164272
C                  0.960268777         0.176953543         0.914876859
C                  0.466284590         0.079312729         0.445564308
C
C          Roll angle (deg) =          1.800000000
C             Target epoch  =  484488067.184933782
C             Number of limb points at this roll angle:            3
C               Limb points
C                  1.358042184        -0.390349186         1.404421386
C                  0.959495690         0.170340551         0.905212642
C                  0.370611049        -0.167047205         0.395076979
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 2.0.0, 01-NOV-2021 (NJB) (JDR)
C
C        Added support for transmission aberration corrections.
C
C        Bug fix: deleted a computation of TMPVEC that had no effect.
C
C        Bug fix: PRVCOR is no longer set to blank before
C        ABCORR is parsed.
C
C        Bug fix: corrected long error message for an unsupported
C        limb type used with the ELLIPSOID LIMB locus.
C
C        Corrected description of iteration count for non-converged
C        corrections.
C
C        Edited the header to comply with NAIF standard. Reduced
C        the number of cuts to present in the output in Example #3.
C        Modified output format in all examples to comply with the
C        maximum line length of header comments.
C
...
C
C     Set the sign S prefixing LT in expression for light time-
C     corrected epochs associated with limb points.
C
      IF ( USELT ) THEN
...
C
     ...
     .      //            '# aberration correction locus. Only the '
     .      //            'TANGENT limb type is supported for this '
     .      //            'locus.'                                  )

            IF ( LMBTYP .EQ. GUIDED ) THEN
               CALL ERRCH  ( '#',  'GUIDED' )
            ELSE
               CALL ERRINT ( '#',  LMBTYP )
            END IF
...
C
                        IF ( XMIT ) THEN
                           CALL STLABX ( PTARG,  STOBS(4), STLPOS )
                        ELSE
...
C
                     IF ( XMIT ) THEN
                        CALL STLABX ( ISRFVC, STOBS(4), STLPOS )
                     ELSE
