...
C     compute the state. These additional kernels may be C-kernels, PCK
...
C        C     file and a leapseconds file. Use a meta-kernel.
C        C
C              CALL FURNSH( 'spkezr_ex1.tm' )
C
C        C
...
C              WRITE(*,'(3F20.6)') (STATE(I), I=1,3 )
C
C              WRITE(*,*) 'V (kilometers/sec) : '
C              WRITE(*,'(3F20.6)') (STATE(I), I=4,6 )
C
...
C
C         R (kilometers)     :
C            229953013.746498   -167125346.211588    -78800343.963573
C         V (kilometers/sec) :
C                   35.380861           28.653402           12.861524
...
C$ Version
C
C-    SPICELIB Version 4.2.0, 01-OCT-2021 (JDR) (NJB)
C
C        Deleted include statement for frmtyp.inc.
C
C        Edited the header to comply with NAIF standard. Changed output
C        format in code example to comply with maximum line length for
C        header comments. Added ABCORR to $Required_Reading.
C
