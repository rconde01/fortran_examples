...
C$ Detailed_Input
C
C     INST     is the NAIF integer ID for the instrument, spacecraft, or
C              other structure for which pointing is requested. For
C              brevity we will refer to this object as the "instrument,"
C              and the frame fixed to this object as the "instrument
C              frame" or "instrument-fixed" frame.
C
C     SCLKDP   is the encoded spacecraft clock time for which pointing
C              is requested.
C
...
C
C              The SPICELIB routine SCTIKS converts a spacecraft clock
C              tolerance duration from its character string
C              representation to ticks. SCFMT performs the inverse
C              conversion.
C
C              The C-matrix returned by CKGP is the one whose time tag
C              is closest to SCLKDP and within TOL units of SCLKDP.
C              (More in $Particulars, below.)
C
C              In general, because using a non-zero tolerance affects
C              selection of the segment from which the data is obtained,
C              users are strongly discouraged from using a non-zero
C              tolerance when reading CKs with continuous data. Using a
C              non-zero tolerance should be reserved exclusively to
C              reading CKs with discrete data because in practice
C              obtaining data from such CKs using a zero tolerance is
C              often not possible due to time round off.
C
C     REF      is the desired reference frame for the returned pointing.
C              The returned C-matrix CMAT gives the orientation of the
C              instrument designated by INST relative to the frame
C              designated by REF. When a vector specified relative to
C              frame REF is left- multiplied by CMAT, the vector is
C              rotated to the frame associated with INST. See the
C              discussion of CMAT below for details.
C
...
C$ Detailed_Output
C
C     CMAT     is a rotation matrix that transforms the components of a
C              vector expressed in the reference frame specified by REF
C              to components expressed in the frame tied to the
C              instrument, spacecraft, or other structure at time CLKOUT
C              (see below).
C
...
C                            (Transpose of CMAT)
C
C     CLKOUT   is the encoded spacecraft clock time associated with the
C              returned C-matrix. This value may differ from the
C              requested time, but never by more than the input
C              tolerance TOL.
C
C              The $Particulars section below describes the search
C              algorithm used by CKGP to satisfy a pointing request.
C              This algorithm determines the pointing instance (and
C              therefore the associated time value) that is returned.
C
...
C
C     1) The following example program uses CKGP to get C-matrices
C        for a set of images whose SCLK counts (un-encoded character
C        string versions) are contained in the array SCLKCH.
C
C        If available, the program will get the corrected pointing
C        values.
C
C        For each C-matrix, a unit pointing vector is constructed and
C        printed.
C
...
C        C
C        C     -- The code for CASSINI spacecraft reference frame
C        C        is -82000.
C        C
C        C    --  Spacecraft clock times for successive CASSINI
C        C        navigation images always differ by more than 1.0.
C        C        This is an acceptable tolerance, and must be
C        C        converted to "ticks" (units of encoded SCLK) for
C        C        input to CKGP.
C        C
C        C     -- The reference frame we want is J2000.
C        C
C        C     -- The CASSINI ISS camera boresight
C        C        in the spacecraft frame is
C        C        (0.0005760, -0.99999982, -0.0001710).
C        C
C              INTEGER               FILEN
...
C              LOGICAL               FOUND
C
C              DATA                  SCLKCH /  '1465644281.0',
C             .                                '1465644351.0' /
C
C              DATA                  ISSFIX /  0.00057600D0,
C             .                               -0.99999982D0,
C             .                               -0.00017100D0  /
C
C              CK         = '04153_04182ca_ISS.bc'
C              SCLK       = 'cas00071.tsc'
C              SC         = -82
C              INST       = -82000
C              TOL        = '1.0'
...
C
C        C
C        C     Load the CK file.
C        C
C              CALL FURNSH ( CK )
C
C        C
C        C     Need to load a CASSINI SCLK kernel to convert from
C        C     clock string to ticks.  Although not required for
C        C     the CASSINI spacecraft clock, most modern spacecraft
C        C     clocks require a leapseconds kernel to be loaded in
C        C     addition to an SCLK kernel.
C        C
C              CALL FURNSH ( SCLK )
C
C        C
...
C        C           coordinates.
C        C
C                    CALL MTXV   ( CMAT, ISSFIX, VINERT )
C                    CALL SCDECD ( SC,   CLKOUT, CLKCH  )
C
C                    WRITE(*,*) 'Requested SCLK time : ', SCLKCH(I)
C                    WRITE(*,*) '   CASSINI SCLK time: ', CLKCH
C                    WRITE(*,'(A,3F11.7)')
C             .             '    CASSINI ISS boresight:', VINERT
C                    WRITE(*,*) ' '
C
C                 ELSE
C
C                    WRITE (*,*) 'Pointing not found for time ',
C             .                                            SCLKCH(I)
C
C                 END IF
C
...
C
C
C         Requested SCLK time : 1465644281.0
C            CASSINI SCLK time: 1/1465644281.171
C            CASSINI ISS boresight:  0.9376789  0.3444125  0.0462419
C
C         Requested SCLK time : 1465644351.0
C            CASSINI SCLK time: 1/1465644351.071
C            CASSINI ISS boresight:  0.9376657  0.3444504  0.0462266
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 5.4.1, 26-MAY-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C        Updated the code, input times and kernel set to work with
C        PDS archived CASSINI data.
C
