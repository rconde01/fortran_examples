...
C              this vector is the "distance" that serves as the subject
C              of the search performed by this routine.
C
...
C              observer-target position vector to account for one-way
C              light time and stellar aberration.
C
C              Any aberration correction accepted by the SPICE routine
C              SPKEZR is accepted here. See the header of SPKEZR for a
...
C              strings to supply to indicate the observer is Earth.
C
C              Case and leading or trailing blanks are not significant
C              in the string OBSRVR.
C
C     RELATE   is a relational operator used to define a constraint on
C              the observer-target distance. The result window found by
C              this routine indicates the time intervals where the
C              constraint is satisfied.
C
C              Supported values of RELATE and corresponding meanings are
C              shown below:
C
...
C              the set of time intervals where the distance is within a
C              specified offset relative to an absolute extremum. The
...
C              ADJUST is set to a positive value, GFDIST will find times
C              when the observer-target distance is within ADJUST km of
C              the specified extreme value.
C
...
C              corresponding range is  between AMAX - ADJUST and AMAX.
C
...
C              shorter than any maximal time interval on which the
...
C              intervals.
C
C              However, STEP must not be *too* short, or the search will
C              take an unreasonable amount of time.
C
...
C
C     MW       is a parameter specifying the length of the SPICE windows
C              in the workspace array WORK (see description below) used
C              by this routine.
C
C              MW should be set to a number at least twice as large as
C              the maximum number of intervals required by any workspace
C              window. In many cases, it's not necessary to compute an
C              accurate estimate of how many intervals are needed;
C              rather, the user can pick a size considerably larger than
C              what's really required.
C
C              However, since excessively large arrays can prevent
C              applications from compiling, linking, or running
C              properly, sometimes MW must be set according to the
C              actual workspace requirement. A rule of thumb for the
C              number of intervals NINTVLS needed is
C
...
C                 2 * NINTVLS
C
C     NW       is a parameter specifying the number of SPICE windows in
C              the workspace array WORK (see description below) used by
C              this routine. NW should be set to the parameter NWDIST;
C              this parameter is declared in the include file gf.inc.
C              (The reason this dimension is an input argument is that
C              this allows run-time error checking to be performed.)
C
...
C              discarded before GFDIST conducts its search.
C
C$ Detailed_Output
C
...
C
C              where MW is a constant declared by the caller and NWDIST
C              is a constant defined in the SPICELIB INCLUDE file
C              gf.inc. See the discussion of MW above.
C
...
C              distance constraint is satisfied.
C
...
C         SPICE(INVALIDDIMENSION) is signaled.
C
C     11) If the result window has size less than 2, the error
C         SPICE(INVALIDDIMENSION) is signaled.
C
...
C             .                    TIMSTR                            )
C
C                    WRITE (*, '(A,F14.7)' ) 'Start time, distance = '//
C             .                              TIMSTR, DIST
C
...
C             .                    TIMSTR                            )
C
C                    WRITE (*, '(A,F14.7)' ) 'Stop time,  distance = '//
C             .                              TIMSTR, DIST
C                 END DO
C
C              END IF
...
C
C
C        Start time, distance = 2007-JAN-08 00:10:02.439  400000.0000000
C        Stop time,  distance = 2007-JAN-13 06:36:42.770  400000.0000000
C        Start time, distance = 2007-FEB-04 07:01:30.094  400000.0000000
C        Stop time,  distance = 2007-FEB-10 09:29:56.659  400000.0000000
C        Start time, distance = 2007-MAR-03 00:19:19.998  400000.0000000
C        Stop time,  distance = 2007-MAR-10 14:03:33.312  400000.0000000
C        Start time, distance = 2007-MAR-29 22:52:52.961  400000.0000000
C        Stop time,  distance = 2007-APR-01 00:00:00.000  404531.9552322
C
C
C        Note that the distance at the final solutions interval's stop
C        time is not close to the reference value of 400000 km. This is
C        because the interval's stop time was determined by the stop
C        time of the confinement window.
C
C
...
C           Earth-Moon distance is at the absolute minimum
C           Earth-Moon distance is > the absolute minimum + 100 km
C           Earth-Moon distance is at a local maximum
C           Earth-Moon distance is at the absolute maximum
...
C        C              of the result window.
C        C
C                       CALL WNFETD ( RESULT, J, START, FINSH )
C
C        C
C        C              Check the distance at the start and stop times.
C        C
...
C             .                       TIMSTR                           )
C
C                       WRITE (*, FMT1 ) '  Start time, distance = '
C             .         //               TIMSTR, DIST
C
C                       CALL SPKPOS ( 'MOON',  FINSH, 'J2000', 'NONE',
C             .                       'EARTH', POS,     LT            )
C                       DIST = VNORM(POS)
C
C                       CALL TIMOUT ( FINSH, 'YYYY-MON-DD HR:MN:SC.###',
C             .                       TIMSTR                           )
C
C                       WRITE (*, FMT1 ) '  Stop time,  distance = '
C             .         //               TIMSTR, DIST
C                    END DO
C
C                 END IF
...
C         Result window:
C          Start time, distance = 2007-FEB-04 07:01:30.094  400000.00000
C          Stop time,  distance = 2007-FEB-04 07:01:30.094  400000.00000
C          Start time, distance = 2007-FEB-10 09:29:56.659  400000.00000
C          Stop time,  distance = 2007-FEB-10 09:29:56.659  400000.00000
C          Start time, distance = 2007-MAR-03 00:19:19.998  400000.00000
C          Stop time,  distance = 2007-MAR-03 00:19:19.998  400000.00000
C          Start time, distance = 2007-MAR-10 14:03:33.312  400000.00000
C          Stop time,  distance = 2007-MAR-10 14:03:33.312  400000.00000
C
C        Condition: distance < 4.00000E+05 km
C         Result window:
C          Start time, distance = 2007-JAN-15 00:00:00.000  393018.60991
C          Stop time,  distance = 2007-FEB-04 07:01:30.094  400000.00000
C          Start time, distance = 2007-FEB-10 09:29:56.659  400000.00000
C          Stop time,  distance = 2007-MAR-03 00:19:19.998  400000.00000
C          Start time, distance = 2007-MAR-10 14:03:33.312  400000.00000
C          Stop time,  distance = 2007-MAR-15 00:00:00.000  376255.45393
C
C        Condition: distance > 4.00000E+05 km
C         Result window:
C          Start time, distance = 2007-FEB-04 07:01:30.094  400000.00000
...
C        Condition: distance is a local minimum
C         Result window:
C          Start time, distance = 2007-JAN-22 12:30:49.458  366925.80411
C          Stop time,  distance = 2007-JAN-22 12:30:49.458  366925.80411
C          Start time, distance = 2007-FEB-19 09:36:29.968  361435.64681
C          Stop time,  distance = 2007-FEB-19 09:36:29.968  361435.64681
C
C        Condition: distance is the absolute minimum
C         Result window:
C          Start time, distance = 2007-FEB-19 09:36:29.968  361435.64681
...
C         Result window:
C          Start time, distance = 2007-FEB-19 01:09:52.706  361535.64681
C          Stop time,  distance = 2007-FEB-19 18:07:45.136  361535.64681
C
C        Condition: distance is a local maximum
C         Result window:
C          Start time, distance = 2007-FEB-07 12:38:29.870  404992.42429
C          Stop time,  distance = 2007-FEB-07 12:38:29.870  404992.42429
C          Start time, distance = 2007-MAR-07 03:37:02.122  405853.45213
C          Stop time,  distance = 2007-MAR-07 03:37:02.122  405853.45213
C
C        Condition: distance is the absolute maximum
C         Result window:
C          Start time, distance = 2007-MAR-07 03:37:02.122  405853.45213
...
C         Result window:
C          Start time, distance = 2007-MAR-06 15:56:00.957  405753.45213
C          Stop time,  distance = 2007-MAR-07 15:00:38.674  405753.45213
C
C
C$ Restrictions
C
...
C
C        Edited second example code and modified the output resolution
C        for the distances in that example to fit in the $Examples
C        section without modifications. Renamed example's meta-kernel.
