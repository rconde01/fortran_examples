...
C            [1, NV]
C
C         the error SPICE(INDEXOUTOFRANGE) is signaled.
C
C$ Files
C
C     None.
...
C
C        C
C        C     Compute the volume of a plate model representing the
C        C     pyramid with one vertex at the origin and the other
C        C     vertices coinciding with the standard basis vectors.
C        C
C              PROGRAM PLTVOL_EX1
C              IMPLICIT NONE
C
C        C
...
C
C        C
C        C     Let the notation
C        C
C        C        < A, B >
C        C
C        C     denote the dot product of vectors A and B.
C        C
...
C
C     [1]  T. Apostol, "Calculus, Vol. II," John Wiley & Sons, 1969.
C
...
C
C        Edited the header to comply with NAIF standard. Added DSK to
C        $Required_Reading. Updated code example comments.
C
