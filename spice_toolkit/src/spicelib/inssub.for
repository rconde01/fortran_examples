...
C
C     LOC      is the index in the input string at which the substring
C              is to be inserted. The range of LOC is 1:LEN(IN). To
C              append to the string, set LOC equal to LEN(IN)+1.
C
C$ Detailed_Output
C
...
C
C     1) The following table illustrates the use of INSSUB.
C
C           IN                SUB      LOC    OUT
C           ----------------- -------  ---    ------------------------
...
C           'Apple a day'     'An '      0     An error is signaled.
C           'Apple a day'     'An '     -3     An error is signaled.
C           'An Apple a day'  ' keeps'  16     An error is signaled.
C
C
C     2) INSSUB could be used to insert substrings at any position
C        within the declared length of the input string. Use it to
C        add an ascii arrow (--->) at the end of the string.
C
C
C        Example code begins here.
C
C
C              PROGRAM INSSUB_EX2
C              IMPLICIT NONE
C
C        C
...
C        C
C              INTEGER               STRSZ
C              PARAMETER           ( STRSZ = 20 )
C
C        C
C        C     Local variables.
C        C
C              CHARACTER*(STRSZ)     STRIN
C              CHARACTER*(STRSZ)     STROUT
C
C        C
C        C     Assign the input string.
C        C
C              DATA                  STRIN / '0123456789         .' /
C
C
C        C
C        C     Insert a substring at index 17. This should leave 5
C        C     blanks after the sequence of numbers. Note that the
C        C     resulting string is truncated.
C        C
C              CALL INSSUB ( STRIN, '--->', 17, STROUT )
C
C              WRITE(*,*) 'Input string:  ', STRIN
C              WRITE(*,*) 'Output string: ', STROUT
C
C              END
C
C
...
C
C
C         Input string:  0123456789         .
C         Output string: 0123456789      --->
C
C
C$ Restrictions
C
...
C
C        Updated long error message for LOC out of range check,
C        providing valid range of values.
C
...
C
C-    Beta Version 2.0.0, 04-JAN-1989 (HAN)
C
C        If the location at which the substring is to be inserted is
C        not in the interval [1, LEN(IN)+1], an error is signaled.
...
C
      IF (  ( LOC .LT. 1 )  .OR.  ( LOC .GT. INLEN+1 )  ) THEN
         ...
         CALL CHKIN  ( 'INSSUB'                                    )
         CALL SETMSG ( 'Index at which the substring is to be '
     .   //            'inserted is out of the valid range [1,#]. '
     .   //            'Requested index was *.'                    )
         CALL ERRINT ( '#', INLEN + 1                              )
         CALL ERRINT ( '*', LOC                                    )
