...
C              identifier for that segment. Together they determine a
C              complete set of ephemeris data, from which a subset is to
C              be extracted.
C
C     BEGIN,
C     END      are the initial and final epochs (ephemeris time) of the
C              subset.
C
C     NEWH     is the file handle assigned to the file in which the new
C              segment is to be written. The file must be open for write
C              access. NEWH and HANDLE may refer to the same file.
C
C$ Detailed_Output
C
C     See $Files section.
...
C
C         is not satisfied (where ALPHA and OMEGA are the initial and
C         final epochs of the segment respectively), the error
C         SPICE(SPKNOTASUBSET) is signaled.
C
C     2)  If the segment type is not supported by the current version of
C         SPKSUB, the error SPICE(SPKTYPENOTSUPP) is signaled.
C
C$ Files
C
...
C
C     1)  ALPHA and OMEGA (DC(1) and DC(2)) are assigned the values
C         specified by BEGIN and END.
C
C     2)  The beginning and ending segment addresses (IC(5) and IC(6))
...
C$ Version
C
C-    SPICELIB Version 9.1.0, 14-APR-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Removed
C        unnecessary $Revisions section. Moved SPK required reading from
