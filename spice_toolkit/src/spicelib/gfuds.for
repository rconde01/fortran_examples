...
C              where:
C
C                 ET      is a double precision value representing
C                         ephemeris time, expressed as seconds past
C                         J2000 TDB, at which to determine the scalar
C                         value.
C
C                 VALUE   is the value of the scalar quantity at ET.
C
C     UDQDEC   is the name of the routine that determines if the scalar
C              quantity calculated by UDFUNS is decreasing. The calling
C              sequence of UDQDEC is:
C
C                 CALL UDQDEC ( UDFUNS, ET, ISDECR )
C
C              where:
C
C                 UDFUNS   is the name of the scalar function as defined
C                          above.
C
C                 ET       is a double precision value representing
C                          ephemeris time, expressed as seconds past
...
C
C                 ISDECR   is a logical output variable indicating
C                          whether or not the scalar value returned by
C                          UDFUNS is decreasing. ISDECR returns .TRUE.
C                          if the time derivative of UDFUNS at ET is
C                          negative.
C
C     RELATE   is the scalar string comparison operator indicating
...
C              geometric quantity value has values between ABSMIN and
C              ABSMIN + ADJUST.
C
...
C
C     STEP     is the double precision time step size to use in
C              the search.
C
...
C              which UDFUNS and UDQDEC require data. See $Particulars
C              for details.
C
...
C              discarded before GFUDS conducts its search.
C
C$ Detailed_Output
C
...
C
C     4)  If the number of workspace windows NW is too small for the
...
C         tree of this routine.
C
C     7)  If the window count NW is less than NWUDS, the error
...
C
C     11) If a non-zero value is provided for ADJUST when RELATE has any
C         value other than 'ABSMIN' or 'ABSMAX', an error is signaled by
...
C        UDFUNS and UDQDEC require data; such data must be provided by
...
C     not modified.
C
C     If a search uses an equality constraint, the time window over
C     which the functions UDFUNS and UDQDEC are called is expanded by 1
C     second. 
C
...
C     time ET is used by UDFUNS or UDQDEC to compute such a state, the
C     window over which the observer state is computed is expanded by
C     1 second.
C
C     The window expansions described above are additive: if both
...
C
C     In addition to the possible expansion of the search window that
C     occurs when both an equality constraint and stellar aberration
C     corrections are used, round-off error should be taken into
C     account when the need for data availability is analyzed.
C
C$ Examples
C
...
C
C     1) Conduct a search on the range-rate of the vector from the Sun
...
C        C     The reference value is .3365 km/s - a range rate value
C        C     known to exist during the confinement window. We're not
...
C        C
C                 CALL GFUDS ( GFQ,    GFDECR, RELATE(J),
...
C        C- Abstract
C        C
C        C     User defined function to detect if the function
C        C     derivative is negative (the function is decreasing)
C        C     at TDB time ET.
C        C
C
C              EXTERNAL              UDFUNS
...
C        Start time, drdt = 2007-FEB-10 06:26:15.438      -0.575837623
C        Stop time,  drdt = 2007-FEB-10 06:26:15.438      -0.575837623
...
C        Start time, drdt = 2007-JAN-26 02:27:33.767       1.154648992
C        Stop time,  drdt = 2007-JAN-26 02:27:33.767       1.154648992
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 21-OCT-2021 (JDR) (NJB)
C
...
C
C-    SPICELIB Version 1.0.0, 16-FEB-2010 (EDW) (NJB)
C
C-&
