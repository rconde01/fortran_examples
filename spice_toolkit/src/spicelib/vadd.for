...
C$ Abstract
C
C     Add two double precision 3-dimensional vectors.
C
C$ Disclaimer
C
...
C
C     V1,
C     V2       are two arbitrary double precision 3-dimensional
C              vectors.
C
C$ Detailed_Output
C
C     VOUT     is the double precision 3-dimensional vector sum of V1
C              and V2.
C
C$ Parameters
C
...
C
C     1) Define two sets of 3-dimensional vectors and compute the sum
...
C              DOUBLE PRECISION      SETB ( 3, SETSIZ )
...
C        C
C              DATA                  SETA / 1.D0,  2.D0,  3.D0,
C             .                             1.D-7, 1.D23, 1.D-9  /
C
C              DATA                  SETB / 4.D0,  5.D0,   6.D0,
C             .                             1.D24, 1.D23,  0.D0  /
C
C        C
...
C              DO I=1, SETSIZ
C
C                 CALL VADD ( SETA(1,I), SETB(1,I), VOUT )
C
C                 WRITE(*,'(A,3E11.2)') 'Vector A  : ',
C             .                        ( SETA(J,I), J=1,3 )
C                 WRITE(*,'(A,3E11.2)') 'Vector B  : ',
C             .                        ( SETB(J,I), J=1,3 )
C                 WRITE(*,'(A,3E11.2)') 'Sum vector: ', VOUT
C                 WRITE(*,*) ' '
C
C              END DO
C
...
C
C
C        Vector A  :    0.10E+01   0.20E+01   0.30E+01
C        Vector B  :    0.40E+01   0.50E+01   0.60E+01
C        Sum vector:    0.50E+01   0.70E+01   0.90E+01
C
C        Vector A  :    0.10E-06   0.10E+24   0.10E-08
C        Vector B  :    0.10E+25   0.10E+24   0.00E+00
C        Sum vector:    0.10E+25   0.20E+24   0.10E-08
C
C
C$ Restrictions
C
