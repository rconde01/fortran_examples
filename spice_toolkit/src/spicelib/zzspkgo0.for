...
C$ Detailed_Output
C
C     STATE    is a 6-dimensional vector that contains the geometric
C              position and velocity of the target body, relative to the
C              observing body, at epoch ET. STATE has six elements: the
C              first three contain the target's position; the last three
C              contain the target's velocity. These vectors are
C              transformed into the specified reference frame. Units are
C              always km and km/sec.
C
...
C     that have been loaded by previous calls to FURNSH or SPKLEF
C     (load ephemeris file).
C
...
C
C     1) Return the geometric state vector of Mars (499) as seen from
C        Earth (399) in the J2000 frame and the one-way light time
C        between them at the epoch July 4, 2003 11:00 AM PST.
C
...
C        C     Local variables.
C        C
C              CHARACTER*(TIMLEN)    EPOCH
C              CHARACTER*(NAMLEN)    REFFRM
C
...
C        C     file and a leapseconds file. Use a meta
C        C     kernel for convenience.
C        C
C              CALL FURNSH ( 'spkgeo_ex1.tm' )
C
C        C
C        C     Define parameters for a state lookup.
C        C
C              TARGET = 499
C              EPOCH  = 'July 4, 2003 11:00 AM PST'
C              REFFRM = 'J2000'
C              OBSRVR = 399
C
C        C
...
C        C
C              CALL SPKGEO ( TARGET, ET, REFFRM, OBSRVR, STATE, LT )
C
C        C
C        C     Output...
C        C
C              WRITE(*,'(A,I3)') 'The position of    : ', TARGET
C              WRITE(*,'(A,I3)') 'As observed from   : ', OBSRVR
C              WRITE(*,'(2A)')   'In reference frame : ', REFFRM
C              WRITE(*,'(2A)')   'At epoch           : ', EPOCH
C              WRITE(*,*) ' '
C
C        C
...
C        C
C              WRITE(*,'(A,3F18.6)') 'R   (km):', ( STATE(I), I=1,3 )
C              WRITE(*,'(A,3F18.6)') 'V (km/s):', ( STATE(I), I=4,6 )
C              WRITE(*,*) ' '
C              WRITE(*,'(A,F19.14)') 'Light time (s) between observer '
C             .                   // 'and target: ', LT
C
C              END
C
...
C
C
C        The position of    : 499
C        As observed from   : 399
C        In reference frame : J2000
C        At epoch           : July 4, 2003 11:00 AM PST
C
C        R   (km):   73826216.435288  -27128030.732406  -18741973.868287
C        V (km/s):         -6.809504          7.513814          3.001290
C
C        Light time (s) between observer and target:  269.70264776317532
C
C
C$ Restrictions
C
C     1)  The ephemeris files to be used by SPKGEO must be loaded
C         by FURNSH or SPKLEF before SPKGEO is called.
C
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 09-OCT-2021 (JDR) (NJB)
C
C        Bug fix: added calls to FAILED after calls to SPKPVN.
C        Previously only one call to SPKPVN was followed by a FAILED
C        call. Moved some FAILED checks so they will be hit whether
C        or not SPKSFS finds a segment.
C
C        Edited the header to comply with NAIF standard. Added complete
C        code example to $Examples section. Removed unnecessary
C        $Revisions section.
C
C        Added reference to FURNSH in $Particulars and $Restrictions
C        sections.
C
...
C        IRFDEF will be fully masked by a frame with identical name
