...
C$ Version
C
C-    SPICELIB Version 1.1.0, 13-OCT-2021 (NJB) (BVS)
C
C        Updated entry point ZZDSKSNS to always initialize FOUND.
C
C        Corrected typo in long error message in entry point ZZDSKBSS.
C        Corrected comments in ZZDSKBSS and in ZZDSKSNS
C
...
C     segments for each body and the expense at which that list was
C     constructed. (The expense of a body list is the number of segment
C     descriptors examined during the construction of the list.) It
C     also contains the highest and lowest file numbers searched during
C     the construction of the list.
C
...
C     Variable  I/O  Description
C     --------  ---  --------------------------------------------------
C     BODYID     I   ID code of body.
C
C$ Detailed_Input
C
C     BODYID     is the integer ID code of the body for which a search
...
C$ Particulars
C
C     ZZDSKBSS sets up a search for segments by ZZDSKSNS. It records
C     the body for which the search is to be conducted.
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 23-NOV-2020 (NJB) (BVS)
C
C        Corrected typo in long error message. Corrected descriptions
C        of inputs. Corrected description of saved information in the
C        Particulars header section.
C
C-    SPICELIB Version 1.0.0, 08-FEB-2017 (NJB)
C
C        Updated version info.
...
C
      IF ( NFT .EQ. 0 ) THEN
         ...
         CALL SETMSG ( 'At least one DSK file must have been loaded '
     .   //            'by ZZDSKLSF before a search can be started.' )
...
C           stored for the body, to see if there is one that satisfies
