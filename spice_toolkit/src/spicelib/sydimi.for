...
C     TABSYM,
C     TABPTR,
C     TABVAL   are the components of an integer symbol table. The table
C              may or may not contain the symbol NAME.
C
C$ Detailed_Output
C
C     The function returns the dimension of the symbol NAME. The
C     dimension of a symbol is the number of values associated with that
C     symbol. If NAME is not in the symbol table, the function returns
C     the value zero.
C
C$ Parameters
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 07-APR-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Updated
C        $Brief_I/O to indicate that TABSYM, TABPTR, TABVAL are input
C        arguments.
C
