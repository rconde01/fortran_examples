C$Procedure SGSEQW ( Generic segments: Sequential writer. )
...
C$ Detailed_Input
C
C     HANDLE   is the handle of a DAF file opened with write access.
C              This is the handle of the file in which a generic segment
C              will be started, or the handle of a file in which a
C              generic segment is currently being written.
C
C     DESCR    is the descriptor for the generic segment that is being
...
C
C     SEGID    is an identifier for the generic segment that is being
...
C
C     NCONST   is the number of constant values to be placed in the
C              generic segment.
C
C     CONST    is an array of NCONST constant values for the generic
C              segment.
C
C     NPKTS    is the number of data packets to write to a generic
C              segment.
C
C     PKTSIZ   is the size of fixed size packets or sizes of variable
C              size packets.
C
...
C              the data packets.
C
C     PKTDAT   is a singly dimensioned array containing the double
...
C                             K = 1
C
C     NREFS    is the number of reference values.
C
...
C              equal to NPKTS.
C
C     REFDAT   is the reference data values.
C
...
C              the step size must be an integer, i.e., DINT(REFDAT(2))
...
C
C     IDXTYP   is the index type to use for the reference values.
C
...
C         of its entry points, the error SPICE(BOGUSENTRY) is signaled.
C
C     2)  See the individual entry points for descriptions of their
C         exceptions.
C
C$ Files
C
...
C     of carbon dioxide, sulfur dioxide, ozone, and other constituents
...
C        C                  packet. In this case, we are going to use an
C        C                  explicit index, which requires a reference
...
C        value, BEGIN_REF, and a step size, STEP_SIZE, that is the
...
C$ Restrictions
C
C     1)  See the individual entry points for any restrictions they may
C         have.
C
...
C$ Version
C
C-    SPICELIB Version 1.2.1, 27-OCT-2021 (JDR)
C
...
C$Procedure SGBWFS ( Generic segments: Begin a fixed size segment. )
...
C              will be written.
C
C     DESCR    is the descriptor for a segment that is being written.
C              This is the packed form of the DAF double precision and
C              integer summaries which contain ND double precision
C              numbers and NI integers.
C
C     SEGID    is an identifier for a segment that is being written.
C              This is a character string containing at most NC printing
C              ASCII characters where
C
C                                /  ND + ( NI + 1 )  \
C                     NC =  8 *  | ----------------- |
C                                \         2         /
...
C               SEGID may be blank.
C
C     NCONST   is the number of constant values to be placed in a
C              segment.
C
C     CONST    is an array of NCONST constant values for a segment.
C
C     PKTSIZ   is the size of fixed size packets. The size of a packet
...
C         and segment, the error SPICE(CALLEDOUTOFORDER) is signaled.
C
C     2)  If the length of the segment identifier, SEGID, is greater
C         than NC, as determined from the ND and NI values for a
C         particular DAF file, the error SPICE(SEGIDTOOLONG) is
C         signaled.
C
C     3)  If the segment identifier contains nonprinting characters, the
C         error SPICE(NONPRINTINGCHARS) is signaled.
C
C     4)  If the number of constant values, NCONST, is negative, the
C         error SPICE(NUMCONSTANTSNEG) is signaled.
C
C     5)  If the packet size, PKTSIZ, is not positive, the error
C         SPICE(NONPOSPACKETSIZE) is signaled.
C
C     6)  If the index type for the reference values is not recognized,
C         the error SPICE(UNKNOWNINDEXTYPE) is signaled.
C
C     7)  If the file table is full, the error SPICE(FILETABLEFULL) is
C         signaled.
C
C$ Files
C
...
C     It contains examples which demonstrate the use of the entry points
...
C     writing. If not, an error is signaled.
C
...
C$Procedure SGBWVS ( Generic segments: Begin a variable size segment. )
...
C         the error SPICE(UNKNOWNINDEXTYPE) is signaled.
C
C     6)  If the file table is full, the error SPICE(FILETABLEFULL) is
C         signaled.
C
C$ Files
C
...
C$Procedure SGWFPK ( Generic segments: Write fixed size packets. )
...
C
C     HANDLE   is the handle of a DAF file opened with write access.
C              This is the handle of a file in which a generic segment
C              has been started and is currently being written.
C
C     NPKTS    is the number of data packets to write to a segment.
C
C     PKTDAT   is a singly dimensioned array containing the fixed size
...
C              reference value and a step size between consecutive
...
C         SPICE(INCOMPATIBLENUMREF) is signaled.
C
...
C         SPICE(UNORDEREDREFS) is signaled.
C
...
C         error SPICE(UNORDEREDREFS) is signaled.
C
...
C         the error SPICE(INCOMPATIBLENUMREF) is signaled.
C
C     9)  If an implicitly indexed generic segment is being written and
C         the second reference value, the step size used for indexing,
C         is not integral, i.e., DINT(REFDAT(2)) .NE. REFDAT(2), the
C         error SPICE(REFVALNOTINTEGER) is signaled.
C
C$ Files
C
C     See HANDLE in the $Detailed_Input section.
...
C$Procedure SGWVPK ( Generic segment: Write variable size packets. )
...
C     NPKTS    is the number of data packets to write to a segment.
C
C     PKTSIZ   is the sizes of variable size packets.
C
...
C
C     PKTDAT   is a singly dimensioned array containing the variable
...
C         currently being written, the error SPICE(CALLEDOUTOFORDER)
C         is signaled.
C
...
C
C        Edited the header to comply with NAIF standard. Corrected
C        typos in comments.
C
C-    SPICELIB Version 1.0.0, 05-APR-1995 (KRG) (WLT)
C
...
C$Procedure SGWES ( Generic segments: End a segment. )
...
C
C     HANDLE   is the handle of a DAF file opened with write access.
C              This is the handle of the file which contains the generic
...
C         error SPICE(CALLEDOUTOFORDER) is signaled.
C
C     2)  If there is no generic segment being written to the file
C         associated with HANDLE, the error SPICE(SEGMENTNOTFOUND) is
C         signaled.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 03-JUN-2021 (JDR)
C
...
C        that scans through the packet data and then incrementing this
