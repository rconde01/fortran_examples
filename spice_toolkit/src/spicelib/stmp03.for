...
C     C1,
C     C2,
C     C3       are the values of the Stumpff functions C_0(X), C_1(X),
C              C_2(X), and C_3(X).
C
C$ Parameters
C
C     TRUNC    is the Maclaurin series for C_3 and C_2 respectively are:
C
C                                     2     3                 k
C                        1     X     X     X              (-X)
C              C_3(X) =  --- - --- + --- - --- + . . . + ----------. . .
...
C$ Version
C
C-    SPICELIB Version 3.29.0, 28-NOV-2021 (BVS)
C
C        Updated for MAC-OSX-M1-64BIT-CLANG_C.
C
C-    SPICELIB Version 3.28.0, 12-AUG-2021 (JDR)
C
...
C
C        Code was updated to correct excessive round-off
