...
C              except in comparisons using the LIKE and NOT LIKE
...
C         error message, instead of signaling errors. The following
...
C     instead cause errors to be signaled. These are listed below.
C
...
C         is called, UTC time values may not be used in queries. If they
C         are, an error is signaled by a routine in the call tree of
C         this routine.
C
C     4)  If an SCLK kernel for the appropriate spacecraft clock has not
C         been loaded before this routine is called, SCLK values for
C         that clock may not be used in queries. If they are, an error
...
C     satisfy the input query. See the EK Required Reading for
C     examples of use of this routine in conjunction with the EK
C     fetch routines.
C
C$ Examples
C
...
C        the different commands of the Deep Impact spacecraft
C        subsystem, and a table with the subsystem id, the parameter
C        name, and the description of that parameters, ordered by
C        subsystem name. Print the number of records of that table.
C
C        Use the EK kernel below to load the Deep Impact spacecraft
C        subsystem commands dictionary.
C
C           dif_cmdict_128_20050620.bdb
C
C
C        Example code begins here.
C
C
C              PROGRAM EKFIND_EX1
C              IMPLICIT NONE
C
C        C
...
C
C        C
C        C     The EK file contains the table 'DIF_COMMANDS',
C        C     and that 'DIF_COMMANDS' contains columns named:
C        C
C        C       SUBSYSTEM, COMMAND, PARAMETER_NAME, DESCRIPTION
C        C
C        C     Define a set of constraints to perform a query on all
C        C     loaded EK files (the SELECT clause).
C        C
C              QUERY = 'Select SUBSYSTEM, COMMAND, PARAMETER_NAME, '
C             .   //   'DESCRIPTION from DIF_COMMANDS '
...
C        C        string.
C        C
C                 WRITE(*,*) 'Number of matching rows: ', NMROWS
C
C              END IF
C
C              END
...
C
C
C         Number of matching rows:         5798
C
C
C     2) Examples of strings containing syntactically valid queries:
C
...
C
C
C     3) Examples of syntactically invalid queries:
C
...
C        an error to be signaled. Previous version line was changed
