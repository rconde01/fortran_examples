C$Procedure ZZSINUTL ( Utilities for generalized ray intercept )
...
C
C     2)  See the entry points for descriptions of errors specific to
C         those routines.
C
C$ Files
C
...
C$ Particulars
C
C     This routine is meant to be used only SPICELIB.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 17-OCT-2021 (NJB) (JDR)
C
...
C$Procedure ZZSUELIN ( Initialize SINCPT utilities for ellipsoid )
...
C$ Particulars
C
C     This routine is meant to be used only by SPICELIB.
C
...
C     routine and related utilities assume the input ray vectors are
C     expressed in a body-fixed frame centered at the target body's
C     center, having its axes aligned with the principal axes of the
C     target body's reference ellipsoid.
C
C$ Examples
C
C     See usage in SINCPT, ZZSFXCOR.
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 30-MAY-2021 (NJB) (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Updated comments.
...
C         exceeds MAXSRF, the error SPICE(INVALIDCOUNT) is signaled.
C
...
C
C     4)  If a body RADII vector has other than exactly thee elements,
C         an error is signaled by a routine in the call tree of this
C         routine.
C
C     5)  If a body RADII vector has any element less-than or equal to
...
C     also computes the radii of the inner and outer bounding
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 07-SEP-2021 (EDW) (JDR)
C
...
C$Procedure ZZRAYSFX ( Callback for ray-surface intercept )
...
C         surface intercept, the error is signaled by a routine in
...
C-    SPICELIB Version 1.0.1, 30-MAY-2021 (NJB) (JDR)
C
C        Updated comments: this routine no longer is considered
C        a DSK subsystem routine.
C
C        Edited the header to comply with NAIF standard.
C
C-    SPICELIB Version 1.0.0, 01-JUN-2016 (NJB)
...
C$Procedure ZZMAXRAD ( Shape model, maximum radius )
...
C
C     The routine considers all segments for the target in the
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 16-JUL-2021 (NJB) (JDR)
C
C        Updated comments: this routine no longer is considered
C        a DSK subsystem routine.
C
...
C$Procedure ZZMINRAD ( Shape model, minimum radius )
...
C$Procedure ZZRAYNP ( Shape model, callback for ray-surface near point )
...
C         input ray, the error is signaled by a routine
