...
C$ Abstract
C
C     Convert an input epoch represented in TDB seconds past the TDB
C     epoch of J2000 to a character string formatted to the
C     specifications of a user's format picture.
C
C$ Disclaimer
C
...
C     --------  ---  --------------------------------------------------
C     ET         I   Epoch in seconds past the ephemeris epoch J2000.
C     PICTUR     I   A format specification for the output string.
C     OUTPUT     O   A string representation of the input epoch.
C
C$ Detailed_Input
C
C     ET       is a double precision representation of time in seconds
...
C              format picture. These are String Markers, Numeric
C              Markers, Meta markers, Modifier Markers and Literal
C              Markers.
C
...
C$ Detailed_Output
C
C     OUTPUT   is a time string equivalent to the input epoch ET,
C              matching the format specified by PICTUR.
C
C$ Parameters
C
C     None.
...
C$ Exceptions
C
C     1)  A leapseconds kernel must be loaded via the routine FURNSH
C         before calling this routine. If a leapsecond kernel has not
C         been loaded, an error is signaled by a routine in the call
C         tree of this routine.
C
C     2)  If PICTUR contains the numeric marker 'YYYY' and the
C         magnitude of year is too large to be displayed as a four-digit
C         integer, TIMOUT will replace it by '****'.
C
C     3)  If the requested precision is higher than 12 decimal places,
C         TIMOUT will truncate the decimal part down to 12, and OUTPUT
C         will have all the remaining digits in the decimal part set to
C         zero.
C
C     4)  Double colon (::), when is not part of one of the supported
C         markers, has no effect and will be presented as is on the
C         output string.
C
C$ Files
C
...
C
C     '::TDT'      meta      all components should be TT (TDT)
C
C     '::TT'       meta      all components should be TT (TDT)
C
...
C        any of the systems UTC, TT or TDT, or TDB. If you do not
...
C        ::UTC, ::TT, ::TDT, or ::TDB you may specify that TIMOUT
C        produce time strings relative to the UTC, TT or TDT, or TDB
C        system respectively.
C
C     Time Zones
C
...
C
C           CALL TPICTR ( 'Fri Jul 26 12:22:09 PDT 1996',
C          .              PICTUR, OK, ERRMSG             )
C
...
C        C
C                 UTCSTR = '26 Nov 2018  23:23:00 UTC'
...
C
C         Input UTC    : 26 Nov 2018  23:23:00 UTC
C         Output       : Mon Nov 26 16:23:00 PDT 2018
C
C
C     2) Convert a UTC time to a string that contains both the
...
C        C     Convert the input UTC time to ephemeris time.
C        C
C              UTCSTR = '26 Nov 2018  16:23:00 UTC'
C              CALL STR2ET ( UTCSTR, ET )
C
C        C
C        C     Create the required time picture. This could be done
C        C     using TPICTR.
C        C
C              PICTUR = 'Wkd Mon DD HR:MN ::UTC-7 YYYY '
C             .      // '(JULIAND.#### JDUTC)'
C
C        C
C        C      Now convert ET to the desired output format.
...
C
C
C         Input UTC: 26 Nov 2018  16:23:00 UTC
C         Output   : Mon Nov 26 09:23  2018 (2458449.1826 JDUTC)
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 3.4.0, 07-AUG-2021 (EDW) (JDR) (NJB)
C
C        Corrected typo preventing correct calculation of decimal
C        values for HR.###... and MN.###... markers with ::UTC+N:M
C        and ::UTC-N:M meta tags.
C
C        Added "::TT" as a time system meta marker equivalent-to/
C        alias-for "::TDT". No change to functionality.
C
C        Corrected OUTPUT argument name in $Brief_I/O section (it was
C        STRING) and improved its description in $Detailed_Output.
C        Fixed call to TPICTR in $Particulars.
C
C        Edited the header to comply with NAIF standard. Removed
C        unnecessary entries in $Revisions section. Converted the
C        existing code fragments into complete examples and added
C        references to required LSKs.
C
C        Updated $Exceptions section, rewording the existing entry
C        and adding three additional cases.
C
...
C        arithmetic operations with double precision arithmetic.
C
...
C
C-    SPICELIB Version 2.0.0, 01-APR-1997 (WLT)
C
...
C
C-    SPICELIB Version 1.0.0, 26-JUL-1996 (WLT) (MJS) (NJB)
C
C-&
...
C
C     Times come in three flavors: TT or TDT, TDB, UTC.  The one used
...
C     be one of the following: UTC, TDB, TT, TDT
C
      UNKNWN = .TRUE.
      GO2JUL = .FALSE.
      DOZONE = .FALSE.
...
C
      ...
      ELSE IF ( TSYS .EQ. 'TDB' ) THEN
         TIMTYP = ID(TDB)
      ELSE IF ( (TSYS .EQ. 'TDT') .OR. (TSYS .EQ. 'TT') ) THEN
         TIMTYP = ID(TDT)
      ELSE
         TIMTYP = ID(UTCP)
...
C     convert our ET to an earth based epoch (TT, TDT) and use this as
C     our base input system.
C
C
      MYET = ET
...
C
      ...
      ELSE IF ( (TIMTYP .EQ. ID(TDT)) .OR.
     .          (TIMTYP .EQ. ID(TT)) ) THEN
...
C
C                 Weekday. If we need to deal with time zones, this is
C                 the time to do it.
C
                  IF ( TIMTYP .EQ. ID(UTCP) ) THEN
