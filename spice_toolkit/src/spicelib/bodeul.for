...
C$ Exceptions
C
C     1)  If any of the PCK keywords required to compute the angles are
C         not available in the kernel pool, an error is signaled by a
C         routine in the call tree of this routine.
C
C     2)  If the number of phase terms is insufficient, the error
C         SPICE(INSUFFICIENTANGLES) is signaled.
C
C     3)  If, for a given body, both forms of the kernel variable names
C
...
C
C     4)  If, for a given body, both forms of the kernel variable names
C
...
C         variable names match.
C
C     5)  If the central body associated with the input BODY, whether
...
C$ Files
C
C     A text or binary PCK containing orientation data for the
...
C$ Version
C
C-    SPICELIB Version 5.0.0, 14-APR-2021 (NJB) (JDR)
C
...
C
C        Error handling was upgraded to check FAILED() between kernel
C        data lookups and computations.
C
C        Now SAVEs all local variables.
C
C        Edited the header to comply with NAIF standard. Moved NAIF_IDS
...
C
         ...
         IF ( .NOT. FOUND ) THEN
            ...
            CALL GDPOOL ( ITEM2, 1, 1, DIM, CONEPC, FOUND2 )
...
C
         ...
         CALL REPMI  ( ITEM, '#', REFID, ITEM            )
         CALL GIPOOL ( ITEM,  1,  1,     DIM, REF, FOUND )

         IF ( .NOT. FOUND ) THEN

            CALL GIPOOL ( ITEM2, 1, 1, DIM, REF, FOUND2 )
...
C
C           Find out whether the maximum phase angle degree has been
C           explicitly set.
C
...
C
               ...
               THETA  = ( TCOEF(K) + T * TCOEF(M) ) * RPD()

            ELSE
C
C              THETA and DTHETA have higher-order terms; there are
C              NPHSCO coefficients for each angle.
C
               THETA = 0.D0
...
C
C        Adjust RA, DEC, W by their librations and nutations.
C
