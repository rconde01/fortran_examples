...
C$ Abstract
C
C     Read lines from a Fortran logical unit and place them in a
...
C$ Detailed_Input
C
C     UNIT     is the Fortran unit number for the input. This may
C              be either the unit number for the terminal, or the
C              unit number of a previously opened text file.
C
C     MAXLIN   is the maximum number of text lines that can be placed
C              into the ARRAY.
C
C$ Detailed_Output
C
C     NUMLIN   is the number of text lines read from the file attached
C              to UNIT and placed into ARRAY. 0 <= NUMLIN <= MAXLIN.
C
...
C              occurred.
C
C     ARRAY    is the array which is to contain the lines of text read
C              from the text file attached to UNIT.
C
...
C
C     2)  If an error occurs while attempting to read from the text file
...
C
C        Edited the header to comply with NAIF standard. Moved $Version
C        history entries for relevant Beta versions to $Revisions
C        section.
C
...
C        account for the pre-increment before the READ that set the end
C        of file.
C
