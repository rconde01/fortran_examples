...
C        -1.797693134862316D+308 as the minimum negative double
C        precision number, it produced a compilation error. As a result,
C        we changed the number to -1.79769313486231D+308, which did not
C        produce a compilation error.
C
...
C
C     1) Return the value of the smallest negative number representable
C        in a double precision variable.
C
...
C        C
C              DOUBLE PRECISION      DPMIN
C
C        C
C        C     Display the smallest negative number representable
C        C     in a double precision variable.
C        C
C              WRITE(*,'(A,E22.14)')
C             .               'Minimum Double Precision Number: ',
C             .               DPMIN()
C
C
C              END
...
C
C
C        Minimum Double Precision Number:  -0.17976931348623+309
C
C
C$ Restrictions
C
...
C
C     [10] "Unix/VMS Compatibility Libraries", Absoft Fortran V3.2,
C          1993; Chapter 3, Support Libraries, page 3-5, dflmax. (for
C          the NeXT)
C
...
C$ Version
C
C-    SPICELIB Version 2.15.1, 13-AUG-2021 (JDR)
C
...
C
C-    SPICELIB Version 1.6.0, 05-JAN-1995 (HAN)
C
...
C
C-    SPICELIB Version 1.5.0, 03-NOV-1993 (HAN) (NJB)
C
...
C        Hewlett Packard UX 9000/750 environment.
C
C        The code was also reformatted so that a utility program
C        can create the source file for a specific environment
C        given a master source file.
C
...
C        for Sun. Documentation in the $Particulars
