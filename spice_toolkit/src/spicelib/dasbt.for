...
C$ Detailed_Input
C
C     BINFIL   is the name of a binary DAS file which is to be converted
...
C$ Exceptions
C
C     1)  If the binary DAS file specified by the filename BINFIL cannot
...
C
C     3)  If, for any reason, the DAS file cannot be read, an error is
...
C         the error SPICE(BADDASFILE) is signaled. THIS ERROR
...
C$ Version
C
C-    SPICELIB Version 3.2.0, 02-JUN-2021 (JDR)
C
