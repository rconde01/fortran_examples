...
C$ Detailed_Input
C
C     SYMMAT   is a symmetric 2x2 matrix. That is, SYMMAT has the
C              form
C
C                 .-        -.
C                 |  A    B  |
...
C
C              In other words, DIAG is similar to SYMMAT, and ROTATE is
C              a change-of-basis matrix that diagonalizes SYMMAT. DIAGS2
C              chooses ROTATE so that its angle of rotation has the
C              smallest possible magnitude. If there are two rotations
C              that meet these criteria (they will be inverses of one
C              another), either rotation may be chosen.
C
C$ Parameters
C
C     None.
...
C     point source) of a body modeled as a tri-axial ellipsoid are
C     ellipses.
C
...
C
C     [1]  T. Apostol, "Calculus, Vol. II," chapter 5, "Eigenvalues of
C          Operators Acting on Euclidean Spaces," John Wiley & Sons,
C          1969.
C
...
C$ Version
C
C-    SPICELIB Version 1.3.0, 17-JUN-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Added
C        "Error free." to $Exceptions section. Removed unnecessary
C        $Revisions section.
C
...
C
C        Explicitly zero out the (2,1) entry of DIAG, since DIAG is
