...
C$ Detailed_Input
C
C     STRING   is the input character string to be tested for a match
C              against the input template. Leading and trailing blanks
C              are ignored.
C
C     TEMPL    is the input template to be tested for a match against
C              the input string. TEMPL may contain wild cards. Leading
C              and trailing blanks are ignored.
C
C     WSTR     is the wild string token used in the input template. The
C              wild string token may represent from zero to any number
C              of characters.
C
C     WCHR     is the wild character token used in the input template.
C              The wild character token represents exactly one
C              character.
C
C$ Detailed_Output
C
C     The function returns .TRUE. when the input string matches the
C     input template, and .FALSE. otherwise. The string and template
C     match whenever the template can expand (through replacement of its
...
C$ Version
C
C-    SPICELIB Version 1.4.0, 17-JUN-2021 (JDR)
C
...
C
C     First let's get everybody's measurements.
C
