...
C
C
C              Neither case nor white space are significant in METHOD,
C              except within double-quoted strings representing
C              surfaces. For example, the string ' eLLipsoid ' is valid.
C
C              Within double-quoted strings representing surfaces, blank
C              characters are significant, but multiple consecutive
C              blanks are considered equivalent to a single blank. Case
C              is not significant. So
C
...
C              observer and the input surface point SPOINT (note: NOT
C              between the observer and the target body's center). The
C              following values of ABCORR apply to the "reception" case
C              in which photons depart from SPOINT at the light-time
...
C                            equations for SPOINT and the
...
C              is valid.
C
C     OBSRVR   is the name of the observing body. The observing body is
C              an ephemeris object: it typically is a spacecraft, an
C              extended body, or a surface point for which ephemeris
C              data are available. OBSRVR is case-insensitive, and
...
C              example both 'MOON' and '301' are legitimate strings that
C              indicate the Moon is the observer.
C
...
C$ Detailed_Output
C
C     TRGEPC   is the "target surface point epoch." TRGEPC is defined as
C              follows: letting LT be the one-way light time between the
C              observer and the input surface point SPOINT, TRGEPC is
C              either the epoch ET-LT, ET+LT or ET depending on whether
C              the requested aberration correction is, respectively, for
C              received radiation, transmitted radiation or omitted. LT
...
C              FIXREF at time TRGEPC to a time-dependent reference frame
C              REF at ET, the routine PXFRM2 should be called. For
C              example, let XFORM be 3x3 matrix representing the
C              rotation from the body-fixed reference frame FIXREF at
C              time TRGEPC to the time-dependent frame REF at time ET.
C              Then SRFVEC can be transformed to the result REFVEC as
C              follows:
C
...
C
C
C     PHASE    is the phase angle at SPOINT, as seen from OBSRVR at time
C              ET. This is the angle between the negative of the vector
C              SRFVEC and the SPOINT-illumination source vector at
C              TRGEPC. Units are radians. The range of PHASE is [0, pi].
C              See $Particulars below for a detailed discussion of the
C              definition.
C
C     INCDNC   is the illumination source incidence angle at SPOINT, as
C              seen from OBSRVR at time ET. This is the angle between
C              the surface normal vector at SPOINT and the SPOINT-source
C              vector at TRGEPC. Units are radians. The range of INCDNC
C              is [0, pi]. See $Particulars below for a detailed
C              discussion of the definition.
C
C     EMISSN   is the emission angle at SPOINT, as seen from OBSRVR at
C              time ET. This is the angle between the surface normal
C              vector at SPOINT and the negative of the vector SRFVEC.
C              Units are radians. The range of EMISSN is [0, pi]. See
C              $Particulars below for a detailed discussion of the
C              definition.
C
...
C
C     2)  If the specified aberration correction is any other
C         unrecognized value, an error is signaled
C         by a routine in the call tree of this routine.
C
C     3)  If any of the target, observer, or illumination source
C         input strings cannot be converted to an integer ID code, the
C         error SPICE(IDCODENOTFOUND) is signaled.
C
C     4)  If OBSRVR and TARGET map to the same NAIF integer ID code,
C         the error SPICE(BODIESNOTDISTINCT) is signaled.
C
C     5)  If the input target body-fixed frame FIXREF is not
...
C
C     6)  If the input frame FIXREF is not centered at the target body,
C         the error SPICE(INVALIDFRAME) is signaled.
C
C     7)  If the input argument METHOD cannot be parsed, an error
C         is signaled by either this routine or a routine in the
C         call tree of this routine.
C
C     8)  If insufficient ephemeris data have been loaded prior to
C         calling ILLUMF, an error is signaled by a
...
C         loaded into the kernel pool prior to calling ILLUMF, an error
C         is signaled by a routine in the call tree of this routine.
C
C     10) If PCK data specifying the target body-fixed frame orientation
C         have not been loaded prior to calling ILLUMF, an error is
...
C         DSK data, and data representing the portion of the surface on
C         which SPOINT is located are not available, an error is
C         signaled by a routine in the call tree of this routine.
C
...
C         surface.
C
C         If, in the DSK case, SPOINT is too far from the surface, an
...
C
C     14) If radii for TARGET are not found in the kernel pool, an error
C         is signaled by a routine in the call tree of this routine.
C
C     15) If the size of the TARGET body radii kernel variable is not
C         three, an error is signaled by a routine in the call tree of
C         this routine.
C
C     16) If any of the three TARGET body radii is less-than or equal to
...
C$ Restrictions
C
C     1)  Results from this routine are not meaningful if the input
C         point lies on a ridge or vertex of a surface represented by
C         DSK data, or if for any other reason the direction of the
C         outward normal vector at the point is undefined.
C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 20-NOV-2021 (JDR) (EDW) (NJB)
C
