...
C     derivatives, given a specified set of axes.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     XFORM    is a state transformation matrix from some frame FRAME1
C              to another frame FRAME2. Pictorially, XFORM has the
...
C                 `-             -'
C
C              where R is a rotation matrix that varies with respect to
C              time and dR/dt is its time derivative.
C
...
C     AXISA,
C     AXISB,
C     AXISC    are the axes desired for the factorization of R.
C
C              All must be in the range from 1 to 3. Moreover
C              it must be the case that AXISA and AXISB are distinct
...
C         routine.
C
C     2)  If AXISB is equal to AXISC or AXISA, an error is signaled by a
C         routine in the call tree of this routine. An arbitrary
C         rotation matrix cannot be expressed using a sequence of Euler
C         angles unless the second rotation axis differs from the other
C         two.
C
C     3)  If the input matrix XFORM is not a rotation matrix, an error
...
C     are in the range produced by XF2EUL and the Euler representation
...
C
C     1) Determine the rate of change of the right ascension and
C        declination of the pole of the moon, from the state
C        transformation matrix that transforms J2000 states to object
C        fixed states.
C
C        Recall that the rotation component of the state transformation
...
C        C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'xf2eul_ex1.tm' )
C
C              CHARACTER*(*)         UTCSTR
C              PARAMETER           ( UTCSTR = 'May 15, 2007' )
C
C        C
C        C     Local variables.
C        C
...
C        C     IAU_MOON.
C        C
C              CALL SXFORM ( 'J2000', 'IAU_MOON', ET, FTMTRX )
C
C        C
C        C     Convert the transformation matrix to
C        C     Euler angles (3-1-3).
C        C
C              CALL XF2EUL ( FTMTRX, 3, 1, 3, EULANG, UNIQUE )
C
C        C
...
C              IF ( UNIQUE ) THEN
C
C                 WRITE(*,'(2A)') 'UTC: ', UTCSTR
C                 WRITE(*,'(A,F20.16)') 'W       = ', EULANG(1)
C                 WRITE(*,'(A,F20.16)') 'DEC     = ',
C             .                  HALFPI() - EULANG(2)
C                 WRITE(*,'(A,F20.16)') 'RA      = ',
C             .                  EULANG(3) - HALFPI()
C                 WRITE(*,'(A,F20.16)') 'dW/dt   = ', EULANG(4)
C                 WRITE(*,'(A,F20.16)') 'dDEC/dt = ', EULANG(5)
C                 WRITE(*,'(A,F20.16)') 'dRA/dt  = ', EULANG(6)
C
C              ELSE
C
C                 WRITE(*,*) 'The values in EULANG are not uniquely '
C             .          //  'determined.'
C
C              END IF
C
...
C
C        UTC: May 15, 2007
C        W       =  -2.6490877296701645
C        DEC     =   1.1869108599473206
C        RA      =  -1.5496443908099826
C        dW/dt   =   0.0000026578085601
C        dDEC/dt =   0.0000000004021737
C        dRA/dt  =   0.0000000039334471
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 26-OCT-2021 (JDR)
C
...
C
C        Added complete code example based on existing fragment.
C        Corrected input argument name in $Exceptions section.
C
...
C$ Abstract
C
C     Compute a state transformation from an Euler angle factorization
C     of a rotation and the derivatives of those Euler angles.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C$ Detailed_Output
C
C     XFORM    is the state transformation matrix corresponding to R
C              and dR/dt as described above. Pictorially,
C
C                 .-             -.
C                 |       |       |
C                 |   R   |   0   |
...
C     three such rotations, provided that this last criterion is met.
C
C     This routine is intended to provide an inverse for XF2EUL.
C
C     The two subroutine calls shown here will not change
C     XFORM except for round off errors.
C
...
C     specific arithmetic implementation.
C
C     1) Suppose you have a set of Euler angles and their derivatives
...
C                     1        2      3
C
C        The following code example will perform the desired
C        computation.
C
C
C        Example code begins here.
C
C
C              PROGRAM EUL2XF_EX1
C              IMPLICIT NONE
C
C        C
C        C     Local variables.
C        C
C              DOUBLE PRECISION      ABGANG ( 6    )
C              DOUBLE PRECISION      RPYANG ( 6    )
C              DOUBLE PRECISION      XFORM  ( 6, 6 )
C
C              LOGICAL               UNIQUE
C
C        C
C        C     Define the initial set of Euler angles.
C        C
C              ABGANG(1) =  0.01D0
C              ABGANG(2) =  0.03D0
C              ABGANG(3) =  0.09D0
...
C
C        C
C        C     Compute the equivalent angles and derivatives for a
C        C     1-2-3 rotation.
C        C
...
C              IF ( UNIQUE )  THEN
C
C                 WRITE(*,'(A)') '1-2-3 equivalent rotation to input '
C             .               // '(radians):'
C                 WRITE(*,'(2(A,F13.9))') 'Roll  ', RPYANG(1),
C             .                           ', dRoll/dt  ', RPYANG(4)
C                 WRITE(*,'(2(A,F13.9))') 'Pitch ', RPYANG(2),
C             .                           ', dPitch/dt ', RPYANG(5)
C                 WRITE(*,'(2(A,F13.9))') 'Yaw   ', RPYANG(3),
C             .                           ', dYaw/dt   ', RPYANG(6)
C
C              ELSE
C
C                 WRITE(*,*) 'The values in RPYANG are not uniquely '
C             .          //  'determined.'
C
C              END IF
C
...
C
C
C        1-2-3 equivalent rotation to input (radians):
C        Roll    0.029998501, dRoll/dt   -0.002999550
C        Pitch  -0.000299950, dPitch/dt   0.000059980
C        Yaw     0.099995501, dYaw/dt    -0.009998650
C
C
C$ Restrictions
C
