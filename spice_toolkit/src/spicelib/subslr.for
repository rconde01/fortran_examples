...
C         routine.
C
C     2)  If transmission aberration corrections are specified, the
...
C         loaded into the kernel pool prior to calling SUBSLR, an error
...
C         have not been loaded prior to calling SUBSLR, an error is
...
C
C     14) If the surface intercept on the target body's reference
...
C
C     15) If the target body is the sun, the error SPICE(INVALIDTARGET)
C         is signaled.
C
...
C        C     aberration corrections. Use the "target surface
C        C     intercept" definition of sub-solar point on the first
C        C     loop iteration, and use the "near point" definition on
C        C     the second.
C        C
C              DO I = 1, NMETH
C
...
C                 WRITE(*,FM) ' '
C                 WRITE(*,FM) '  Sub-solar point altitude            '
C             .   // '(km) = ', SPGALT
C                 WRITE(*,FM) '  Sub-solar planetographic longitude '
C             .   // '(deg) = ', SPGLON
C                 WRITE(*,FM) '  Sun''s planetographic longitude     '
C             .   // '(deg) = ', SUPGLN
C                 WRITE(*,FM) '  Sub-solar planetographic latitude  '
C             .   // '(deg) = ', SPGLAT
C                 WRITE(*,FM) '  Sun''s planetographic latitude      '
C             .   // '(deg) = ', SUPGLT
C                 WRITE(*,FM) '  Sub-solar planetocentric longitude '
C             .   // '(deg) = ', SPCLON
C                 WRITE(*,FM) '  Sun''s planetocentric longitude     '
C             .   // '(deg) = ', SUPCLN
C                 WRITE(*,FM) '  Sub-solar planetocentric latitude  '
C             .   // '(deg) = ', SPCLAT
C                 WRITE(*,FM) '  Sun''s planetocentric latitude      '
C             .   // '(deg) = ', SUPCLT
C                 WRITE(*,FM) ' '
C
C              END DO
...
C$ Version
C
C-    SPICELIB Version 3.1.0, 01-NOV-2021 (JDR) (NJB) (EDW)
C
...
C        Changed code example and its output to comply with maximum
