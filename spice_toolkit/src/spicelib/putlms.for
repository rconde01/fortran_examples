...
C$ Abstract
C
C     Store a long error message.
C
C     PUTLMS is a low-level data structure access routine.
C     DO NOT CALL THIS ROUTINE. USE SETMSG, NOT PUTLMS, TO SET THE
C     CURRENT LONG ERROR MESSAGE.
C
C$ Disclaimer
C
...
C$ Detailed_Input
C
C     MSG      is the current long error message. This value will be
C              saved.
C
C$ Detailed_Output
C
...
C$ Restrictions
C
C     1)  Calls to this routine by routines outside of the SPICELIB
C         error handling system may interfere with error processing.
C
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 27-AUG-2021 (JDR)
C
...
C$ Version
C
C-    SPICELIB Version 1.0.3, 03-JUN-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Added
C        $Index_Entries entry. Removed unnecessary $Revisions section.
C
...
C$ Index_Entries
C
C     get current long error message
C
C-&
