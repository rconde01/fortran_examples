...
C
C     LON      is the planetographic longitude of the input point.
C              This is the angle between the prime meridian and the
C              meridian containing the input point. For bodies
C              having prograde (aka direct) rotation, the direction
C              of increasing longitude is positive west: from the +X
...
C
C              LON is measured in radians. On input, the range
C              of longitude is unrestricted.
C
C     LAT      is the planetographic latitude of the input point.
C              For a point P on the reference spheroid, this is the
C              angle between the XY plane and the outward normal
C              vector at P. For a point P not on the reference
C              spheroid, the planetographic latitude is that of the
C              closest point to P on the spheroid.
C
C              LAT is measured in radians. On input, the
...
C
C     RE       is the equatorial radius of a reference spheroid.
C              This spheroid is a volume of revolution: its
C              horizontal cross sections are circular. The shape of
C              the spheroid is defined by an equatorial radius RE and
C              a polar radius RP. Units of RE must match those of
C              ALT.
C
C     F        is the flattening coefficient of the body, a
C              dimensionless value defined as:
C
C                 (RE - RP) / RE
C
...
C              See the discussion below in the $Particulars header
...
C           Latitude  (deg)        =    45.000000000000000
C           Altitude  (km)         =    300.00000000000000
C
...
C
C           X (km)                 =    1.6047030223125209E-013
C           Y (km)                 =   -2620.6789148181788
C           Z (km)                 =    2592.4089088569663
C
C
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 27-MAY-2021 (JDR)
C
C        Edits to header to comply with NAIF standard.
C
