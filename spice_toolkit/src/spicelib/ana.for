...
C
C     The function returns the correct article, 'A' or 'AN', needed to
C     modify a word WORD, appropriately capitalized.
C
C$ Detailed_Input
C
C     WORD     is any English word for which you want to write the
C              correct phrase "a(an) response(answer)".  The case of the
C              letters of word do not matter.
C
...
C              the same word.
C
C     CASE     is a character that describes how the value returned in
C              ANA should be capitalized. The rules are:
C
C                 'U'  ---  ANA is returned in all caps ( A, AN )
C                 'C'  ---  ANA is returned capitalized ( A, An )
C                 'L'  ---  ANA is returned lower case  ( a, an )
C
C              The case of CASE does not matter. Any value other than
C              those specified result in ANA being returned in all lower
C              case.
C
C$ Detailed_Output
C
C     The function returns the correct indefinite article needed to
C     modify the word contained in WORD.
C
C     ANA should be declared to be
C
C        CHARACTER*(2)
C
C     (or CHARACTER*(N) where N > 1) in the calling program.
C
C$ Parameters
C
C     None.
...
C     Error free.
C
C     1)  If the uppercase value of CASE is not 'U', 'C' or 'L', it
C         shall be treated as 'L'.
C
C$ Files
C
C     None.
...
C
C     [1]  Merriam-Webster (Ed.), "Webster's Ninth New Collegiate
C          Dictionary," 10th edition, 1990.
C
...
C$ Version
C
C-    SPICELIB Version 1.1.3, 24-NOV-2021 (JDR)
C
...
C
C     If we got this far we can determine the ANA by
