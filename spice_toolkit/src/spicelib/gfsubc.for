...
C              name.
C
C     METHOD   is the string name of the method to use for the subpoint
...
C              legitimate strings to indicate that the observer is the
C              Earth.
C
...
C              subpoint vector. See the discussion of RELATE above for
...
C              'ABSMIN' and ADJUST is set to a positive value, GFSUBC
C              finds times when the subpoint position vector coordinate
C              is within ADJUST radians/kilometers of the specified
C              extreme value.
C
...
C              discarded before GFSUBC conducts its search.
C
C$ Detailed_Output
C
...
C
C     1) Find the time during 2007 for which the subpoint position
C        vector of the Sun on Earth in the IAU_EARTH frame lies within
C        a geodetic latitude-longitude "box" defined as
C
...
C        longitude. The sub-observer point latitude varies between
C        approximately 23.44 degrees and -23.44 degrees during the
C        year. The sub-observer point longitude varies between -180
C        degrees and 180 degrees in one day.
C
...
C        C     Load kernels.
C        C
C              CALL FURNSH ('gfsubc_ex1.tm')
C
C              TIMFMT = 'YYYY-MON-DD HR:MN:SC.###### ::TDB ::RND'
C
C        C
...
C        C     latitude- longitude box restriction conditions apply to
C        C     the subpoint vector.
C        C
...
C        C     search serves as the confinement window of the
C        C     subsequent search.
C        C
...
C
C        C
C        C           Determine the latitude and longitude of the
C        C           subpoint at the event interval boundaries.
C        C
C                    CALL SUBPNT ( METHOD, TARGET, LEFT,
C             .                    FIXREF, ABCORR, OBSRVR,
C             .                    LPOS, TRGEPC, SRFVEC   )
C                    CALL RECLAT ( LPOS, RAD(1), LON(1), LAT(1) )
C
C                    CALL SUBPNT ( METHOD, TARGET, RIGHT,
C             .                    FIXREF, ABCORR, OBSRVR,
C             .                    RPOS, TRGEPC, SRFVEC   )
C                    CALL RECLAT ( RPOS, RAD(2), LON(2), LAT(2) )
C
C
C                    WRITE(*,'(2A,2F14.8)') 'From : ',    BEGSTR,
C             .                              LAT(1)*DPR(), LON(1)*DPR()
C                    WRITE(*,'(2A,2F14.8)') 'To   : ',    ENDSTR,
C             .                              LAT(2)*DPR(), LON(2)*DPR()
C                    WRITE(*,*) ' '
C
C                 END DO
C
...
C
C
C               Time (TDB)                      LAT (deg)      LON (deg)
C               ---------------------------    -----------   -----------
C        From : 2007-MAY-05 06:12:59.452307    16.05435608   86.00000000
C        To   : 2007-MAY-05 06:16:59.436479    16.05514776   85.00000417
C
C        From : 2007-MAY-06 06:12:54.398070    16.33714720   86.00000000
C        To   : 2007-MAY-06 06:16:54.383826    16.33792651   85.00000417
C
C        From : 2007-MAY-07 06:12:49.917541    16.61544356   86.00000000
C        To   : 2007-MAY-07 06:16:49.904901    16.61621026   85.00000417
C
C        From : 2007-MAY-08 06:12:46.017221    16.88916258   86.00000000
C        To   : 2007-MAY-08 06:16:46.006200    16.88991646   85.00000417
C
C        From : 2007-AUG-06 06:22:12.099776    16.68071740   86.00000000
C        To   : 2007-AUG-06 06:26:12.080859    16.67996165   85.00000417
C
C        From : 2007-AUG-07 06:22:05.362314    16.40641076   86.00000000
C        To   : 2007-AUG-07 06:26:05.341799    16.40564259   85.00000417
C
C        From : 2007-AUG-08 06:21:58.050893    16.12767782   86.00000000
C        To   : 2007-AUG-08 06:25:58.028786    16.12689748   85.00000417
C
C
C$ Restrictions
C
...
C
C        Modified code example's output to comply with maximum line
C        length of header comments. Added SAVE statements for CNFINE,
C        WORK, RESULT1, RESULT2, RESULT3 and RESULT4 variables in code
C        example.
C
