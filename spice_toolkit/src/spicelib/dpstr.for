...
C     Error free.
C
C     1)  If SIGDIG is less than one, this routine returns one
C         significant digit in the output string.
C
C$ Files
C
C     None.
...
C$ Restrictions
C
C     1)  The format of the string returned by this routine is used in
C         DPSTRF which is in the call tree to DPFMT. Changes to the
C         format of the output string may have unexpected consequences
C         for these SPICE routines. Please check those routines before
C         modifying this routine.
C
C     2)  The maximum number of significant digits returned is 14.
C
C     3)  If the output string is not declared to be adequately large
C         (at least SIGDIG + 6), the numeric string will be truncated to
C         the side opposite its justification.
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 20-AUG-2021 (JDR)
C
...
C
C     We might have accidentally made copy as big as 10 by the
