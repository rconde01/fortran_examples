...
C$ Exceptions
C
C     1)  If the value of BODY is the ID code of a barycenter --codes 0,
C         1, ..., 9, the error SPICE(BARYCENTERIDCODE) is signaled.
C
C     2)  If FRAME is not one of the known SPICE inertial reference
C         frames, the error SPICE(INVALIDREFFRAME) is signaled.
C
C     3)  If FIRST is greater than or equal to LAST, the error
C         SPICE(BADDESCRTIMES) is signaled.
C
C     4)  If the value of TYPE is outside the range 2 to 1000
C         (inclusive), the error SPICE(UNKNOWNPCKTYPE) is signaled. This
C         does not ensure that the TYPE is a legitimate PCK segment
C         type, but it is a simple check that helps avoid problems that
C         arise from uninitialized values or improperly ordered calling
C         arguments.
C
C$ Files
C
...
C$ Version
C
C-    SPICELIB Version 1.0.2, 20-AUG-2021 (JDR)
C
