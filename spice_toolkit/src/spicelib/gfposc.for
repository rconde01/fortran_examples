...
C
C     FRAME    is the name of the reference frame in which to perform
C              state look-ups and coordinate calculations.
C
C              The SPICE frame subsystem must recognize the FRAME
C              name.
C
C     ABCORR   is the description of the aberration corrections
...
C              strings to indicate the observer is the Earth.
C
C     CRDSYS   is the name of the coordinate system for which the
...
C              coordinates is the center of the frame FRAME.
C
C     RELATE   is the relational operator used to define a constraint on
C              the selected coordinate of the observer-target vector.
C              The result window found by this routine indicates the
C              time intervals where the constraint is satisfied.
...
C              extremum constraint: this requires the quantity to be
...
C              'ABSMIN' and ADJUST is set to a positive value, GFPOSC
C              finds times when the position vector coordinate is within
...
C              decreasing. However, STEP must not be *too* short, or the
C              search will take an unreasonable amount of time.
C
C              For coordinates other than 'LONGITUDE' and 'RIGHT
C              ASCENSION', the step size must be shorter than the
C              shortest interval, within the confinement window, over
C              which the coordinate is monotone increasing or
C              decreasing.
C
...
C              discarded before GFPOSC conducts its search.
C
C$ Detailed_Output
C
...
C
C        C
C        C     The latitude varies relatively slowly, ~46 degrees
C        C     during the year. The extrema occur approximately every
C        C     six months. Search using a step size less than half
C        C     that value (180 days). For this example use ninety days
...
C
C
C         Event time: 2007-JUN-21 17:54:13.172475 (TDB)
C
C
C     2) A minor modification of the program listed in Example 1; find
...
C
C
C         Event time: 2007-DEC-22 06:04:32.635539 (TDB)
C
C
C     3) Find the time during 2007 for which the Z component of the
...
C
C
C         Event time: 2007-MAR-21 00:01:25.500673 (TDB)
C         Event time: 2007-SEP-23 09:46:39.579484 (TDB)
C
C
C     4) Find the times between Jan 1, 2007 and Jan 1, 2008
...
C
C        C
C        C     This search requires a change in the step size since the
C        C     Moon's orbit about the earth (earth-moon barycenter) has
C        C     a twenty-eight day period. Use a step size something
C        C     less than half that value. In this case, we use twelve
C        C     days.
C        C
C              STEP   = SPD() * 12.D0
C              ADJUST = 0.D0
...
C
C
C         Event time: 2007-JAN-10 16:26:18.784521 (TDB)
C         Event time: 2007-FEB-07 12:39:35.055710 (TDB)
C         Event time: 2007-MAR-07 03:38:07.308330 (TDB)
C         Event time: 2007-APR-03 08:38:55.191516 (TDB)
C         Event time: 2007-APR-30 10:56:49.819340 (TDB)
C         Event time: 2007-MAY-27 22:03:28.834302 (TDB)
C         Event time: 2007-JUN-24 14:26:23.617432 (TDB)
C         Event time: 2007-JUL-22 08:43:50.113902 (TDB)
C         Event time: 2007-AUG-19 03:28:33.515939 (TDB)
C         Event time: 2007-SEP-15 21:07:13.940711 (TDB)
C         Event time: 2007-OCT-13 09:52:30.791223 (TDB)
C         Event time: 2007-NOV-09 12:32:50.039258 (TDB)
C         Event time: 2007-DEC-06 16:54:31.199770 (TDB)
C
C
C     5) Find times between Jan 1, 2007 and Jan 1, 2008 when the
...
C                                            frame definitions
C              earth_latest_high_prec.bpc    High precision earth PCK
C
C           \begindata
C
C              KERNELS_TO_LOAD = ( 'de421.bsp',
...
C
C        C
C        C     This search uses a step size of four hours since the
C        C     time for all declination zero-to-max-to-zero passes
C        C     within the search window exceeds eight hours.
C        C
C        C     The example uses an 83 degree elevation because of its
C        C     rare occurrence and short duration.
C        C
...
C
C
C         From : 2007-FEB-26 03:18:48.229281 (TDB)
C         To   : 2007-FEB-26 03:31:29.734931 (TDB)
C
C         From : 2007-MAR-25 01:12:38.550572 (TDB)
C         To   : 2007-MAR-25 01:23:53.909469 (TDB)
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 1.2.0, 03-NOV-2021 (JDR) (NJB)
C
C        Edited the header to comply with NAIF standard.
C
C        Added initialization of QCPARS(10) to pacify Valgrind.
C
C        Added entries #5 and #9 to $Exceptions section.
C
