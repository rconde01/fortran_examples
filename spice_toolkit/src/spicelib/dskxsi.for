...
C$ Detailed_Input
C
C     PRI      is a logical flag indicating whether to perform a
C              prioritized or unprioritized DSK segment search. In an
C              unprioritized search, no segment masks another: data from
C              all specified segments are used to define the surface of
C              interest.
C
C              The search is unprioritized if and only if PRI is set to
C              .FALSE. In the N0066 SPICE Toolkit, this is the only
C              allowed value.
C
...
C
C     NSURF,
C     SRFLST   are, respectively, a count of surface ID codes in a list
C              and an array containing the list. Only DSK segments for
C              the body designated by TARGET and having surface IDs in
C              this list will be considered in the intercept
C              computation. If the list is empty, all DSK segments for
C              TARGET will be considered.
C
C     ET       is the epoch of the intersection computation, expressed
C              as seconds past J2000 TDB. This epoch is used only for
C              DSK segment selection. Segments used in the intercept
C              computation must include ET in their time coverage
C              intervals.
C
...
C              orientation relative to the frame of any DSK segment used
C              in the computation.
C
C     VERTEX,
C     RAYDIR   are, respectively, the vertex and direction vector of the
C              ray to be used in the intercept computation.
C
C              Both the vertex and ray's direction vector must be
C              represented in the reference frame designated by FIXREF.
C              The vertex is considered to be an offset from the target
C              body.
C
C     MAXD,
C     MAXI     are, respectively, the declared sizes of the arrays DC
C              and IC. MAXD must be at least DCSIZE, while MAXI must be
C              at least ICSIZE. See the $Parameters section for details.
C
C$ Detailed_Output
C
...
C              surface at multiple points, the one closest to the ray's
C              vertex is selected.
C
...
C     HANDLE,
C     DLADSC,
C     DSKDSC   are, respectively, the DSK file handle, DLA descriptor,
C              and DSK descriptor of the DSK file and segment that
C              contributed the surface data on which the intercept was
C              found.
C
C              These outputs are defined if and only if FOUND is .TRUE.
C
C     DC,
C     IC       are, respectively, double precision and integer arrays
C              that may contain additional information associated with
C              the segment contributing the surface data on which the
C              intercept was found. The information is DSK data
C              type-dependent.
C
C                 For DSK type 2 segments
C
C                    IC(1) is the intercept plate ID. DC is unused.
C
C              These outputs are defined if and only if FOUND is .TRUE.
C
C              The declared length of DC must be at least DCSIZE; the
C              declared length of IC must be at least ICSIZE. See the
C              $Parameters section for details.
C
...
C     ray-surface intercept algorithm. These are discussed in the
...
C         the error SPICE(ARRAYTOOSMALL) is signaled.
C
C     7)  If NSURF is less than 0, the error SPICE(INVALIDCOUNT)
C         is signaled.
C
C     8)  If an error occurs during the intercept computation, the error
...
C
C
C        Use the meta-kernel shown below to load example SPICE
C        kernels.
C
C
C           KPL/MK
...
C           \begindata
C
C              KERNELS_TO_LOAD = ( 'phobos512.bds' )
C
C           \begintext
C
...
C
C        C
C        C     Prompt for the name of the meta-kernel.
C        C
...
C
C        C
C        C     Set the epoch for interval selection.
C        C
C              ET     = 0.D0
C
C              WRITE (*,*) ' '
...
C        platform, using as input the meta-kernel dskxsi_ex1.tm, the
C        output was:
C
C
C        Enter meta-kernel name >  dskxsi_ex1.tm
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 06-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Updated code example to prompt for input meta-kernel name and
C        set input time to zero.
C
