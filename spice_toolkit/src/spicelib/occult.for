...
C$ Abstract
C
C     Determine the occultation condition (not occulted, partially
C     occulted, etc.) of one target relative to another target as seen
C     by an observer at a given time.
C
C     The surfaces of the target bodies may be represented by triaxial
C     ellipsoids, points, or by topographic data provided by DSK files.
C
C$ Disclaimer
C
...
C         not recognized, an error is signaled by a routine in
C         the call tree of this routine.
C
C     2)  If the input shapes are not accepted, an error is signaled by
C         a routine in the call tree of this routine.
C
C     3)  If both input shapes are points, an error is signaled by a
...
C         OBSRVR) are the same, an error is signaled
C         by a routine in the call tree of this routine.
C
C     6)  If the loaded kernels provide insufficient data to compute any
...
C         of this routine.
C
C     8)  If the aberration correction specification ABCORR is invalid,
C         an error is signaled by a routine in the call tree of this
C         routine.
C
C     9)  If either SHAPE1 or SHAPE2 specifies that the target surface
C         is represented by DSK data, and no DSK files are loaded for
C         the specified target, an error is signaled by a routine in
C         the call tree of this routine.
C
C     10) If either SHAPE1 or SHAPE2 specifies that the target surface
...
C        instant specified by the argument ET. If aberration
...
C              earth_latest_high_prec.bpc       Earth latest binary PCK
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 26-OCT-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Extended
C        $Abstract description.
C
C        Edited meta-kernel and code example to comply with NAIF
C        standards for Example sections.
C
