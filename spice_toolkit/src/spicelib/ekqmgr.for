...
C
C     2)  See the headers of the entry points for descriptions of
...
C$ Version
C
C-    SPICELIB Version 2.2.0, 27-AUG-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header of EKQMGR umbrella routine and all its entry
C        points. Removed unnecessary $Revisions section.
C
...
C         returned.
C
C     3)  If a file open error occurs, the error is signaled by a
C         routine in the call tree of this routine. HANDLE is undefined
C         in this case.
C
C     4)  If loading the input file would cause the maximum number of
C         loaded EK files to be exceeded, the error
C         SPICE(EKFILETABLEFULL) is signaled. HANDLE is undefined in
...
C         loaded DAS files to be exceeded, an error is signaled by a
C         routine in the call tree of this routine. HANDLE is undefined
C         in this case. This routine will attempt to unload the file
C         from the DAS system.
C
...
C         SPICE(EKSEGMENTTABLEFULL) is signaled. HANDLE is undefined in
C         this case. This routine will attempt to unload the file from
C         the DAS system.
C
...
C         SPICE(EKCOLDESCTABLEFULL) is signaled. HANDLE is undefined in
C         this case. This routine will attempt to unload the file from
C         the DAS system.
C
C     8)  If loading the input file would cause the maximum allowed
C         number of columns having distinct attributes in loaded EK
C         files to be exceeded, the error SPICE(EKCOLATTRTABLEFULL) is
C         signaled. HANDLE is undefined in this case. This routine will
C         attempt to unload the file from the DAS system.
C
...
C         the error SPICE(EKIDTABLEFULL) is signaled. HANDLE is
C         undefined in this case. This routine will attempt to unload
C         the file from the DAS system.
C
C     10) If the input file does not contain at least one segment, the
C         error SPICE(EKNOSEGMENTS) is signaled.
C
C$ Files
C
...
C     specific arithmetic implementation.
C
C     1) Load two EK files and perform a query on them. During query
C        execution, all files will be searched.
C
C        Use the EK kernel below to load the Cassini Science Plan
C        SPICE E-Kernel File based upon the integrated science
C        plan #78.
C
C           S78_CIMSSSUPa.bep
C
C        Use the EK kernel below to load the data based upon the
C        integrated science plan #79.
C
C           S79_CIMSSSUPa.bep
C
C
...
C
C        C
C        C     Include the EK Maximum length of an input query,
C        C     MAXQRY.
C        C
C              INCLUDE 'ekqlimit.inc'
C
...
C
C        C
C        C     Set up the array holding the EK file names.
C        C
C              DATA                  EKNAMS / 'S78_CIMSSSUPa.bep',
C             .                               'S79_CIMSSSUPa.bep'  /
C
C        C
C        C     Load the EK files. This call could be replaced by a call
C        C     to FURNSH (in this case, a meta-kernel listing the EKs
C        C     to be loaded could also be used).
C        C
C              DO I = 1, 2
C
C                 CALL EKLEF ( EKNAMS(I), HANDLE )
C                 WRITE(*,'(2A)') 'Loading EK: ', EKNAMS(I)
C
C              END DO
C
C        C
C        C     The EK files contain a table 'CASSINI_SP_OBSERVATION',
C        C     that contains columns named:
C        C
C        C        NOTES, OBSERVATION_ID, OBSERVATION_TITLE,
C        C        OBS_DESCRIPTION, SCIENCE_OBJECTIVE, SEQUENCE,
C        C        SUBSYSTEM
C        C
C        C     Define a set of constraints to perform a query on all
C        C     loaded EK files (the SELECT clause).
C        C
C              QUERY = 'Select SUBSYSTEM, SCIENCE_OBJECTIVE, '
C             .   //   'OBSERVATION_ID from CASSINI_SP_OBSERVATION '
C             .   //   'order by SUBSYSTEM'
C
C        C
...
C              IF ( ERROR ) THEN
C
C                 WRITE(*,'(2A)') 'SELECT clause error: ', ERRMSG
C
C              ELSE
C
C        C
C        C        If no error, NMROWS contains the number of rows
C        C        matching the constraints specified in the query
C        C        string.
C        C
C                 WRITE(*,'(A,I3)') 'Number of matching rows: ', NMROWS
C
C              END IF
C
C
...
C
C
C        Loading EK: S78_CIMSSSUPa.bep
C        Loading EK: S79_CIMSSSUPa.bep
C
C        Number of matching rows:   9
C
C
C     2) Repeat the previous exercise, using the same input kernels,
C        but this time unloading the previous file before each new
C        file is loaded. Unloading files prevents them from being
C        searched during query execution.
C
C
C        Example code begins here.
C
...
C        C           string.
C        C
C                    WRITE(*,'(A,I3)') 'Number of matching rows: ',
C             .                        NMROWS
C
C                 END IF
C
C        C
C        C        Unload the current file. Unloading files prevents
C        C        them from being searched during query execution.
C        C
C                 CALL EKUEF ( HANDLE )
C                 WRITE(*,'(2A)') 'Unloading EK: ', EKNAMS(I)
C                 WRITE(*,*)
C
C              END DO
C
...
C
C        Loading EK: S78_CIMSSSUPa.bep
C        Number of matching rows:   4
C        Unloading EK: S78_CIMSSSUPa.bep
C
C        Loading EK: S79_CIMSSSUPa.bep
C        Number of matching rows:   5
C        Unloading EK: S79_CIMSSSUPa.bep
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 2.2.0, 06-JUL-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Added complete
C        code examples based on existing fragments.
C
C-    SPICELIB Version 2.1.0, 09-FEB-2015 (NJB)
C
C        Now uses ERRHAN to insert DAS file name into
...
C$ Files
C
C     This routine unloads a binary EK file from the EK query system.
C
C$ Particulars
C
...
C
C     1) Load two EK files and perform a query on them. During query
C        execution, all files will be searched. Unload the previous
C        file before each new file is loaded. Unloading files prevents
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 06-JUL-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard. Added complete
...
C     specific arithmetic implementation.
C
C     1) Suppose we have several EK files. Load one at a time and
C        display, right after, the number of loaded EK tables in the
C        system.
C
C        Use the EK kernel below to load the Cassini Science Plan
C        SPICE E-Kernel File based upon integrated science plan. This
C        kernel contains 3 tables.
C
C           S79_CIMSSSUPa.bep
C
C        Use the EK kernel below to load the Cassini Spacecraft
C        Sequence Status SPICE E-Kernel File based upon integrated
C        Predicted Events File. This kernel contains 1 table.
C
C           S79_status_pf.bes
C
C
...
C
C        C
C        C     Load the first EK.
C        C
C              CALL FURNSH ( 'S79_CIMSSSUPa.bep' )
C
C        C
C        C     Display the number of EK tables in the system after
C        C     the first EK file is loaded.
C        C
C              CALL EKNTAB ( N )
C              WRITE(*,'(A,I3)') 'EK tables in the system (1 EK):', N
C
C        C
C        C     Load the second EK.
C        C
C              CALL FURNSH ( 'S79_status_pf.bes' )
C
C        C
C        C     Display the number of EK tables in the system after
C        C     the second EK file is loaded.
C        C
C              CALL EKNTAB ( N )
C              WRITE(*,'(A,I3)') 'EK tables in the system (2 EK):', N
C
C              END
C
C
...
C
C
C        EK tables in the system (1 EK):  3
C        EK tables in the system (2 EK):  4
C
C
C$ Restrictions
C
...
C
C        Removed the requirement of loading the files via EKELF from the
C        $Files section.
C
C-    SPICELIB Version 2.0.0, 16-NOV-2001 (NJB)
C
...
C$ Detailed_Output
C
C     TABLE    is the name of the N'th loaded table. If TABLE
C              is too small to accommodate the name, the name will
C              be truncated on the right.
C
C$ Parameters
C
C     TNAMSZ   is the maximum allowed table name length. See the
C              include file ektnamsz.inc for the actual value of
C              this parameter.
C
C$ Exceptions
C
...
C     specific arithmetic implementation.
C
C     1) Dump the names of all the loaded tables.
C
...
C              ---------                 --------
C              S78_CIMSSSUPa.bep         Cassini Science Plan #78
C              S79_CIMSSSUPa.bep         Cassini Science Plan #79
C              S79_status_pf.bes         Cassini Spacecraft Sequence
C                                        Status #79
C
C
C           \begindata
C
C              KERNELS_TO_LOAD = ( 'S78_CIMSSSUPa.bep',
C                                  'S79_CIMSSSUPa.bep',
C                                  'S79_status_pf.bes'  )
C
C           \begintext
...
C
C
C              PROGRAM EKTNAM_EX1
C              IMPLICIT NONE
C
C        C
C        C     Include EK parameter declarations:
C        C
C        C        EK Table Name Size
C        C
...
C        C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'ektnam_ex1.tm' )
C
C        C
C        C     Local variables.
C        C
...
C
C        C
C        C     Load the EK files. Use a meta-kernel for convenience.
C        C
C              CALL FURNSH ( META )
C
C        C
C        C     Get the number of loaded tables. The count refers to the
C        C     number of logical tables; if multiple EKs contain data
C        C     for the same table, these EKs collectively contribute
C        C     only one table to the count.
C        C
C              CALL EKNTAB ( NTAB )
C
C              WRITE(*,'(A,I3)') 'Number of tables in EK subsystem:',
C             .                   NTAB
C
C              DO TAB = 1, NTAB
C
C        C
C        C        Get the name of the current table, and display it.
C        C
C                 CALL EKTNAM ( TAB, TABNAM )
C                 WRITE(*,'(2A)') '   TABLE = ', TABNAM
C
C              END DO
C
C              END
...
C
C
C        Number of tables in EK subsystem:  4
C           TABLE = CASSINI_SP_REQUEST
C           TABLE = CASSINI_SP_OBSERVATION
C           TABLE = CASSINI_SP_REQ_OBS
C           TABLE = CASSINI_STATUS
C
C
C$ Restrictions
C
...
C        code example.
C
C        Added description of TNAMSZ parameter.
C
C-    SPICELIB Version 2.0.0, 16-NOV-2001 (NJB)
C
C        Bug fix: When an already loaded kernel is opened with EKOPR,
...
C     specific arithmetic implementation.
C
C     1) Examine an EK. Dump the names and attributes of the columns in
C        each loaded table. EKCCNT is used to obtain column counts.
C
C
C        Example code begins here.
C
C
C              PROGRAM EKCCNT_EX1
C              IMPLICIT NONE
C
C        C
C        C     Include EK parameter declarations:
C        C
C        C        ekattdsc.inc: EK Column Attribute Descriptor
C        C                      Parameters
C        C        ekcnamsz.inc: EK Column Name Size
C        C        ektnamsz.inc: EK Table Name Size
C        C        ektype.inc:   EK Data Types
C        C
C              INCLUDE 'ekattdsc.inc'
C              INCLUDE 'ekcnamsz.inc'
C              INCLUDE 'ektnamsz.inc'
C              INCLUDE 'ektype.inc'
C
C        C
C        C     Local parameters.
C        C
...
C        C     Local variables.
C        C
C              CHARACTER*(CNAMSZ)      COLNAM
C              CHARACTER*(FILEN)       EKFILE
C              CHARACTER*(TNAMSZ)      TABNAM
C
...
C
C        C
C        C     Prompt for the EK file name.
C        C
C              CALL PROMPT ( 'Enter name of EK to examine > ', EKFILE )
C
C              CALL FURNSH ( EKFILE )
C
C        C
...
C              CALL EKNTAB ( NTAB )
C
C              WRITE(*,*) 'Number of tables in EK:', NTAB
C
C              DO TAB = 1, NTAB
C
C        C
...
C        C
C                    WRITE (*,'(A,I2)') '    SIZE   = ', ATTDSC(ATTSIZ)
C
C        C
...
C
C        When this program was executed on a Mac/Intel/gfortran/64-bit
C        platform, using the EK named S79_CIMSSSUPa.bep to load the
C        Cassini Science Plan SPICE E-Kernel File based upon the
C        integrated science plan, the output was:
C
C
C        Enter name of EK to examine > S79_CIMSSSUPa.bep
C         Number of tables in EK:           3
C         ------------------------------------------------------------
C         TABLE = CASSINI_SP_REQUEST
C
C         COLUMN = SUBSYSTEM
...
C         COLUMN = REQUEST_ID
C            TYPE   =  CHR
C            STRING LENGTH = VARIABLE.
C            SIZE   =  1
C            INDEXED
C            NULL VALUES NOT ALLOWED
C         COLUMN = REQUEST_TITLE
C            TYPE   =  CHR
C            STRING LENGTH = VARIABLE.
C            SIZE   =  1
C            INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = BEGIN_TIME
C            TYPE   =  TIME
C            SIZE   =  1
C            INDEXED
...
C            INDEXED
C            NULL VALUES NOT ALLOWED
C         COLUMN = POINTING_AGREEMENT
C            TYPE   =  CHR
C            STRING LENGTH = 80
C            SIZE   = -1
C            NOT INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = PRIMARY_POINTING
C            TYPE   =  CHR
C            STRING LENGTH = VARIABLE.
C            SIZE   =  1
C            INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = SECONDARY_POINTING
C            TYPE   =  CHR
C            STRING LENGTH = VARIABLE.
C            SIZE   =  1
C            INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = REQ_DESCRIPTION
C            TYPE   =  CHR
C            STRING LENGTH = 80
C            SIZE   = -1
C            NOT INDEXED
...
C            INDEXED
C            NULL VALUES NOT ALLOWED
C         COLUMN = OBSERVATION_ID
C            TYPE   =  CHR
C            STRING LENGTH = VARIABLE.
C            SIZE   =  1
C            INDEXED
C            NULL VALUES NOT ALLOWED
C         COLUMN = OBSERVATION_TITLE
C            TYPE   =  CHR
C            STRING LENGTH = VARIABLE.
C            SIZE   =  1
C            INDEXED
...
C            INDEXED
C            NULL VALUES NOT ALLOWED
C         COLUMN = SCIENCE_OBJECTIVE
C            TYPE   =  CHR
C            STRING LENGTH = 80
C            SIZE   = -1
C            NOT INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = OBS_DESCRIPTION
C            TYPE   =  CHR
C            STRING LENGTH = 80
C            SIZE   = -1
C
...
C
C
C        Warning: incomplete output. Only 100 out of 129 lines have been
C        provided.
C
C
C$ Restrictions
...
C         SPICE(TABLENOTLOADED) is signaled.
C
C     2)  If the input argument CINDEX is out of range, the error
...
C
C        When this program was executed on a Mac/Intel/gfortran/64-bit
C        platform, using the EK file named vo_sedr.bdb to load the
C        Viking Orbiter Image SEDR Data, the output was:
C
C
C        Enter name of EK to examine > vo_sedr.bdb
C         Number of tables in EK:           1
C         ------------------------------------------------------------
C         TABLE = VIKING_SEDR_DATA
C
C         COLUMN = IMAGE_ID
C            TYPE   =  CHR
C            STRING LENGTH =  6
...
C            INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = IMAGE_NUMBER
C            TYPE   =  INT
C            SIZE   =  1
C            INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = SPACECRAFT_ID
C            TYPE   =  CHR
C            STRING LENGTH =  3
C            SIZE   =  1
C            INDEXED
...
C            INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = INSTRUMENT_ID
C            TYPE   =  CHR
C            STRING LENGTH =  4
C            SIZE   =  1
C            INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = GAIN_MODE_ID
C            TYPE   =  CHR
C            STRING LENGTH =  4
C            SIZE   =  1
C            NOT INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = FLOOD_MODE_ID
C            TYPE   =  CHR
C            STRING LENGTH =  3
C            SIZE   =  1
C            NOT INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = OFFSET_MODE_ID
C            TYPE   =  CHR
C            STRING LENGTH =  3
C            SIZE   =  1
C            NOT INDEXED
...
C            INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = EXPOSURE_DURATION
C            TYPE   =  DP
C            SIZE   =  1
C            INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = PLATFORM_IN_MOTION
C            TYPE   =  CHR
C            STRING LENGTH =  3
C            SIZE   =  1
C            NOT INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = PLATFORM_CONE
C            TYPE   =  DP
C            SIZE   =  1
C            NOT INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = PLATFORM_CLOCK
C            TYPE   =  DP
C            SIZE   =  1
C            NOT INDEXED
C            NULL VALUES ALLOWED
C         COLUMN = PLATFORM_TWIST
C            TYPE   =  DP
C            SIZE   =  1
C            NOT INDEXED
C            NULL VALUES ALLOWED
...
C        Added IMPLICIT NONE statement.
C
C        Edited header to comply with NAIF standard.
C        Added complete code example from existing code fragment.
C
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 27-AUG-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
...
C     specific arithmetic implementation.
C
C     1) This example demonstrates how to fetch integer, double
C        precision and character string values from a column when such
C        column corresponds to either a variable-size array or to a
C        static-size array.
C
...
C        C
C              CHARACTER*(*)         EKNAME
C              PARAMETER           ( EKNAME  = 'eknelt_ex1.bdb' )
C
C              CHARACTER*(*)         TABLE
C              PARAMETER           ( TABLE   = 'TAB' )
C
C              INTEGER               CHRSLN
C              PARAMETER           ( CHRSLN = 5   )
C
C              INTEGER               COL1SZ
C              PARAMETER           ( COL1SZ = 3   )
C
C              INTEGER               DECLEN
...
C              CHARACTER*(CHRSLN)    COL3   ( MXC3SZ )
C              CHARACTER*(STRSIZ)    CVALS  ( MXC3SZ )
C              CHARACTER*(ERRLEN)    ERRMSG
C              CHARACTER*(NAMLEN)    IFNAME
C              CHARACTER*(MAXQRY)    QUERY
C
C              DOUBLE PRECISION      COL2   ( MXC2SZ )
C              DOUBLE PRECISION      DVALS  ( MXC2SZ )
C
C              INTEGER               COL1   ( COL1SZ )
C              INTEGER               ELTIDX
C              INTEGER               HANDLE
C              INTEGER               I
C              INTEGER               IVALS ( COL1SZ )
C              INTEGER               J
C              INTEGER               NELT
C              INTEGER               NMROWS
C              INTEGER               NRESVC
...
C        C
C              CNAMES(1) = 'IARRAY'
C              CDECLS(1) = 'DATATYPE = INTEGER, SIZE = 3'
C
C              CNAMES(2) = 'DARRAY'
C              CDECLS(2) = 'DATATYPE = DOUBLE PRECISION, ' //
C             .            'SIZE = VARIABLE'
C
C              CNAMES(3) = 'CARRAY'
C              CDECLS(3) = 'DATATYPE = CHARACTER*(5), ' //
C             .            'SIZE = VARIABLE'
C
C        C
...
C             .                 CNAMES(1), COL1SZ, COL1,  .FALSE. )
C
C        C
C        C        Add I items to DARRAY
C        C
C                 DO J = 1, I
C                    COL2(J) = J + I*200.D0
C                 END DO
C
C                 CALL EKACED ( HANDLE,    SEGNO, RECNO,
C             .                 CNAMES(2), I,     COL2,  .FALSE. )
C
C        C
C        C        Add 3+I items to CARRAY
C        C
C                 DO J = 1, 3+I
C                    CALL REPMI ( 'ST#', '#', J + I*100, COL3(J) )
C                 END DO
C
C                 CALL EKACEC ( HANDLE,    SEGNO, RECNO,
C             .                 CNAMES(3), I+3,   COL3,  .FALSE. )
C
C              END DO
C
C        C
...
C                    IF ( ISNULL ) THEN
C
C                       WRITE(*,'(A)') '   COLUMN = IARRAY: <Null>'
C
C                    ELSE
C
C                       WRITE(*,'(A,3I6)') '   COLUMN = IARRAY:',
C             .                            ( IVALS(I), I = 1, COL1SZ )
C
C                    END IF
C
C        C
...
C                    IF ( ISNULL ) THEN
C
C                       WRITE(*,'(A)') '   COLUMN = DARRAY: <Null>'
C
C                    ELSE
C
C                       WRITE(*,'(A,4F6.1)') '   COLUMN = DARRAY:',
C             .                              ( DVALS(I), I = 1, NELT )
C
C                    END IF
C
...
C                    IF ( ISNULL ) THEN
C
C                       WRITE(*,'(A)') '   COLUMN = CARRAY: <Null>'
C
C                    ELSE
C
C                       WRITE(*,'(8A)') '   COLUMN = CARRAY: ',
C             .                     ( CVALS(I)(:RTRIM(CVALS(I))) //' ',
C             .                                         I = 1, NELT   )
C
C                    END IF
C
...
C
C        ROW  =   1
C           COLUMN = IARRAY:   101   201   301
C           COLUMN = DARRAY: 201.0
C           COLUMN = CARRAY: ST101 ST102 ST103 ST104
C
C        ROW  =   2
C           COLUMN = IARRAY:   102   202   302
C           COLUMN = DARRAY: 401.0 402.0
C           COLUMN = CARRAY: ST201 ST202 ST203 ST204 ST205
C
C        ROW  =   3
C           COLUMN = IARRAY:   103   203   303
C           COLUMN = DARRAY: 601.0 602.0 603.0
C           COLUMN = CARRAY: ST301 ST302 ST303 ST304 ST305 ST306
C
C        ROW  =   4
C           COLUMN = IARRAY:   104   204   304
C           COLUMN = DARRAY: 801.0 802.0 803.0 804.0
C           COLUMN = CARRAY: ST401 ST402 ST403 ST404 ST405 ST406 ST407
C
C
...
C$ Exceptions
C
C     1)  If the input argument ELMENT is less than 1, the error
C         SPICE(INVALIDINDEX) is signaled and FOUND is returned .FALSE.
...
C     2)  If SELIDX is outside of the range established by the
C         last query passed to EKSRCH, the error SPICE(INVALIDINDEX)
C         is signaled and FOUND is returned .FALSE.
C
C     3)  If the input argument ROW is less than 1 or greater than the
C         number of rows matching the query, the error
C         SPICE(INVALIDINDEX) is signaled and FOUND is returned .FALSE.
C
...
C     specific arithmetic implementation.
C
C     1) Perform a query on an EK file that contains a database with
C        the Supplementary Engineering Data Records of the Viking
C        Project in order to retrieve the IMAGE_ID values (character
C        strings) that correspond to the images with IMAGE_NUMBER
C        smaller than a given value, ordered by IMAGE_NUMBER.
C
C
C        Use the EK kernel below to load the information from the
C        original Supplementary Engineering Data Record (SEDR) data
...
C        C
C        C     Include the EK Maximum length of an input query,
C        C     MAXQRY, and the maximum length of literal string
C        C     values, MAXSTR, from eklimit.inc.
C        C
C              INCLUDE 'ekqlimit.inc'
C
C        C
...
C
C        C
C        C     The table 'VIKING_SEDR_DATA' has a column 'IMAGE_ID'
C        C     of scalar strings.
C        C
C        C     Define a set of constraints to perform a query on
C        C     all loaded EK files (the SELECT clause). In this
C        C     case select the column 'IMAGE_ID' from table
C        C     'VIKING_SEDR_DATA' sorted by 'IMAGE_NUMBER'.
C        C
C              QUERY = 'Select IMAGE_ID from VIKING_SEDR_DATA '
C             .   //   'where IMAGE_NUMBER < 25860000 '
C             .   //   'order by IMAGE_NUMBER'
C
C        C
C        C     Query the EK system for data rows matching the
...
C
C        C
C        C        Fetch the character data. We know the query returned
C        C        one column and the column contains only scalar data,
C        C        so the index of all elements is 1.
C        C
C                 SELIDX = 1
C                 ELTIDX = 1
C
C        C
C        C        Loop over each row found matching the query.
C        C
C                 DO ROWNO = 1, NMROWS
C
C        C
C        C           Use EKGC to retrieve the string from
C        C
C                    CALL EKGC ( SELIDX,  ROWNO,  ELTIDX,
C             .                  CDATA,   ISNULL, FOUND   )
C
C                    IF ( ISNULL ) THEN
C
C                       WRITE (*,'(A,I3,A)') 'Row ', ROWNO,
C             .                  ': Character data: <Null>'
C
C                    ELSE
C
C                       WRITE (*,'(A,I3,2A)') 'Row ', ROWNO,
C             .                  ': Character data: ', CDATA
C                    END IF
C
C                 END DO
C
...
C
C
C        Row   1: Character data: 168C09
C        Row   2: Character data: 168C10
C        Row   3: Character data: 168C11
C        Row   4: Character data: 168C12
C        Row   5: Character data: 169C01
C        Row   6: Character data: 169C02
C        Row   7: Character data: 169C03
C        Row   8: Character data: 169C04
C        Row   9: Character data: 169C05
C        Row  10: Character data: 169C09
C        Row  11: Character data: 169C11
C        Row  12: Character data: 169C19
C        Row  13: Character data: 169C23
C        Row  14: Character data: 169C25
C        Row  15: Character data: 169C26
C        Row  16: Character data: 169C30
C        Row  17: Character data: 169C32
C        Row  18: Character data: 169C33
C        Row  19: Character data: 169C37
C        Row  20: Character data: 169C39
C        Row  21: Character data: 169C40
C        Row  22: Character data: 169C44
C        Row  23: Character data: 169C46
C        Row  24: Character data: 169C47
C        Row  25: Character data: 169C51
C        Row  26: Character data: 169C53
C
C
C     2) This example demonstrates how to fetch string values from a
C        column in three different cases: single values, variable-size
C        arrays and static-size arrays.
C
C        Create an EK that contains a table TAB that has the following
C        columns:
C
...
C        C
C              CHARACTER*(*)         EKNAME
C              PARAMETER           ( EKNAME  = 'ekgc_ex2.bdb' )
C
C              CHARACTER*(*)         TABLE
C              PARAMETER           ( TABLE   = 'TAB' )
C
C              INTEGER               CHRSLN
C              PARAMETER           ( CHRSLN = 3   )
C
C              INTEGER               COL3SZ
...
C              CHARACTER*(DECLEN)    CDECLS ( NCOLS  )
C              CHARACTER*(CNAMSZ)    CNAMES ( NCOLS  )
C              CHARACTER*(STRSIZ)    COL1
C              CHARACTER*(CHRSLN)    COL2   ( MXC2SZ )
C              CHARACTER*(CHRSLN)    COL3   ( COL3SZ )
C              CHARACTER*(STRSIZ)    CVALS  ( MXC2SZ )
C              CHARACTER*(ERRLEN)    ERRMSG
C              CHARACTER*(NAMLEN)    IFNAME
C              CHARACTER*(MAXQRY)    QUERY
C
...
C              CDECLS(2) = 'DATATYPE = CHARACTER*(3), ' //
C             .            'SIZE = VARIABLE'
C
C              CNAMES(3) = 'CHR_COL_3'
C              CDECLS(3) = 'DATATYPE = CHARACTER*(3), ' //
C             .            'SIZE = 3'
C
C        C
C        C     Start the segment.
...
C        C        Add CHR_COL_1
C        C
C                 CALL REPMI ( 'Column #2 has $ elements.',
C             .                '$', I*2, COL1              )
C
C                 CALL EKACEC ( HANDLE,    SEGNO, RECNO,
...
C
C        C
C        C        Add I*2 items to CHR_COL_2
C        C
C                 DO J = 1, I*2
C                    CALL INTSTR( J + I*100, COL2(J) )
C                 END DO
C
C                 CALL EKACEC ( HANDLE,    SEGNO, RECNO,
C             .                 CNAMES(2), I*2,   COL2, .FALSE. )
C
C        C
C        C        Add 3 items to CHR_COL_3
C        C
C                 DO J = 1, 3
C                    CALL INTSTR( I + J*100, COL3(J) )
C                 END DO
C
C                 CALL EKACEC ( HANDLE,    SEGNO, RECNO,
...
C                    IF ( ISNULL ) THEN
C
C                       WRITE(*,*) '  COLUMN = CHR_COL_1: <Null>'
C
C                    ELSE
C
C                       WRITE(*,*) '  COLUMN = CHR_COL_1: ', CVALS(1)
C
C                    END IF
C
C        C
...
C
C        ROW  =   1
C           COLUMN = CHR_COL_1: Column #2 has 2 elements.
C           COLUMN = CHR_COL_2: 101 102
C           COLUMN = CHR_COL_3: 101 201 301
C
C        ROW  =   2
C           COLUMN = CHR_COL_1: Column #2 has 4 elements.
C           COLUMN = CHR_COL_2: 201 202 203 204
C           COLUMN = CHR_COL_3: 102 202 302
C
C        ROW  =   3
C           COLUMN = CHR_COL_1: Column #2 has 6 elements.
C           COLUMN = CHR_COL_2: 301 302 303 304 305 306
C           COLUMN = CHR_COL_3: 103 203 303
C
C        ROW  =   4
C           COLUMN = CHR_COL_1: Column #2 has 8 elements.
C           COLUMN = CHR_COL_2: 401 402 403 404 405 406 407 408
C           COLUMN = CHR_COL_3: 104 204 304
C
C
C        Note that after run completion, a new EK file exists in the
C        output directory.
C
C
C     3) See $Examples in EKQMGR.
C
C        In this example, the names and data types of the columns from
C        which to fetch data are not known in advance.
C
C$ Restrictions
C
C     None.
...
C     1) Perform a query on an EK file that contains a database with
C        the Supplementary Engineering Data Records of the Viking
C        Project in order to retrieve the PLATFORM_CLOCK values (double
C        precision) that correspond to the images with IMAGE_NUMBER
C        smaller than a given value, ordered by IMAGE_NUMBER.
C
C
C        Use the EK kernel below to load the information from the
...
C
C        C
C        C     The table 'VIKING_SEDR_DATA' has a column
C        C     'PLATFORM_CLOCK' of double precision values.
C        C
C        C     Define a set of constraints to perform a query on
C        C     all loaded EK files (the SELECT clause). In this
C        C     case select the column 'PLATFORM_CLOCK' from table
C        C     'VIKING_SEDR_DATA' sorted by 'IMAGE_NUMBER'.
C        C
C              QUERY = 'Select PLATFORM_CLOCK from VIKING_SEDR_DATA '
C             .   //   'where IMAGE_NUMBER < 25860000 '
C             .   //   'order by IMAGE_NUMBER'
C
C        C
...
C
C        C
C        C           Use EKGD to retrieve the string from
C        C
C                    CALL EKGD ( SELIDX, ROWNO,  ELTIDX,
C             .                  DDATA,  ISNULL, FOUND   )
C
C                    IF ( ISNULL ) THEN
C
C                       WRITE (*,'(A,I3,A)') 'Row ', ROWNO,
C             .                  ': Double precision data: <Null>'
C
C                    ELSE
C
C                       WRITE (*,'(A,I3,A,F10.6)') 'Row ', ROWNO,
C             .                  ': Double precision data: ', DDATA
C
C                    END IF
C
C                 END DO
...
C
C
C        Row   1: Double precision data: 119.880000
C        Row   2: Double precision data: 119.270000
C        Row   3: Double precision data: 119.880000
C        Row   4: Double precision data: 119.270000
C        Row   5: Double precision data: 119.880000
C        Row   6: Double precision data: 119.270000
C        Row   7: Double precision data: 120.140000
C        Row   8: Double precision data: 119.520000
C        Row   9: Double precision data: 120.140000
C        Row  10: Double precision data: 120.140000
C        Row  11: Double precision data: 120.140000
C        Row  12: Double precision data: 221.920000
C        Row  13: Double precision data: 221.920000
C        Row  14: Double precision data: 221.920000
C        Row  15: Double precision data: 120.140000
C        Row  16: Double precision data: 120.140000
C        Row  17: Double precision data: 120.140000
C        Row  18: Double precision data: 120.220000
C        Row  19: Double precision data: 120.220000
C        Row  20: Double precision data: 120.220000
C        Row  21: Double precision data: 120.370000
C        Row  22: Double precision data: 120.370000
C        Row  23: Double precision data: 120.370000
C        Row  24: Double precision data: 120.290000
C        Row  25: Double precision data: 120.290000
C        Row  26: Double precision data: 120.290000
C
C
C     2) Perform a query on an EK file that contains a database with
C        the Supplementary Engineering Data Records of the Viking
C        Project in order to retrieve the IMAGE_TIME values (double
C        precision time) that correspond to the images with
C        IMAGE_NUMBER smaller than a given value, ordered by
C        IMAGE_NUMBER.
C
C
C        Use the EK kernel below to load the information from the
...
C        C
C        C     The table 'VIKING_SEDR_DATA' has a column
C        C     'IMAGE_TIME' of time values (stored as double
C        C     precision items).
C        C
C        C     Define a set of constraints to perform a query on
C        C     all loaded EK files (the SELECT clause). In this
C        C     case select the column 'IMAGE_TIME' from table
C        C     'VIKING_SEDR_DATA' sorted by 'IMAGE_NUMBER'.
C        C
C              QUERY = 'Select IMAGE_TIME from VIKING_SEDR_DATA '
C             .   //   'where IMAGE_NUMBER < 25860000 '
C             .   //   'order by IMAGE_NUMBER'
C
C        C
...
C                    ELSE
C
C                       CALL ET2UTC ( DDATA, 'C', 3, UTCSTR )
C
C                       WRITE (*,'(A,I3,2A)') 'Row ', ROWNO,
C             .                               ': Time data:  ', UTCSTR
C
...
C
C
C        Row   1: Time data:  1976 JUN 16 16:50:55.925
C        Row   2: Time data:  1976 JUN 16 16:51:00.269
C        Row   3: Time data:  1976 JUN 16 20:56:53.051
C        Row   4: Time data:  1976 JUN 16 20:56:57.395
C        Row   5: Time data:  1976 JUN 17 01:02:50.177
C        Row   6: Time data:  1976 JUN 17 01:02:54.521
C        Row   7: Time data:  1976 JUN 17 05:08:56.263
C        Row   8: Time data:  1976 JUN 17 05:09:00.607
C        Row   9: Time data:  1976 JUN 17 06:30:28.424
C        Row  10: Time data:  1976 JUN 17 06:30:46.174
C        Row  11: Time data:  1976 JUN 17 06:30:55.168
C        Row  12: Time data:  1976 JUN 17 11:17:47.471
C        Row  13: Time data:  1976 JUN 17 11:18:05.221
C        Row  14: Time data:  1976 JUN 17 11:18:14.215
C        Row  15: Time data:  1976 JUN 17 13:20:23.634
C        Row  16: Time data:  1976 JUN 17 13:20:41.384
C        Row  17: Time data:  1976 JUN 17 13:20:50.378
C        Row  18: Time data:  1976 JUN 17 15:23:17.717
C        Row  19: Time data:  1976 JUN 17 15:23:35.467
C        Row  20: Time data:  1976 JUN 17 15:23:44.461
C        Row  21: Time data:  1976 JUN 17 17:26:20.760
C        Row  22: Time data:  1976 JUN 17 17:26:38.510
C        Row  23: Time data:  1976 JUN 17 17:26:47.504
C        Row  24: Time data:  1976 JUN 17 19:29:23.803
C        Row  25: Time data:  1976 JUN 17 19:29:41.553
C        Row  26: Time data:  1976 JUN 17 19:29:50.547
C
C
C     3) This example demonstrates how to fetch double precision values
C        from a column in three different cases: single values,
...
C        C
C              CHARACTER*(*)         EKNAME
C              PARAMETER           ( EKNAME  = 'ekgd_ex3.bdb' )
C
C              CHARACTER*(*)         TABLE
C              PARAMETER           ( TABLE   = 'TAB' )
C
...
C              CHARACTER*(MAXQRY)    QUERY
C
C              DOUBLE PRECISION      COL1
C              DOUBLE PRECISION      COL2   ( MXC2SZ )
C              DOUBLE PRECISION      COL3   ( COL3SZ )
C              DOUBLE PRECISION      DVALS  ( MXC2SZ )
C
C              INTEGER               ELTIDX
C              INTEGER               HANDLE
...
C        C
C              CNAMES(1) = 'DP_COL_1'
C              CDECLS(1) = 'DATATYPE = DOUBLE PRECISION, ' //
C             .            'INDEXED  = TRUE'
C
C              CNAMES(2) = 'DP_COL_2'
C              CDECLS(2) = 'DATATYPE = DOUBLE PRECISION, ' //
...
C
C              CNAMES(3) = 'DP_COL_3'
C              CDECLS(3) = 'DATATYPE = DOUBLE PRECISION, ' //
C             .            'SIZE = 3'
C
C        C
C        C     Start the segment.
...
C        C           DP_COL_1 was the first column selected, the
C        C           selection index SELIDX is set to 1.
C        C
C                    SELIDX = 1
C                    ELTIDX = 1
...
C                    IF ( ISNULL ) THEN
C
C                       WRITE(*,*) '  COLUMN = DP_COL_1: <Null>'
C
C                    ELSE
C
C                       WRITE(*,*) '  COLUMN = DP_COL_1:', DVALS(1)
C
C                    END IF
C
C        C
...
C                    IF ( ISNULL ) THEN
C
C                       WRITE(*,*) '  COLUMN = DP_COL_2: <Null>'
C
C                    ELSE
C
C                       WRITE(*,'(A,4F6.1)') '   COLUMN = DP_COL_2:',
C             .                              ( DVALS(I), I = 1, NELT )
C
C                    END IF
C
...
C                    IF ( ISNULL ) THEN
C
C                       WRITE(*,*) '  COLUMN = DP_COL_3: <Null>'
C
C                    ELSE
C
C                       WRITE(*,'(A,3F6.1)') '   COLUMN = DP_COL_3:',
C             .                              ( DVALS(I), I = 1, COL3SZ )
C
C                    END IF
C
C                 END DO
...
C
C        ROW  =   1
C           COLUMN = DP_COL_1:   100.00000000000000
C           COLUMN = DP_COL_2: 201.0
C           COLUMN = DP_COL_3: 101.0 201.0 301.0
C
C        ROW  =   2
C           COLUMN = DP_COL_1:   200.00000000000000
C           COLUMN = DP_COL_2: 401.0 402.0
C           COLUMN = DP_COL_3: 102.0 202.0 302.0
C
C        ROW  =   3
C           COLUMN = DP_COL_1:   300.00000000000000
C           COLUMN = DP_COL_2: 601.0 602.0 603.0
C           COLUMN = DP_COL_3: 103.0 203.0 303.0
C
C        ROW  =   4
C           COLUMN = DP_COL_1:   400.00000000000000
C           COLUMN = DP_COL_2: 801.0 802.0 803.0 804.0
C           COLUMN = DP_COL_3: 104.0 204.0 304.0
C
C
C        Note that after run completion, a new EK file exists in the
C        output directory.
C
C     4) See $Examples in EKQMGR.
C
C        In this example, the names and data types of the columns from
C        which to fetch data are not known in advance.
C
...
C     1) Perform a query on an EK file that contains a database with
C        the Supplementary Engineering Data Records of the Viking
C        Project in order to retrieve the IMAGE_NUMBER values (integer
C        numbers) that correspond to the images with IMAGE_NUMBER
C        smaller than a given value, ordered by IMAGE_NUMBER.
C
C
C        Use the EK kernel below to load the information from the
...
C        C
C        C     The table 'VIKING_SEDR_DATA' has a column
C        C     'IMAGE_NUMBER' of double precision values.
C        C
C        C     Define a set of constraints to perform a query on
C        C     all loaded EK files (the SELECT clause). In this
C        C     case select the column 'IMAGE_NUMBER' from table
C        C     'VIKING_SEDR_DATA' sorted by 'IMAGE_NUMBER'.
C        C
C              QUERY = 'Select IMAGE_NUMBER from VIKING_SEDR_DATA '
C             .   //   'where IMAGE_NUMBER < 25860000 '
C             .   //   'order by IMAGE_NUMBER'
C
C        C
...
C        C           Use EKGD to retrieve the string from
C        C
C                    CALL EKGI ( SELIDX, ROWNO,  ELTIDX,
C             .                  IDATA,  ISNULL, FOUND   )
C
C                    IF ( ISNULL ) THEN
C
C                       WRITE (*,'(A,I3,A)') 'Row ', ROWNO,
...
C                    ELSE
C
C                       WRITE (*,'(A,I3,A,I10)') 'Row ', ROWNO,
C             .                  ': Integer data: ', IDATA
C
C                    END IF
C
C                 END DO
...
C
C
C        Row   1: Integer data:   25837050
C        Row   2: Integer data:   25837051
C        Row   3: Integer data:   25840344
C        Row   4: Integer data:   25840345
C        Row   5: Integer data:   25843638
C        Row   6: Integer data:   25843639
C        Row   7: Integer data:   25846934
C        Row   8: Integer data:   25846935
C        Row   9: Integer data:   25848026
C        Row  10: Integer data:   25848030
C        Row  11: Integer data:   25848032
C        Row  12: Integer data:   25851874
C        Row  13: Integer data:   25851878
C        Row  14: Integer data:   25851880
C        Row  15: Integer data:   25853516
C        Row  16: Integer data:   25853520
C        Row  17: Integer data:   25853522
C        Row  18: Integer data:   25855162
C        Row  19: Integer data:   25855166
C        Row  20: Integer data:   25855168
C        Row  21: Integer data:   25856810
C        Row  22: Integer data:   25856814
C        Row  23: Integer data:   25856816
C        Row  24: Integer data:   25858458
C        Row  25: Integer data:   25858462
C        Row  26: Integer data:   25858464
C
C
C     2) This example demonstrates how to fetch integer values
C        from a column in three different cases: single values,
C        variable-size arrays and static-size arrays.
C
C        Create an EK that contains a table TAB that has the following
...
C        C
C              CHARACTER*(*)         EKNAME
C              PARAMETER           ( EKNAME  = 'ekgi_ex2.bdb' )
C
C              CHARACTER*(*)         TABLE
C              PARAMETER           ( TABLE   = 'TAB' )
C
...
C-    SPICELIB Version 2.0.1, 22-SEP-2004 (EDW)
C
C        Edited version 1.1.0 entry to not include
