...
C
C
C              ROOT1 and ROOT2 are both arrays of length 2. The first
C              element of each array is the real part of a root; the
C              second element contains the complex part of the same
C              root.
C
...
C
C
C              When A is zero and B is non-zero, ROOT1 and ROOT2 both
C              represent the root
C
C                 - C / B.
C
C$ Parameters
...
C         SPICE(DEGENERATECASE) is signaled. The output arguments
C         are not modified.
C
C$ Files
C
...
C             r   +  r  -  1  =  0.
C
C          The following code fragment does the job.
C
C
C             C
...
C$ Restrictions
C
C     1)  No checks for overflow of the roots are performed.
C
