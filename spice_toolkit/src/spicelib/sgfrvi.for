...
C$ Abstract
C
C     Find the reference value associated with the value X and its 
C     index in a generic segment. The segment is identified by a DAF 
C     file handle and segment descriptor.
C
C$ Disclaimer
C
...
C              X.
C
C     X        is a value for which the associated reference value
...
C$ Exceptions
C
C     1)  If the reference directory structure is unrecognized, the
C         error SPICE(UNKNOWNREFDIR) is signaled. The most likely cause
C         of this error is that an upgrade to your version of the SPICE
C         toolkit is needed.
C
...
C         the error SPICE(INDEXTOOLARGE) is signaled.
C
C$ Files
C
...
C$ Restrictions
C
C     1)  The segment described by DESCR MUST be a generic segment,
...
C$ Version
C
C-    SPICELIB Version 1.2.1, 26-OCT-2021 (JDR)
C
...
C
     ...
     .   //            ' anomaly is your version of SPICELIB needs'
...
C        sure, based on the type of index we have.
C
...
C              want or the reference value immediately preceding it
...
C           is not a reference directory value, and the second when
C           it is.
C
            IF ( .NOT. ISDIRV ) THEN
C
