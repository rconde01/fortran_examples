...
C$ Exceptions
C
C     1)  If ZZSGP4 is called, the error SPICE(BOGUSENTRY) is signaled.
C
C$ Files
C
C     None.
...
C
C        Correction of documentation error in listing of GEOPHS
C        constants. The definition of index 5, "High altitude bound
C        for atmospheric model in km.", and index 6, "Low altitude
C        bound for atmospheric model in km." were switched.
C
C-    SPICELIB Version 1.0.0, 15-SEP-2014 (EDW)
C
...
C$ Exceptions
C
C     1)  If radius of perigee has value less-than 1, the error
C         SPICE(SUBORBITAL) is signaled.
C
C$ Files
C
C     None.
...
C        bound for atmospheric model in km." were switched.
C
C-    SPICELIB Version 1.0.0, 11-DEC-2014 (EDW)
C
...
C$ Detailed_Input
C
C     T        is the time in minutes from the elements epoch
...
C$ Exceptions
C
C     1)  If the mean motion is less than 0, the error
C         SPICE(BADMEANMOTION) is signaled.0D0.
C
C     2)  If the mean eccentricity is not bounded by [-0, the error
C         SPICE(BADMECCENTRICITY) is signaled.001, 1].
C
C     3)  If the mean semimajor axis has value less-than ,, the error
C         SPICE(BADMSEMIMAJOR) is signaled.95.
C
C     4)  If the perturbed eccentricity is not bounded by [0, 1], the
C         error SPICE(BADPECCENTRICITY) is signaled.
C
C     5)  If the semi-latus rectum has value less-than 0, the error
C         SPICE(BADSEMILATUS) is signaled.
C
C     6)  If the scaled orbit radial distance has value less than 1, the
C         error SPICE(ORBITDECAY) is signaled.
C
C$ Files
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 30-MAY-2021 (BVS) (JDR)
C
C        Added missing 'D' to literal DP constants.
C
