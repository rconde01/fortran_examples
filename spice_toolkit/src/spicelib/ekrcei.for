...
C
C     NVALS,
C     IVALS    are, respectively, the number of values found in the
C              specified column entry and the set of values themselves.
C
C              For columns having fixed-size entries, when a a column
...
C
C     8)  If an I/O error occurs while reading the indicated file, the
