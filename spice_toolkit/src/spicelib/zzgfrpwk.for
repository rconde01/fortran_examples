C$Procedure    ZZGFRPWK ( Geometry finder report work done on a task )
      ...
      SUBROUTINE ZZGFRPWK ( UNIT,   TOTAL, WAIT,
...
C     WAIT      I-O  ZZGFTSWK, ZZGFWKAD, ZZGFWKMO
...
C           END    = 'completed. '
C
C           CALL ZZGFTSWK ( TOTAL, WAIT, 1, BEGIN, END )
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0 05-NOV-2021 (NJB)
C
C        Changed name of argument FREQ to WAIT. Updated entry points
C        affected by this change. Deleted description of argument
C        UNIT from header of entry point ZZGFTSWK.
C
...
C$Procedure ZZGFTSWK ( Geometry finder total sum of work to be done. )
      ...
      ENTRY ZZGFTSWK ( TOTAL, WAIT, TCHECK, BEGIN, END )
...
C     WAIT       I   Minimum wait time between reports.
...
C                positive number.
C
C     WAIT       is the minimum duration between reports in seconds. If
C                WAIT = 5 then a work progress report will be sent to
C                the output device no more frequently than every 5
C                seconds.  Since writing to the output device takes
C                time, the smaller WAIT is, the greater the overhead
C                taken up by the work reporter will be. (A value of 2
C                or greater should not burden your application
C                appreciably.)
C
C                Regardless of the value of WAIT, the number of calls
C                to ZZGFWKIN between reports will not be less than that
C                specified by the argument TCHECK. See the description
C                below.
C
C     TCHECK     is an integer used to the tell the reporter how often
C                to sample the system clock; TCHECK specifies the
C                number of calls to ZZGFWKIN between system clock
C                samples. If TCHECK = 7, then on every seventh call to
C                ZZGFWKIN, the system clock will be sampled to
C                determine if WAIT seconds have elapsed since the last
C                report time.  Sampling the system clock takes time.
C                Not a lot of time, but it does take time. If ZZGFWKIN
C                is being called from a loop that does not take a lot
C                of time for each pass, the sampling of the system
C                clock can become a significant overhead cost in
C                itself.  On the VAX the sampling of the system clock
C                used here takes about 37 double precision multiplies.
C                If thousands of multiplies take place between calls to
C                ZZGFWKIN, the sampling time is insignificant.  On the
C                other hand, if only a hundred or so multiplies occur
C                between calls to ZZGFWKIN, the sampling of the system
C                clock can become a significant fraction of your
C                overhead.  TCHECK allows you to tailor the work
C                reporter to your application.
C
...
C-    SPICELIB Version 1.1.0 05-NOV-2021 (NJB)
C
C        Changed name of argument FREQ to WAIT. Updated header to
C        clarify roles of WAIT and TCHECK. Deleted description of
C        UNIT from $Detailed_Input header section.
C
C-    SPICELIB Version 1.0.0 17-FEB-2009 (NJB) (LSE) (WLT) (IMU)
C
C-&
...
C
      ENTIRE = TOTAL
      STEP   = MIN ( 3600.0D0, MAX( 0.0D0, WAIT ) )
      CHECK  = MAX ( 1, TCHECK )
...
C$Procedure ZZGFWKAD ( Geometry finder work reporting adjustment )
      ...
      ENTRY ZZGFWKAD ( WAIT, TCHECK, BEGIN, END )
...
C
C     2) If WAIT is less than 0.D0, the value 0.D0 is stored.
C        If WAIT is greater than 3600, the value 3600 is stored.
C
C$ Files
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 1.1.0 21-FEB-2021 (NJB)
C
C        Changed name of argument FREQ to WAIT. Updated header to
C        clarify roles of WAIT and TCHECK. Corrected Exceptions
C        section's description of handling of WAIT values less than
C        zero.
C
C-    SPICELIB Version 1.0.0 17-FEB-2009 (NJB) (LSE) (WLT) (IMU)
C
...
C-&
      ...
      STEP   = MIN  ( 3600.0D0,  MAX( 0.D0, WAIT ) )
      CHECK  = MAX  ( 1, TCHECK )
      START  = BEGIN
      FINISH = END
...
C$Procedure ZZGFWKMO ( Geometry finder work reporting monitor )
      ...
      ENTRY ZZGFWKMO ( UNIT, TOTAL, WAIT, TCHECK, BEGIN, END, INCR )
...
C     WAIT       O   Minimum wait time between reports.
...
C-    SPICELIB Version 1.1.0 21-FEB-2021 (NJB)
C
C        Changed name of argument FREQ to WAIT. Updated Brief_I/O
C        description of this argument.
C
C-    SPICELIB Version 1.0.0 17-FEB-2009 (NJB)
C
C-&
