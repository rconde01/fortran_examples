...
C              discarded before GFTFOV conducts its search.
C
C$ Detailed_Output
C
C     RESULT   is a SPICE window representing the set of time intervals,
C              within the confinement period, when the target body is
C              visible; that is, when the target body intersects the
C              space bounded by the specified instrument's field of
C              view.
C
...
C         value, an error is signaled by a routine
...
C         routine.
C
C     7)  If the body model specifier TSHAPE is invalid, an error is
C         signaled by either this routine or a routine in the call tree
C         of this routine.
C
...
C         the limit (pi/2)-MARGIN radians, an error is signaled
...
C         of this routine.
C
C     16) If the output SPICE window RESULT has size less than 2, the
C         error SPICE(WINDOWTOOSMALL) is signaled.
C
C     17) If the output SPICE window RESULT has insufficient capacity
...
C          2004-JUN-11 10:36:56.283772     2004-JUN-11 11:09:04.397165
C          2004-JUN-11 11:23:56.020645     2004-JUN-11 11:56:04.733535
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 06-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Modified code example's output to comply with maximum line
C        length of header comments. Updated Example's kernels set to use
