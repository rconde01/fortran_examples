...
C              be set.
C
C              If NAME has values associated with it, they are removed,
C              and VALUE becomes the only value associated with NAME. If
C              NAME is not in the symbol table, a new symbol is created,
...
C         name table, the error SPICE(NAMETABLEFULL) is signaled.
C
C     2)  If the addition of a new symbol causes an overflow in the
C         pointer table, the error SPICE(POINTERTABLEFULL) is signaled.
C
C     3)  If the addition of a new symbol causes an overflow in the
C         value table, the error SPICE(VALUETABLEFULL) is signaled.
C
C$ Files
C
C     None.
...
C$ Particulars
C
C     If NAME has values associated with it, they are removed, and VALUE
C     becomes the only value associated with NAME. If NAME is not in the
C     symbol table, a new symbol is created, provided there is room in
C     the symbol table.
C
C$ Examples
C
