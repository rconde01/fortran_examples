...
C              the file ID word.
C
C     BINFIL   is the name of the binary DAF file to be created.
C
C$ Detailed_Output
C
C     None.
...
C
C     3)  If an error occurs while attempting to decode data in the DAF
C         transfer file, the error SPICE(BADDAFTRANSFERFILE) is
C         signaled.
C
C     4)  If the DAF file cannot be written, an error is signaled by a
...
C           entire data portion of the DAF transfer file, so there is
C           no more data.
C
            MORE = .FALSE.
