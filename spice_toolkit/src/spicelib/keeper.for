...
C     SRCFIL     O   KDATA, KINFO
C     FOUND      O   KDATA, KINFO
C     FILSIZ     P   Maximum file name length.
C     MAXFIL     P   Maximum number of files that can be loaded.
C
C$ Detailed_Input
C
...
C
C     1) Load a meta-kernel with a PCK, an LSK and an SPK, and
C        separately, a text kernel and a binary PCK. Loop over the
C        loaded kernels, outputting file information for each of
C        them.
C
C        Then unload the text kernels, check that they have been
C        unloaded, and finally unload all remaining kernels
C        and clear the kernel pool using KCLEAR.
C
C
...
C
C
C        Use the PCK kernel below as the binary PCK required for the
C        example.
C
C           earth_latest_high_prec.bpc
C
C
C        Use the FK kernel below as the text kernel required for the
C        example.
C
C           RSSD0002.TF
C
...
C        C
C              INTEGER               FNAMLN
C              PARAMETER           ( FNAMLN = 256 )
C
C              INTEGER               FTYPLN
C              PARAMETER           ( FTYPLN = 33 )
C
C              INTEGER               SRCLEN
C              PARAMETER           ( SRCLEN = 256 )
C
C        C
C        C     Local variables.
C        C
C              CHARACTER*(FNAMLN)    FILE
C              CHARACTER*(FTYPLN)    FILTYP
C              CHARACTER*(SRCLEN)    SRCFIL
C
C              INTEGER               COUNT
C              INTEGER               HANDLE
C              INTEGER               WHICH
...
C
C        C
C        C     Load several kernel files.
C        C
C              CALL FURNSH ( 'keeper_ex1.tm'              )
C              CALL FURNSH ( 'RSSD0002.TF'                )
C              CALL FURNSH ( 'earth_latest_high_prec.bpc' )
C
C        C
C        C     Count the number of loaded kernel files.
C        C
C              CALL KTOTAL ( 'ALL', COUNT )
C
C              WRITE(*,'(A,I2)') 'The total number of kernels after '
C             . //               'final FURNSH: ', COUNT
C              WRITE(*,*) ' '
C
C        C
C        C     Retrieve the data for all the loaded kernels and
C        C     test an index for which there should be no kernel.
C        C
C              WRITE(*,'(A)') 'Overview of loaded kernels, by index:'
C              WRITE(*,*) ' '
C
C              DO WHICH= 1, COUNT + 1
C
C                 CALL KDATA ( WHICH, 'ALL',   FILE, FILTYP,
C             .                SRCFIL, HANDLE, FOUND        )
C
C                 IF ( FOUND ) THEN
C
C                    WRITE(*,*) '  Index : ', WHICH
C                    WRITE(*,*) '  File  : ', FILE
C                    WRITE(*,*) '  Type  : ', FILTYP
C                    WRITE(*,*) '  Source: ', SRCFIL
C                    WRITE(*,*) '  Handle: ', HANDLE
C                    WRITE(*,*) ' '
C
C                 ELSE
C
C                    WRITE(*,*) '  No kernel found with index: ', WHICH
C
C                 END IF
C
C              END DO
C
C        C
C        C     Unload the text kernels.
C        C
C              CALL KTOTAL ( 'TEXT', COUNT )
C
C              WRITE(*,*) ' '
C              WRITE(*,'(A,I2,A)') 'Unloading', COUNT,
C             .                    ' text kernels...'
C              WRITE(*,*) ' '
C
C              DO WHILE ( COUNT .NE. 0 )
C
C                 CALL KDATA (      1, 'TEXT',  FILE, FILTYP,
C             .                SRCFIL, HANDLE, FOUND        )
C
C        C
C        C        If the kernel is found in the pool, unload it.
C        C
C                 IF ( FOUND ) THEN
C
C                    CALL UNLOAD ( FILE )
C
C        C
C        C           Check if the file has been unloaded.
C        C
C                    CALL KINFO ( FILE, FILTYP, SRCFIL, HANDLE, FOUND )
C
C                    IF ( FOUND ) THEN
C
C                       WRITE(*,'(A)') '  Error unloading ' // FILE
C
C                    ELSE
C
C                       WRITE(*,'(A)') '  Success unloading ' // FILE
C
C                    END IF
C
C        C
C        C        Something is not working. Inform NAIF.
C        C
C                 ELSE
C
C                    WRITE(*,*) ' ERROR: No kernel found with index: ',
C             .                 WHICH
C
C                 END IF
C
C        C
C        C        Check if we have more text kernels to unload from
C        C        the kernel pool. Note that unloading a text kernel
C        C        or meta-kernel implies that the kernel pool is
C        C        cleared, and any kernel(s) that were not to be
C        C        unloaded are re-loaded. Therefore the COUNT value
C        C        changes, and the indexing of the files within the
C        C        kernel pool too.
C        C
C                 CALL KTOTAL ( 'TEXT', COUNT )
C
...
C              WRITE(*,*) ' '
C              WRITE(*,'(A,I2)') 'The total number of kernels after '
C             . //               'UNLOAD calls: ', COUNT
C
C        C
C        C     Clear the KEEPER system, retrieve the number of loaded
C        C     after the clear.
C        C
C              CALL KCLEAR()
C
...
C
C
C        The total number of kernels after final FURNSH:  6
C
C        Overview of loaded kernels, by index:
C
C           Index :            1
C           File  : keeper_ex1.tm
C           Type  : META
C           Source:
C           Handle:            0
C
...
C
C           Index :            6
C           File  : earth_latest_high_prec.bpc
C           Type  : PCK
C           Source:
C           Handle:            2
C
C           No kernel found with index:            7
C
C        Unloading 3 text kernels...
C
C          Success unloading naif0012.tls
C          Success unloading pck00009.tpc
C          Success unloading RSSD0002.TF
C
C        The total number of kernels after UNLOAD calls:  3
C
C        The total number of kernels after KCLEAR      :  0
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 5.1.0, 29-DEC-2021 (JDR) (NJB)
C
C        Changed argument name SOURCE to SRCFIL for consistency with
C        other routines.
C
C        Updated KEEPER umbrella routine and all entry points' headers
C        to comply with NAIF standard.
C
C        Updated description of input argument KIND in headers of entry
C        points KDATA and KTOTAL. Updated $Brief_I/O table to include
C        WHICH short description and sort arguments in the order they
C        are declared.
C
C        Added a restriction about specifying kernels using relative 
C        paths to the FURNSH entry point header $Restrictions section. 
C
...
C
C           Changed to use SEPOOL instead of STPOOL to reduce loading
C           time for large meta-kernels due to n^2 delay in STPOOL.
C
...
C     array SRCFIL.  If the file is loaded directly, its source
...
C$ Parameters
C
C     FILSIZ   is the maximum file name length that can be accommodated
C              by the kernel pool.
C
C     MAXFIL   is the number of entries that can be stored in KEEPER's
C              kernel database. In this version of the toolkit MAXFIL
C              is set to 5300. Each time a kernel is loaded via
...
C$ Exceptions
C
C     1)  If a problem is encountered while trying to load FILE, an
...
C         assignment, the error is signaled by a routine in the call
C         tree of this routine, and this routine will return. Any files
C         loaded prior to encountering the failure, including those
C         referenced by the KERNELS_TO_LOAD assignment, will remain
C         loaded.
C
...
C         combined name and path length exceeding FILSIZ characters,
C         the error SPICE(FILENAMETOOLONG) is signaled.
C
C     10) If a kernel pool variable name length exceeds its maximum
C         allowed length (see Kernel Required Reading, kernel.req), an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C$ Files
...
C     specific arithmetic implementation.
C
C     1) Load the leapseconds kernel naif0007.tls and the planetary
...
C
C
C     2) This example illustrates how you could create a meta-kernel
C        file for a program that requires several text and binary
C        kernels.
C
...
C           KPL/MK
C
C           File name: furnsh_ex2.tm
C
C           Here are the SPICE kernels required for my application
C           program.
C
C           Note that kernels are loaded in the order listed. Thus
C           we need to list the highest priority kernel last.
C
C
C           \begindata
C
...
C
C        is continued across several lines in the right hand side of
C        the assignment of the kernel variable KERNELS_TO_LOAD.
C
...
C
C           CALL FURNSH ( 'furnsh_ex2.tm' )
C
C        This will cause each of the kernels listed in your meta-kernel
C        to be loaded.
C
C
C     3) This example illustrates how you can simplify the previous
C        kernel list by using PATH_SYMBOLS.
C
C
C           KPL/MK
C
C           File name: furnsh_ex3.tm
C
C           Here are the SPICE kernels required for my application
C           program.
C
...
C
C
C     4) This example illustrates continuation of path values. The
...
C
C
C     5) Load a meta-kernel containing three kernels, and separately,
C        a text kernel and a binary PCK. Count the number of loaded
C        files before and after calling KCLEAR.
C
C
C        Use the meta-kernel shown below to load the required SPICE
C        kernels.
...
C
C     3)  When a kernel is specified with a relative path, this path 
C         should be valid at the time when FURNSH is called and stay 
C         valid for the rest of the application run. This is required
C         because SPICE stores kernel names as provided by the caller
C         and uses them to open and close binary kernels as needed 
C         by the DAF/DAS handle manager subsystem (behind the scenes,
C         to allow reading many more binary kernels than available
C         logical units), and to automatically reload into the POOL
C         the rest of text kernels that should stay loaded when a 
C         particular text kernel is unloaded.
C         
C         Changing the working directory from within an application 
C         during an application run after calling FURNSH to load 
C         kernels specified using relative paths is likely to 
C         invalidate stored paths and prevent open/close and unload
C         operations mentioned above. A simple workaround when this
C         is needed is to specify kernels using absolute paths.
C
C$ Literature_References
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 5.0.1, 08-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C        Created complete code example from existing code fragments.
C
C        Added KERNEL to $Required_Reading section.
C
C        Added FILSIZ to the $Declarations section.
C
C        Added a restriction about specifying kernels using relative 
C        paths to the $Restrictions section. 
C
C-    SPICELIB Version 5.0.0, 01-FEB-2017 (NJB) (BVS)
C
C        Updated to support use of DSKs.
...
C$ Abstract
C
C     Return the number of kernels of a specified type that are
C     currently loaded via the FURNSH interface.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
...
C$ Detailed_Input
C
C     KIND     is a list of types of kernels to count when computing
C              loaded kernels. KIND should consist of a list of words of
C              kernels to examine. Recognized types are
C
...
C
C              When KIND consists of multiple words, the words must
C              be separated by blanks. Examples of valid lists are the
C              strings
C
C                 'SPK CK TEXT'
C                 'SPK CK text'
...
C
C     1) Load a meta-kernel with a PCK, an LSK and an SPK, and
C        separately, a text kernel and a binary PCK. Show the
C        total number of kernels and meta-kernels loaded. Determine the
C        number of text kernels loaded, and the number of binary
C        kernels.
C
C        Unload all kernels and clear the kernel pool using
C        KCLEAR, and check that no kernels are loaded.
C
C
C        Use the meta-kernel shown below to load the required SPICE
C        kernels.
...
C
C        C
C        C     Count the number of meta-kernels.
C        C
C              CALL KTOTAL ( 'META', COUNT )
C
C              WRITE(*,'(A,I2)') 'The total number of meta-kernels  '
C             . //               '            : ', COUNT
C
C        C
C        C     Count the number of text kernels.
C        C
C              CALL KTOTAL ( 'TEXT', COUNT )
C
C              WRITE(*,'(A,I2)') 'The total number of text kernels  '
C             . //               '            : ', COUNT
C
C        C
C        C     Count the number of binary kernels. These kernels
C        C     are of type CK, DSK, EK, PCK or SPK.
C        C
C              CALL KTOTAL ( 'CK DSK EK PCK SPK', COUNT )
C              WRITE(*,'(A,I2)') 'The total number of binary kernels'
C             . //               '            : ', COUNT
C
C        C
C        C     Clear the KEEPER system, retrieve the number of loaded
...
C
C        The total number of kernels after final FURNSH:  6
C        The total number of meta-kernels              :  1
C        The total number of text kernels              :  3
C        The total number of binary kernels            :  2
C        The total number of kernels after KCLEAR      :  0
C
C
C$ Restrictions
...
C$ Version
C
C-    SPICELIB Version 5.0.1, 25-OCT-2021 (JDR) (NJB)
C
C        Edited the header to comply with NAIF standard.
C        Created complete code example from existing code fragments.
C
C        Updated $Detailed_Input description of input argument KIND to
C        illustrate use of multi-word lists. Added KERNEL to the list
C        of required readings.
C
C-    SPICELIB Version 5.0.0, 01-FEB-2017 (NJB) (BVS)
C
C        Updated to support use of DSKs.
...
C     SRCFIL     O   Name of the source file used to load FILE.
C     HANDLE     O   The handle attached to FILE.
C     FOUND      O   .TRUE. if the specified file could be located.
C
C$ Detailed_Input
C
C     WHICH    is the number of the kernel to fetch (matching the type
C              specified by KIND) from the list of kernels that have
C              been loaded through the routine FURNSH but that have not
C              been unloaded through the routine UNLOAD.
C
C              The range of WHICH is 1 to COUNT, where COUNT is the
C              number of kernels loaded via FURNSH of type KIND. This
C              count may be obtained by calling KTOTAL. See the
C              $Examples section for an illustrative example.
C
...
C                 'ALL'
C
C              See the routine KTOTAL for examples of the use of KIND.
C
C$ Detailed_Output
C
C     FILE     is the name of the file having index WHICH in the
C              sequence of files of type KIND that is currently loaded
C              via FURNSH. FILE will be blank if there is not such
C              kernel loaded.
C
...
C
C     SRCFIL   is the name of the source file that was used to
C              specify FILE as one to load. If FILE was loaded
C              directly via a call to FURNSH, SRCFIL will be blank.
C              If there is no file matching the specification of
C              WHICH and KIND, SRCFIL will be blank.
C
...
C         and KIND, FOUND will be .FALSE., FILE, FILTYP, and SRCFIL will
C         be blank and HANDLE will be set to zero.
C
C     2)  If any of FILE, FILTYP or SRCFIL output strings has length
C         too short to contain the corresponding output string, the
C         string is truncated on the right.
C
C$ Files
C
C     None.
...
C$ Particulars
C
C     This routine allows you to determine which kernels have been
C     loaded via FURNSH and to obtain information sufficient to directly
C     query those files.
C
C$ Examples
C
...
C     specific arithmetic implementation.
C
C     1) Load a meta-kernel with a PCK, an LSK and an SPK and loop over
C        the loaded kernels, outputting file information for each of
C        them.
C
C        Use the meta-kernel shown below to load the required SPICE
C        kernels.
...
C         Handle:            0
C
C         No kernel found with index:            5
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 5.1.0, 08-AUG-2021 (JDR) (NJB)
C
C        Changed argument name SOURCE to SRCFIL for consistency with
C        other routines.
C
...
C        Created complete code example from existing code fragments.
C
C        Updated $Detailed_Input description of input arguments KIND, to
C        illustrate use of multi-word lists, and WHICH, to describe its
C        range.
C
C        Added entry #2 to $Exceptions section. Added KERNEL to the list
C        of required readings.
C
C-    SPICELIB Version 5.0.0, 01-FEB-2017 (NJB) (BVS)
C
...
C              specify FILE as one to load. If FILE was loaded
C              directly via a call to FURNSH, SRCFIL will be blank.
C              If FILE is not on the list of loaded kernels, SRCFIL
C              will be blank
C
C     HANDLE   is the handle attached to FILE if it is a binary
C              kernel. If FILE is a text kernel or meta-text kernel
...
C     Error free.
C
C     1)  If the specified file is not on the list of files that are
C         currently loaded via the interface FURNSH, FOUND will be
C         .FALSE., HANDLE will be set to zero and FILTYP and SRCFIL will
C         be set to blanks.
C
C     2)  If any of  FILTYP or SRCFIL output strings has length too
C         short to contain the corresponding output string, the string
...
C     specific arithmetic implementation.
C
C     1) Suppose you wish to determine the types of kernels loaded
C        by a given meta-kernel. The following code example shows
C        how you might use this routine to do this.
C
C        Use the meta-kernel shown below to load the required SPICE
C        kernels.
C
...
C        C
C              INTEGER               FILLEN
C              PARAMETER           ( FILLEN = 32 )
C
C              INTEGER               TYPLEN
C              PARAMETER           ( TYPLEN = 33 )
C
C              INTEGER               SRCLEN
C              PARAMETER           ( SRCLEN = 256 )
C
...
C        C
C              CALL FURNSH ( 'kinfo_ex1.tm' )
C
C        C
C        C     Find out the total number of kernels in the kernel pool.
C        C
C              CALL KTOTAL ( 'all', COUNT )
C
C              IF ( COUNT .EQ. 0 ) THEN
C
C                 WRITE(*,*) 'No files loaded at this time.'
C
C              ELSE
C
C                 WRITE(*,*) 'The loaded files files are: '
C                 WRITE(*,*) ' '
C
C              END IF
C
C        C
C        C     Find the file name, type and source for each of the
C        C     kernels in the kernel pool and print its type.
C        C
C              DO WHICH= 1, COUNT
C
C                 CALL KDATA ( WHICH,  'all',  FILE, FILTYP,
...
C                 CALL KINFO ( FILE, FILTYP, SRCFIL, HANDLE, FOUND )
C
C                 IF ( EQSTR( FILTYP, "SPK" ) ) THEN
C
C                    WRITE(*,*) FILE, ' is an SPK file.'
C
C                 ELSE IF ( EQSTR( FILTYP, "CK" ) ) THEN
C
C                    WRITE(*,*) FILE, ' is a CK file.'
C
C                 ELSE IF ( EQSTR( FILTYP, "PCK" ) ) THEN
C
C                    WRITE(*,*) FILE, ' is a PCK file.'
C
C                 ELSE IF ( EQSTR( FILTYP, "DSK" ) ) THEN
C
C                    WRITE(*,*) FILE, ' is a DSK file.'
C
C                 ELSE IF ( EQSTR( FILTYP, "EK" ) ) THEN
C
C                    WRITE(*,*) FILE, ' is an EK file.'
C
C                 ELSE IF ( EQSTR( FILTYP, "META" ) ) THEN
C
C                    WRITE(*,*) FILE, ' is a meta-kernel file.'
C
C                 ELSE
C
C                    WRITE(*,*) FILE, ' is a text kernel.'
C
C                 END IF
C
C              END DO
...
C
C
C         The loaded files files are:
C
C         kinfo_ex1.tm                     is a meta-kernel file.
C         de421.bsp                        is an SPK file.
C         pck00008.tpc                     is a text kernel.
C         naif0009.tls                     is a text kernel.
C
C
C$ Restrictions
C
...
C$ Version
C
C-    SPICELIB Version 5.1.0, 08-AUG-2021 (JDR)
C
C        Changed argument name SOURCE to SRCFIL for consistency with
C        other routines.
C
...
C        Created complete code example from existing code fragments.
C
C        Added entry #2 to $Exceptions section.
C
C-    SPICELIB Version 5.0.0, 01-FEB-2017 (NJB) (BVS)
C
C        Updated to support use of DSKs.
...
C$ Exceptions
C
C     1)  If an error occurs when setting a kernel pool watch or
C         checking watched variables, the error is signaled by a routine
...
C     specific arithmetic implementation.
C
C     1) Load a meta-kernel containing three kernels, and count the
C        number of files in the kernel pool before and after calling
C        KCLEAR.
C
C        Use the meta-kernel shown below to load the required SPICE
C        kernels.
...
C
C
C        The total number of kernels after FURNSH:  4
C        The total number of kernels after KCLEAR:  0
C
C
C$ Restrictions
C
C     1)  Calling this routine will wipe out any kernel pool data
C         inserted via the SPICELIB API routines to put data into the
C         kernel pool (PCPOOL, PDPOOL and PIPOOL).
C
C$ Literature_References
C
C     None.
...
C        code example from existing code fragments.
C
C        Improved $Restrictions section.
C
C-    SPICELIB Version 5.0.0, 01-FEB-2017 (NJB) (BVS)
C
C        Updated to support use of DSKs.
...
C
C        Then unload the text kernels, check that they have been
C        unloaded, and finally unload the meta-kernel.
C
C
C        Use the meta-kernel shown below to load the required SPICE
C        kernels.
...
C                 ELSE
C
C                    WRITE(*,*) ' ERROR: No kernel found but KTOTAL '
C             .      //         'returns ', COUNT
C
C                 END IF
C
...
C
C        C
C        C     Unload the meta-kernel and retrieve the number of loaded
C        C     after the clear.
C        C
C              CALL UNLOAD ( 'unload_ex1.tm' )
C
C              CALL KTOTAL ( 'ALL', COUNT )
C
C              WRITE(*,*) ' '
C              WRITE(*,'(A,I2)') 'The total number of kernels after '
C             . //               'final UNLOAD: ', COUNT
C
C              END
C
C
...
C        The total number of kernels after UNLOAD calls:  3
C
C        The total number of kernels after final UNLOAD:  1
C
C
C$ Restrictions
C
C     1)  See the note regarding the unloading of Text and meta-text
C         Kernels.
C
C$ Literature_References
C
