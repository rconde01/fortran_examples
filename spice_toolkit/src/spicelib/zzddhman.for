...
C$ Version
C
C-    SPICELIB Version 2.2.0, 01-OCT-2021 (NJB)
C
C        Corrected typos in a long error message and in comments.
C
...
C        environment.  See ZZPLTCHK's header for details.
C
C-&
...
C     may not exceed UTSIZE.  UINDEX refers to a unit of interest
C     within the table.
C
      INTEGER               NUT
...
C$ Version
C
C-    SPICELIB Version 2.1.1, 01-OCT-2021 (NJB)
C
...
C     to prevent one architecture's code from stepping on another's.
C
      TMPSTR = ARCH
      CALL UCASE ( TMPSTR, TMPSTR )
      FILARC = ISRCHC ( TMPSTR, NUMARC, STRARC )
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 01-OCT-2021 (NJB)
C
C        Fixed typo in long error message and in comments.
C
C-    SPICELIB Version 1.0.0, 06-NOV-2001 (FST)
C
C-&
...
C     matches that listed in the file table for this handle.  This is
C     to prevent one architectures code from stepping on another's.
C
      TMPSTR = ARCH
      CALL UCASE ( TMPSTR, TMPSTR )
      FILARC = ISRCHC ( TMPSTR, NUMARC, STRARC )
...
C                is to have its logical unit lock released.
C
C     ARCH       is the expected architecture of the file associated
C                with HANDLE.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 01-OCT-2021 (NJB)
C
C        Corrected typo in comments.
C
C-    SPICELIB Version 1.0.0, 06-NOV-2001 (FST)
