...
C
C     FIRST,
C     LAST     are, respectively, the start and stop times of the time
C              interval over which the segment defines the state of
C              BODY.
C
...
C              interpolate the states contained in the input packets.
C              All components of the state vectors are interpolated by
C              polynomials of the specified degree, except near the
C              segment boundaries, or if the total number of states in
C              the segment is too few to allow interpolation using the
C              specified degree.
C
C              If the actual interpolation degree is reduced, the
C              highest degree feasible degree valid for the
C              interpolation type is used.
C
C     N        is the number of packets in the input packet array.
C
C     PACKTS   is a time-ordered array of data packets representing
C              geometric states of BODY relative to CENTER, specified
C              relative to FRAME. The packet structure depends on the
C              segment subtype as follows:
C
...
C
C                 where x, y, z represent Cartesian position components
C                 and  vx, vy, vz represent Cartesian velocity
C                 components. Note well: vx, vy, and vz *are not
C                 necessarily equal* to the time derivatives of x, y,
C                 and z. This packet structure mimics that of the
C                 Rosetta/MEX orbit file from which the data are taken.
C
...
C                 components.
C
C              Position units are kilometers, velocity units are
C              kilometers per second, and acceleration units are
C              kilometers per second per second.
C
C     EPOCHS   is an array of epochs corresponding to the members of the
C              packets array. The epochs are specified as seconds past
C              J2000, TDB.
C
C$ Detailed_Output
C
...
C              polynomial. If the value of MAXDEG is increased, the
C              SPICELIB routine SPKPVN must be changed accordingly. In
C              particular, the size of the record passed to SPKRnn and
C              SPKEnn must be increased, and comments describing the
C              record size must be changed.
C
C$ Exceptions
C
C     If any of the following exceptions occur, this routine will
C     return without creating a new segment.
C
...
C
C     6)  If the number of packets N is not at least 2, the error
...
C$ Version
C
C-    SPICELIB Version 1.1.1, 09-APR-2021 (JDR)
C
