...
C$ Abstract
C
C     Compare two character sets, as indicated by a relational operator.
C
C$ Disclaimer
C
...
C$ Exceptions
C
C     1)  If the set relational operator is not recognized, the error
...
C       If the operator is not recognized, an error is now signaled.
C
C-&
...
C
C     We can mimic this in an algorithm.  The main trick is to figure
