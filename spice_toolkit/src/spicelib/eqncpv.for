...
C
C     EQEL     is an array of 9 double precision numbers that are the
C              equinoctial elements for some orbit expressed relative to
C              the equatorial frame of the central body defined as
C
C              -  The Z-axis of the equatorial frame is the direction
C                 of the pole of the central body relative to some
C                 inertial frame;
C
C              -  The X-axis is given by the cross product of the Z-axis
C                 of the inertial frame with the direction of the pole
C                 of the central body; and
C
C              -  The Y-axis completes a right handed frame.
C
C              If the X-axis of the equatorial frame is aligned with the
C              X-axis of the inertial frame, then the X-axis of the
C              equatorial frame will be located at 90 degrees + RAPOL in
C              the inertial frame.
C
C              The specific arrangement of the elements is spelled out
C              below:
C
C                 EQEL(1)   is the semi-major axis (A) of the orbit in
C                           km.
C
...
C                           the epoch of the elements measured in
C                           radians.
C
...
C
C                 EQEL(9)   is the rate of the longitude of the
C                           ascending node ( dNODE/dt). This rate is
C                           measured in radians per second.
C
C              where
C
C                 INC       is the inclination of the orbit,
C
C                 ARGP      is the argument of periapse,
C
C                 NODE      is longitude of the ascending node, and
C
C                 E         is eccentricity of the orbit.
C
C     RAPOL    is the Right Ascension of the pole of the reference plane
...
C$ Detailed_Output
C
C     STATE    is the state of the object described by EQEL relative to
C              the inertial frame used to define RAPOL and DECPOL. Units
...
C         greater than 0.9, the error SPICE(ECCOUTOFRANGE) is signaled.
C
...
C$ Restrictions
C
C     1)  The equinoctial elements used by this routine are taken
...
C
C         This will convert the Sine formulation to the Tangent
C         formulation.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.3, 14-APR-2021 (JDR)
C
C        Edited the header to comply with NAIF standard. Added SPK
C        required reading.
C
...
C
C-    SPICELIB Version 1.0.0, 08-JAN-1997 (WLT) (RAJ)
C
C-&
