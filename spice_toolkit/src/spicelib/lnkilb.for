...
C         SPICE(INVALIDNODE) is signaled. POOL will not be
C         modified.
C
...
C
C     4)  If either of LIST or NEXT are valid node numbers but are not
C         allocated, the error SPICE(UNALLOCATEDNODE) is signaled. POOL
...
C         to be signaled, even though HEAD is 0 at that point.
C
C$ Restrictions
C
C     1)  Linked list pools must be initialized via the routine
C         LNKINI. Failure to initialize a linked list pool
C         will almost certainly lead to confusing results.
C
C     2)  For efficiency, discovery check-in is used in this routine.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.1, 24-NOV-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Added note about efficiency in $Restrictions section.
C
