...
C$ Abstract
C
C     Return a specified pointing instance from a CK type 02 segment.
...
C
C     DESCR    is the packed descriptor of the data type 2 CK segment.
C
...
C                 RECORD( 3  ) = TDB seconds per tick rate
C
C                 RECORD( 4  ) = Q1
C                 RECORD( 5  ) = Q2
C                 RECORD( 6  ) = Q3
...
C
C              Note that the RECORD returned by this routine is slightly
C              different from that returned by the SPICELIB routine
C              CKR02. The second element of the record returned by that
C              routine contains the SCLK time at which pointing was
C              requested, whereas this routine returns the SCLK time of
C              the right endpoint of the interval for which the constant
C              angular velocity model is valid.
C
C$ Parameters
C
C     None.
