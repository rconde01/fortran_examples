...
C
C     BODYID   is the integer ID code of a body for which topographic
C              data are present in the specified DSK file.
C
C     SRFIDS   is an initialized SPICE set data structure.
C
C              SRFIDS optionally may contain a set of surface ID codes
C              on input; on output, the ID codes already present in
C              SRFIDS will be combined with surface ID code set found
C              for the body designated by BODYID in the file DSKFNM.
C
...
C$ Detailed_Output
C
C     SRFIDS   is a SPICE set data structure that contains the union
C              of its contents upon input with the set of ID codes of
C              the surfaces associated with the body designated by
C              BODYID, for which segments were found in the indicated
C              DSK file.
C
C              The elements of SPICE sets are unique; each ID code in
C              SRFIDS appears only once, even if the DSK file contains
C              multiple segments for that ID code.
C
C              See the $Examples section below for a complete example
C              program showing how to retrieve body and surface ID codes
C              from a DSK file.
C
C$ Parameters
C
...
C         to contain the actual number of ID codes of surfaces covered
C         by the indicated DSK file, the error SPICE(CELLTOOSMALL) is
C         signaled.
C
C$ Files
C
...
C
C     1) Examine a DSK file and identify the set of central bodies
C        associated with the segments in the file. For each body, find
C        the set of surfaces associated with that body.
C
C
...
C
C        Edited the header to comply with NAIF standard. Update problem
C        statement in $Examples section. Corrected short error message
C        in entries #2, #3 and #5 in $Exceptions section.
C
