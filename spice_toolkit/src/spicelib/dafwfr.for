...
C         open DAF file, an error is signaled by a routine in the call
C         tree of this routine.
C
C     2)  If the specified DAF file is not open for write access, an
...
C         SPICE(DAFREADFAIL) is signaled.
C
C$ Files
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 4.1.0, 06-AUG-2021 (JDR)
C
C        Added IMPLICIT NONE statement.
C
C        Edited the header to comply with NAIF standard.
C
C        Moved DAF required reading from $Literature_References to
C        $Required_Reading section. Fixed typo in $Brief_I/O: ND
C        argument was listed twice. Second should be NI. Removed
