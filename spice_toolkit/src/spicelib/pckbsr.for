...
C
C     STSIZE   is the maximum number of segments that can be buffered at
C              any given time by PCKSFS.
C
C$ Exceptions
C
...
C
C     1) Suppose that the data of interest are contained in a PCK file,
C        and that we want to generate a table containing the
C        descriptors of the PCK segments, or a message indicating that
C        no segment was found, for various request times. We are
C        interested in the data coverage of the segments in the file.
C
C        The code example below loads PCK files and performs searches
C        for various epochs, generating a table containing the segment
C        descriptors, if found, or a message indicating that a segment
C        descriptor was not found.
C
C        Use the PCK kernel below as input file for the code example.
C
...
C              PARAMETER           ( DELTA  = 50000000.D0 )
C
C              INTEGER               BODY
C              PARAMETER           ( BODY   = 3000        )
C
C              INTEGER               DESCSZ
...
C        C
C              CALL PCKLOF ( 'earth_latest_high_prec.bpc', PCKHDL )
C
C        C
C        C     Search for segments using evenly spaced epochs.
C        C
C              ET = -86400.D0
C
C              DO I = 1, NEPOCH
...
C
C        C
C        C        Locate the applicable segment (handle and
C        C        descriptor).
C        C
...
C
C                    WRITE(*,*) '   Segment ID: ', SEGID
C                    WRITE(*,*) '   Body ID   : ', BODYID
C                    WRITE(*,*) '   Frame ID  : ', FRAMID
C                    WRITE(*,*) '   PCK Type  : ', PCKTYP
C                    WRITE(*,*) '   Start ET  : ', BEGET
C                    WRITE(*,*) '   End ET    : ', ENDET
C
C                 ELSE
C
...
C
C
C         Epoch =   -86400.000000000000
C            ***** SEGMENT NOT FOUND *****
C
C         Epoch =    49913600.000000000
C            Segment ID: Earth PCK, ITRF93 Frame
C            Body ID   :         3000
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :   -43135.816087188054
C            End ET    :    86343752.179112613
C
C         Epoch =    99913600.000000000
C            Segment ID: Earth PCK, ITRF93 Frame
C            Body ID   :         3000
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :    86343752.179112613
C            End ET    :    172730640.17431438
C
C         Epoch =    149913600.00000000
C            Segment ID: Earth PCK, ITRF93 Frame
C            Body ID   :         3000
C            Frame ID  :           17
C            PCK Type  :            2
...
C            End ET    :    172730640.17431438
C
C         Epoch =    199913600.00000000
C            Segment ID: Earth PCK, ITRF93 Frame
C            Body ID   :         3000
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :    172730640.17431438
C            End ET    :    259117528.16951615
C
C         Epoch =    249913600.00000000
C            Segment ID: Earth PCK, ITRF93 Frame
C            Body ID   :         3000
C            Frame ID  :           17
C            PCK Type  :            2
...
C            End ET    :    259117528.16951615
C
C         Epoch =    299913600.00000000
C            Segment ID: Earth PCK, ITRF93 Frame
C            Body ID   :         3000
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :    259117528.16951615
C            End ET    :    345504416.16470283
C
C         Epoch =    349913600.00000000
C            Segment ID: Earth PCK, ITRF93 Frame
C            Body ID   :         3000
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :    345504416.16470283
C            End ET    :    431891304.15988630
C
C         Epoch =    399913600.00000000
C            Segment ID: Earth PCK, ITRF93 Frame
C            Body ID   :         3000
C            Frame ID  :           17
C            PCK Type  :            2
...
C            End ET    :    431891304.15988630
C
C         Epoch =    449913600.00000000
C            Segment ID: Earth PCK, ITRF93 Frame
C            Body ID   :         3000
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :    431891304.15988630
C            End ET    :    518278192.15506977
C
C
C     2) In this example multiple PCK files are loaded and searched for
C        segments.
C
C        Use the PCK kernel below as the first input file for the code
C        example.
C
C           earth_latest_high_prec.bpc
C
C
C        Use the PCK kernel below as the second input file for the code
C        example.
C
...
C
C        C
C        C     Load the PCK files.
C        C
C              CALL PCKLOF ( 'earth_latest_high_prec.bpc', PCKHD1 )
C              CALL PCKLOF ( 'earth_720101_070426.bpc',    PCKHD2 )
C
C        C
C        C     Search for segments using evenly spaced epochs.
C        C
...
C
C        C
C        C     Unload the PCK files.
C        C
C              CALL PCKUOF ( PCKHD1 )
C              CALL PCKUOF ( PCKHD2 )
C
C              END
C
C
...
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :   -74415076.797098771
C            End ET    :    11979698.742793124
C
C         Epoch =    49913600.000000000
C            Handle    :            2
C            Segment ID: Earth PCK, ITRF93 Frame
...
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :    11979698.742793124
C            End ET    :    98374474.282683983
C
C         Epoch =    99913600.000000000
C            Handle    :            2
C            Segment ID: Earth PCK, ITRF93 Frame
...
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :    98374474.282683983
C            End ET    :    184769249.82257757
C
C         Epoch =    149913600.00000000
C            Handle    :            2
C            Segment ID: Earth PCK, ITRF93 Frame
...
C            Frame ID  :           17
C            PCK Type  :            2
C            Start ET  :    184769249.82257757
C            End ET    :    230817665.18534085
C
C         Epoch =    249913600.00000000
C            Handle    :            1
C            Segment ID: Earth PCK, ITRF93 Frame
...
C$ Version
C
C-    SPICELIB Version 2.1.0, 26-OCT-2021 (JDR) (BVS) (NJB)
C
C        Updated entry point PCKSFS to always initialize FOUND.
C
C        Edited the header of the PCKBSR umbrella and all its entry
C        points to comply with NAIF standard. Added complete code
C        examples from existing fragments.
C
C        Changed SAVE statements to save each variable individually.
C
...
C        Bug fix:
C
C           In the PCKSFS 'MAKE ROOM' state, when the suspended
C           activity is 'ADD TO FRONT' and no segment table room is
C           available, the body table's pointer to the current segment
C           list is now set to null. Previously the pointer was allowed
C           to go stale.
C
...
C$ Detailed_Input
C
C     FNAME    is the character name of the file to be loaded.
C
C$ Detailed_Output
C
C     HANDLE   is the integer handle assigned to the file upon loading.
C              Other PCK routines will subsequently use this number to
C              refer to the file.
C
C$ Parameters
C
C     FTSIZE   is the maximum number of PCK files that may be loaded
C              simultaneously under any circumstances. FTSIZE is
C              currently set to match the maximum number of DAF files
C              that may be loaded simultaneously.
C
C$ Exceptions
C
C     1)  If an attempt is made to open more DAF files than is
...
C         by the parameter FTSIZE in the PCK subsystem, and if the DAF
C         system has room to load another file, the error
C         SPICE(PCKFILETABLEFULL) is signaled. The current setting of
C         FTSIZE does not allow this situation to arise: the DAF system
C         will trap the error before this routine has the chance.
C
C     3)  This routine makes use of DAF file system routines and is
C         subject to all of the constraints imposed by the DAF file
C         system. See the DAF Required Reading daf.req or individual DAF
C         routines for details.
C
C$ Files
C
C     A file specified by FNAME, to be loaded. The file is assigned a
C     handle by PCKLOF, which will be used by other routines to refer
C     to it.
C
C$ Particulars
C
C     If there is room for a new file in the file table, PCKLOF creates
C     an entry for it, and opens the file for reading.
C
C     Also, if the file table is empty, PCKLOF initializes it.
C
C$ Examples
C
C     See $Examples in PCKBSR.
C
C$ Restrictions
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 2.0.2, 26-OCT-2021 (JDR)
C
...
C$ Detailed_Input
C
C     HANDLE   is the integer handle assigned to the PCK file upon
C              loading.
C
C$ Detailed_Output
C
...
C$ Particulars
C
C     A PCK file is removed from consideration during a search by the
C     readers by a call to PCKUOF.
C
C     The file table entry corresponding to the file referenced by
C     HANDLE is removed and the file is closed. Any segment table
C     entry which came from the specified file is also deleted.
C
...
C$ Version
C
C-    SPICELIB Version 4.1.3, 09-JUL-2020 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Improved documentation of $Detailed_Input and $Particulars
C        sections.
C
...
C$ Detailed_Output
C
C     HANDLE   is the handle of the binary PCK file containing a
C              located segment.
C
C     DESCR    is the descriptor of a located segment.
C
C     IDENT    is the identifier of a located segment.
C
C     FOUND    is a logical flag indicating whether a requested segment
C              was found or not.
C
C$ Parameters
C
...
C$ Version
C
C-    SPICELIB Version 4.3.0, 13-OCT-2021 (JDR) (NJB)
C
C        Edited the header to comply with NAIF standard.
C
