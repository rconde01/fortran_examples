...
C$ Revisions
C
C-     Beta Version 2.0.0, 01-JAN-1989 (HAN)
C
C         Code was added to handle the following exceptional
C         inputs.
C
C         If the dimension of the array is less than one, any
C         value of LOC is invalid. The old version did not check
C         the dimension of the array, and as a result, its output
C         was unpredictable.
C
C         If the location at which the elements are to be removed is
C         not in the interval [1, NA], an error is signaled.
...
C         number of elements can be removed, an error is signaled.
C         In the old version, only those elements that could be
C         removed were removed, and no error was signaled.
C
C-&
...
C     value of LOC is invalid, and an error is signaled.
C
