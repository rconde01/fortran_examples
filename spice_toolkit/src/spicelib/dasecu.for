...
C$ Detailed_Output
C
C     COMNTS   is a logical flag indicating whether or not any comments
...
C         attached to HANDLE, the error is signaled by a routine in the
C         call tree of this routine.
C
C     2)  If an error occurs while writing to the text file attached to
C         COMLUN, the error is signaled by a routine in the call tree of
C         this routine.
C
C$ Files
C
...
C$ Restrictions
C
C     1)  The maximum length of a single line comment in the comment
...
C
C-    SPICELIB Version 1.0.0, 05-JAN-1993 (KRG)
C
C-&
