...
C$ Detailed_Input
C
C     LEFT     is the amount to be added to the left endpoint of each
C              interval in the input window. The amount LEFT is signed.
C
C     RIGHT    is the amount to be subtracted from the right endpoint of
C              each interval in the window. The amount RIGHT is signed.
C
...
C$ Version
C
C-    SPICELIB Version 1.1.0, 06-JUL-2021 (JDR) (NJB)
C
...
C
C        Added entry #1 in $Exceptions section. Extended LEFT and RIGHT
C        description in $Detailed_Input.
C
