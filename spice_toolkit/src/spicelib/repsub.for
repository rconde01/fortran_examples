...
C         the error SPICE(PASTENDSTR) is signaled.
C
...
C
C     to insert ' Imelda Marcos' between the first and second commas
...
C     then
C
C        CALL REPSUB ( IN, LEFT, RIGHT, ' Imelda Marcos', OUT )
C
C     The output (OUT) will have the value:
C
C        'The defendant, Imelda Marcos, was found innocent.'
C
C$ Restrictions
C
C     1)  The memory used by STRING and OUT must be disjoint. The memory
