...
C$ Detailed_Input
C
C     INST     is a string indicates the name of an instrument, such as
C              a spacecraft-mounted framing camera, the field of view
...
C              defined, as is normally done in a frame kernel. It must
C              also have an associated reference frame and a FOV shape,
C              boresight and boundary vertices (or reference vector and
C              reference angles) defined, as is usually done in an
C              instrument kernel.
C
C              See the header of the SPICELIB routine GETFOV for a
C              description of the required parameters associated with an
C              instrument.
C
C     TSHAPE   is a string indicating the geometric model used to
C              represent the location and shape of the target body. The
C              target body may be represented by either an ephemeris
C              object or a ray emanating from the observer.
C
...
C              representing the target. RAYDIR is used if and only if
C              TSHAPE (see description above) indicates the target is
C              modeled as a ray.
C
C     TARGET   is the name of the target body, the appearances of which
C              in the specified instrument's field of view are sought.
C              The body must be an ephemeris object.
C
C              Optionally, you may supply the integer NAIF ID code for
C              the body as a string. For example both 'MOON' and '301'
C              are legitimate strings that designate the Moon.
C
...
C              target is NOT modeled as ray, as indicated by the input
C              argument TSHAPE.
C
...
C              target. Examples of such names are 'IAU_SATURN' (for
C              Saturn) and 'ITRF93' (for the Earth).
C
C              If the target is an ephemeris object modeled as an
C              ellipsoid, TFRAME must designate a body-fixed reference
C              frame centered on the target body.
C
C              If the target is an ephemeris object modeled as a point,
C              TFRAME is ignored; TFRAME should be left blank.
C
C              If the target is modeled as a ray, TFRAME may designate
C              any reference frame. Since light time corrections are not
C              supported for rays, the orientation of the frame is
C              always evaluated at the epoch associated with the
C              observer, as opposed to the epoch associated with the
C              light-time corrected position of the frame center.
C
C              Case and leading or trailing blanks bracketing a
C              non-blank frame name are not significant in the string
C              TFRAME.
C
C     ABCORR   is a string indicating the aberration corrections to be
C              applied when computing the target's position and
C              orientation. The supported values of ABCORR depend on the
...
C                 'S'          Stellar aberration correction, reception
C                              case.
C                 'XS'         Stellar aberration correction,
C                              transmission case.
C
C              If the target is an ephemeris object, the aberration
C              correction options are those supported by the SPICE SPK
C              system. For remote sensing applications, where the
C              apparent position and orientation of the target seen by
C              the observer are desired, normally either of the
C              corrections
C
C                 'LT+S'
C                 'CN+S'
C
C              should be used. These and the other supported options are
C              described below.
C
C              Supported aberration correction options for observation
C              (the case where radiation is received by observer at ET)
C              are:
C
C                 'NONE'       No correction.
...
C              (the case where radiation is emitted from observer at ET)
C              are:
C
...
C              observed. The instrument designated by INST is treated as
C              if it were co-located with the observer.
C
...
C
C     UDSTEP   is an externally specified routine that computes a time
C              step used to find transitions of the state being
...
C                 GFSTEP
C
C              may be used as the step size function. If GFSTEP is used,
C              the step size must be set by calling GFSSTP prior to
...
C              computes an intermediate time T such that either [T1, T]
C              or [T, T2] contains the time of the state transition. The
C              calling sequence for UDREFN is:
C
...
C
C                 T2    is a time when the visibility state is S2. T2 is
C                       expressed as seconds past J2000 TDB and is
...
C              routine GFREFN may be used as the refinement function.
C
C     RPT      is a logical variable that controls whether progress
C              reporting is enabled. When RPT is .TRUE., progress
C              reporting is enabled and the routines UDREPI, UDREPU, and
C              UDREPF (see descriptions below) are used to report
C              progress.
C
C     UDREPI   is a user-defined subroutine that initializes a progress
C              report. When progress reporting is enabled, UDREPI is
C              called at the start of a search. The calling sequence of
C              UDREPI is
C
...
C                 CNFINE
C
C              is the confinement window specifying the time period
...
C
C              The SPICELIB routine GFREPI may be used as the actual
C              argument corresponding to UDREPI. If so, the SPICELIB
C              routines GFREPU and GFREPF must be the actual arguments
C              corresponding to UDREPU and UDREPF.
C
C     UDREPU   is a user-defined subroutine that updates the progress
C              report for a search. The calling sequence of UDREPU is
C
...
C
C              Here IVBEG, IVEND are the bounds of an interval that is
C              contained in some interval belonging to the confinement
C              window. The confinement window is associated with some
C              root finding activity. It is used to determine how much
C              total time is being searched in order to find the events
C              of interest.
C
C              ET is an epoch belonging to the interval [IVBEG, IVEND].
C
...
C
C              The SPICELIB routine GFREPU may be used as the actual
C              argument corresponding to UDREPU. If so, the SPICELIB
C              routines GFREPI and GFREPF must be the actual arguments
C              corresponding to UDREPI and UDREPF.
C
C     UDREPF   is a user-defined subroutine that finalizes a progress
C              report. UDREPF has no arguments.
C
C              The SPICELIB routine GFREPF may be used as the actual
C              argument corresponding to UDREPF. If so, the SPICELIB
C              routines GFREPI and GFREPU must be the actual arguments
C              corresponding to UDREPI and UDREPU.
C
C     BAIL     is a logical variable indicating whether or not interrupt
C              handling is enabled. When BAIL is set to .TRUE., the
C              input function UDBAIL (see description below) is used to
C              determine whether an interrupt has been issued.
C
C     UDBAIL   is the name of a user defined logical function that
C              indicates whether an interrupt signal has been issued
C              (for example, from the keyboard).  UDBAIL has no
C              arguments and returns a LOGICAL value. The return value
C              is .TRUE. if an interrupt has been issued; otherwise the
C              value is .FALSE.
C
C              GFFOVE uses UDBAIL only when BAIL (see above) is set to
C              .TRUE., indicating that interrupt handling is enabled.
C              When interrupt handling is enabled, GFFOVE and routines
C              in its call tree will call UDBAIL to determine whether to
C              terminate processing and return immediately.
C
C              If interrupt handing is not enabled, a logical function
C              must still be passed to GFFOVE as an input argument. The
C              SPICELIB function
C
C                 GFBAIL
C
...
C              discarded before GFFOVE conducts its search.
C
C$ Detailed_Output
C
...
C         that if the step size is non-positive, an error is signaled by
...
C         translated to a NAIF ID code, an error is signaled by
C         a routine in the call tree of this routine.
C
C     4)  If the specified aberration correction is not a supported
C         value for the target type (ephemeris object or ray), an error
...
C         the body, an error is signaled by a routine in the
C         call tree of this routine.
C
C     6)  If the target body coincides with the observer body OBSRVR, an
C         error is signaled by a routine in the call tree of this
C         routine.
C
C     7)  If the body model specifier TSHAPE is not recognized, an error
C         is signaled by a routine in the call tree of this routine.
C
C     8)  If a target body-fixed reference frame associated with a
C         non-point target is not recognized, an error is signaled by a
C         routine in the call tree of this routine.
C
C     9)  If a target body-fixed reference frame is not centered at the
C         corresponding target body, an error is signaled by a routine
C         in the call tree of this routine.
C
C     10) If the instrument name INST does not have corresponding NAIF
C         ID code, an error is signaled by a routine in the call
C         tree of this routine.
C
C     11) If the FOV parameters of the instrument are not present in
C         the kernel pool, an error is signaled by a routine
C         in the call tree of this routine.
C
C     12) If the FOV boundary has more than MAXVRT vertices, an error
...
C         the limit (pi/2)-SPICE_GF_MARGIN radians, an error is signaled
C         by a routine in the call tree of this routine. If the FOV is
C         any other shape, the same error check will be applied with the
C         instrument boresight vector serving the role of R.
C
C     14) If the loaded kernels provide insufficient data to compute a
C         requested state vector, an error is signaled by a
C         routine in the call tree of this routine.
C
...
C         visibility condition is met, an error is signaled
C         by a routine in the call tree of this routine.
C
C     17) If the result window has size less than 2, the error
C         SPICE(WINDOWTOOSMALL) is signaled.
C
C     18) If the convergence tolerance size is non-positive, the error
C         SPICE(INVALIDTOLERANCE) is signaled.
C
C     19) If the step size is non-positive, an error is signaled by a
C         routine in the call tree of this routine.
C
C     20) If the ray's direction vector is zero, an error is signaled by
C         a routine in the call tree of this routine.
C
C     21) If operation of this routine is interrupted, the output result
...
C              pck00010.tpc                    Satellite orientation
C                                              and radii
C              041014R_SCPSE_01066_04199.bsp   CASSINI, planetary and
C                                              Saturn satellite
C                                              ephemeris
C              cas_v40.tf                      Cassini FK
C              04161_04164ra.bc                Cassini bus CK
C              cas00071.tsc                    Cassini SCLK kernel
C              cas_iss_v10.ti                  Cassini IK
C
C
C           \begindata
C
...
C                                  'pck00010.tpc',
C                                  '041014R_SCPSE_01066_04199.bsp',
C                                  'cas_v40.tf',
C                                  '04161_04164ra.bc',
C                                  'cas00071.tsc',
C                                  'cas_iss_v10.ti'            )
...
C        C
C              CHARACTER*(*)         META
C              PARAMETER           ( META   = 'gffove_ex1.tm' )
C
C              CHARACTER*(*)         TIMFMT
C              PARAMETER           ( TIMFMT =
C             .      'YYYY-MON-DD HR:MN:SC.######::TDB' )
C
C              INTEGER               LBCELL
...
C        C
C        C     Pass the step size (1 second) to the GF default step
C        C     size put/get system.
C        C
C              CALL GFSSTP ( 1.D0 )
C
C        C
...
C             .              TARGET,  TFRAME,  ABCORR,  OBSRVR,
C             .              TOL,     GFSTEP,  GFREFN,  RPT,
C             .              GFREPI,  GFREPU,  GFREPF,  BAIL,
C             .              GFBAIL,  CNFINE,  RESULT          )
C
C              N = WNCARD( RESULT )
C
C              IF ( N .EQ. 0 ) THEN
...
C              ELSE
C
C                 WRITE (*, '(A)' ) '  Visibility start time (TDB)'
C             .    //               '           Stop time (TDB)'
C                 WRITE (*, '(A)' ) '  ---------------------------'
C             .    //               '     ---------------------------'
C
...
C        Target visibility search 100.00% done.
C
C          Visibility start time (TDB)           Stop time (TDB)
C          ---------------------------     ---------------------------
C          2004-JUN-11 07:35:27.066980     2004-JUN-11 08:48:03.954696
C          2004-JUN-11 09:02:56.580045     2004-JUN-11 09:35:04.038509
C          2004-JUN-11 09:49:56.476397     2004-JUN-11 10:22:04.242879
C          2004-JUN-11 10:36:56.283771     2004-JUN-11 11:09:04.397165
C          2004-JUN-11 11:23:56.020645     2004-JUN-11 11:56:04.733536
C
C
C        Note that the progress report has the format shown below:
C
C           Target visibility search   6.02% done.
C
...
C        the output was:
C
C           Target visibility search  13.63% done.
...
C        C     of the spacecraft relative to the solar system
C        C     barycenter; the result is the ray's direction vector.
C        C
...
C          Visibility start time (TDB)           Stop time (TDB)
C          ---------------------------     ---------------------------
C          2004-JUN-11 06:30:00.000000     2004-JUN-11 12:00:00.000000
C
C
C$ Restrictions
C
C     1)  The kernel files to be used by GFFOVE must be loaded (normally
C         via the SPICELIB routine FURNSH) before GFFOVE is called.
C
...
C$ Version
C
C-    SPICELIB Version 1.0.2, 06-AUG-2021 (JDR)
C
C        Edited the header to comply with NAIF standard.
C
C        Modified code examples' output to comply with maximum line
C        length of header comments. Updated Examples' kernels set to use
C        PDS archived data. Added SAVE statements for CNFINE and RESULT
C        variables in code examples.
C
C        Updated description of RESULT argument in $Brief_I/O,
C        $Detailed_Input and $Detailed_Output.
C
C        Added entries #17 and #22 in $Exceptions section.
C
C        Corrected reporting message in UDREPI description.
C
