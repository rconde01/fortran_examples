...
C$ Exceptions
C
C     1)  If improper inputs are supplied, or if there is insufficient
C         room to store the requested addition, an error is signaled by
C         a routine in the call tree of this routine.
C
C     2)  If the length of NAME exceeds the maximum allowed length for a
C         body name, the name stored in the kernel pool will be
C         truncated on the right.
C
C     3)  If a name-code definition inserted into this routine seems to
