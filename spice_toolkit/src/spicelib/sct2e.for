...
C         output argument ET will not be modified. This routine assumes
C         that that an SCLK kernel appropriate to the spacecraft clock
C         identified by the input argument SC has been loaded.
C
...
C         ET will not be modified. When using SCLK kernels that map SCLK
C         to a time system other than ET (also called barycentric
...
C         SPICE(NOTSUPPORTED) is signaled. The output argument ET
C         will not be modified.
C
C     4)  If the input argument SCLKDP is invalid, an error is signaled
...
C
C        Moved the required readings present in $Literature_References
C        section to $Required_Reading.
C
